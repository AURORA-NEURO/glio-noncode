...
| `sources` | array | exactly 17 source records |
...
Every source record contains a source ID, title, URL, retrieval date, version or release label, scope, and content address. Source IDs are unique. Operation source joins and case source joins must resolve to the source registry before evaluation.
...
Cases retain expected state, expected result state, expected issue codes, expected count summaries, source IDs, payload summary, and a content address. Raw payload keys that could expose unbounded study records are rejected by the artifact safety gate.
