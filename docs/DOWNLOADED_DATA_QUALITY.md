...
## D469 runtime registry admission

D469 aggregates D468 strict and release runtime decisions into a deterministic
multi-runtime registry. It rejects duplicate runtime identities and content
addresses, folds any blocked runtime into a blocked aggregate, persists the
canonical four-file registry, and exposes independent audits and bounded
queries while retaining each upstream diff and runtime address.

The real-data demonstration admits the D468 strict and release outputs:

```text
python examples/downloaded_data_quality_d469_runtime_registry_demo.py D468_STRICT_DIR D468_RELEASE_DIR --destination D469_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```
...
```text
C:/Users/murar/AppData/Local/Temp/glio-noncode-d188-real-demo-20260921/d469-example-real/summary.json
```

## D470 runtime registry history

D470 records D469 runtime-registry snapshots in an append-only,
content-addressed history. It preserves stable registry and history identity,
requires an optimistic expected head for appends, rejects duplicate snapshots
and registry addresses, folds deterministic transitions, and exposes latest
readiness plus bounded history queries. Its compact D469 adapter verifies the
exact four-file registry without reopening earlier import layers.

The real-data demonstration records the blocked D469 registry and the ready
D469 registry rebuilt from the D468 release runtime:

```text
python examples/downloaded_data_quality_d470_runtime_registry_history_demo.py D469_REGISTRY_DIR D468_RELEASE_DIR --destination D470_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```
...
```text
C:/Users/murar/AppData/Local/Temp/glio-noncode-d188-real-demo-20260921/d470-example-real/summary.json
```

## D471 registry-history comparison

D471 compares two D470 runtime-registry histories without reopening source
records. It matches ordinal snapshots, preserves both history and entry
addresses, classifies added, removed, changed, and unchanged entries, folds
quality direction and state transitions, and emits an independently auditable
exact four-file diff with bounded queries.
...
The real-data demonstration rebuilds the baseline history from the persisted
D469 blocked registry and compares it with the persisted D470 history:

```text
python examples/downloaded_data_quality_d471_history_diff_demo.py D469_REGISTRY_DIR D470_HISTORY_DIR --destination D471_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```
...
## D475 registry-history comparison

D475 compares two D474 runtime-registry histories without reopening source
records. It matches ordinal snapshots, preserves both history and entry
addresses, classifies added, removed, changed, and unchanged entries, folds
quality direction and state transitions, and emits an independently auditable
exact four-file diff with bounded queries.
...
The real-data demonstration rebuilds the baseline history from the persisted
D473 blocked registry and compares it with the persisted D474 history:

```text
python examples/downloaded_data_quality_d475_history_diff_demo.py D473_REGISTRY_DIR D474_HISTORY_DIR --destination D475_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```
...
```text
C:/Users/murar/AppData/Local/Temp/glio-noncode-d188-real-demo-20260921/d475-example-real/summary.json
```

## D476 history-diff runtime evaluation

D476 evaluates D475 history diffs under explicit strict and release policies.
Policies bound added, removed, and changed items, allowed direction, upstream
acceptance, state transitions, and unchanged behavior. Each decision retains
the diff and policy addresses, emits bounded checks, persists an exact
four-file runtime, and exposes independent runtime and query audits.

The real-data demonstration evaluates the D475 comparison under both policies:

```text
python examples/downloaded_data_quality_d476_history_diff_runtime_demo.py D475_DIFF_DIR --destination D476_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```
...
## D480 history-diff runtime evaluation

D480 evaluates D479 release-batch history diffs under explicit strict and
release policies. Policies control minimum items, added/removed/changed
budgets, allowed direction, required state movement, unchanged behavior, and
upstream acceptance. Each runtime retains the diff and policy addresses,
emits fifteen deterministic checks, persists an exact four-file artifact, and
exposes independent runtime and query audits.

The real-data demonstration evaluates the ZIP-derived D479 diff:

```text
python examples/downloaded_data_quality_d480_release_batch_history_diff_runtime_demo.py D479_DIFF_DIR --destination D480_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```

The strict policy blocked the diff on its added budget, while the release
policy accepted the improved `blocked->ready` diff as `ready` with 15/15
checks. Both runtime audits passed 15/15. The release query returned 59/59
rows without truncation and its query audit passed 12/12. The rerun summary is
stored at:
...
## D484 history-diff runtime evaluation

D484 evaluates a D483 history diff under independently configurable strict
and release policies. It enforces minimum-item, added, removed, and changed
budgets; allowed direction; required state transitions; unchanged handling;
and upstream acceptance. Each decision is content-addressed, persisted as an
exact four-file runtime, reload-verified, and exposed through policy, checks,
comparison, readiness, address, and bounds projections. Independent runtime
and query audits verify the full decision and every bounded result row.

The real-data demonstration evaluates the D483 comparison derived from the
downloaded product archive:

```text
python examples/downloaded_data_quality_d484_history_diff_runtime_registry_history_diff_runtime_demo.py C:/Users/murar/AppData/Local/Temp/glio-noncode-d188-real-demo-20260921/d483-example-real/diff --destination D484_OUTPUT_DIR --source-zip C:/Users/murar/Downloads/GLIO_NONCODE_vNext_Product_Rebuild_2026-08-20.zip
```

The strict zero-added policy blocks on the one added history item, while the
release policy accepts the `improved` `blocked->ready` transition. Both
runtime audits pass 15/15. The release query returns all 59 rows without
truncation, and its query audit passes 12/12. The rerun summary is stored at:
