...
The exact runtime-registry history diff now has a portable [deterministic archive](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE.md). It wraps the four-file baseline/candidate diff in a fixed five-member ZIP, records per-member byte receipts, rejects unsafe or non-canonical members, and exposes independent 18-check archive and 13-check query audits across CLI, HTTP, schema, capability, public inventory, Actions, and the real downloaded-ZIP demo. The demo currently reads 25 catalog members, produces 160 review actions, reloads a 4,783-byte archive, replays 48 query rows, and reports `release_ready: true` without publishing source paths, payload bytes, or private attribution fields.
