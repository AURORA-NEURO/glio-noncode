...
95% confidence intervals. Adjusted results also include residual standard
error and adjusted R-squared as per-feature model-fit diagnostics.
...
The exact-prefixed registry implementation consumes the durable runtime package directly, supports repeated runtime inputs in the CLI and HTTP API, verifies both registry packages and registry projections, and is covered by dedicated persistence, tamper, API, schema, and inventory regression tests.
...
The execution-ledger runtime registry history now has the same deep comparison boundary in [execution-ledger runtime registry history diff](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION_LEDGER_RUNTIME_REGISTRY_HISTORY_DIFF.md). It compares same-registry baseline and candidate histories by ordinal, retains both addressed snapshots and ordered field deltas, derives improved/regressed/changed/unchanged direction, persists an exact four-file package with artifact byte receipts, and exposes independent diff and query audits across CLI, HTTP, schemas, capabilities, public inventory, Actions, and the downloaded-ZIP demo.

That diff now has a portable [deterministic archive](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION_LEDGER_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE.md). It packages a canonical outer manifest plus the four history-diff projections with fixed ZIP metadata, per-member receipts, strict nested replay, atomic persistence, an 18-check archive audit, eight bounded query resources, and a 12-check query audit across the CLI, HTTP API, schemas, capabilities, inventory, Actions, and downloaded-ZIP demonstration.
...
That exact archive now has a resumable [addressed transfer](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER.md). The supplied downloaded ZIP produces a 5,077-byte archive split into five 1,024-byte chunks; atomic complete/partial receiver directories, out-of-order idempotent assembly, strict nested verification, 18 transfer checks, 14 bounded receiver-query rows, and 12 query-audit checks are exercised end to end, and reassembly preserves the original content address without exposing source paths or chunk bytes.

The exact execution-ledger runtime registry history-diff archive now also has a deep [addressed transfer](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION_LEDGER_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER.md). The supplied downloaded ZIP produces a 5,183-byte archive split into six 1,024-byte chunks; complete and partial receiver directories, reverse-order idempotent assembly, strict nested replay, 18 transfer checks, nine bounded transfer-query resources, and 12 query-audit checks are exercised end to end, with exact reload and reassembly evidence.

That exact addressed transfer now has a deterministic [recovery checkpoint](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION_LEDGER_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY.md). Complete and partial receiver states are reloaded into public recovery snapshots; each missing chunk becomes one addressed resume action; received/missing indexes and bytes are conserved; and `resume` versus `assemble`, safety, checkpoint, and next-index projections are independently audited. Nine bounded recovery resources, 18 recovery checks, 12 query-audit checks, CLI/API/schema/capability surfaces, Actions coverage, and the real downloaded-ZIP demonstration are included.

The exact-prefixed transfer now has a deterministic [recovery plan](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY.md). Against the supplied ZIP it preserves received chunks `[0, 4]`, identifies `[1, 2, 3]` as missing, emits three addressed resume actions, conserves 2,005 received bytes plus 3,072 remaining bytes, passes 17/17 recovery checks and 12/12 query-audit checks, and also verifies the complete `assemble` state.

The exact-prefixed recovery plan now has a verifiable [execution receipt](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION.md). Against the supplied ZIP it records chunk `1` as applied while `2,3` remain pending, reports `in_progress` with 3,029 current received bytes and 2,048 remaining bytes, and exercises planned, complete, and blocked receipts. The 18-check execution audit, nine-row bounded query, 12-check query audit, JSON/CSV/Markdown projections, CLI, local HTTP API, schemas, capabilities, public inventory, and Actions coverage all pass.

That exact transfer now has a [verifiable execution receipt](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION.md). It records applied, pending, and rejected outcomes for addressed recovery actions; replays planned, in-progress, complete, and blocked states; conserves indexes and bytes; and exposes independent 18-check execution and 12-check query audits. The real downloaded-ZIP demo persists and reloads the planned receiver execution, an in-progress receipt with chunk `1` applied and chunks `2,3` pending, a complete receipt, and a blocked receipt.

That exact execution now has a durable [runtime handoff](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION_RUNTIME.md). It composes the receipt, independent audit, bounded query, and query audit into five ordered stages; persists an exact seven-file package atomically with manifest byte receipts; rejects non-canonical and tampered reloads; and exposes sixteen runtime checks plus twelve query-assurance checks through CLI, local HTTP, schemas, capabilities, and the public inventory. The real downloaded-ZIP demo persists and reloads in-progress, complete, and blocked runtime packages while preserving the execution state and byte/index evidence.

That exact receipt now has a focused [execution outcome/query boundary](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION_LEDGER_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER_RECOVERY_EXECUTION.md). It records applied, pending, and rejected actions with conserved byte/index partitions; derives planned, in-progress, complete, and blocked states; and exposes independent 18-check execution and 12-check query audits over nine bounded resources. The downloaded-ZIP demonstration persists and reloads in-progress, complete, and blocked receipts, including the 2,111-byte progress branch, complete assembly branch, and 4,096-byte blocked remainder.
Downloaded-data quality diffs now have a policy-governed release gate. It
classifies every value-free transition as safe, review, or blocked; applies
content-addressed direction and count thresholds; emits promote/hold/block
decisions; independently audits gate and query closure; and exposes CLI/API
surfaces for real downloaded-ZIP review. See [docs/DOWNLOADED_DATA_QUALITY.md](docs/DOWNLOADED_DATA_QUALITY.md).
