...
That exact archive now has a resumable [addressed transfer](docs/HISTORY_DIFF_RUNTIME_REGISTRY_HISTORY_DIFF_ARCHIVE_TRANSFER.md). Five 1,024-byte chunks, atomic complete/partial receiver directories, out-of-order idempotent assembly, strict nested verification, 18 transfer checks, 14 bounded receiver-query rows, and 12 query-audit checks are exercised against the supplied downloaded ZIP; the reassembled archive preserves its original content address without exposing source paths, chunk bytes, agent attribution, or language metadata.
