"""Aggregate D344 decisions with the D345 runtime registry contract.

The input directories are exact four-file D344 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d345_runtime_registry_demo.py \
        artifacts/d344-real/strict \
        artifacts/d344-real/release \
        --destination artifacts/d345-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d345-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D345 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d345-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D344 runtimes with D345")
    parser.add_argument("strict_directory", type=Path, help="blocked D344 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D344 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d345-release-registry")
