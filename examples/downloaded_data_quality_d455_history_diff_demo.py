"""Compare D454 registry histories with the D455 history diff contract.

The baseline is the blocked-only D454 history and the candidate is the
blocked-to-ready D454 history. D455 preserves both history addresses,
classifies each ordinal, folds release movement, and emits independent audits.

Example::

    python examples/downloaded_data_quality_d455_history_diff_demo.py \
        artifacts/d454-baseline/history \
        artifacts/d454-real/history \
        --destination artifacts/d455-real \
...
from glio_noncode import downloaded_data_quality_d455_history_diff_audit as audit_model
from glio_noncode import downloaded_data_quality_d455_history_diff_query as query_model
from glio_noncode import downloaded_data_quality_d455_history_diff_query_audit as query_audit_model


def build_demo(baseline_directory: str | Path, candidate_directory: str | Path, destination: str | Path | None = None, *, source_zip: str | Path | None = None, diff_id: str = "glio-noncode-d455-history-diff", change_filter: str = "added") -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D455 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D454 registry histories with D455")
    parser.add_argument("baseline_directory", type=Path, help="baseline D454 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D454 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d455-history-diff")
