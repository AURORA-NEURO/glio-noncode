"""Record D253 registry decisions in an append-only D254 history.

The mixed D253 registry is the blocked baseline. A ready-only D253 registry is
built from the release D252 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d254_runtime_registry_history_demo.py \
        artifacts/d253-real/registry \
        artifacts/d252-real/release \
        --destination artifacts/d254-real \
...
from glio_noncode import downloaded_data_quality_d253_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d254-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D254 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D253 registry decisions with D254")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D253 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D252 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d254-release-history")
