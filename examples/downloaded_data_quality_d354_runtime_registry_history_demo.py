"""Record D353 registry decisions in an append-only D354 history.

The mixed D353 registry is the blocked baseline. A ready-only D353 registry is
built from the release D352 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d354_runtime_registry_history_demo.py \
        artifacts/d353-real/registry \
        artifacts/d352-real/release \
        --destination artifacts/d354-real \
...
from glio_noncode import downloaded_data_quality_d353_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d354-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D354 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D353 registry decisions with D354")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D353 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D352 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d354-release-history")
