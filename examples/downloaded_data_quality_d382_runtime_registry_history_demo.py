"""Record D381 registry decisions in an append-only D382 history.

The mixed D381 registry is the blocked baseline. A ready-only D381 registry is
built from the release D380 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d382_runtime_registry_history_demo.py \
        artifacts/d381-real/registry \
        artifacts/d380-real/release \
        --destination artifacts/d382-real \
...
from glio_noncode import downloaded_data_quality_d381_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d382-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D382 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D381 registry decisions with D382")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D381 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D380 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d382-release-history")
