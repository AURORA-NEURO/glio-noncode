"""Aggregate D432 decisions with the D433 runtime registry contract.

The input directories are exact four-file D432 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d433_runtime_registry_demo.py \
        artifacts/d432-real/strict \
        artifacts/d432-real/release \
        --destination artifacts/d433-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d433-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D433 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d433-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D432 runtimes with D433")
    parser.add_argument("strict_directory", type=Path, help="blocked D432 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D432 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d433-release-registry")
