"""Aggregate D408 decisions with the D409 runtime registry contract.

The input directories are exact four-file D408 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d409_runtime_registry_demo.py \
        artifacts/d408-real/strict \
        artifacts/d408-real/release \
        --destination artifacts/d409-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d409-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D409 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d409-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D408 runtimes with D409")
    parser.add_argument("strict_directory", type=Path, help="blocked D408 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D408 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d409-release-registry")
