"""Record D289 registry decisions in an append-only D290 history.

The mixed D289 registry is the blocked baseline. A ready-only D289 registry is
built from the release D288 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d290_runtime_registry_history_demo.py \
        artifacts/d289-real/registry \
        artifacts/d288-real/release \
        --destination artifacts/d290-real \
...
from glio_noncode import downloaded_data_quality_d289_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d290-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D290 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D289 registry decisions with D290")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D289 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D288 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d290-release-history")
