"""Aggregate D336 decisions with the D337 runtime registry contract.

The input directories are exact four-file D336 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d337_runtime_registry_demo.py \
        artifacts/d336-real/strict \
        artifacts/d336-real/release \
        --destination artifacts/d337-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d337-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D337 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d337-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D336 runtimes with D337")
    parser.add_argument("strict_directory", type=Path, help="blocked D336 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D336 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d337-release-registry")
