"""Aggregate D472 decisions with the D473 runtime registry contract.

The input directories are exact four-file D472 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d473_runtime_registry_demo.py \
        artifacts/d472-real/strict \
        artifacts/d472-real/release \
        --destination artifacts/d473-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d473-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D473 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d473-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D472 runtimes with D473")
    parser.add_argument("strict_directory", type=Path, help="blocked D472 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D472 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d473-release-registry")
