"""Aggregate D260 decisions with the D261 runtime registry contract.

The input directories are exact four-file D260 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d261_runtime_registry_demo.py \
        artifacts/d260-real/strict \
        artifacts/d260-real/release \
        --destination artifacts/d261-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d261-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D261 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d261-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D260 runtimes with D261")
    parser.add_argument("strict_directory", type=Path, help="blocked D260 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D260 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d261-release-registry")
