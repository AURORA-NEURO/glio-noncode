"""Record D321 registry decisions in an append-only D322 history.

The mixed D321 registry is the blocked baseline. A ready-only D321 registry is
built from the release D320 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d322_runtime_registry_history_demo.py \
        artifacts/d321-real/registry \
        artifacts/d320-real/release \
        --destination artifacts/d322-real \
...
from glio_noncode import downloaded_data_quality_d321_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d322-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D322 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D321 registry decisions with D322")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D321 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D320 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d322-release-history")
