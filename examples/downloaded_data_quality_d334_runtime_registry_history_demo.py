"""Record D333 registry decisions in an append-only D334 history.

The mixed D333 registry is the blocked baseline. A ready-only D333 registry is
built from the release D332 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d334_runtime_registry_history_demo.py \
        artifacts/d333-real/registry \
        artifacts/d332-real/release \
        --destination artifacts/d334-real \
...
from glio_noncode import downloaded_data_quality_d333_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d334-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D334 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D333 registry decisions with D334")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D333 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D332 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d334-release-history")
