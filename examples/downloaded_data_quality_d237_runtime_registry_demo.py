"""Aggregate D236 decisions with the D237 runtime registry contract.

The input directories are exact four-file D236 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d237_runtime_registry_demo.py \
        artifacts/d236-real/strict \
        artifacts/d236-real/release \
        --destination artifacts/d237-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d237-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D237 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d237-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D236 runtimes with D237")
    parser.add_argument("strict_directory", type=Path, help="blocked D236 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D236 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d237-release-registry")
