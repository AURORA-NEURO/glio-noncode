"""Compare D202 registry histories with the D203 history diff contract.

The baseline is the blocked-only D202 history and the candidate is the
blocked-to-ready D202 history. D203 preserves both history addresses,
classifies each ordinal, folds the release direction, and exposes independent
diff and query audits. The compact module name keeps the new surface portable
on Windows while retaining the full typed D202 history dependency.

Example::

    python examples/downloaded_data_quality_d203_history_diff_demo.py \
        artifacts/d202-baseline/history \
        artifacts/d202-real/history \
        --destination artifacts/d203-real \
...
def build_demo(
    ...
    *,
    ...
    diff_id: str = "glio-noncode-d203-history-diff",
    change_filter: str = "added",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D203 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D202 registry histories with D203")
    parser.add_argument("baseline_directory", type=Path, help="baseline D202 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D202 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d203-history-diff")
