"""Aggregate D416 decisions with the D417 runtime registry contract.

The input directories are exact four-file D416 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d417_runtime_registry_demo.py \
        artifacts/d416-real/strict \
        artifacts/d416-real/release \
        --destination artifacts/d417-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d417-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D417 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d417-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D416 runtimes with D417")
    parser.add_argument("strict_directory", type=Path, help="blocked D416 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D416 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d417-release-registry")
