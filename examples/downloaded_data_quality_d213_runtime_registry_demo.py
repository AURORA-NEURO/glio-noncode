"""Aggregate D212 decisions with the D213 runtime registry contract.

The input directories are exact four-file D212 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d213_runtime_registry_demo.py \
        artifacts/d212-real/strict \
        artifacts/d212-real/release \
        --destination artifacts/d213-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d213-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D213 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d213-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D212 runtimes with D213")
    parser.add_argument("strict_directory", type=Path, help="blocked D212 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D212 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d213-release-registry")
