"""Aggregate D200 decisions with the D201 runtime registry contract.

The input directories are exact four-file D200 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_demo.py \
        artifacts/d200-real/strict \
        artifacts/d200-real/release \
        --destination artifacts/d201-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d201-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D201 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d201-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D200 runtimes with D201")
    parser.add_argument("strict_directory", type=Path, help="blocked D200 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D200 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d201-release-registry")
