"""Aggregate D376 decisions with the D377 runtime registry contract.

The input directories are exact four-file D376 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d377_runtime_registry_demo.py \
        artifacts/d376-real/strict \
        artifacts/d376-real/release \
        --destination artifacts/d377-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d377-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D377 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d377-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D376 runtimes with D377")
    parser.add_argument("strict_directory", type=Path, help="blocked D376 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D376 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d377-release-registry")
