"""Record D277 registry decisions in an append-only D278 history.

The mixed D277 registry is the blocked baseline. A ready-only D277 registry is
built from the release D276 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d278_runtime_registry_history_demo.py \
        artifacts/d277-real/registry \
        artifacts/d276-real/release \
        --destination artifacts/d278-real \
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d278-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D278 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D277 registry decisions with D278")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D277 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D276 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d278-release-history")
