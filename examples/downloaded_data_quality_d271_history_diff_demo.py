"""Compare D270 registry histories with the D271 history diff contract.

The baseline is the blocked-only D270 history and the candidate is the
blocked-to-ready D270 history. D271 preserves both history addresses,
classifies each ordinal, folds release movement, and emits independent audits.

Example::

    python examples/downloaded_data_quality_d271_history_diff_demo.py \
        artifacts/d270-baseline/history \
        artifacts/d270-real/history \
        --destination artifacts/d271-real \
...
def build_demo(baseline_directory: str | Path, candidate_directory: str | Path, destination: str | Path | None = None, *, source_zip: str | Path | None = None, diff_id: str = "glio-noncode-d271-history-diff", change_filter: str = "added") -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D271 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D270 registry histories with D271")
    parser.add_argument("baseline_directory", type=Path, help="baseline D270 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D270 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d271-history-diff")
