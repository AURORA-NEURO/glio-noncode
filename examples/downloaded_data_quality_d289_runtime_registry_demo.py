"""Aggregate D288 decisions with the D289 runtime registry contract.

The input directories are exact four-file D288 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d289_runtime_registry_demo.py \
        artifacts/d288-real/strict \
        artifacts/d288-real/release \
        --destination artifacts/d289-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d289-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D289 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d289-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D288 runtimes with D289")
    parser.add_argument("strict_directory", type=Path, help="blocked D288 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D288 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d289-release-registry")
