"""Record D373 registry decisions in an append-only D374 history.

The mixed D373 registry is the blocked baseline. A ready-only D373 registry is
built from the release D372 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d374_runtime_registry_history_demo.py \
        artifacts/d373-real/registry \
        artifacts/d372-real/release \
        --destination artifacts/d374-real \
...
from glio_noncode import downloaded_data_quality_d373_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d374-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D374 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D373 registry decisions with D374")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D373 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D372 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d374-release-history")
