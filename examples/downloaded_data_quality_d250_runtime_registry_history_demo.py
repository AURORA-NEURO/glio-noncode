"""Record D249 registry decisions in an append-only D250 history.

The mixed D249 registry is the blocked baseline. A ready-only D249 registry is
built from the release D248 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d250_runtime_registry_history_demo.py \
        artifacts/d249-real/registry \
        artifacts/d248-real/release \
        --destination artifacts/d250-real \
...
from glio_noncode import downloaded_data_quality_d249_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d250-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D250 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D249 registry decisions with D250")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D249 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D248 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d250-release-history")
