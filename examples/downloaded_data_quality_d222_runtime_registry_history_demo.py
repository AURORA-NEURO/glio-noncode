"""Record D221 registry decisions in an append-only D222 history.

The mixed D221 registry is the blocked baseline. A ready-only D221 registry is
built from the release D220 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d222_runtime_registry_history_demo.py \
        artifacts/d221-real/registry \
        artifacts/d220-real/release \
        --destination artifacts/d222-real \
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d222-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D222 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D221 registry decisions with D222")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D221 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D220 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d222-release-history")
