"""Record D241 registry decisions in an append-only D242 history.

The mixed D241 registry is the blocked baseline. A ready-only D241 registry is
built from the release D240 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d242_runtime_registry_history_demo.py \
        artifacts/d241-real/registry \
        artifacts/d240-real/release \
        --destination artifacts/d242-real \
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d242-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D242 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D241 registry decisions with D242")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D241 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D240 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d242-release-history")
