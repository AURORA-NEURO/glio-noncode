"""Aggregate D308 decisions with the D309 runtime registry contract.

The input directories are exact four-file D308 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d309_runtime_registry_demo.py \
        artifacts/d308-real/strict \
        artifacts/d308-real/release \
        --destination artifacts/d309-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d309-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D309 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d309-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D308 runtimes with D309")
    parser.add_argument("strict_directory", type=Path, help="blocked D308 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D308 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d309-release-registry")
