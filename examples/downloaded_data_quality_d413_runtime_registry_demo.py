"""Aggregate D412 decisions with the D413 runtime registry contract.

The input directories are exact four-file D412 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d413_runtime_registry_demo.py \
        artifacts/d412-real/strict \
        artifacts/d412-real/release \
        --destination artifacts/d413-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d413-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D413 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d413-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D412 runtimes with D413")
    parser.add_argument("strict_directory", type=Path, help="blocked D412 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D412 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d413-release-registry")
