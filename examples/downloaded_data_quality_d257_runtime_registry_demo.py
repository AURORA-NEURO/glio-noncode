"""Aggregate D256 decisions with the D257 runtime registry contract.

The input directories are exact four-file D256 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d257_runtime_registry_demo.py \
        artifacts/d256-real/strict \
        artifacts/d256-real/release \
        --destination artifacts/d257-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d257-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D257 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d257-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D256 runtimes with D257")
    parser.add_argument("strict_directory", type=Path, help="blocked D256 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D256 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d257-release-registry")
