"""Aggregate D440 decisions with the D441 runtime registry contract.

The input directories are exact four-file D440 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d441_runtime_registry_demo.py \
        artifacts/d440-real/strict \
        artifacts/d440-real/release \
        --destination artifacts/d441-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d441-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D441 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d441-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D440 runtimes with D441")
    parser.add_argument("strict_directory", type=Path, help="blocked D440 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D440 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d441-release-registry")
