"""Record D361 registry decisions in an append-only D362 history.

The mixed D361 registry is the blocked baseline. A ready-only D361 registry is
built from the release D360 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d362_runtime_registry_history_demo.py \
        artifacts/d361-real/registry \
        artifacts/d360-real/release \
        --destination artifacts/d362-real \
...
from glio_noncode import downloaded_data_quality_d361_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d362-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D362 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D361 registry decisions with D362")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D361 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D360 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d362-release-history")
