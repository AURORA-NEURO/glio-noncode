"""Aggregate D300 decisions with the D301 runtime registry contract.

The input directories are exact four-file D300 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d301_runtime_registry_demo.py \
        artifacts/d300-real/strict \
        artifacts/d300-real/release \
        --destination artifacts/d301-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d301-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D301 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d301-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D300 runtimes with D301")
    parser.add_argument("strict_directory", type=Path, help="blocked D300 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D300 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d301-release-registry")
