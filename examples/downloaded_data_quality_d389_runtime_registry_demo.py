"""Aggregate D388 decisions with the D389 runtime registry contract.

The input directories are exact four-file D388 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d389_runtime_registry_demo.py \
        artifacts/d388-real/strict \
        artifacts/d388-real/release \
        --destination artifacts/d389-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d389-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D389 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d389-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D388 runtimes with D389")
    parser.add_argument("strict_directory", type=Path, help="blocked D388 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D388 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d389-release-registry")
