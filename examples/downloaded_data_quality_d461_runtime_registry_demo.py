"""Aggregate D460 decisions with the D461 runtime registry contract.

The input directories are exact four-file D460 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d461_runtime_registry_demo.py \
        artifacts/d460-real/strict \
        artifacts/d460-real/release \
        --destination artifacts/d461-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d461-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D461 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d461-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D460 runtimes with D461")
    parser.add_argument("strict_directory", type=Path, help="blocked D460 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D460 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d461-release-registry")
