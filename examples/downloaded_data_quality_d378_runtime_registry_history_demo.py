"""Record D377 registry decisions in an append-only D378 history.

The mixed D377 registry is the blocked baseline. A ready-only D377 registry is
built from the release D376 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d378_runtime_registry_history_demo.py \
        artifacts/d377-real/registry \
        artifacts/d376-real/release \
        --destination artifacts/d378-real \
...
from glio_noncode import downloaded_data_quality_d377_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d378-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D378 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D377 registry decisions with D378")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D377 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D376 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d378-release-history")
