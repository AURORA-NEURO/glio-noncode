"""Record D229 registry decisions in an append-only D230 history.

The mixed D229 registry is the blocked baseline. A ready-only D229 registry is
built from the release D228 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d230_runtime_registry_history_demo.py \
        artifacts/d229-real/registry \
        artifacts/d228-real/release \
        --destination artifacts/d230-real \
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d230-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D230 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D229 registry decisions with D230")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D229 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D228 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d230-release-history")
