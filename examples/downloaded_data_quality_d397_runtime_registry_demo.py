"""Aggregate D396 decisions with the D397 runtime registry contract.

The input directories are exact four-file D396 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d397_runtime_registry_demo.py \
        artifacts/d396-real/strict \
        artifacts/d396-real/release \
        --destination artifacts/d397-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d397-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D397 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d397-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D396 runtimes with D397")
    parser.add_argument("strict_directory", type=Path, help="blocked D396 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D396 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d397-release-registry")
