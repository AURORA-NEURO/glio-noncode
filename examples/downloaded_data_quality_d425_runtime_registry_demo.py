"""Aggregate D424 decisions with the D425 runtime registry contract.

The input directories are exact four-file D424 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d425_runtime_registry_demo.py \
        artifacts/d424-real/strict \
        artifacts/d424-real/release \
        --destination artifacts/d425-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d425-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D425 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d425-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D424 runtimes with D425")
    parser.add_argument("strict_directory", type=Path, help="blocked D424 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D424 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d425-release-registry")
