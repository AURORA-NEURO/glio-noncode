"""Aggregate D280 decisions with the D281 runtime registry contract.

The input directories are exact four-file D280 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d281_runtime_registry_demo.py \
        artifacts/d280-real/strict \
        artifacts/d280-real/release \
        --destination artifacts/d281-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d281-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D281 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d281-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D280 runtimes with D281")
    parser.add_argument("strict_directory", type=Path, help="blocked D280 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D280 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d281-release-registry")
