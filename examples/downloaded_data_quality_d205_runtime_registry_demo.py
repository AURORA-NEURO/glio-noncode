"""Aggregate D204 decisions with the D205 runtime registry contract.

The input directories are exact four-file D204 runtimes. Keeping a strict
...
also verifies duplicate identity rejection and persists audited projections.

Example::

    python examples/downloaded_data_quality_d205_runtime_registry_demo.py \
        artifacts/d204-real/strict \
        artifacts/d204-real/release \
        --destination artifacts/d205-real \
...
from glio_noncode import downloaded_data_quality_d205_runtime_registry_audit as audit_model
from glio_noncode import downloaded_data_quality_d205_runtime_registry_query as query_model
from glio_noncode import downloaded_data_quality_d205_runtime_registry_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d205-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D205 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d205-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D204 runtimes with D205")
    parser.add_argument("strict_directory", type=Path, help="blocked D204 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D204 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d205-release-registry")
