"""Record D401 registry decisions in an append-only D402 history.

The mixed D401 registry is the blocked baseline. A ready-only D401 registry is
...
Example::
    ...
    python examples/downloaded_data_quality_d402_runtime_registry_history_demo.py \
        artifacts/d401-real/registry \
        artifacts/d384-real/release \
        --destination artifacts/d402-real \
...
from glio_noncode import downloaded_data_quality_d401_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d402-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D402 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D401 registry decisions with D402")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D401 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D400 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d402-release-history")
