"""Aggregate D224 decisions with the D225 runtime registry contract.

The input directories are exact four-file D224 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d225_runtime_registry_demo.py \
        artifacts/d224-real/strict \
        artifacts/d224-real/release \
        --destination artifacts/d225-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d225-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D225 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d225-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D224 runtimes with D225")
    parser.add_argument("strict_directory", type=Path, help="blocked D224 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D224 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d225-release-registry")
