"""Record D349 registry decisions in an append-only D350 history.

The mixed D349 registry is the blocked baseline. A ready-only D349 registry is
built from the release D348 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d350_runtime_registry_history_demo.py \
        artifacts/d349-real/registry \
        artifacts/d348-real/release \
        --destination artifacts/d350-real \
...
from glio_noncode import downloaded_data_quality_d349_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d350-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D350 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D349 registry decisions with D350")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D349 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D348 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d350-release-history")
