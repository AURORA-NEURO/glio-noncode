"""Record D297 registry decisions in an append-only D298 history.

The mixed D297 registry is the blocked baseline. A ready-only D297 registry is
built from the release D296 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d298_runtime_registry_history_demo.py \
        artifacts/d297-real/registry \
        artifacts/d296-real/release \
        --destination artifacts/d298-real \
...
from glio_noncode import downloaded_data_quality_d297_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d298-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D298 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D297 registry decisions with D298")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D297 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D296 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d298-release-history")
