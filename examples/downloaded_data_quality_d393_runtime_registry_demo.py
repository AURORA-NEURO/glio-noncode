"""Aggregate D392 decisions with the D393 runtime registry contract.

The input directories are exact four-file D392 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d393_runtime_registry_demo.py \
        artifacts/d392-real/strict \
        artifacts/d392-real/release \
        --destination artifacts/d393-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d393-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D393 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d393-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D392 runtimes with D393")
    parser.add_argument("strict_directory", type=Path, help="blocked D392 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D392 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d393-release-registry")
