"""Aggregate D400 decisions with the D401 runtime registry contract.

The input directories are exact four-file D400 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d401_runtime_registry_demo.py \
        artifacts/d400-real/strict \
        artifacts/d400-real/release \
        --destination artifacts/d401-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d401-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D401 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d401-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D400 runtimes with D401")
    parser.add_argument("strict_directory", type=Path, help="blocked D400 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D400 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d401-release-registry")
