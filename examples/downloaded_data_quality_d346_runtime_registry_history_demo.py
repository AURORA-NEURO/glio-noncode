"""Record D345 registry decisions in an append-only D346 history.

The mixed D345 registry is the blocked baseline. A ready-only D345 registry is
built from the release D344 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d346_runtime_registry_history_demo.py \
        artifacts/d345-real/registry \
        artifacts/d344-real/release \
        --destination artifacts/d346-real \
...
from glio_noncode import downloaded_data_quality_d345_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d346-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D346 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D345 registry decisions with D346")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D345 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D344 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d346-release-history")
