"""Record D413 registry decisions in an append-only D414 history.

The mixed D413 registry is the blocked baseline. A ready-only D413 registry is
built from the release D384 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d414_runtime_registry_history_demo.py \
        artifacts/d413-real/registry \
        artifacts/d384-real/release \
        --destination artifacts/d414-real \
...
from glio_noncode import downloaded_data_quality_d413_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d414-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D414 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D413 registry decisions with D414")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D413 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D412 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d414-release-history")
