"""Aggregate D384 decisions with the D385 runtime registry contract.

The input directories are exact four-file D384 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d385_runtime_registry_demo.py \
        artifacts/d384-real/strict \
        artifacts/d384-real/release \
        --destination artifacts/d385-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d385-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D385 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d385-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D384 runtimes with D385")
    parser.add_argument("strict_directory", type=Path, help="blocked D384 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D384 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d385-release-registry")
