"""Record D217 registry decisions in an append-only D218 history.

The mixed D217 registry is the blocked baseline. A ready-only D217 registry is
built from the release D216 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d218_runtime_registry_history_demo.py \
        artifacts/d217-real/registry \
        artifacts/d216-real/release \
        --destination artifacts/d218-real \
...
from glio_noncode import downloaded_data_quality_d217_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d218-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D218 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D217 registry decisions with D218")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D217 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D216 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d218-release-history")
