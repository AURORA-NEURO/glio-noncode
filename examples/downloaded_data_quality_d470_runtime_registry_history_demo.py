"""Record D469 registry decisions in an append-only D470 history.

The mixed D469 registry is the blocked baseline. A ready-only D469 registry is
built from the release D468 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d470_runtime_registry_history_demo.py \
        artifacts/d469-real/registry \
        artifacts/d468-real/release \
        --destination artifacts/d470-real \
...
from glio_noncode import downloaded_data_quality_d470_runtime_registry_history_audit as audit_model
from glio_noncode import downloaded_data_quality_d470_runtime_registry_history_query as query_model
from glio_noncode import downloaded_data_quality_d470_runtime_registry_history_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d470-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D470 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D469 registry decisions with D470")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D469 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D468 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d470-release-history")
