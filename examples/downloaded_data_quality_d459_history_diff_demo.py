"""Compare D458 registry histories with the D459 history diff contract.

The baseline is the blocked-only D458 history and the candidate is the
blocked-to-ready D458 history. D459 preserves both history addresses,
classifies each ordinal, folds release movement, and emits independent audits.

Example::

    python examples/downloaded_data_quality_d459_history_diff_demo.py \
        artifacts/d458-baseline/history \
        artifacts/d458-real/history \
        --destination artifacts/d459-real \
...
from glio_noncode import downloaded_data_quality_d459_history_diff_audit as audit_model
from glio_noncode import downloaded_data_quality_d459_history_diff_query as query_model
from glio_noncode import downloaded_data_quality_d459_history_diff_query_audit as query_audit_model


def build_demo(baseline_directory: str | Path, candidate_directory: str | Path, destination: str | Path | None = None, *, source_zip: str | Path | None = None, diff_id: str = "glio-noncode-d459-history-diff", change_filter: str = "added") -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D459 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D458 registry histories with D459")
    parser.add_argument("baseline_directory", type=Path, help="baseline D458 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D458 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d459-history-diff")
