"""Record D357 registry decisions in an append-only D358 history.

The mixed D357 registry is the blocked baseline. A ready-only D357 registry is
built from the release D356 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d358_runtime_registry_history_demo.py \
        artifacts/d357-real/registry \
        artifacts/d356-real/release \
        --destination artifacts/d358-real \
...
from glio_noncode import downloaded_data_quality_d357_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d358-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D358 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D357 registry decisions with D358")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D357 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D356 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d358-release-history")
