"""Record D449 registry decisions in an append-only D450 history.

The mixed D449 registry is the blocked baseline. A ready-only D449 registry is
built from the release D448 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d450_runtime_registry_history_demo.py \
        artifacts/d449-real/registry \
        artifacts/d448-real/release \
        --destination artifacts/d450-real \
...
from glio_noncode import downloaded_data_quality_d449_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d450-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D450 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D449 registry decisions with D450")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D449 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D448 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d450-release-history")
