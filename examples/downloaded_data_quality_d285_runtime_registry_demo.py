"""Aggregate D284 decisions with the D285 runtime registry contract.

The input directories are exact four-file D284 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d285_runtime_registry_demo.py \
        artifacts/d284-real/strict \
        artifacts/d284-real/release \
        --destination artifacts/d285-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d285-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D285 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d285-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D284 runtimes with D285")
    parser.add_argument("strict_directory", type=Path, help="blocked D284 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D284 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d285-release-registry")
