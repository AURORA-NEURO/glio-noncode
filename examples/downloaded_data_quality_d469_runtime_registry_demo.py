"""Aggregate D468 decisions with the D469 runtime registry contract.

The input directories are exact four-file D468 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d469_runtime_registry_demo.py \
        artifacts/d468-real/strict \
        artifacts/d468-real/release \
        --destination artifacts/d469-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d469-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D469 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d469-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D468 runtimes with D469")
    parser.add_argument("strict_directory", type=Path, help="blocked D468 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D468 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d469-release-registry")
