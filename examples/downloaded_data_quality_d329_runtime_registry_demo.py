"""Aggregate D328 decisions with the D329 runtime registry contract.

The input directories are exact four-file D328 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d329_runtime_registry_demo.py \
        artifacts/d328-real/strict \
        artifacts/d328-real/release \
        --destination artifacts/d329-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d329-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D329 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d329-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D328 runtimes with D329")
    parser.add_argument("strict_directory", type=Path, help="blocked D328 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D328 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d329-release-registry")
