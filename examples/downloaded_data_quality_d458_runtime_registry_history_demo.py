"""Record D457 registry decisions in an append-only D458 history.

The mixed D457 registry is the blocked baseline. A ready-only D457 registry is
built from the release D456 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d458_runtime_registry_history_demo.py \
        artifacts/d457-real/registry \
        artifacts/d456-real/release \
        --destination artifacts/d458-real \
...
from glio_noncode import downloaded_data_quality_d458_runtime_registry_history as history_model
from glio_noncode import downloaded_data_quality_d458_runtime_registry_history_audit as audit_model
from glio_noncode import downloaded_data_quality_d458_runtime_registry_history_query as query_model
from glio_noncode import downloaded_data_quality_d458_runtime_registry_history_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d458-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D458 history."""

    blocked_registry = history_model.load_registry_snapshot(blocked_registry_directory)
    release_runtime = history_model.load_runtime_snapshot(release_runtime_directory)
    ready_registry = history_model.build_registry_snapshot((release_runtime,), registry_id=blocked_registry.registry_id)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D457 registry decisions with D458")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D457 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D456 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d458-release-history")
