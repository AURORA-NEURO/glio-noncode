"""Compare D450 registry histories with the D451 history diff contract.

The baseline is the blocked-only D450 history and the candidate is the
blocked-to-ready D450 history. D451 preserves both history addresses,
classifies each ordinal, folds release movement, and emits independent audits.

Example::

    python examples/downloaded_data_quality_d451_history_diff_demo.py \
        artifacts/d450-baseline/history \
        artifacts/d450-real/history \
        --destination artifacts/d451-real \
...
def build_demo(baseline_directory: str | Path, candidate_directory: str | Path, destination: str | Path | None = None, *, source_zip: str | Path | None = None, diff_id: str = "glio-noncode-d451-history-diff", change_filter: str = "added") -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D451 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D450 registry histories with D451")
    parser.add_argument("baseline_directory", type=Path, help="baseline D450 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D450 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d451-history-diff")
