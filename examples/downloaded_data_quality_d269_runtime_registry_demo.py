"""Aggregate D268 decisions with the D269 runtime registry contract.

The input directories are exact four-file D268 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d269_runtime_registry_demo.py \
        artifacts/d268-real/strict \
        artifacts/d268-real/release \
        --destination artifacts/d269-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d269-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D269 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d269-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D268 runtimes with D269")
    parser.add_argument("strict_directory", type=Path, help="blocked D268 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D268 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d269-release-registry")
