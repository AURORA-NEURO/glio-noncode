"""Aggregate D312 decisions with the D313 runtime registry contract.

The input directories are exact four-file D312 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d313_runtime_registry_demo.py \
        artifacts/d312-real/strict \
        artifacts/d312-real/release \
        --destination artifacts/d313-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d313-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D313 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d313-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D312 runtimes with D313")
    parser.add_argument("strict_directory", type=Path, help="blocked D312 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D312 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d313-release-registry")
