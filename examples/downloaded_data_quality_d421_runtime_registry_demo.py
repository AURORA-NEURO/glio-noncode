"""Aggregate D420 decisions with the D421 runtime registry contract.

The input directories are exact four-file D420 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d421_runtime_registry_demo.py \
        artifacts/d420-real/strict \
        artifacts/d420-real/release \
        --destination artifacts/d421-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d421-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D421 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d421-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D420 runtimes with D421")
    parser.add_argument("strict_directory", type=Path, help="blocked D420 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D420 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d421-release-registry")
