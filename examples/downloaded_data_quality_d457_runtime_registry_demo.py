"""Aggregate D456 decisions with the D457 runtime registry contract.

The input directories are exact four-file D456 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d457_runtime_registry_demo.py \
        artifacts/d456-real/strict \
        artifacts/d456-real/release \
        --destination artifacts/d457-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d457-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D457 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d457-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D456 runtimes with D457")
    parser.add_argument("strict_directory", type=Path, help="blocked D456 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D456 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d457-release-registry")
