"""Evaluate the D475 history diff as a D476 release decision.

The D475 comparison is derived from the downloaded source archive and carries
one added snapshot plus one unchanged snapshot. D476 evaluates that diff under
strict and release policies, persisting independent runtime and query audits.

Example::

    python examples/downloaded_data_quality_d476_history_diff_runtime_demo.py \
        artifacts/d475-real/diff \
        --destination artifacts/d476-real \
...
from glio_noncode import downloaded_data_quality_d476_history_diff_runtime_audit as audit_model
from glio_noncode import downloaded_data_quality_d476_history_diff_runtime_query as query_model
from glio_noncode import downloaded_data_quality_d476_history_diff_runtime_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    """Build, audit, query, and optionally persist two D476 decisions."""

    diff = diff_model.load_diff(diff_directory)
    strict_policy = runtime_model.build_policy("glio-noncode-d476-strict-policy", diff.diff_id, maximum_added=strict_max_added, maximum_removed=0, maximum_changed=0, allowed_directions=("improved", "changed", "unchanged"), require_accepted=True, require_state_change=False, allow_unchanged=True)
    release_policy = runtime_model.build_policy("glio-noncode-d476-release-policy", diff.diff_id, maximum_added=release_max_added, maximum_removed=0, maximum_changed=release_max_changed, allowed_directions=("improved",), require_accepted=True, require_state_change=True, allow_unchanged=True)
    strict = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d476-strict-runtime", policy=strict_policy)
    release = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d476-release-runtime", policy=release_policy)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a D475 history diff with D476")
