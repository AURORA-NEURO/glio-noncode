"""Aggregate D348 decisions with the D349 runtime registry contract.

The input directories are exact four-file D348 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d349_runtime_registry_demo.py \
        artifacts/d348-real/strict \
        artifacts/d348-real/release \
        --destination artifacts/d349-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d349-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D349 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d349-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D348 runtimes with D349")
    parser.add_argument("strict_directory", type=Path, help="blocked D348 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D348 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d349-release-registry")
