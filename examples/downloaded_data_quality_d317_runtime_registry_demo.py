"""Aggregate D316 decisions with the D317 runtime registry contract.

The input directories are exact four-file D316 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d317_runtime_registry_demo.py \
        artifacts/d316-real/strict \
        artifacts/d316-real/release \
        --destination artifacts/d317-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d317-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D317 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d317-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D316 runtimes with D317")
    parser.add_argument("strict_directory", type=Path, help="blocked D316 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D316 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d317-release-registry")
