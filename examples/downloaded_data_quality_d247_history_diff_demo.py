"""Compare D246 registry histories with the D247 history diff contract.

The baseline is the blocked-only D246 history and the candidate is the
blocked-to-ready D246 history. D247 preserves both history addresses,
classifies each ordinal, folds release movement, and emits independent audits.

Example::

    python examples/downloaded_data_quality_d247_history_diff_demo.py \
        artifacts/d246-baseline/history \
        artifacts/d246-real/history \
        --destination artifacts/d247-real \
...
def build_demo(baseline_directory: str | Path, candidate_directory: str | Path, destination: str | Path | None = None, *, source_zip: str | Path | None = None, diff_id: str = "glio-noncode-d247-history-diff", change_filter: str = "added") -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D247 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D246 registry histories with D247")
    parser.add_argument("baseline_directory", type=Path, help="baseline D246 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D246 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d247-history-diff")
