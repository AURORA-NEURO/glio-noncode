"""Aggregate D464 decisions with the D465 runtime registry contract.

The input directories are exact four-file D464 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d465_runtime_registry_demo.py \
        artifacts/d464-real/strict \
        artifacts/d464-real/release \
        --destination artifacts/d465-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d465-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D465 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d465-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D464 runtimes with D465")
    parser.add_argument("strict_directory", type=Path, help="blocked D464 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D464 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d465-release-registry")
