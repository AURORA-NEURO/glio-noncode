"""Aggregate D368 decisions with the D369 runtime registry contract.

The input directories are exact four-file D368 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d369_runtime_registry_demo.py \
        artifacts/d368-real/strict \
        artifacts/d368-real/release \
        --destination artifacts/d369-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d369-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D369 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d369-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D368 runtimes with D369")
    parser.add_argument("strict_directory", type=Path, help="blocked D368 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D368 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d369-release-registry")
