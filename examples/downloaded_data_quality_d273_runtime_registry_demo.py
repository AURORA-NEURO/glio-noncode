"""Aggregate D272 decisions with the D273 runtime registry contract.

The input directories are exact four-file D272 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d273_runtime_registry_demo.py \
        artifacts/d272-real/strict \
        artifacts/d272-real/release \
        --destination artifacts/d273-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d273-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D273 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d273-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D272 runtimes with D273")
    parser.add_argument("strict_directory", type=Path, help="blocked D272 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D272 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d273-release-registry")
