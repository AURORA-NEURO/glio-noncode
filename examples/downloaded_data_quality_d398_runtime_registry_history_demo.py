"""Record D397 registry decisions in an append-only D398 history.

The mixed D397 registry is the blocked baseline. A ready-only D397 registry is
built from the release D396 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d398_runtime_registry_history_demo.py \
        artifacts/d397-real/registry \
        artifacts/d396-real/release \
        --destination artifacts/d398-real \
...
from glio_noncode import downloaded_data_quality_d397_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d398-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D398 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D397 registry decisions with D398")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D397 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D396 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d398-release-history")
