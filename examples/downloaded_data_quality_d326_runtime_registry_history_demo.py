"""Record D325 registry decisions in an append-only D326 history.

The mixed D325 registry is the blocked baseline. A ready-only D325 registry is
built from the release D324 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d326_runtime_registry_history_demo.py \
        artifacts/d325-real/registry \
        artifacts/d324-real/release \
        --destination artifacts/d326-real \
...
from glio_noncode import downloaded_data_quality_d325_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d326-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D326 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D325 registry decisions with D326")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D325 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D324 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d326-release-history")
