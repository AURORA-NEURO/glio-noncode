"""Record D341 registry decisions in an append-only D342 history.

The mixed D341 registry is the blocked baseline. A ready-only D341 registry is
built from the release D340 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d342_runtime_registry_history_demo.py \
        artifacts/d341-real/registry \
        artifacts/d340-real/release \
        --destination artifacts/d342-real \
...
from glio_noncode import downloaded_data_quality_d341_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d342-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D342 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D341 registry decisions with D342")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D341 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D340 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d342-release-history")
