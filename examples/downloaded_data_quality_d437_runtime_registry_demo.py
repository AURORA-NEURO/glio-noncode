"""Aggregate D436 decisions with the D437 runtime registry contract.

The input directories are exact four-file D436 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d437_runtime_registry_demo.py \
        artifacts/d436-real/strict \
        artifacts/d436-real/release \
        --destination artifacts/d437-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d437-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D437 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d437-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D436 runtimes with D437")
    parser.add_argument("strict_directory", type=Path, help="blocked D436 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D436 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d437-release-registry")
