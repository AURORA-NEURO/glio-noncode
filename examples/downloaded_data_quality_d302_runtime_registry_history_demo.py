"""Record D301 registry decisions in an append-only D302 history.

The mixed D301 registry is the blocked baseline. A ready-only D301 registry is
built from the release D300 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d302_runtime_registry_history_demo.py \
        artifacts/d301-real/registry \
        artifacts/d300-real/release \
        --destination artifacts/d302-real \
...
from glio_noncode import downloaded_data_quality_d301_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d302-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D302 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D301 registry decisions with D302")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D301 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D300 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d302-release-history")
