"""Aggregate D404 decisions with the D405 runtime registry contract.

The input directories are exact four-file D404 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d405_runtime_registry_demo.py \
        artifacts/d404-real/strict \
        artifacts/d404-real/release \
        --destination artifacts/d405-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d405-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D405 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d405-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D404 runtimes with D405")
    parser.add_argument("strict_directory", type=Path, help="blocked D404 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D404 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d405-release-registry")
