"""Record D389 registry decisions in an append-only D390 history.

The mixed D389 registry is the blocked baseline. A ready-only D389 registry is
built from the release D388 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d390_runtime_registry_history_demo.py \
        artifacts/d389-real/registry \
        artifacts/d388-real/release \
        --destination artifacts/d390-real \
...
from glio_noncode import downloaded_data_quality_d389_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d390-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D390 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D389 registry decisions with D390")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D389 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D388 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d390-release-history")
