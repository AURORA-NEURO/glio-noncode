"""Evaluate the D235 history diff as a D236 release decision.

The D235 comparison is derived from the downloaded source archive and carries
one added ready snapshot plus one unchanged blocked snapshot. D236 evaluates
that diff under strict and release policies, persisting independent runtime
and query audits.

Example::

    python examples/downloaded_data_quality_d236_history_diff_runtime_demo.py \
        artifacts/d235-real/diff \
        --destination artifacts/d236-real \
...
def build_demo(
    ...
    *,
    ...
    """Build, audit, query, and optionally persist two D236 decisions."""

    diff = diff_model.load_diff(diff_directory)
    strict_policy = runtime_model.build_policy("glio-noncode-d236-strict-policy", diff.diff_id, maximum_added=strict_max_added, maximum_removed=0, maximum_changed=0, allowed_directions=("improved", "changed", "unchanged"), require_accepted=True, require_state_change=False, allow_unchanged=True)
    release_policy = runtime_model.build_policy("glio-noncode-d236-release-policy", diff.diff_id, maximum_added=release_max_added, maximum_removed=0, maximum_changed=0, allowed_directions=("improved",), require_accepted=True, require_state_change=True, allow_unchanged=True)
    strict = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d236-strict-runtime", policy=strict_policy)
    release = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d236-release-runtime", policy=release_policy)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a D235 history diff with D236")
    parser.add_argument("diff_directory", type=Path, help="exact four-file D235 diff directory")
