"""Aggregate D320 decisions with the D321 runtime registry contract.

The input directories are exact four-file D320 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d321_runtime_registry_demo.py \
        artifacts/d320-real/strict \
        artifacts/d320-real/release \
        --destination artifacts/d321-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d321-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D321 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d321-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D320 runtimes with D321")
    parser.add_argument("strict_directory", type=Path, help="blocked D320 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D320 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d321-release-registry")
