"""Record D317 registry decisions in an append-only D318 history.

The mixed D317 registry is the blocked baseline. A ready-only D317 registry is
built from the release D316 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d318_runtime_registry_history_demo.py \
        artifacts/d317-real/registry \
        artifacts/d316-real/release \
        --destination artifacts/d318-real \
...
from glio_noncode import downloaded_data_quality_d317_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d318-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D318 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D317 registry decisions with D318")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D317 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D316 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d318-release-history")
