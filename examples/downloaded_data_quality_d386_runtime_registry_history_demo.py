"""Record D385 registry decisions in an append-only D386 history.

The mixed D385 registry is the blocked baseline. A ready-only D385 registry is
...
Example::
    ...
    python examples/downloaded_data_quality_d386_runtime_registry_history_demo.py \
        artifacts/d385-real/registry \
        artifacts/d384-real/release \
        --destination artifacts/d386-real \
...
from glio_noncode import downloaded_data_quality_d385_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d386-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D386 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D385 registry decisions with D386")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D385 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D384 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d386-release-history")
