"""Record D369 registry decisions in an append-only D370 history.

The mixed D369 registry is the blocked baseline. A ready-only D369 registry is
built from the release D368 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d370_runtime_registry_history_demo.py \
        artifacts/d369-real/registry \
        artifacts/d368-real/release \
        --destination artifacts/d370-real \
...
from glio_noncode import downloaded_data_quality_d369_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d370-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D370 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D369 registry decisions with D370")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D369 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D368 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d370-release-history")
