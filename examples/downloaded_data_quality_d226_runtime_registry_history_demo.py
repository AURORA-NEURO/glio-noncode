"""Record D225 registry decisions in an append-only D226 history.

The mixed D225 registry is the blocked baseline. A ready-only D225 registry is
built from the release D224 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d226_runtime_registry_history_demo.py \
        artifacts/d225-real/registry \
        artifacts/d224-real/release \
        --destination artifacts/d226-real \
...
from glio_noncode import downloaded_data_quality_d225_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d226-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D226 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D225 registry decisions with D226")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D225 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D224 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d226-release-history")
