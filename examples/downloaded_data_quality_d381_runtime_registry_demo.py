"""Aggregate D380 decisions with the D381 runtime registry contract.

The input directories are exact four-file D380 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d381_runtime_registry_demo.py \
        artifacts/d380-real/strict \
        artifacts/d380-real/release \
        --destination artifacts/d381-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d381-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D381 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d381-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D380 runtimes with D381")
    parser.add_argument("strict_directory", type=Path, help="blocked D380 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D380 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d381-release-registry")
