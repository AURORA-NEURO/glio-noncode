"""Record D233 registry decisions in an append-only D234 history.

The mixed D233 registry is the blocked baseline. A ready-only D233 registry is
built from the release D232 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d234_runtime_registry_history_demo.py \
        artifacts/d233-real/registry \
        artifacts/d232-real/release \
        --destination artifacts/d234-real \
...
from glio_noncode import downloaded_data_quality_d233_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d234-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D234 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D233 registry decisions with D234")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D233 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D232 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d234-release-history")
