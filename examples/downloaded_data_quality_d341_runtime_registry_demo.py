"""Aggregate D340 decisions with the D341 runtime registry contract.

The input directories are exact four-file D340 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d341_runtime_registry_demo.py \
        artifacts/d340-real/strict \
        artifacts/d340-real/release \
        --destination artifacts/d341-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d341-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D341 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d341-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D340 runtimes with D341")
    parser.add_argument("strict_directory", type=Path, help="blocked D340 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D340 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d341-release-registry")
