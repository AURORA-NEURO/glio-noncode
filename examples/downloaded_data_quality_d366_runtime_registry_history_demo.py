"""Record D365 registry decisions in an append-only D366 history.

The mixed D365 registry is the blocked baseline. A ready-only D365 registry is
built from the release D364 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d366_runtime_registry_history_demo.py \
        artifacts/d365-real/registry \
        artifacts/d364-real/release \
        --destination artifacts/d366-real \
...
from glio_noncode import downloaded_data_quality_d365_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d366-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D366 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D365 registry decisions with D366")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D365 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D364 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d366-release-history")
