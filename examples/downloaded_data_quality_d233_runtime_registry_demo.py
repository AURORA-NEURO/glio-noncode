"""Aggregate D232 decisions with the D233 runtime registry contract.

The input directories are exact four-file D232 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d233_runtime_registry_demo.py \
        artifacts/d232-real/strict \
        artifacts/d232-real/release \
        --destination artifacts/d233-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d233-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D233 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d233-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D232 runtimes with D233")
    parser.add_argument("strict_directory", type=Path, help="blocked D232 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D232 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d233-release-registry")
