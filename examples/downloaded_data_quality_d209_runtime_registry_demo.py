"""Aggregate D208 decisions with the D209 runtime registry contract.

The input directories are exact four-file D208 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d209_runtime_registry_demo.py \
        artifacts/d208-real/strict \
        artifacts/d208-real/release \
        --destination artifacts/d209-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d209-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D209 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d209-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D208 runtimes with D209")
    parser.add_argument("strict_directory", type=Path, help="blocked D208 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D208 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d209-release-registry")
