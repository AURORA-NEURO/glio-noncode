"""Record D393 registry decisions in an append-only D394 history.

The mixed D393 registry is the blocked baseline. A ready-only D393 registry is
built from the release D392 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d394_runtime_registry_history_demo.py \
        artifacts/d393-real/registry \
        artifacts/d392-real/release \
        --destination artifacts/d394-real \
...
from glio_noncode import downloaded_data_quality_d393_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d394-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D394 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D393 registry decisions with D394")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D393 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D392 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d394-release-history")
