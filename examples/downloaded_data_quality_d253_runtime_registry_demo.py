"""Aggregate D252 decisions with the D253 runtime registry contract.

The input directories are exact four-file D252 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d253_runtime_registry_demo.py \
        artifacts/d252-real/strict \
        artifacts/d252-real/release \
        --destination artifacts/d253-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d253-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D253 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d253-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D252 runtimes with D253")
    parser.add_argument("strict_directory", type=Path, help="blocked D252 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D252 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d253-release-registry")
