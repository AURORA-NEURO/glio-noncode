"""Aggregate D364 decisions with the D365 runtime registry contract.

The input directories are exact four-file D364 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d365_runtime_registry_demo.py \
        artifacts/d364-real/strict \
        artifacts/d364-real/release \
        --destination artifacts/d365-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d365-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D365 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d365-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D364 runtimes with D365")
    parser.add_argument("strict_directory", type=Path, help="blocked D364 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D364 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d365-release-registry")
