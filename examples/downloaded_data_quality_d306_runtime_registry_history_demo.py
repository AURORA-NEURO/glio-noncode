"""Record D305 registry decisions in an append-only D306 history.

The mixed D305 registry is the blocked baseline. A ready-only D305 registry is
built from the release D304 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d306_runtime_registry_history_demo.py \
        artifacts/d305-real/registry \
        artifacts/d304-real/release \
        --destination artifacts/d306-real \
...
from glio_noncode import downloaded_data_quality_d305_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d306-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D306 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D305 registry decisions with D306")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D305 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D304 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d306-release-history")
