"""Record D281 registry decisions in an append-only D282 history.

The mixed D281 registry is the blocked baseline. A ready-only D281 registry is
built from the release D280 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d282_runtime_registry_history_demo.py \
        artifacts/d281-real/registry \
        artifacts/d280-real/release \
        --destination artifacts/d282-real \
...
from glio_noncode import downloaded_data_quality_d281_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d282-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D282 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D281 registry decisions with D282")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D281 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D280 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d282-release-history")
