"""Aggregate D444 decisions with the D445 runtime registry contract.

The input directories are exact four-file D444 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d445_runtime_registry_demo.py \
        artifacts/d444-real/strict \
        artifacts/d444-real/release \
        --destination artifacts/d445-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d445-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D445 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d445-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D444 runtimes with D445")
    parser.add_argument("strict_directory", type=Path, help="blocked D444 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D444 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d445-release-registry")
