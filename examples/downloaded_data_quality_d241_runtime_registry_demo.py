"""Aggregate D240 decisions with the D241 runtime registry contract.

The input directories are exact four-file D240 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d241_runtime_registry_demo.py \
        artifacts/d240-real/strict \
        artifacts/d240-real/release \
        --destination artifacts/d241-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d241-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D241 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d241-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D240 runtimes with D241")
    parser.add_argument("strict_directory", type=Path, help="blocked D240 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D240 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d241-release-registry")
