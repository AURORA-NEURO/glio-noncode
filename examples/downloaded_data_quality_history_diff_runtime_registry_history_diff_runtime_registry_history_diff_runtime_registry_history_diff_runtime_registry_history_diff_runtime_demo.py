"""Evaluate the D199 history diff as a D200 release decision.

The D199 comparison is derived from the downloaded source archive and carries
one added ready snapshot plus one unchanged blocked snapshot.  D200 evaluates
that diff twice: a zero-addition strict policy blocks it, while a one-addition
improved-transition release policy is ready.  Both decisions are persisted
with independent runtime and query audits.

Example::

    python examples/downloaded_data_quality_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_demo.py \
        artifacts/d199-real/diff \
        --destination artifacts/d200-real \
...
from glio_noncode import downloaded_data_quality_runtime_history_release_evidence_history_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_audit as audit_model
from glio_noncode import downloaded_data_quality_runtime_history_release_evidence_history_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_query as query_model
from glio_noncode import downloaded_data_quality_runtime_history_release_evidence_history_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    """Build, audit, query, and optionally persist two D200 decisions."""

    diff = diff_model.load_diff(diff_directory)
    strict_policy = runtime_model.build_policy(
        "glio-noncode-d200-strict-policy",
        diff.diff_id,
    ...
    release_policy = runtime_model.build_policy(
        "glio-noncode-d200-release-policy",
        diff.diff_id,
    ...
    strict = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d200-strict-runtime", policy=strict_policy)
    release = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d200-release-runtime", policy=release_policy)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a D199 history diff with D200")
    parser.add_argument("diff_directory", type=Path, help="exact four-file D199 diff directory")
