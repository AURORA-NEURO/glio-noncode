"""Record D329 registry decisions in an append-only D330 history.

The mixed D329 registry is the blocked baseline. A ready-only D329 registry is
built from the release D328 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d330_runtime_registry_history_demo.py \
        artifacts/d329-real/registry \
        artifacts/d328-real/release \
        --destination artifacts/d330-real \
...
from glio_noncode import downloaded_data_quality_d329_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d330-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D330 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D329 registry decisions with D330")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D329 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D328 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d330-release-history")
