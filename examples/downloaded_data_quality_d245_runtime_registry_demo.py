"""Aggregate D244 decisions with the D245 runtime registry contract.

The input directories are exact four-file D244 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d245_runtime_registry_demo.py \
        artifacts/d244-real/strict \
        artifacts/d244-real/release \
        --destination artifacts/d245-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d245-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D245 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d245-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D244 runtimes with D245")
    parser.add_argument("strict_directory", type=Path, help="blocked D244 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D244 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d245-release-registry")
