"""Aggregate D448 decisions with the D449 runtime registry contract.

The input directories are exact four-file D448 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d449_runtime_registry_demo.py \
        artifacts/d448-real/strict \
        artifacts/d448-real/release \
        --destination artifacts/d449-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d449-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D449 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d449-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D448 runtimes with D449")
    parser.add_argument("strict_directory", type=Path, help="blocked D448 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D448 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d449-release-registry")
