"""Record D337 registry decisions in an append-only D338 history.

The mixed D337 registry is the blocked baseline. A ready-only D337 registry is
built from the release D336 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d338_runtime_registry_history_demo.py \
        artifacts/d337-real/registry \
        artifacts/d336-real/release \
        --destination artifacts/d338-real \
...
from glio_noncode import downloaded_data_quality_d337_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d338-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D338 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D337 registry decisions with D338")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D337 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D336 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d338-release-history")
