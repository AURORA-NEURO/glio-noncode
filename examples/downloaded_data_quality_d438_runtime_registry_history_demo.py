"""Record D437 registry decisions in an append-only D438 history.

The mixed D437 registry is the blocked baseline. A ready-only D437 registry is
built from the release D436 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d438_runtime_registry_history_demo.py \
        artifacts/d437-real/registry \
        artifacts/d436-real/release \
        --destination artifacts/d438-real \
...
from glio_noncode import downloaded_data_quality_d437_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d438-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D438 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D437 registry decisions with D438")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D437 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D436 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d438-release-history")
