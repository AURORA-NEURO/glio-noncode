"""Record D417 registry decisions in an append-only D418 history.

The mixed D417 registry is the blocked baseline. A ready-only D417 registry is
...
Example::
    ...
    python examples/downloaded_data_quality_d418_runtime_registry_history_demo.py \
        artifacts/d417-real/registry \
        artifacts/d384-real/release \
        --destination artifacts/d418-real \
...
from glio_noncode import downloaded_data_quality_d417_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d418-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D418 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D417 registry decisions with D418")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D417 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D416 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d418-release-history")
