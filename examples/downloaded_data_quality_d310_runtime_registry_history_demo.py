"""Record D309 registry decisions in an append-only D310 history.

The mixed D309 registry is the blocked baseline. A ready-only D309 registry is
built from the release D308 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d310_runtime_registry_history_demo.py \
        artifacts/d309-real/registry \
        artifacts/d308-real/release \
        --destination artifacts/d310-real \
...
from glio_noncode import downloaded_data_quality_d309_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d310-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D310 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D309 registry decisions with D310")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D309 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D308 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d310-release-history")
