"""Record D293 registry decisions in an append-only D294 history.

The mixed D293 registry is the blocked baseline. A ready-only D293 registry is
built from the release D292 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d294_runtime_registry_history_demo.py \
        artifacts/d293-real/registry \
        artifacts/d292-real/release \
        --destination artifacts/d294-real \
...
from glio_noncode import downloaded_data_quality_d293_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d294-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D294 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D293 registry decisions with D294")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D293 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D292 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d294-release-history")
