"""Aggregate D428 decisions with the D429 runtime registry contract.

The input directories are exact four-file D428 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d429_runtime_registry_demo.py \
        artifacts/d428-real/strict \
        artifacts/d428-real/release \
        --destination artifacts/d429-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d429-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D429 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d429-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D428 runtimes with D429")
    parser.add_argument("strict_directory", type=Path, help="blocked D428 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D428 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d429-release-registry")
