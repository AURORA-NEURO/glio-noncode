"""Aggregate D292 decisions with the D293 runtime registry contract.

The input directories are exact four-file D292 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d293_runtime_registry_demo.py \
        artifacts/d292-real/strict \
        artifacts/d292-real/release \
        --destination artifacts/d293-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d293-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D293 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d293-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D292 runtimes with D293")
    parser.add_argument("strict_directory", type=Path, help="blocked D292 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D292 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d293-release-registry")
