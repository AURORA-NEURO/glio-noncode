"""Record D261 registry decisions in an append-only D262 history.

The mixed D261 registry is the blocked baseline. A ready-only D261 registry is
built from the release D260 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d262_runtime_registry_history_demo.py \
        artifacts/d261-real/registry \
        artifacts/d260-real/release \
        --destination artifacts/d262-real \
...
from glio_noncode import downloaded_data_quality_d261_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d262-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D262 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D261 registry decisions with D262")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D261 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D260 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d262-release-history")
