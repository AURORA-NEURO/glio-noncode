"""Compare D206 registry histories with the D207 history diff contract.

The baseline is the blocked-only D206 history and the candidate is the
blocked-to-ready D206 history. D207 preserves both history addresses,
classifies each ordinal, folds the release direction, and exposes independent
diff and query audits.

Example::

    python examples/downloaded_data_quality_d207_history_diff_demo.py \
        artifacts/d206-baseline/history \
        artifacts/d206-real/history \
        --destination artifacts/d207-real \
...
def build_demo(
    ...
    *,
    ...
    diff_id: str = "glio-noncode-d207-history-diff",
    change_filter: str = "added",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D207 history diff."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare D206 registry histories with D207")
    parser.add_argument("baseline_directory", type=Path, help="baseline D206 history directory")
    parser.add_argument("candidate_directory", type=Path, help="candidate D206 history directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--diff-id", default="glio-noncode-d207-history-diff")
