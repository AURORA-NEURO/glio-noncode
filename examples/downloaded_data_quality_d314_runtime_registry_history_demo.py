"""Record D313 registry decisions in an append-only D314 history.

The mixed D313 registry is the blocked baseline. A ready-only D313 registry is
built from the release D312 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d314_runtime_registry_history_demo.py \
        artifacts/d313-real/registry \
        artifacts/d312-real/release \
        --destination artifacts/d314-real \
...
from glio_noncode import downloaded_data_quality_d313_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d314-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D314 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D313 registry decisions with D314")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D313 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D312 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d314-release-history")
