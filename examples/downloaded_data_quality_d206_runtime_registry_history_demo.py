"""Record D205 registry decisions in an append-only D206 history.

The mixed D205 registry is the blocked baseline. A ready-only D205 registry is
built from the release D204 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. The example persists the
history, independent audit, readiness-filtered query, and query audit.

Example::

    python examples/downloaded_data_quality_d206_runtime_registry_history_demo.py \
        artifacts/d205-real/registry \
        artifacts/d204-real/release \
        --destination artifacts/d206-real \
...
from glio_noncode import downloaded_data_quality_d205_runtime_registry as registry_model
from glio_noncode import downloaded_data_quality_d206_runtime_registry_history as history_model
from glio_noncode import downloaded_data_quality_d206_runtime_registry_history_audit as audit_model
from glio_noncode import downloaded_data_quality_d206_runtime_registry_history_query as query_model
from glio_noncode import downloaded_data_quality_d206_runtime_registry_history_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d206-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D206 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D205 registry decisions with D206")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D205 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D204 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d206-release-history")
