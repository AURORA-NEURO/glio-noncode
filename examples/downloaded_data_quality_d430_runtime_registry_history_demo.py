"""Record D429 registry decisions in an append-only D430 history.

The mixed D429 registry is the blocked baseline. A ready-only D429 registry is
...
Example::
    ...
    python examples/downloaded_data_quality_d430_runtime_registry_history_demo.py \
        artifacts/d429-real/registry \
        artifacts/d384-real/release \
        --destination artifacts/d430-real \
...
from glio_noncode import downloaded_data_quality_d429_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d430-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D430 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D429 registry decisions with D430")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D429 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D428 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d430-release-history")
