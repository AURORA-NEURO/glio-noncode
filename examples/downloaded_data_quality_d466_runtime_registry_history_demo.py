"""Record D465 registry decisions in an append-only D466 history.

The mixed D465 registry is the blocked baseline. A ready-only D465 registry is
built from the release D464 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d466_runtime_registry_history_demo.py \
        artifacts/d465-real/registry \
        artifacts/d464-real/release \
        --destination artifacts/d466-real \
...
from glio_noncode import downloaded_data_quality_d466_runtime_registry_history_audit as audit_model
from glio_noncode import downloaded_data_quality_d466_runtime_registry_history_query as query_model
from glio_noncode import downloaded_data_quality_d466_runtime_registry_history_query_audit as query_audit_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d466-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D466 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D465 registry decisions with D466")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D465 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D464 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d466-release-history")
