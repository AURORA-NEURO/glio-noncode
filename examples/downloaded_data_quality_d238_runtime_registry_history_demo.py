"""Record D237 registry decisions in an append-only D238 history.

The mixed D237 registry is the blocked baseline. A ready-only D237 registry is
built from the release D236 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d238_runtime_registry_history_demo.py \
        artifacts/d237-real/registry \
        artifacts/d236-real/release \
        --destination artifacts/d238-real \
...
from glio_noncode import downloaded_data_quality_d237_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d238-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D238 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D237 registry decisions with D238")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D237 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D236 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d238-release-history")
