"""Aggregate D248 decisions with the D249 runtime registry contract.

The input directories are exact four-file D248 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d249_runtime_registry_demo.py \
        artifacts/d248-real/strict \
        artifacts/d248-real/release \
        --destination artifacts/d249-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d249-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D249 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d249-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D248 runtimes with D249")
    parser.add_argument("strict_directory", type=Path, help="blocked D248 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D248 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d249-release-registry")
