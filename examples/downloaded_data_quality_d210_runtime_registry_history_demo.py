"""Record D209 registry decisions in an append-only D210 history.

The mixed D209 registry is the blocked baseline. A ready-only D209 registry is
built from the release D208 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d210_runtime_registry_history_demo.py \
        artifacts/d209-real/registry \
        artifacts/d208-real/release \
        --destination artifacts/d210-real \
...
from glio_noncode import downloaded_data_quality_d209_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d210-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D210 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D209 registry decisions with D210")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D209 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D208 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d210-release-history")
