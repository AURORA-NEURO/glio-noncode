"""Evaluate the D203 history diff as a D204 release decision.

The D203 comparison is derived from the downloaded source archive and carries
one added ready snapshot plus one unchanged blocked snapshot. D204 evaluates
that diff twice: a zero-addition strict policy blocks it, while a one-addition
improved-transition release policy is ready. Both decisions are persisted with
independent runtime and query audits.

Example::

    python examples/downloaded_data_quality_d204_history_diff_runtime_demo.py \
        artifacts/d203-real/diff \
        --destination artifacts/d204-real \
...
def build_demo(
    ...
    *,
    ...
    """Build, audit, query, and optionally persist two D204 decisions."""

    diff = diff_model.load_diff(diff_directory)
    strict_policy = runtime_model.build_policy(
        "glio-noncode-d204-strict-policy",
        diff.diff_id,
    ...
    release_policy = runtime_model.build_policy(
        "glio-noncode-d204-release-policy",
        diff.diff_id,
    ...
    strict = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d204-strict-runtime", policy=strict_policy)
    release = runtime_model.run_runtime(diff, runtime_id="glio-noncode-d204-release-runtime", policy=release_policy)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a D203 history diff with D204")
    parser.add_argument("diff_directory", type=Path, help="exact four-file D203 diff directory")
