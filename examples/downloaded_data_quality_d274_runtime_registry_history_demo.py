"""Record D273 registry decisions in an append-only D274 history.

The mixed D273 registry is the blocked baseline. A ready-only D273 registry is
built from the release D272 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d274_runtime_registry_history_demo.py \
        artifacts/d273-real/registry \
        artifacts/d272-real/release \
        --destination artifacts/d274-real \
...
from glio_noncode import downloaded_data_quality_d273_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d274-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D274 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D273 registry decisions with D274")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D273 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D272 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d274-release-history")
