"""Aggregate D324 decisions with the D325 runtime registry contract.

The input directories are exact four-file D324 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d325_runtime_registry_demo.py \
        artifacts/d324-real/strict \
        artifacts/d324-real/release \
        --destination artifacts/d325-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d325-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D325 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d325-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D324 runtimes with D325")
    parser.add_argument("strict_directory", type=Path, help="blocked D324 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D324 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d325-release-registry")
