"""Aggregate D228 decisions with the D229 runtime registry contract.

The input directories are exact four-file D228 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d229_runtime_registry_demo.py \
        artifacts/d228-real/strict \
        artifacts/d228-real/release \
        --destination artifacts/d229-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d229-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D229 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d229-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D228 runtimes with D229")
    parser.add_argument("strict_directory", type=Path, help="blocked D228 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D228 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d229-release-registry")
