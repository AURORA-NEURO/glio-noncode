"""Record D269 registry decisions in an append-only D270 history.

The mixed D269 registry is the blocked baseline. A ready-only D269 registry is
built from the release D268 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d270_runtime_registry_history_demo.py \
        artifacts/d269-real/registry \
        artifacts/d268-real/release \
        --destination artifacts/d270-real \
...
from glio_noncode import downloaded_data_quality_d269_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d270-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D270 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D269 registry decisions with D270")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D269 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D268 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d270-release-history")
