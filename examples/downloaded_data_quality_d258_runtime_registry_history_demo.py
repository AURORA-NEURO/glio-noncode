"""Record D257 registry decisions in an append-only D258 history.

The mixed D257 registry is the blocked baseline. A ready-only D257 registry is
built from the release D256 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d258_runtime_registry_history_demo.py \
        artifacts/d257-real/registry \
        artifacts/d256-real/release \
        --destination artifacts/d258-real \
...
from glio_noncode import downloaded_data_quality_d257_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d258-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D258 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D257 registry decisions with D258")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D257 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D256 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d258-release-history")
