"""Aggregate D216 decisions with the D217 runtime registry contract.

The input directories are exact four-file D216 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d217_runtime_registry_demo.py \
        artifacts/d216-real/strict \
        artifacts/d216-real/release \
        --destination artifacts/d217-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d217-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D217 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d217-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D216 runtimes with D217")
    parser.add_argument("strict_directory", type=Path, help="blocked D216 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D216 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d217-release-registry")
