"""Aggregate D452 decisions with the D453 runtime registry contract.

The input directories are exact four-file D452 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d453_runtime_registry_demo.py \
        artifacts/d452-real/strict \
        artifacts/d452-real/release \
        --destination artifacts/d453-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d453-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D453 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d453-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D452 runtimes with D453")
    parser.add_argument("strict_directory", type=Path, help="blocked D452 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D452 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d453-release-registry")
