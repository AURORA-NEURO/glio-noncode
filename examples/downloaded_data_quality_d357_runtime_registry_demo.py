"""Aggregate D356 decisions with the D357 runtime registry contract.

The input directories are exact four-file D356 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d357_runtime_registry_demo.py \
        artifacts/d356-real/strict \
        artifacts/d356-real/release \
        --destination artifacts/d357-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d357-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D357 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d357-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D356 runtimes with D357")
    parser.add_argument("strict_directory", type=Path, help="blocked D356 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D356 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d357-release-registry")
