"""Aggregate D332 decisions with the D333 runtime registry contract.

The input directories are exact four-file D332 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d333_runtime_registry_demo.py \
        artifacts/d332-real/strict \
        artifacts/d332-real/release \
        --destination artifacts/d333-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d333-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D333 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d333-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D332 runtimes with D333")
    parser.add_argument("strict_directory", type=Path, help="blocked D332 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D332 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d333-release-registry")
