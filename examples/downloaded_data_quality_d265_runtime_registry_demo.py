"""Aggregate D264 decisions with the D265 runtime registry contract.

The input directories are exact four-file D264 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d265_runtime_registry_demo.py \
        artifacts/d264-real/strict \
        artifacts/d264-real/release \
        --destination artifacts/d265-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d265-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D265 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d265-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D264 runtimes with D265")
    parser.add_argument("strict_directory", type=Path, help="blocked D264 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D264 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d265-release-registry")
