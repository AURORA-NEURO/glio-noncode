"""Record D213 registry decisions in an append-only D214 history.

The mixed D213 registry is the blocked baseline. A ready-only D213 registry is
built from the release D212 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d214_runtime_registry_history_demo.py \
        artifacts/d213-real/registry \
        artifacts/d212-real/release \
        --destination artifacts/d214-real \
...
from glio_noncode import downloaded_data_quality_d213_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d214-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D214 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D213 registry decisions with D214")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D213 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D212 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d214-release-history")
