"""Record D445 registry decisions in an append-only D446 history.

The mixed D445 registry is the blocked baseline. A ready-only D445 registry is
built from the release D444 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d446_runtime_registry_history_demo.py \
        artifacts/d445-real/registry \
        artifacts/d444-real/release \
        --destination artifacts/d446-real \
...
from glio_noncode import downloaded_data_quality_d445_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d446-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D446 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D445 registry decisions with D446")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D445 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D444 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d446-release-history")
