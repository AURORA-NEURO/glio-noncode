"""Record D285 registry decisions in an append-only D286 history.

The mixed D285 registry is the blocked baseline. A ready-only D285 registry is
built from the release D284 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d286_runtime_registry_history_demo.py \
        artifacts/d285-real/registry \
        artifacts/d284-real/release \
        --destination artifacts/d286-real \
...
from glio_noncode import downloaded_data_quality_d285_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d286-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D286 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D285 registry decisions with D286")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D285 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D284 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d286-release-history")
