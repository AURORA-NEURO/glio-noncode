"""Record D441 registry decisions in an append-only D442 history.

The mixed D441 registry is the blocked baseline. A ready-only D441 registry is
built from the release D440 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d442_runtime_registry_history_demo.py \
        artifacts/d441-real/registry \
        artifacts/d440-real/release \
        --destination artifacts/d442-real \
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d442-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D442 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D441 registry decisions with D442")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D441 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D440 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d442-release-history")
