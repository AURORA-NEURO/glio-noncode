"""Aggregate D304 decisions with the D305 runtime registry contract.

The input directories are exact four-file D304 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d305_runtime_registry_demo.py \
        artifacts/d304-real/strict \
        artifacts/d304-real/release \
        --destination artifacts/d305-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d305-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D305 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d305-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D304 runtimes with D305")
    parser.add_argument("strict_directory", type=Path, help="blocked D304 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D304 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d305-release-registry")
