"""Record D245 registry decisions in an append-only D246 history.

The mixed D245 registry is the blocked baseline. A ready-only D245 registry is
built from the release D244 runtime and appended with an optimistic head check,
producing a durable ``blocked->ready`` transition. History and query audits
are persisted for inspection.

Example::

    python examples/downloaded_data_quality_d246_runtime_registry_history_demo.py \
        artifacts/d245-real/registry \
        artifacts/d244-real/release \
        --destination artifacts/d246-real \
...
from glio_noncode import downloaded_data_quality_d245_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d246-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D246 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D245 registry decisions with D246")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D245 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D244 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d246-release-history")
