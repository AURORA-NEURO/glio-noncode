"""Record D201 registry decisions in an append-only D202 history.

The mixed D201 registry is the blocked baseline. A ready-only D201 registry is
built from the release D200 runtime and appended with an optimistic head check,
...
Example::
    ...
    python examples/downloaded_data_quality_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_demo.py \
        artifacts/d201-real/registry \
        artifacts/d200-real/release \
        --destination artifacts/d202-real \
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d202-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D202 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D201 registry decisions with D202")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D201 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D200 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d202-release-history")
