"""Aggregate D352 decisions with the D353 runtime registry contract.

The input directories are exact four-file D352 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d353_runtime_registry_demo.py \
        artifacts/d352-real/strict \
        artifacts/d352-real/release \
        --destination artifacts/d353-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d353-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D353 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d353-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D352 runtimes with D353")
    parser.add_argument("strict_directory", type=Path, help="blocked D352 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D352 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d353-release-registry")
