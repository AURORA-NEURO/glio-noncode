"""Record D409 registry decisions in an append-only D410 history.

The mixed D409 registry is the blocked baseline. A ready-only D409 registry is
...
Example::
    ...
    python examples/downloaded_data_quality_d410_runtime_registry_history_demo.py \
        artifacts/d409-real/registry \
        artifacts/d384-real/release \
        --destination artifacts/d410-real \
...
from glio_noncode import downloaded_data_quality_d409_runtime_registry as registry_model
...
def build_demo(
    ...
    *,
    ...
    history_id: str = "glio-noncode-d410-release-history",
) -> dict[str, Any]:
    """Build, append, audit, query, and optionally persist one D410 history."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Record D409 registry decisions with D410")
    parser.add_argument("blocked_registry_directory", type=Path, help="mixed D409 registry directory")
    parser.add_argument("release_runtime_directory", type=Path, help="ready D408 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--history-id", default="glio-noncode-d410-release-history")
