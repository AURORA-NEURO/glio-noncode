"""Aggregate D276 decisions with the D277 runtime registry contract.

The input directories are exact four-file D276 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d277_runtime_registry_demo.py \
        artifacts/d276-real/strict \
        artifacts/d276-real/release \
        --destination artifacts/d277-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d277-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D277 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d277-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D276 runtimes with D277")
    parser.add_argument("strict_directory", type=Path, help="blocked D276 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D276 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d277-release-registry")
