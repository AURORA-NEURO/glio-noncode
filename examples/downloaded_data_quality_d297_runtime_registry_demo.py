"""Aggregate D296 decisions with the D297 runtime registry contract.

The input directories are exact four-file D296 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d297_runtime_registry_demo.py \
        artifacts/d296-real/strict \
        artifacts/d296-real/release \
        --destination artifacts/d297-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d297-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D297 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d297-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D296 runtimes with D297")
    parser.add_argument("strict_directory", type=Path, help="blocked D296 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D296 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d297-release-registry")
