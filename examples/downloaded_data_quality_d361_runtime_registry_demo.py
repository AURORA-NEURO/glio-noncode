"""Aggregate D360 decisions with the D361 runtime registry contract.

The input directories are exact four-file D360 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d361_runtime_registry_demo.py \
        artifacts/d360-real/strict \
        artifacts/d360-real/release \
        --destination artifacts/d361-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d361-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D361 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d361-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D360 runtimes with D361")
    parser.add_argument("strict_directory", type=Path, help="blocked D360 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D360 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d361-release-registry")
