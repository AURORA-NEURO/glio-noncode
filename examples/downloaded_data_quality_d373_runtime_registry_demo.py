"""Aggregate D372 decisions with the D373 runtime registry contract.

The input directories are exact four-file D372 runtimes. Keeping a strict
...
Example::
    ...
    python examples/downloaded_data_quality_d373_runtime_registry_demo.py \
        artifacts/d372-real/strict \
        artifacts/d372-real/release \
        --destination artifacts/d373-real \
...
def build_demo(
    ...
    *,
    ...
    registry_id: str = "glio-noncode-d373-release-registry",
) -> dict[str, Any]:
    """Build, audit, query, and optionally persist one D373 registry."""
    ...
    try:
        registry_model.build_registry((strict, strict), registry_id="glio-noncode-d373-duplicate-check")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Aggregate D372 runtimes with D373")
    parser.add_argument("strict_directory", type=Path, help="blocked D372 runtime directory")
    parser.add_argument("release_directory", type=Path, help="ready D372 runtime directory")
    parser.add_argument("--destination", type=Path)
    parser.add_argument("--source-zip", type=Path, help="optional provenance path for the downloaded source archive")
    parser.add_argument("--registry-id", default="glio-noncode-d373-release-registry")
