...
from .intake_architecture_contracts import IntakeArchitectureFixture, IntakeArchitectureRuntime
...
def intake_architecture_invariants(runtime: IntakeArchitectureRuntime) -> tuple[str, ...]:
    ...
    if len(runtime.evaluation.results) != runtime.evaluation.passed_cases + runtime.evaluation.failed_cases:
    ...
    if any(item.observed_state.value == "accepted" for item in runtime.evaluation.results if item.scenario.value != "positive"):
