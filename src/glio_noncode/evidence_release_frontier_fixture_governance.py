"""Detailed fixture governance plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseFixtureGovernance:
    row_count: int
    accepted: bool
...
def build_evidence_release_fixture_governance(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseFixtureGovernance:
    ...
    return EvidenceReleaseFixtureGovernance(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_fixture_governance(receipt: EvidenceReleaseFixtureGovernance, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "row_count") != expected:
        return False
    return all(
...
def summarize_fixture_governance(receipt: EvidenceReleaseFixtureGovernance) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "row_count"),
...
__all__ = ["EvidenceReleaseFixtureGovernance", "validate_evidence_release_fixture_governance", "build_evidence_release_fixture_governance", "summarize_fixture_governance"]
