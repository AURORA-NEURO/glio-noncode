"""Typed contracts for catalogs of module639 policy-decision transports."""
# ruff: noqa: E501
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_639_ct import PACKAGE_PREFIX as TRANSPORT_PREFIX, PAYLOAD_NAMES, VERSION as TRANSPORT_VERSION
...
class Entry:
    ...
    def __post_init__(self) -> None:
        _label(self.entry_id, "module640 entry ID")
        _label(self.package_id, "module640 package ID")
        _address(self.package_address, "module640 package address", TRANSPORT_PREFIX)
        for field in ("manifest_address", "diff_address", "policy_address", "audit_address"):
            _address(getattr(self, field), f"module640 {field}")
        if self.policy_state not in {"ready", "blocked"} or not isinstance(self.policy_accepted, bool) or not isinstance(self.audit_accepted, bool):
            raise ValidationError("module640 entry decision fields are invalid")
        _count(self.package_byte_count, "module640 package byte count", MAX_PACKAGE_BYTES)
        _count(self.member_count, "module640 member count", MAX_MEMBER_COUNT)
        _address(self.content_address, "module640 entry address", ENTRY_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_entry(self) != self.content_address:
            raise ValidationError("module640 entry address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Entry":
        _strict(value, set(ENTRY_FIELDS), "module640 entry")
...
class Catalog:
    ...
    def __post_init__(self) -> None:
        _label(self.catalog_id, "module640 catalog ID")
        if self.version != VERSION or self.boundary != BOUNDARY or self.state not in STATES:
            raise ValidationError("module640 catalog identity or state is invalid")
        if not isinstance(self.entries, tuple) or len(self.entries) > MAX_ENTRIES or tuple(item.entry_id for item in self.entries) != tuple(sorted(item.entry_id for item in self.entries)) or len({item.entry_id for item in self.entries}) != len(self.entries) or len({item.package_address for item in self.entries}) != len(self.entries):
            raise ValidationError("module640 catalog entries are not canonical")
        for field in ("entry_count", "accepted_count", "ready_count", "blocked_count"):
            _count(getattr(self, field), f"module640 {field}", MAX_ENTRIES)
        if self.entry_count != len(self.entries) or self.accepted_count != sum(item.audit_accepted for item in self.entries) or self.ready_count != sum(item.policy_state == "ready" for item in self.entries) or self.blocked_count != sum(item.policy_state == "blocked" for item in self.entries):
            raise ValidationError("module640 catalog aggregates do not replay")
        ...
        if self.state != expected or _has_forbidden_key(self.to_dict()):
            raise ValidationError("module640 catalog state or public boundary does not replay")
        _address(self.content_address, "module640 catalog address", CATALOG_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_catalog(self) != self.content_address:
            raise ValidationError("module640 catalog address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Catalog":
        _strict(value, set(CATALOG_FIELDS), "module640 catalog")
...
def entry_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module640 catalog entry", "type": "object", "additionalProperties": False, "required": list(ENTRY_FIELDS), "properties": {"policy_state": {"enum": ["ready", "blocked"]}, "policy_accepted": {"type": "boolean"}, "audit_accepted": {"type": "boolean"}, "package_byte_count": {"type": "integer"}, "member_count": {"type": "integer", "maximum": MAX_MEMBER_COUNT}, "content_address": {"type": "string"}}}


def catalog_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module640 transport catalog", "type": "object", "additionalProperties": False, "required": list(CATALOG_FIELDS), "properties": {"version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "entry_count": {"type": "integer", "maximum": MAX_ENTRIES}, "entries": {"type": "array", "maxItems": MAX_ENTRIES}, "state": {"enum": list(STATES)}, "content_address": {"type": "string"}}}
