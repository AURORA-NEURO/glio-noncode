"""Deterministic recovery plans for exact execution-ledger archive transfers."""
...
DEFAULT_RECOVERY_ID = RECOVERY_PREFIX
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction:
    ...
    def __init__(self, index: int, offset: int, size: int, content_address: str, action_address: str) -> None:
        ...
        if not self.action_address.startswith("pending:") and not self.action_address.endswith(":pending") and address_action(self) != self.action_address:
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery:
    ...
    def __init__(self, recovery_id: str, version: str, boundary: str, transfer_id: str, transfer_address: str, archive_address: str, archive_size: int, chunk_count: int, received_indices: Sequence[int], missing_indices: Sequence[int], received_bytes: int, remaining_bytes: int, actions: Sequence[ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction | Mapping[str, Any]], action_count: int, state: str, decision: str, safe_to_resume: bool, checkpointed: bool, next_index: int, content_address: str) -> None:
        ...
        self.actions = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction) else ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction.from_mapping(item) for item in _sequence(actions, "recovery actions", MAX_ACTIONS))
    ...
    def _validate(self) -> None:
        ...
        if not self.content_address.startswith("pending:") and not self.content_address.endswith(":pending") and address_recovery(self) != self.content_address:
    ...
    def from_mapping(cls, value: Mapping[str, Any]):
        ...
        return cls(value["recovery_id"], value["version"], value["boundary"], value["transfer_id"], value["transfer_address"], value["archive_address"], value["archive_size"], value["chunk_count"], value["received_indices"], value["missing_indices"], value["received_bytes"], value["remaining_bytes"], tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction.from_mapping(item) for item in _sequence(value["actions"], "recovery actions", MAX_ACTIONS)), value["action_count"], value["state"], value["decision"], value["safe_to_resume"], value["checkpointed"], value["next_index"], value["content_address"])


def address_action(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction):
...
def address_recovery(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery):
...
def _actions(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransfer, missing: Sequence[int]) -> tuple[ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction, ...]:
    actions = []
    for index in missing:
        chunk = value.chunks[index]
        pending = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction(chunk.index, chunk.offset, chunk.size, chunk.content_address, "pending:recovery-action")
        actions.append(ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryAction(pending.index, pending.offset, pending.size, pending.content_address, address_action(pending)))
    return tuple(actions)


def _received_parts(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransfer | transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferAssembler) -> tuple[transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransfer, dict[int, bytes]]:
    if isinstance(value, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferAssembler):
        return value.value, dict(value._parts)
    if isinstance(value, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransfer):
        try:
            return value, dict(value.payload_bytes())
        except ValidationError:
            return value, {}
    raise ValidationError("recovery builder requires a typed transfer or assembler")


def build_recovery(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransfer | transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferAssembler, *, recovery_id: str | None = None, checkpointed: bool = False):
    """Derive a path-free recovery plan from complete or partial transfer state."""
    transfer, parts = _received_parts(value)
    ...
    body = {
        "recovery_id": DEFAULT_RECOVERY_ID if recovery_id is None else _label(recovery_id, "recovery ID"),
        "version": VERSION,
        "boundary": BOUNDARY,
        "transfer_id": transfer.transfer_id,
        "transfer_address": transfer.content_address,
        "archive_address": transfer.archive_address,
        "archive_size": transfer.archive_size,
        "chunk_count": transfer.chunk_count,
        "received_indices": received,
        "missing_indices": missing,
        "received_bytes": received_bytes,
        "remaining_bytes": transfer.archive_size - received_bytes,
        "actions": actions,
        "action_count": len(actions),
        "state": "complete" if not missing else "partial",
        "decision": "assemble" if not missing else "resume",
        "safe_to_resume": True,
        "checkpointed": bool(checkpointed),
        "next_index": missing[0] if missing else -1,
    }
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery(**body, content_address="pending:recovery")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery(**body, content_address=address_recovery(provisional))


def build_recovery_from_directory(source: str | Path, *, recovery_id: str | None = None):
...
def recovery_from_mapping(value: Mapping[str, Any]):
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery.from_mapping(value)


def recovery_json(value) -> str:
    return canonical_json(recovery_from_mapping(value.to_dict()).to_dict())


def recovery_csv(value) -> str:
...
def render_recovery_markdown(value) -> str:
    value = recovery_from_mapping(value.to_dict())
    lines = ["# Execution-ledger history-diff archive transfer recovery", "", f"- Recovery: `{value.recovery_id}`", f"- Transfer: `{value.transfer_id}`", f"- State: `{value.state}`", f"- Decision: `{value.decision}`", f"- Received: `{len(value.received_indices)}/{value.chunk_count}` chunks", f"- Received bytes: `{value.received_bytes}`", f"- Remaining bytes: `{value.remaining_bytes}`", f"- Next index: `{value.next_index}`", f"- Checkpointed: `{str(value.checkpointed).lower()}`", f"- Address: `{value.content_address}`", "", "| index | offset | bytes | chunk address | action address |", "| ---: | ---: | ---: | --- | --- |"]
