"""Independent assurance for runtime-registry federation archive transfers."""
...
class TransferAudit:
    ...
    def __init__(self, transfer_address: str, archive_address: str, checks: Sequence[TransferAuditCheck], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, TransferAuditCheck) else TransferAuditCheck.from_mapping(item) for item in _sequence(checks, "audit checks", MAX_CHECKS))
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> TransferAudit:
        ...
        return cls(value["transfer_address"], value["archive_address"], tuple(TransferAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "audit checks", MAX_CHECKS)), value["check_count"], value["passed_count"], value["failed_count"], value["accepted"], value["content_address"])
...
def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> TransferAuditCheck:
    provisional = TransferAuditCheck(ordinal, check_id, bool(passed), detail, evidence, "pending:check")
    return TransferAuditCheck(ordinal, check_id, bool(passed), detail, evidence, address_check(provisional))
...
def _replay_checks(value: transfer_model.ArchiveTransfer) -> tuple[TransferAuditCheck, ...]:
    ...
    return (
        ...
        _check(7, "archive-linkage", archive_address.startswith(transfer_model.archive_model.ARCHIVE_PREFIX + ":"), "transfer retains the archive address namespace", evidence),
...
def render_audit_markdown(value: TransferAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Runtime Registry Federation Archive Transfer Audit", "", f"- Transfer: `{value.transfer_address}`", f"- Archive: `{value.archive_address}`", f"- Checks: `{value.passed_count}/{value.check_count}`", f"- Accepted: `{str(value.accepted).lower()}`", f"- Address: `{value.content_address}`", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | `{item.check_id}` | `{str(item.passed).lower()}` | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Runtime registry federation archive transfer audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"type": "string", "enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 16}, "content_address": {"type": "string", "pattern": "^" + CHECK_PREFIX + ":"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Runtime registry federation archive transfer audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"transfer_address": {"type": "string", "pattern": "^" + transfer_model.TRANSFER_PREFIX + ":"}, "archive_address": {"type": "string", "pattern": "^" + transfer_model.archive_model.ARCHIVE_PREFIX + ":"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "check_count": {"type": "integer", "const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "content_address": {"type": "string", "pattern": "^" + AUDIT_PREFIX + ":"}}}
