...
class HistoryDiffArchiveTransferRecoveryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryAuditCheck:
...
class HistoryDiffArchiveTransferRecoveryAudit:
    """Independent audit receipt for a path-free recovery plan."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, recovery_address: str, recovery_id: str, checks: Sequence[HistoryDiffArchiveTransferRecoveryAuditCheck], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryAuditCheck) else HistoryDiffArchiveTransferRecoveryAuditCheck.from_mapping(item) for item in _sequence(checks, "recovery audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryAudit:
        ...
        return cls(value["recovery_address"], value["recovery_id"], tuple(HistoryDiffArchiveTransferRecoveryAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "recovery audit checks", MAX_CHECKS)), value["check_count"], value["passed_count"], value["failed_count"], value["accepted"], value["content_address"])


def address_check(value: HistoryDiffArchiveTransferRecoveryAuditCheck) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryAuditCheck):
        raise ValidationError("recovery audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: HistoryDiffArchiveTransferRecoveryAudit) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryAudit):
        raise ValidationError("recovery audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> HistoryDiffArchiveTransferRecoveryAuditCheck:
    pending = HistoryDiffArchiveTransferRecoveryAuditCheck(ordinal, check_id, bool(passed), detail, evidence, "pending:recovery-audit-check")
    return HistoryDiffArchiveTransferRecoveryAuditCheck(ordinal, check_id, bool(passed), detail, evidence, address_check(pending))


def _expected_actions(value: recovery_model.HistoryDiffArchiveTransferRecovery) -> tuple[dict[str, Any], ...]:
    actions = []
    for item in value.actions:
        pending = recovery_model.HistoryDiffArchiveTransferRecoveryAction(item.index, item.offset, item.size, item.content_address, "pending:recovery-action")
        actions.append(recovery_model.HistoryDiffArchiveTransferRecoveryAction(pending.index, pending.offset, pending.size, pending.content_address, recovery_model.address_action(pending)).to_dict())
    return tuple(actions)


def audit_recovery(value: recovery_model.HistoryDiffArchiveTransferRecovery) -> HistoryDiffArchiveTransferRecoveryAudit:
    """Recompute all recovery conservation and action-plan invariants."""
    if not isinstance(value, recovery_model.HistoryDiffArchiveTransferRecovery):
    ...
    provisional = HistoryDiffArchiveTransferRecoveryAudit(**body, content_address="pending:recovery-audit")
    return HistoryDiffArchiveTransferRecoveryAudit(**body, content_address=address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryAudit:
    return HistoryDiffArchiveTransferRecoveryAudit.from_mapping(value)


def verify_audit(value: HistoryDiffArchiveTransferRecoveryAudit) -> HistoryDiffArchiveTransferRecoveryAudit:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryAudit):
...
def audit_json(value: HistoryDiffArchiveTransferRecoveryAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: HistoryDiffArchiveTransferRecoveryAudit) -> str:
...
def render_audit_markdown(value: HistoryDiffArchiveTransferRecoveryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Federation runtime-registry history-diff archive transfer recovery audit", "", f"- Recovery: `{value.recovery_id}`", f"- Checks: `{value.passed_count}/{value.check_count}`", f"- Accepted: `{str(value.accepted).lower()}`", f"- Address: `{value.content_address}`", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | `{item.check_id}` | `{str(item.passed).lower()}` | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer recovery audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"type": "string", "enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 16}, "content_address": {"type": "string", "pattern": "^" + CHECK_PREFIX + ":"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer recovery audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"recovery_address": {"type": "string", "pattern": "^" + recovery_model.RECOVERY_PREFIX + ":"}, "recovery_id": {"type": "string"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "check_count": {"type": "integer", "const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "content_address": {"type": "string", "pattern": "^" + AUDIT_PREFIX + ":"}}}
