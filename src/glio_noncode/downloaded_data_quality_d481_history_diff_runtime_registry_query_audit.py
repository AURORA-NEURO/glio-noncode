"""Independent replay audit for D481 registry queries."""
...
from . import downloaded_data_quality_d481_history_diff_runtime_registry_query as query_model
...
class QueryAuditCheck:
    ...
    def __init__(self, ordinal: int, check_id: str, passed: bool, actual: Any, expected: Any, detail: str, content_address: str) -> None:
        self.ordinal = _count(ordinal, "runtime registry query audit ordinal", MAX_CHECKS, lower=1); self.check_id = _label(check_id, "runtime registry query audit check ID"); self.passed = _bool(passed, "runtime registry query audit result"); self.actual = _text(actual, "runtime registry query audit actual", 32768); self.expected = _text(expected, "runtime registry query audit expected", 32768); self.detail = _text(detail, "runtime registry query audit detail", 4096); self.content_address = _address(content_address, "runtime registry query audit check address", CHECK_PREFIX); self._validate()
    def _validate(self) -> None:
        if self.check_id not in CHECK_IDS: raise ValidationError("runtime registry query audit identity is unsupported")
        if not self.content_address.startswith("pending:") and _address_for(self, CHECK_PREFIX) != self.content_address: raise ValidationError("runtime registry query audit check address does not replay")
        if not _public(self.to_dict()): raise ValidationError("runtime registry query audit check crosses the public boundary")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "QueryAuditCheck":
        value = _mapping(value, "runtime registry query audit check"); _strict(value, set(cls.FIELDS), "runtime registry query audit check"); return cls(*(value[field] for field in cls.FIELDS))


def address_check(value: QueryAuditCheck) -> str:
    if not isinstance(value, QueryAuditCheck): raise ValidationError("runtime registry query audit check address requires a typed check")
...
class QueryAudit:
    ...
    def __init__(self, query_id: str, registry_id: str, query_address: str, checks: Any, check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.query_id = _label(query_id, "runtime registry query audit query ID"); self.registry_id = _label(registry_id, "runtime registry query audit registry ID"); self.query_address = _address(query_address, "runtime registry query audit query address", query_model.QUERY_PREFIX); self.checks = tuple(item if isinstance(item, QueryAuditCheck) else QueryAuditCheck.from_mapping(_mapping(item, "runtime registry query audit check")) for item in _sequence(checks, "runtime registry query audit checks", MAX_CHECKS)); self.check_count = _count(check_count, "runtime registry query audit check count", MAX_CHECKS); self.passed_count = _count(passed_count, "runtime registry query audit passed count", MAX_CHECKS); self.failed_count = _count(failed_count, "runtime registry query audit failed count", MAX_CHECKS); self.accepted = _bool(accepted, "runtime registry query audit acceptance"); self.content_address = _address(content_address, "runtime registry query audit address", AUDIT_PREFIX); self._validate()
    def _validate(self) -> None:
        if self.check_count != len(self.checks) or self.check_count != MAX_CHECKS or self.passed_count + self.failed_count != self.check_count or self.passed_count != sum(item.passed for item in self.checks) or self.accepted != (self.failed_count == 0): raise ValidationError("runtime registry query audit counters do not replay")
        if tuple(item.ordinal for item in self.checks) != tuple(range(1, MAX_CHECKS + 1)) or tuple(item.check_id for item in self.checks) != CHECK_IDS: raise ValidationError("runtime registry query audit checks are not canonical")
        if not self.content_address.startswith("pending:") and _address_for(self, AUDIT_PREFIX) != self.content_address: raise ValidationError("runtime registry query audit address does not replay")
        if not _public(self.to_dict()): raise ValidationError("runtime registry query audit crosses the public boundary")
    def to_dict(self) -> dict[str, Any]: return {"query_id": self.query_id, "registry_id": self.registry_id, "query_address": self.query_address, "checks": [item.to_dict() for item in self.checks], "check_count": self.check_count, "passed_count": self.passed_count, "failed_count": self.failed_count, "accepted": self.accepted, "content_address": self.content_address}
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "QueryAudit":
        value = _mapping(value, "runtime registry query audit"); _strict(value, set(cls.FIELDS), "runtime registry query audit"); return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: QueryAudit) -> str:
    if not isinstance(value, QueryAudit): raise ValidationError("runtime registry query audit address requires a typed audit")
...
def audit_query(value: query_model.RegistryQuery, registry: registry_model.RuntimeRegistry) -> QueryAudit:
    ...
    checks = (
        ...
        _finding(2, "registry_identity", value.registry_id == registry.registry_id, value.registry_id, registry.registry_id, "query identity must point to the requested registry"),
    ...
    provisional = QueryAudit(value.query_id, value.registry_id, value.content_address, checks, len(checks), sum(item.passed for item in checks), sum(not item.passed for item in checks), all(item.passed for item in checks), f"pending:{AUDIT_PREFIX}"); return _seal(provisional, address_audit)
def verify_audit(value: QueryAudit) -> QueryAudit:
    if not isinstance(value, QueryAudit): raise ValidationError("runtime registry query audit verification requires a typed audit")
...
def render_audit_markdown(value: QueryAudit) -> str:
    value = verify_audit(value); lines = [f"# History diff runtime registry query audit {value.query_id}", "", f"- Accepted: {str(value.accepted).lower()}", f"- Checks: {value.passed_count}/{value.check_count}", "", "| Check | Result | Detail |", "| --- | --- | --- |"] ; lines.extend(f"| {item.check_id} | {str(item.passed).lower()} | {item.detail} |" for item in value.checks); return "\n".join(lines) + "\n"
def check_schema() -> dict[str, Any]: return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "HistoryDiffRuntimeRegistryQueryAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {field: {"type": "integer" if field == "ordinal" else "boolean" if field == "passed" else "string"} for field in CHECK_FIELDS}}
def audit_schema() -> dict[str, Any]: return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "HistoryDiffRuntimeRegistryQueryAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {field: {"type": "array" if field == "checks" else "integer" if field.endswith("count") else "boolean" if field == "accepted" else "string"} for field in AUDIT_FIELDS}}
