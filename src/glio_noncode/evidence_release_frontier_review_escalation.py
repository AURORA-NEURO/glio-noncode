"""Detailed review escalation plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseReviewEscalation:
...
def build_evidence_release_review_escalation(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseReviewEscalation:
    ...
    return EvidenceReleaseReviewEscalation(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_review_escalation(receipt: EvidenceReleaseReviewEscalation, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "escalation_count") != expected:
        return False
    return all(
...
def summarize_review_escalation(receipt: EvidenceReleaseReviewEscalation) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "escalation_count"),
...
__all__ = ["EvidenceReleaseReviewEscalation", "validate_evidence_release_review_escalation", "build_evidence_release_review_escalation", "summarize_review_escalation"]
