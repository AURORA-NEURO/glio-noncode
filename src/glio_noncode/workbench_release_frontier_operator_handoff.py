"""Deep operator handoff receipt for workbench release review.
...
class WorkbenchReleaseOperatorHandoff:
...
def build_workbench_release_operator_handoff(values: Iterable[Any] = (), *, require_non_empty: bool = True) -> WorkbenchReleaseOperatorHandoff:
    rows = tuple(
        {
            "sequence": index,
    ...
    return WorkbenchReleaseOperatorHandoff(**body, content_address=content_hash(body))


def validate_workbench_release_operator_handoff(receipt: WorkbenchReleaseOperatorHandoff) -> bool:
...
__all__ = ["WorkbenchReleaseOperatorHandoff", "build_workbench_release_operator_handoff", "validate_workbench_release_operator_handoff"]
