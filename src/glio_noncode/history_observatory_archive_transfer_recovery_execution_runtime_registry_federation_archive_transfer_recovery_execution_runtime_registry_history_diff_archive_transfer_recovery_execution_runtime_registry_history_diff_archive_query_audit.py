...
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_query as query_model
...
class RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck:
    FIELDS = CHECK_FIELDS
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck:
...
def address_check(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck):
        raise ValidationError("history diff archive query audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit:
    FIELDS = AUDIT_FIELDS

    def __init__(self, query_id: str, archive_id: str, archive_address: str, version: str, boundary: str, check_count: int, passed_count: int, failed_count: int, accepted: bool, checks: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck) else RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck.from_mapping(item) for item in _sequence(checks, "history diff archive query audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit:
        value = _mapping(value, "history diff archive query audit")
        _strict(value, set(cls.FIELDS), "history diff archive query audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit):
        raise ValidationError("history diff archive query audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def audit_query(query: query_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQuery, archive: archive_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit:
    if not isinstance(query, query_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQuery) or not isinstance(archive, archive_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchive):
        raise ValidationError("history diff archive query audit requires a typed query and archive")
    checks: list[RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck] = []

    def add(check_id: str, passed: bool, detail: str, evidence: Sequence[str] = ()) -> None:
        provisional = RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck(len(checks) + 1, check_id, bool(passed), detail, evidence, CHECK_PREFIX + ":pending")
        checks.append(RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck(provisional.ordinal, provisional.check_id, provisional.passed, provisional.detail, provisional.evidence_addresses, address_check(provisional)))

    expected: query_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQuery | None = None
    try:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit(query.query_id, query.archive_id, query.archive_address, VERSION, BOUNDARY, len(checks), passed_count, len(checks) - passed_count, passed_count == len(checks), checks, AUDIT_PREFIX + ":pending")
    return RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit(provisional.query_id, provisional.archive_id, provisional.archive_address, provisional.version, provisional.boundary, provisional.check_count, provisional.passed_count, provisional.failed_count, provisional.accepted, provisional.checks, address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit:
    return RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit.from_mapping(value)


def verify_audit(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit):
...
def audit_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit) -> str:
...
def render_audit_markdown(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
...
__all__ = ["VERSION", "BOUNDARY", "AUDIT_PREFIX", "CHECK_PREFIX", "MAX_CHECKS", "CHECK_IDS", "CHECK_FIELDS", "AUDIT_FIELDS", "RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAuditCheck", "RecoveryExecutionRuntimeRegistryHistoryDiffArchiveQueryAudit", "address_check", "address_audit", "audit_query", "audit_from_mapping", "verify_audit", "audit_json", "audit_csv", "render_audit_markdown", "check_schema", "audit_schema", "capabilities"]
