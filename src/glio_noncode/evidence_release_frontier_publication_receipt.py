"""Detailed publication receipt plane for the D14 C13-C16 release boundary.
...
class EvidenceReleasePublicationReceipt:
    receipt_count: int
    accepted: bool
...
def build_evidence_release_publication_receipt(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleasePublicationReceipt:
    ...
    return EvidenceReleasePublicationReceipt(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_publication_receipt(receipt: EvidenceReleasePublicationReceipt, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "receipt_count") != expected:
        return False
    return all(
...
def summarize_publication_receipt(receipt: EvidenceReleasePublicationReceipt) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "receipt_count"),
...
__all__ = ["EvidenceReleasePublicationReceipt", "validate_evidence_release_publication_receipt", "build_evidence_release_publication_receipt", "summarize_publication_receipt"]
