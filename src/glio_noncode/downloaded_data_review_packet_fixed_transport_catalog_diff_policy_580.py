"""Build, verify, query, and persist source-free module-579 transport catalogs."""
...
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_579 as transport_model
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_580_ct as contract_model
...
def _audit_model() -> Any:
    from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_579_aud as model
...
def _entry(package: Any, entry_id: str) -> contract_model.Entry:
    ...
    receipt = _audit_model().audit_package(package_value)
    if not receipt.accepted:
        raise ValidationError("module580 catalog requires an accepted module579 transport audit")
    manifest = package_value.manifest
    provisional = contract_model.Entry(entry_id, manifest.package_id, package_value.package_address, manifest.content_address, manifest.diff_address, manifest.policy_address, manifest.audit_address, manifest.policy_state, manifest.policy_accepted, manifest.audit_accepted, len(package_value.package_bytes), len(manifest.members), contract_model.ENTRY_PREFIX + ":pending")
    return contract_model.Entry(entry_id, manifest.package_id, package_value.package_address, manifest.content_address, manifest.diff_address, manifest.policy_address, manifest.audit_address, manifest.policy_state, manifest.policy_accepted, manifest.audit_accepted, len(package_value.package_bytes), len(manifest.members), contract_model.address_entry(provisional))
...
def build_catalog(packages: Sequence[Any], *, entry_ids: Sequence[str] | None = None, catalog_id: str = DEFAULT_CATALOG_ID) -> contract_model.Catalog:
    if isinstance(packages, (str, bytes, bytearray)) or not isinstance(packages, Sequence) or len(packages) > MAX_ENTRIES:
        raise ValidationError("module580 packages must be a bounded sequence")
    if entry_ids is not None and (isinstance(entry_ids, (str, bytes, bytearray)) or not isinstance(entry_ids, Sequence) or len(entry_ids) != len(packages)):
        raise ValidationError("module580 entry IDs must align with packages")
    ...
    if len({item.package_address for item in built_entries}) != len(built_entries):
        raise ValidationError("module580 catalog contains duplicate package addresses")
    entries = tuple(sorted(built_entries, key=lambda item: item.entry_id))
    body = {"catalog_id": catalog_id, "version": VERSION, "boundary": BOUNDARY, "entry_count": len(entries), "accepted_count": sum(item.audit_accepted for item in entries), "ready_count": sum(item.policy_state == "ready" for item in entries), "blocked_count": sum(item.policy_state == "blocked" for item in entries), "state": "empty", "entries": entries, "content_address": CATALOG_PREFIX + ":pending"}
    state = "empty" if not entries else "ready" if body["ready_count"] == len(entries) else "blocked" if body["blocked_count"] == len(entries) else "mixed"
    body["state"] = state
...
def _catalog_from_mapping(value: Mapping[str, Any]) -> contract_model.Catalog:
    if not isinstance(value, Mapping):
        raise ValidationError("module580 catalog must be an object")
...
def load_catalog(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> contract_model.Catalog:
    if isinstance(value, Mapping):
        ...
        raw = _strict_json_loads(read_text(value, field="module580 catalog", max_bytes=32 * 1024 * 1024))
...
def verify_catalog(value: Any) -> contract_model.Catalog:
    ...
    if not isinstance(catalog, contract_model.Catalog) or contract_model.address_catalog(catalog) != catalog.content_address or _has_forbidden_key(catalog.to_dict()):
        raise ValidationError("module580 catalog address or public boundary does not replay")
...
def write_catalog(value: Any, destination: str | Path, *, allow_existing: bool = False) -> contract_model.Catalog:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module580 catalog destination already exists")
    _validate_parent(path.parent, "module580 catalog destination")
    atomic_write_text(path, catalog_json(catalog), field="module580 catalog destination")
    return catalog
...
def query_catalog(value: Any, *, resource: str = "summary", state: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if resource not in {"summary", "entries", "lineage", "state"} or state not in {"", "ready", "blocked"} or not isinstance(text, str) or len(text) > 4096 or not isinstance(offset, int) or offset < 0 or not isinstance(limit, int) or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("module580 catalog query filters are invalid")
    if resource == "summary":
        values: tuple[Any, ...] = (catalog.summary(),)
    elif resource in {"entries", "lineage"}:
        values = tuple(item.to_dict() for item in catalog.entries)
    else:
        values = (catalog.summary(),)
    if state:
        values = tuple(item for item in values if (item.get("policy_state") if isinstance(item, dict) else item.get("state")) == state)
    total = len(values)
    if text:
        values = tuple(item for item in values if text.casefold() in canonical_json(item).casefold())
    selected = values[offset:offset + limit]
    result = {"catalog_address": catalog.content_address, "resource": resource, "state": state, "text": text, "offset": offset, "limit": limit, "total": total, "matched": len(values), "returned": len(selected), "truncated": offset + len(selected) < len(values), "values": selected}
...
def render_catalog_markdown(value: Any) -> str:
    catalog = verify_catalog(value)
    lines = ["# Module580 Fixed Policy Transport Catalog", "", f"- Catalog: `{catalog.content_address}`", f"- State: **{catalog.state}**", f"- Entries: **{catalog.entry_count}**", f"- Audited: **{catalog.accepted_count}/{catalog.entry_count}**", "", "| entry | policy state | policy accepted | bytes | package |", "| --- | --- | --- | ---: | --- |"]
