"""Build, verify, query, and persist portable release-bundle diff policy packets."""
...
from .downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_contracts import (
    BOUNDARY, FILE_NAMES, MANIFEST_PREFIX, MAX_PACKAGE_BYTES, MEDIA_TYPES, PACKAGE_FORMAT, PACKAGE_PREFIX, PAYLOAD_NAMES,
    DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage,
    DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageManifest,
    DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageMember,
...
MAX_LIMIT = 256
VERSION = "downloaded-data-review-packet-diff-policy-release-certificate-bundle-diff-policy-package-v1"
...
def _zip_member(name: str) -> zipfile.ZipInfo:
    if name not in FILE_NAMES:
        raise ValidationError("release bundle diff policy packet member is outside the fixed file set")
...
def _parts(raw: bytes) -> tuple[tuple[zipfile.ZipInfo, bytes], ...]:
    try:
        ...
        raise ValidationError(f"cannot read release bundle diff policy packet ZIP: {exc}") from exc


def _read_package(value: bytes | bytearray | str | Path | DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage) -> bytes:
    if isinstance(value, DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage):
        raw = value.package_bytes
    elif isinstance(value, (bytes, bytearray)):
        raw = bytes(value)
    else:
        raw = read_bytes(value, field="release bundle diff policy packet", max_bytes=MAX_PACKAGE_BYTES)
    if len(raw) > MAX_PACKAGE_BYTES:
        raise ValidationError("release bundle diff policy packet exceeds its byte bound")
    return raw


def _review(diff: Any, policy: Any, audit: Any) -> str:
    return "\n".join((
        "# Downloaded Data Release Bundle Diff Policy Packet", "",
        f"- Diff: `{diff.content_address}`", f"- Policy: `{policy.content_address}`", f"- Policy state: **{policy.state}**",
        ...
        f"| diff | `{diff.content_address}` |", f"| policy | `{policy.content_address}` |", f"| policy audit | `{audit.content_address}` |", "",
        "This packet carries longitudinal structural evidence, the applied gate, and an independent audit. It does not carry source archive bytes or record values.", "",
    ))
...
def _member(path: str, payload: bytes) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageMember:
    body = {"relative_path": path, "media_type": MEDIA_TYPES[path], "byte_count": len(payload), "byte_address": address_member_bytes(payload)}
    provisional = DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageMember(**body, content_address=PACKAGE_PREFIX + "-member:pending")
    return DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageMember(**body, content_address=address_member(provisional))
...
def build_package(diff: Any, policy: Any, audit: Any | None = None, *, package_id: str = PACKAGE_PREFIX) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage:
    diff_value = diff_model.verify_diff(diff)
    policy_value = policy_model.verify_policy(policy)
    if policy_value.diff_address != diff_value.content_address:
        raise ValidationError("release bundle diff policy packet policy does not refer to the supplied diff")
    ...
    if audit_value.policy_address != policy_value.content_address or not audit_value.accepted:
        raise ValidationError("release bundle diff policy packet requires an accepted independent policy audit")
    ...
    provisional = DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageManifest(**body, content_address=MANIFEST_PREFIX + ":pending")
    manifest = DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageManifest(**body, content_address=address_manifest(provisional))
    ...
    if len(raw) > MAX_PACKAGE_BYTES:
        raise ValidationError("release bundle diff policy packet exceeds its byte bound")
    package = DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage(manifest=manifest, package_bytes=raw, package_address=hash_bytes(raw, prefix=PACKAGE_PREFIX))
    return verify_package(package)


def _manifest_and_bodies(raw: bytes) -> tuple[DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageManifest, dict[str, bytes]]:
    parts = _parts(raw)
    names = tuple(info.filename for info, _payload in parts)
    if names != FILE_NAMES or any(not _safe_member(name) for name in names):
        raise ValidationError("release bundle diff policy packet ZIP members are not the exact safe file set")
    if any(info.date_time != _ZIP_EPOCH or info.compress_type != zipfile.ZIP_STORED or info.external_attr != _FIXED_ATTRIBUTES or info.extra or info.comment for info, _payload in parts):
        raise ValidationError("release bundle diff policy packet ZIP metadata is not deterministic")
    ...
    for descriptor in members:
        ...
        if descriptor.byte_count != len(payload) or descriptor.byte_address != address_member_bytes(payload) or descriptor.content_address != address_member(descriptor):
            raise ValidationError(f"release bundle diff policy packet member does not replay: {descriptor.relative_path}")
    ...
    if raw != canonical_raw:
        raise ValidationError("release bundle diff policy packet ZIP bytes are not deterministic")
    return manifest, bodies


def verify_package(value: bytes | bytearray | str | Path | DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage:
    raw = _read_package(value)
    manifest, bodies = _manifest_and_bodies(raw)
    diff = diff_model.load_diff(_strict_json_loads(bodies["diff.json"].decode(_UTF8)))
    ...
    for path in ("diff.json", "policy.json", "policy-audit.json"):
        if bodies[path] != (canonical_json(_strict_json_loads(bodies[path].decode(_UTF8))) + "\n").encode(_UTF8):
            raise ValidationError(f"release bundle diff policy packet payload is not canonical: {path}")
    if manifest.diff_address != diff.content_address or manifest.policy_address != policy.content_address or manifest.audit_address != audit.content_address or policy.diff_address != diff.content_address or audit.policy_address != policy.content_address:
        raise ValidationError("release bundle diff policy packet lineage does not replay")
    if manifest.policy_state != policy.state or manifest.policy_accepted != policy.accepted or not manifest.audit_accepted or not audit.accepted:
        raise ValidationError("release bundle diff policy packet decision evidence does not replay")
    if _has_forbidden_key({"manifest": manifest.to_dict(), "diff": diff.to_dict(), "policy": policy.to_dict(), "audit": audit.to_dict()}):
        raise ValidationError("release bundle diff policy packet crosses the public boundary")
    if bodies["review.md"].decode(_UTF8) != _review(diff, policy, audit):
        raise ValidationError("release bundle diff policy packet Markdown does not replay")
    package = DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage(manifest=manifest, package_bytes=raw, package_address=hash_bytes(raw, prefix=PACKAGE_PREFIX))
    if isinstance(value, DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage) and value.to_dict() != package.to_dict():
        raise ValidationError("typed release bundle diff policy packet does not replay its bytes")
    return package


def load_package(value: bytes | bytearray | str | Path | DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage) -> tuple[Any, Any, Any, str]:
    package = verify_package(value)
    _, bodies = _manifest_and_bodies(package.package_bytes)
    return (diff_model.load_diff(_strict_json_loads(bodies["diff.json"].decode(_UTF8))), policy_model.load_policy(_strict_json_loads(bodies["policy.json"].decode(_UTF8))), policy_audit_model.load_audit(_strict_json_loads(bodies["policy-audit.json"].decode(_UTF8))), bodies["review.md"].decode(_UTF8))
...
def write_package(value: Any, destination: str | Path, *, allow_existing: bool = False) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackage:
    package = verify_package(value)
    path = Path(destination)
    if path.exists() and not allow_existing:
        raise ValidationError("release bundle diff policy packet destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("release bundle diff policy packet destination is not a file")
    _validate_parent(path.parent, "release bundle diff policy packet destination")
    atomic_write_bytes(path, package.package_bytes, field="release bundle diff policy packet destination")
    return package
...
def query_package(value: Any, *, resource: str = "summary", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    if resource not in {"summary", "members", "manifest", "diff", "policy", "audit", "review"} or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("release bundle diff policy packet query is invalid")
    package = verify_package(value)
...
__all__ = ["build_package", "capabilities", "load_package", "manifest_schema", "package_csv", "package_json", "package_schema", "query_package", "render_package_markdown", "verify_package", "write_package"]
