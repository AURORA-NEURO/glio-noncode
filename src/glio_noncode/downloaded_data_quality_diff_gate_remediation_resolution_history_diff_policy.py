...
from . import (
    downloaded_data_quality_diff_gate_remediation_resolution_history_diff as diff_model,
)
from .errors import ValidationError
from .serialization import canonical_json, content_hash

VERSION = "downloaded-data-quality-diff-gate-remediation-resolution-history-diff-policy-v1"
BOUNDARY = "public_downloaded_data_quality_diff_gate_remediation_resolution_history_diff_policy"
POLICY_PREFIX = "glio-noncode-download-quality-diff-gate-remediation-resolution-history-diff-policy"
...
class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy:
        value = _mapping(value, "history diff policy")
        _strict(value, set(cls.FIELDS), "history diff policy")
        return cls(*(value[field] for field in cls.FIELDS))


def address_policy(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy):
...
def default_policy(*, policy_id: str = DEFAULT_POLICY_ID, allowed_directions: Sequence[str] = ("improved", "unchanged"), require_candidate_ready: bool = True, max_added_count: int = diff_model.MAX_ITEMS, max_removed_count: int = 0, max_changed_count: int = 0, max_improved_delta: int = diff_model.MAX_ITEMS, max_regressed_delta: int = 0, require_state_progression: bool = True) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy:
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy(policy_id, allowed_directions, require_candidate_ready, max_added_count, max_removed_count, max_changed_count, max_improved_delta, max_regressed_delta, require_state_progression, POLICY_PREFIX + ":pending")
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy(provisional.policy_id, provisional.allowed_directions, provisional.require_candidate_ready, provisional.max_added_count, provisional.max_removed_count, provisional.max_changed_count, provisional.max_improved_delta, provisional.max_regressed_delta, provisional.require_state_progression, address_policy(provisional))


class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule:
    FIELDS = RULE_FIELDS
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule:
        value = _mapping(value, "history diff policy rule")
        _strict(value, set(cls.FIELDS), "history diff policy rule")
        return cls(*(value[field] for field in cls.FIELDS))


def address_rule(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule):
...
def _rule_inputs_for_values(left_entry_count: int, right_entry_count: int, direction: str, candidate_release_ready: bool, added_count: int, removed_count: int, changed_count: int, unchanged_count: int, improved_delta: int, regressed_delta: int, state_transition: str, policy: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy, public_ok: bool = True) -> tuple[tuple[str, bool, str, str, str], ...]:
...
def _rule_inputs(diff: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff, policy: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy) -> tuple[tuple[str, bool, str, str, str], ...]:
...
def _rule(value: tuple[str, bool, str, str, str], ordinal: int) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule:
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule(**body)
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule(**(body | {"content_address": address_rule(provisional)}))


class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation:
    FIELDS = EVALUATION_FIELDS

    def __init__(self, evaluation_id: str, version: str, boundary: str, policy_id: str, policy_address: str, policy: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy | Mapping[str, Any], diff_id: str, diff_address: str, left_entry_count: int, right_entry_count: int, direction: str, candidate_release_ready: bool, added_count: int, removed_count: int, changed_count: int, unchanged_count: int, improved_delta: int, regressed_delta: int, state_transition: str, state: str, decision: str, accepted: bool, release_ready: bool, rules: Sequence[DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule | Mapping[str, Any]], rule_count: int, passed_rule_count: int, failed_rule_count: int, content_address: str) -> None:
        ...
        self.policy = policy if isinstance(policy, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy) else DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy.from_mapping(policy)
        ...
        self.rules = tuple(item if isinstance(item, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule) else DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule.from_mapping(item) for item in _sequence(rules, "history diff policy evaluation rules", MAX_RULES))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation:
        value = _mapping(value, "history diff policy evaluation")
        _strict(value, set(cls.FIELDS), "history diff policy evaluation")
        return cls(*(value[field] for field in cls.FIELDS))


def address_evaluation(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation):
...
def evaluate(diff: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff, *, policy: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy | None = None, evaluation_id: str = EVALUATION_PREFIX) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation:
    if not isinstance(diff, diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff):
        raise ValidationError("history diff policy evaluation requires a typed diff")
    policy = default_policy() if policy is None else policy
    if not isinstance(policy, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy):
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation(**body, content_address=EVALUATION_PREFIX + ":pending")
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation(**body, content_address=address_evaluation(provisional))


def policy_from_mapping(value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy:
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy.from_mapping(value)


def evaluation_from_mapping(value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation:
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation.from_mapping(value)


def evaluation_json(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation) -> str:
    return canonical_json(evaluation_from_mapping(value.to_dict()).to_dict())


def evaluation_csv(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation) -> str:
...
def render_evaluation_markdown(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation) -> str:
    value = evaluation_from_mapping(value.to_dict())
...
def policy_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data quality diff gate remediation resolution history diff policy", "type": "object", "additionalProperties": False, "required": list(POLICY_FIELDS), "properties": {"policy_id": {"type": "string"}, "allowed_directions": {"type": "array", "items": {"enum": list(diff_model.DIRECTIONS)}, "minItems": 1, "maxItems": len(diff_model.DIRECTIONS)}, "require_candidate_ready": {"type": "boolean"}, "max_added_count": {"type": "integer", "minimum": 0}, "max_removed_count": {"type": "integer", "minimum": 0}, "max_changed_count": {"type": "integer", "minimum": 0}, "max_improved_delta": {"type": "integer", "minimum": 0}, "max_regressed_delta": {"type": "integer", "minimum": 0}, "require_state_progression": {"type": "boolean"}, "content_address": {"type": "string"}}}


def rule_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data quality diff gate remediation resolution history diff policy rule", "type": "object", "additionalProperties": False, "required": list(RULE_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_RULES}, "rule_id": {"enum": list(RULE_IDS)}, "passed": {"type": "boolean"}, "observed": {"type": "string"}, "limit": {"type": "string"}, "detail": {"type": "string"}, "content_address": {"type": "string"}}}


def evaluation_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data quality diff gate remediation resolution history diff policy evaluation", "type": "object", "additionalProperties": False, "required": list(EVALUATION_FIELDS), "properties": {"evaluation_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "policy_id": {"type": "string"}, "policy_address": {"type": "string"}, "diff_id": {"type": "string"}, "diff_address": {"type": "string"}, "direction": {"enum": list(diff_model.DIRECTIONS)}, "candidate_release_ready": {"type": "boolean"}, "added_count": {"type": "integer", "minimum": 0}, "removed_count": {"type": "integer", "minimum": 0}, "changed_count": {"type": "integer", "minimum": 0}, "improved_delta": {"type": "integer"}, "regressed_delta": {"type": "integer"}, "state_transition": {"type": "string"}, "state": {"enum": list(STATES)}, "decision": {"enum": list(DECISIONS)}, "accepted": {"type": "boolean"}, "release_ready": {"type": "boolean"}, "rules": {"type": "array", "items": rule_schema(), "minItems": MAX_RULES, "maxItems": MAX_RULES}, "rule_count": {"type": "integer", "minimum": MAX_RULES, "maximum": MAX_RULES}, "passed_rule_count": {"type": "integer", "minimum": 0, "maximum": MAX_RULES}, "failed_rule_count": {"type": "integer", "minimum": 0, "maximum": MAX_RULES}, "content_address": {"type": "string"}}}
...
__all__ = ["BOUNDARY", "DECISIONS", "DEFAULT_POLICY_ID", "EVALUATION_FIELDS", "EVALUATION_PREFIX", "MAX_RULES", "POLICY_FIELDS", "POLICY_PREFIX", "RULE_FIELDS", "RULE_IDS", "RULE_PREFIX", "STATES", "VERSION", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicy", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyEvaluation", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffPolicyRule", "address_evaluation", "address_policy", "address_rule", "capabilities", "default_policy", "evaluate", "evaluation_csv", "evaluation_from_mapping", "evaluation_json", "evaluation_schema", "policy_from_mapping", "policy_schema", "render_evaluation_markdown", "rule_schema"]
