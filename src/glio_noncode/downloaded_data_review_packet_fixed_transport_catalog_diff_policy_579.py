"""Build, verify, query, and persist module-578 policy decision transports."""
...
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_579_ct as contract_model
...
def _zip_bytes(manifest: contract_model.Manifest, payloads: Mapping[str, bytes]) -> bytes:
    ...
    if len(value) > MAX_PACKAGE_BYTES:
        raise ValidationError("module579 package exceeds its byte ceiling")
    return value
...
def build_package(diff: Any, policy: Any, audit: Any = None, *, package_id: str = DEFAULT_PACKAGE_ID) -> contract_model.Transport:
    ...
    if policy_value.diff_address != diff_value.content_address:
        raise ValidationError("module579 policy does not point to the supplied diff")
    audit_value = _audit_model().audit_policy(policy_value, diff=diff_value) if audit is None else _audit_model().verify_audit(audit)
    if audit_value.diff_address != diff_value.content_address or audit_value.policy_address != policy_value.content_address or not audit_value.accepted:
        raise ValidationError("module579 audit lineage or acceptance is invalid")
    ...
    package = contract_model.Transport(manifest, package_bytes, hash_bytes(package_bytes, prefix=PACKAGE_PREFIX))
    return verify_package(package)


def _read_zip(value: Any) -> tuple[bytes, dict[str, bytes], dict[str, zipfile.ZipInfo]]:
    raw = value.package_bytes if isinstance(value, contract_model.Transport) else bytes(value) if isinstance(value, (bytes, bytearray)) else read_bytes(value, field="module579 package", max_bytes=MAX_PACKAGE_BYTES)
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(raw), "r") as archive:
            ...
            if len(infos) != len(FILE_NAMES) or tuple(info.filename for info in infos) != FILE_NAMES or len(set(info.filename for info in infos)) != len(FILE_NAMES):
                raise ValidationError("module579 package member set or order is not fixed")
            ...
            for info in infos:
                if info.date_time != contract_model.FIXED_ZIP_DATE or info.compress_type != zipfile.ZIP_STORED or info.create_system != 0 or info.create_version != 20 or info.extract_version != 20 or info.flag_bits != 0 or info.internal_attr != 0 or info.external_attr != _FIXED_ATTRIBUTES or info.extra or info.comment:
                    raise ValidationError("module579 ZIP metadata is not canonical")
                if info.file_size > MAX_MEMBER_BYTES:
                    raise ValidationError("module579 package member exceeds its byte ceiling")
                body = archive.read(info)
                if len(body) != info.file_size or len(body) > MAX_MEMBER_BYTES:
                    raise ValidationError("module579 package member size is invalid")
        ...
        raise ValidationError("module579 package is not a readable fixed ZIP") from exc


def _manifest_and_bodies(value: Any) -> tuple[contract_model.Transport, dict[str, bytes]]:
    raw, bodies, _records = _read_zip(value)
    parsed_manifest = _strict_json_loads(bodies["manifest.json"].decode("utf-8"))
    if bodies["manifest.json"] != (canonical_json(parsed_manifest) + "\n").encode("utf-8"):
        raise ValidationError("module579 manifest is not canonical")
    manifest = contract_model.Manifest.from_mapping(parsed_manifest)
    if contract_model.address_manifest(manifest) != manifest.content_address:
        raise ValidationError("module579 manifest address does not replay")
    for path in PAYLOAD_NAMES:
        ...
        if member.byte_count != len(body) or member.byte_address != contract_model.address_member_bytes(body) or member.content_address != contract_model.address_member(member):
            raise ValidationError("module579 member address or byte count does not replay")
    canonical_raw = _zip_bytes(manifest, bodies)
    if raw != canonical_raw:
        raise ValidationError("module579 ZIP bytes are not deterministic")
    package = contract_model.Transport(manifest, raw, hash_bytes(raw, prefix=PACKAGE_PREFIX))
    return package, bodies


def verify_package(value: Any) -> contract_model.Transport:
    package, _bodies = _manifest_and_bodies(value)
    if _has_forbidden_key(package.to_dict()) or package.package_address != hash_bytes(package.package_bytes, prefix=PACKAGE_PREFIX):
        raise ValidationError("module579 package address or public boundary does not replay")
    return package
...
def load_package(value: Any) -> tuple[Any, Any, Any, str]:
    ...
    audit = _audit_model().verify_audit(_strict_json_loads(bodies["policy-audit.json"].decode("utf-8")))
    ...
    if package.manifest.diff_address != diff.content_address or package.manifest.policy_address != policy.content_address or package.manifest.audit_address != audit.content_address or policy.diff_address != diff.content_address or audit.diff_address != diff.content_address or audit.policy_address != policy.content_address or not audit.accepted or review != _review(diff, policy, audit):
        raise ValidationError("module579 nested lineage or review does not replay")
    if package.manifest.policy_state != policy.state or package.manifest.policy_accepted != policy.accepted or package.manifest.audit_accepted != audit.accepted:
        raise ValidationError("module579 policy decision flags do not replay")
...
def write_package(value: Any, destination: str | Path, *, allow_existing: bool = False) -> contract_model.Transport:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module579 package destination already exists")
    _validate_parent(path.parent, "module579 package destination")
    atomic_write_bytes(path, package.package_bytes, field="module579 package destination")
    return package
...
def query_package(value: Any, *, resource: str = "summary", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if resource not in {"summary", "manifest", "members", "diff", "policy", "audit", "review"}:
        raise ValidationError("module579 query resource is unsupported")
    if not isinstance(text, str) or len(text) > 4096 or not isinstance(offset, int) or offset < 0 or not isinstance(limit, int) or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("module579 query bounds are invalid")
    ...
    total = len(values)
    if text:
        values = tuple(item for item in values if text.casefold() in canonical_json(item).casefold())
    selected = values[offset:offset + limit]
    result = {"package_address": package.package_address, "resource": resource, "text": text, "offset": offset, "limit": limit, "total": total, "matched": len(values), "returned": len(selected), "truncated": offset + len(selected) < len(values), "values": selected}
...
def render_package_markdown(value: Any) -> str:
    package = verify_package(value)
    diff, policy, audit, _review_text = load_package(package)
    lines = ["# Module579 Fixed Policy Decision Transport", "", f"- Package: `{package.package_address}`", f"- State: **{policy.state}**", f"- Policy accepted: **{str(policy.accepted).lower()}**", f"- Audit: **{audit.passed_count}/{audit.check_count}**", f"- Diff: `{diff.content_address}`", "", "| member | bytes | content address |", "| --- | ---: | --- |"]
...
def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module579 transport member", "type": "object", "additionalProperties": False, "required": ["relative_path", "media_type", "byte_count", "byte_address", "content_address"]}
