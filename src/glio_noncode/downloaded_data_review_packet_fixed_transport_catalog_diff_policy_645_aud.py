"""Independently audit module645 longitudinal transport catalog diffs."""
...
class AuditCheck:
    ...
    def __post_init__(self) -> None:
        if self.check_id not in CHECK_IDS or not isinstance(self.passed, bool) or not isinstance(self.detail, str) or not self.detail or len(self.detail) > 2048:
            raise ValidationError("module645 audit check is invalid")
        _address(self.content_address, "module645 audit check address", CHECK_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_check(self) != self.content_address:
            raise ValidationError("module645 audit check address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "AuditCheck":
        if set(value) != set(CHECK_FIELDS):
            raise ValidationError("module645 audit check fields are not exact")
...
class Audit:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.diff_id, str) or not self.diff_id or tuple(item.check_id for item in self.checks) != CHECK_IDS or self.check_count != len(self.checks) or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0):
            raise ValidationError("module645 audit aggregates do not replay")
        _address(self.diff_address, "module645 audit diff address", diff_model.DIFF_PREFIX)
        _address(self.content_address, "module645 audit address", AUDIT_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_audit(self) != self.content_address:
            raise ValidationError("module645 audit address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Audit":
        if set(value) != set(AUDIT_FIELDS):
            raise ValidationError("module645 audit fields are not exact")
...
def audit_diff(value: Any, *, left: Any = None, right: Any = None) -> Audit:
    try:
        ...
        checks = tuple(_check(check_id, False, "unavailable", "replayable", f"module645 diff audit unavailable: {exc}") for check_id in CHECK_IDS)
...
def verify_audit(value: Any) -> Audit:
    ...
    if not isinstance(audit, Audit) or address_audit(audit) != audit.content_address or _has_forbidden_key(audit.to_dict()):
        raise ValidationError("module645 audit address or public boundary does not replay")
    return Audit.from_mapping(audit.to_dict())


def load_audit(value: bytes | bytearray | str | Path | dict[str, Any]) -> Audit:
    raw = value if isinstance(value, dict) else _strict_json_loads(bytes(value).decode("utf-8")) if isinstance(value, (bytes, bytearray)) else _strict_json_loads(read_text(value, field="module645 audit", max_bytes=32 * 1024 * 1024))
...
def write_audit(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Audit:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module645 audit destination already exists")
    _validate_parent(path.parent, "module645 audit destination")
    atomic_write_text(path, audit_json(audit), field="module645 audit destination")
    return audit
...
def query_audit(value: Any, *, passed: bool | None = None, text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if (passed is not None and not isinstance(passed, bool)) or not isinstance(text, str) or len(text) > 4096 or not isinstance(offset, int) or offset < 0 or not isinstance(limit, int) or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("module645 audit query bounds are invalid")
...
def render_audit_markdown(value: Any) -> str:
    audit = verify_audit(value)
    lines = ["# Module645 Catalog Diff Audit", "", f"- Diff: `{audit.diff_address}`", f"- Result: **{'accepted' if audit.accepted else 'blocked'}**", f"- Checks: **{audit.passed_count}/{audit.check_count}**", "", "| check | passed | detail |", "| --- | --- | --- |"]
    lines.extend(f"| `{item.check_id}` | {str(item.passed).lower()} | {item.detail} |" for item in audit.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module645 diff audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS)}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module645 catalog diff audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS)}
