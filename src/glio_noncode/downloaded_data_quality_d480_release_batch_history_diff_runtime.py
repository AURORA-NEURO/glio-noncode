"""Policy-driven release decisions over D479 release-batch history diffs."""
...
def _public(value: Any) -> bool:
    return diff_model.history_model.batch_model.runtime_model.diff_model.history_model._public(value)
...
class RuntimeSummary:
    ...
    def __init__(self, runtime_id: str, diff_id: str, diff_address: str, policy_address: str, check_count: int, passed_count: int, failed_count: int, release_ready: bool, state: str, direction: str, state_transition: str, item_count: int, added_count: int, removed_count: int, changed_count: int, unchanged_count: int, accepted: bool, content_address: str) -> None:
        ...
        self.unchanged_count = _count(unchanged_count, "history diff runtime summary unchanged count", diff_model.MAX_ITEMS)
...
class DiffRuntime:
    ...
    def __init__(self, runtime_id: str, diff_id: str, diff_address: str, version: str, boundary: str, policy: Mapping[str, Any] | RuntimePolicy, checks: Sequence[RuntimeCheck | Mapping[str, Any]], check_count: int, passed_count: int, failed_count: int, release_ready: bool, state: str, direction: str, state_transition: str, item_count: int, added_count: int, removed_count: int, changed_count: int, unchanged_count: int, accepted: bool, manifest: Mapping[str, Any] | RuntimeManifest, summary: Mapping[str, Any] | RuntimeSummary, content_address: str) -> None:
        ...
        self.state = state; self.direction = direction; self.state_transition = _text(state_transition, "history diff runtime state transition", 128)
        self.item_count = _count(item_count, "history diff runtime item count", diff_model.MAX_ITEMS); self.added_count = _count(added_count, "history diff runtime added count", diff_model.MAX_ITEMS); self.removed_count = _count(removed_count, "history diff runtime removed count", diff_model.MAX_ITEMS); self.changed_count = _count(changed_count, "history diff runtime changed count", diff_model.MAX_ITEMS); self.unchanged_count = _count(unchanged_count, "history diff runtime unchanged count", diff_model.MAX_ITEMS)
    ...
    def _validate(self) -> None:
        if self.version != VERSION or self.boundary != BOUNDARY or self.policy.diff_id != self.diff_id or self.check_count != len(self.checks) or self.check_count != MAX_CHECKS or self.passed_count + self.failed_count != self.check_count or self.passed_count != sum(item.passed for item in self.checks) or self.release_ready != (self.state == "ready") or self.added_count + self.removed_count + self.changed_count + self.unchanged_count != self.item_count:
            raise ValidationError("history diff runtime identity or counters do not replay")
        if tuple(item.ordinal for item in self.checks) != tuple(range(1, MAX_CHECKS + 1)) or tuple(item.check_id for item in self.checks) != CHECK_IDS:
            raise ValidationError("history diff runtime checks are not canonical")
...
def _check(ordinal: int, check_id: str, passed: bool, actual: Any, expected: Any, detail: str) -> RuntimeCheck:
    ...
    provisional = RuntimeCheck(**body); return RuntimeCheck(**(body | {"content_address": address_check(provisional)}))
...
def build_runtime(diff: diff_model.HistoryDiff, *, runtime_id: str = DEFAULT_RUNTIME_ID, policy: RuntimePolicy | Mapping[str, Any] | None = None) -> DiffRuntime:
    ...
    runtime_policy = policy if isinstance(policy, RuntimePolicy) else RuntimePolicy.from_mapping(_mapping(policy, "history diff runtime policy")) if policy is not None else build_policy(f"{runtime_id}-policy", diff.diff_id)
    same_state = diff.state_transition.split("->", 1)[0] == diff.state_transition.split("->", 1)[-1] if "->" in diff.state_transition else True
    checks = (
        _check(1, "diff_present", bool(diff.diff_id and diff.content_address), (diff.diff_id, bool(diff.content_address)), "diff identity and address", "a diff must be present"),
        _check(2, "diff_accepted", (not runtime_policy.require_accepted) or diff.accepted, diff.accepted, True, "upstream diff must be accepted by policy"),
        ...
        _check(4, "added_budget", diff.added_count <= runtime_policy.maximum_added, diff.added_count, runtime_policy.maximum_added, "added snapshots must remain within budget"),
        _check(5, "removed_budget", diff.removed_count <= runtime_policy.maximum_removed, diff.removed_count, runtime_policy.maximum_removed, "removed snapshots must remain within budget"),
        _check(6, "changed_budget", diff.changed_count <= runtime_policy.maximum_changed, diff.changed_count, runtime_policy.maximum_changed, "changed snapshots must remain within budget"),
        ...
        _check(8, "state_transition", (not runtime_policy.require_state_change) or not same_state, diff.state_transition, "different states", "policy may require a state transition"),
        _check(9, "unchanged_policy", runtime_policy.allow_unchanged or diff.unchanged_count == 0, diff.unchanged_count, 0, "unchanged snapshots must follow policy"),
        _check(10, "comparison_conservation", diff.added_count + diff.removed_count + diff.changed_count + diff.unchanged_count == diff.item_count, diff.item_count, "sum of classifications", "comparison counts must conserve items"),
        ...
        _check(12, "address_integrity", diff.summary.right_history_address == diff.right_history_address and diff.summary.left_history_address == diff.left_history_address, True, "history addresses retained", "diff address lineage must remain intact"),
        ...
        _check(14, "release_disposition", True, "pending checks", "all preceding checks", "release readiness is the conjunction of policy checks"),
        _check(15, "public_boundary", _public(diff.to_dict()) and _public(runtime_policy.to_dict()), True, "public value", "runtime inputs must remain value-only"),
    )
    disposition = all(item.passed for item in checks[:13])
    checks = checks[:13] + (_check(14, "release_disposition", disposition, disposition, True, "release readiness is the conjunction of policy checks"), checks[14])
    passed = sum(item.passed for item in checks); failed = len(checks) - passed; release_ready = disposition and checks[14].passed and diff.accepted; state = "ready" if release_ready else "blocked"
...
def verify_runtime(value: DiffRuntime, diff: diff_model.HistoryDiff | None = None) -> DiffRuntime:
    ...
    if diff is not None and (diff_model.verify_diff(diff).content_address != value.diff_address or diff.diff_id != value.diff_id):
        raise ValidationError("history diff runtime does not link to the supplied diff")
    return value


def runtime_from_mapping(value: Mapping[str, Any], diff: diff_model.HistoryDiff | None = None) -> DiffRuntime:
    return verify_runtime(DiffRuntime.from_mapping(value), diff)
...
def capabilities() -> dict[str, Any]: return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "files": FILES, "check_ids": CHECK_IDS, "directions": diff_model.DIRECTIONS, "states": STATES, "max_checks": MAX_CHECKS, "features": ("history diff release policy", "added removed and changed budgets", "direction and transition controls", "exact four-file persistence", "canonical reload and tamper rejection", "JSON CSV and Markdown projections"), "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}
