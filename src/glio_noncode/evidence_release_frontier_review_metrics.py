"""Detailed review metrics plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseReviewMetrics:
    review_count: int
    accepted: bool
...
def build_evidence_release_review_metrics(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseReviewMetrics:
    ...
    return EvidenceReleaseReviewMetrics(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_review_metrics(receipt: EvidenceReleaseReviewMetrics, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "review_count") != expected:
        return False
    return all(
...
def summarize_review_metrics(receipt: EvidenceReleaseReviewMetrics) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "review_count"),
...
__all__ = ["EvidenceReleaseReviewMetrics", "validate_evidence_release_review_metrics", "build_evidence_release_review_metrics", "summarize_review_metrics"]
