"""Append-only history for federation recovery-execution runtime registries."""
...
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_registry as registry_model
...
class RecoveryExecutionRuntimeRegistryHistoryEntry:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryEntry:
        value = _mapping(value, "runtime registry history entry")
        _strict(value, set(cls.FIELDS), "runtime registry history entry")
        return cls(*(value[field] for field in cls.FIELDS))


def address_entry(value: RecoveryExecutionRuntimeRegistryHistoryEntry) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryEntry):
...
def entry_from_registry(value: registry_model.RecoveryExecutionRuntimeRegistry, ordinal: int, transition: str, previous_registry_address: str) -> RecoveryExecutionRuntimeRegistryHistoryEntry:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryEntry(**body)
    return RecoveryExecutionRuntimeRegistryHistoryEntry(**(body | {"content_address": address_entry(provisional)}))


class RecoveryExecutionRuntimeRegistryHistoryEntries:
    """Ordered and addressed snapshot projection persisted beside a history."""

    FIELDS = ENTRIES_FIELDS

    def __init__(self, entries: Sequence[RecoveryExecutionRuntimeRegistryHistoryEntry | Mapping[str, Any]], content_address: str) -> None:
        self.entries = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryEntry) else RecoveryExecutionRuntimeRegistryHistoryEntry.from_mapping(item) for item in _sequence(entries, "runtime registry history entries", MAX_ENTRIES))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryEntries:
...
def address_entries(value: Sequence[RecoveryExecutionRuntimeRegistryHistoryEntry]) -> str:
    typed = tuple(value)
    if any(not isinstance(item, RecoveryExecutionRuntimeRegistryHistoryEntry) for item in typed):
...
class RecoveryExecutionRuntimeRegistryHistoryManifest:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryManifest:
        value = _mapping(value, "runtime registry history manifest")
        _strict(value, set(cls.FIELDS), "runtime registry history manifest")
        return cls(*(value[field] for field in cls.FIELDS))


def address_manifest(value: RecoveryExecutionRuntimeRegistryHistoryManifest) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryManifest):
...
class RecoveryExecutionRuntimeRegistryHistorySummary:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistorySummary:
        value = _mapping(value, "runtime registry history summary")
        _strict(value, set(cls.FIELDS), "runtime registry history summary")
        return cls(*(value[field] for field in cls.FIELDS))


def address_summary(value: RecoveryExecutionRuntimeRegistryHistorySummary) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistorySummary):
...
class RecoveryExecutionRuntimeRegistryHistory:
    """Deterministic append-only history of one registry identity."""

    FIELDS = HISTORY_FIELDS

    def __init__(self, history_id: str, registry_id: str, version: str, boundary: str, entry_count: int, latest_registry_address: str, latest_entry_count: int, latest_accepted_count: int, latest_ready_count: int, latest_blocked_count: int, initial_count: int, improved_count: int, regressed_count: int, unchanged_count: int, changed_count: int, state: str, accepted: bool, manifest: RecoveryExecutionRuntimeRegistryHistoryManifest | Mapping[str, Any], summary: RecoveryExecutionRuntimeRegistryHistorySummary | Mapping[str, Any], entries: Sequence[RecoveryExecutionRuntimeRegistryHistoryEntry | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.manifest = manifest if isinstance(manifest, RecoveryExecutionRuntimeRegistryHistoryManifest) else RecoveryExecutionRuntimeRegistryHistoryManifest.from_mapping(manifest)
        self.summary = summary if isinstance(summary, RecoveryExecutionRuntimeRegistryHistorySummary) else RecoveryExecutionRuntimeRegistryHistorySummary.from_mapping(summary)
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistory:
        value = _mapping(value, "runtime registry history")
        _strict(value, set(cls.FIELDS), "runtime registry history")
        return cls(*(value[field] for field in cls.FIELDS))


def address_history(value: RecoveryExecutionRuntimeRegistryHistory) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistory):
...
def build_history(registries: Sequence[registry_model.RecoveryExecutionRuntimeRegistry], *, history_id: str = DEFAULT_HISTORY_ID) -> RecoveryExecutionRuntimeRegistryHistory:
    typed = tuple(_sequence(registries, "runtime registry history registries", MAX_ENTRIES))
    if any(not isinstance(item, registry_model.RecoveryExecutionRuntimeRegistry) for item in typed):
    ...
    entries: list[RecoveryExecutionRuntimeRegistryHistoryEntry] = []
    previous: RecoveryExecutionRuntimeRegistryHistoryEntry | None = None
    ...
    summary_provisional = RecoveryExecutionRuntimeRegistryHistorySummary(**summary_body)
    summary = RecoveryExecutionRuntimeRegistryHistorySummary(**(summary_body | {"content_address": address_summary(summary_provisional)}))
    ...
    manifest_provisional = RecoveryExecutionRuntimeRegistryHistoryManifest(**manifest_body)
    manifest = RecoveryExecutionRuntimeRegistryHistoryManifest(**(manifest_body | {"content_address": address_manifest(manifest_provisional)}))
    body = {
        "history_id": history_id,
        "registry_id": registry_id,
        "version": VERSION,
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistory(**body)
    return RecoveryExecutionRuntimeRegistryHistory(**(body | {"content_address": address_history(provisional)}))


def verify_history(value: RecoveryExecutionRuntimeRegistryHistory) -> RecoveryExecutionRuntimeRegistryHistory:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistory):
        raise ValidationError("runtime registry history verification requires a typed history")
    return RecoveryExecutionRuntimeRegistryHistory.from_mapping(value.to_dict())


def history_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistory:
    return RecoveryExecutionRuntimeRegistryHistory.from_mapping(value)


def history_json(value: RecoveryExecutionRuntimeRegistryHistory) -> str:
    return canonical_json(verify_history(value).to_dict())


def history_csv(value: RecoveryExecutionRuntimeRegistryHistory) -> str:
...
def render_history_markdown(value: RecoveryExecutionRuntimeRegistryHistory) -> str:
    value = verify_history(value)
    lines = [
        "# Federation archive recovery execution runtime registry history",
        "",
...
def entries_json(value: RecoveryExecutionRuntimeRegistryHistoryEntries) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryHistoryEntries.from_mapping(value.to_dict()).to_dict())


def summary_json(value: RecoveryExecutionRuntimeRegistryHistorySummary) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryHistorySummary.from_mapping(value.to_dict()).to_dict())


def manifest_json(value: RecoveryExecutionRuntimeRegistryHistoryManifest) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryHistoryManifest.from_mapping(value.to_dict()).to_dict())
...
def persist_history(value: RecoveryExecutionRuntimeRegistryHistory, destination: str | Path, *, overwrite: bool = False) -> Path:
    value = verify_history(value)
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".federation-recovery-execution-runtime-registry-history-", dir=str(destination.parent)))
    try:
        entries = RecoveryExecutionRuntimeRegistryHistoryEntries(value.entries, address_entries(value.entries))
...
def load_history(destination: str | Path) -> RecoveryExecutionRuntimeRegistryHistory:
    destination = Path(destination)
    ...
    manifest = RecoveryExecutionRuntimeRegistryHistoryManifest.from_mapping(documents["manifest.json"])
    entries = RecoveryExecutionRuntimeRegistryHistoryEntries.from_mapping(documents["entries.json"])
    summary = RecoveryExecutionRuntimeRegistryHistorySummary.from_mapping(documents["summary.json"])
...
def run_history(registries: Sequence[registry_model.RecoveryExecutionRuntimeRegistry], *, history_id: str = DEFAULT_HISTORY_ID, destination: str | Path | None = None, overwrite: bool = False) -> RecoveryExecutionRuntimeRegistryHistory:
...
__all__ = ["VERSION", "BOUNDARY", "HISTORY_PREFIX", "ENTRY_PREFIX", "ENTRIES_PREFIX", "MANIFEST_PREFIX", "SUMMARY_PREFIX", "DEFAULT_HISTORY_ID", "FILES", "ARTIFACT_FILES", "MANIFEST_ARTIFACT_FILES", "TRANSITIONS", "STATES", "MAX_ENTRIES", "MAX_HISTORY_BYTES", "ENTRY_FIELDS", "ENTRIES_FIELDS", "MANIFEST_FIELDS", "SUMMARY_FIELDS", "HISTORY_FIELDS", "RecoveryExecutionRuntimeRegistryHistoryEntry", "RecoveryExecutionRuntimeRegistryHistoryEntries", "RecoveryExecutionRuntimeRegistryHistoryManifest", "RecoveryExecutionRuntimeRegistryHistorySummary", "RecoveryExecutionRuntimeRegistryHistory", "address_entry", "address_entries", "address_manifest", "address_summary", "address_history", "entry_from_registry", "build_history", "verify_history", "history_from_mapping", "history_json", "history_csv", "render_history_markdown", "entries_json", "summary_json", "manifest_json", "persist_history", "load_history", "run_history", "entry_schema", "entries_schema", "manifest_schema", "summary_schema", "history_schema", "capabilities"]
