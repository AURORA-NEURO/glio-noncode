"""Typed contracts for fixed transport handoffs of module-578 policy decisions."""
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_578_ct import BOUNDARY as POLICY_BOUNDARY, POLICY_PREFIX
...
def _address(value: Any, field: str, prefix: str | None = None, *, allow_pending: bool = False) -> str:
    ...
    if allow_pending and value.endswith(":pending"):
        return value
    if ":" not in value or len(value.rsplit(":", 1)[-1]) != 64 or any(char not in "0123456789abcdef" for char in value.rsplit(":", 1)[-1]) or "/" in value or "\\" in value or '"' in value:
...
class Member:
    """One fixed payload member and its byte/content addresses."""
    ...
    def __post_init__(self) -> None:
        if self.relative_path not in PAYLOAD_NAMES or MEDIA_TYPES[self.relative_path] != self.media_type:
            raise ValidationError("module579 member path or media type is unsupported")
        _count(self.byte_count, "module579 member byte count", MAX_MEMBER_BYTES)
        _address(self.byte_address, "module579 member byte address", MEMBER_PREFIX + "-bytes")
        _address(self.content_address, "module579 member content address", MEMBER_PREFIX, allow_pending=True)
    ...
    def from_mapping(cls, value: dict[str, Any]) -> Member:
        if set(value) != {"relative_path", "media_type", "byte_count", "byte_address", "content_address"}:
            raise ValidationError("module579 member fields are not exact")
...
def address_member(value: Member) -> str:
    if not isinstance(value, Member):
        raise ValidationError("module579 member addressing requires the typed contract")
...
def address_member_bytes(payload: bytes) -> str:
    if not isinstance(payload, bytes) or len(payload) > MAX_MEMBER_BYTES:
        raise ValidationError("module579 member payload exceeds its byte bound")
...
class Manifest:
    ...
    def __post_init__(self) -> None:
        _label(self.package_id, "module579 package ID")
        if self.version != VERSION or self.boundary != BOUNDARY or self.package_format != PACKAGE_FORMAT:
            raise ValidationError("module579 manifest identity is not current")
        _address(self.diff_address, "module579 diff address")
        _address(self.policy_address, "module579 policy address")
        _address(self.audit_address, "module579 audit address")
        if self.policy_state not in {"ready", "blocked"} or not isinstance(self.policy_accepted, bool) or not isinstance(self.audit_accepted, bool):
            raise ValidationError("module579 decision flags are invalid")
        if tuple(item.relative_path for item in self.members) != PAYLOAD_NAMES:
            raise ValidationError("module579 payload order is not canonical")
        _address(self.content_address, "module579 manifest address", MANIFEST_PREFIX, allow_pending=True)
    ...
    def from_mapping(cls, value: dict[str, Any]) -> Manifest:
        ...
        if set(value) != fields:
            raise ValidationError("module579 manifest fields are not exact")
...
def address_manifest(value: Manifest) -> str:
    if not isinstance(value, Manifest):
        raise ValidationError("module579 manifest addressing requires the typed contract")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=MANIFEST_PREFIX)


@dataclass(frozen=True)
class Transport:
    """A deterministic ZIP carrying diff, policy, audit, and review evidence."""
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.package_bytes, bytes) or len(self.package_bytes) > MAX_PACKAGE_BYTES:
            raise ValidationError("module579 package bytes exceed their bound")
        _address(self.package_address, "module579 package address", PACKAGE_PREFIX)
        if self.manifest.content_address.endswith(":pending"):
            raise ValidationError("module579 manifest must be addressed")
