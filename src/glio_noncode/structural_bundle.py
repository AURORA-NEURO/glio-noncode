"""Compact JSON, CSV, and Markdown bundles for structural evidence."""
...
from .structural_lineage import build_structural_lineage
from .structural_public_data import StructuralFixtureCatalog, StructuralFixtureState
...
class StructuralBundleFormat(StrEnum):
    """Supported compact evidence bundle renderings."""
...
class StructuralBundleEntry:
    """One operation summary without copying raw structural payloads."""
    ...
    def __post_init__(self) -> None:
        ...
        if not self.evidence_address.startswith("sha256:"):
            raise ValidationError("structural evidence address must be content-addressed")
...
class StructuralEvidenceBundle:
    """Quality-gated structural bundle with stable manifest metadata."""
    ...
    entries: tuple[StructuralBundleEntry, ...]
    ...
    def render(self, output_format: StructuralBundleFormat | str) -> str:
        selected = StructuralBundleFormat(str(output_format))
        if selected == StructuralBundleFormat.JSON:
            return json.dumps(self.to_dict(), ensure_ascii=False, indent=2, sort_keys=True) + "\n"
        if selected == StructuralBundleFormat.CSV:
            buffer = io.StringIO()
            fields = (
                "entry_id",
                "entry_class",
...
class StructuralEvidenceBundleBuilder:
    """Build a compact bundle only after the structural quality gate passes."""

    _capability_by_operation = {
        "reconstruction": "GNC-D02-C01",
        "consensus": "GNC-D02-C02",
        "complex_resolution": "GNC-D02-C03",
        "copy_number": "GNC-D02-C04",
    }
    ...
    def build(
        ...
        *,
        ...
        catalog = StructuralFixtureCatalog.from_file(path)
        quality = evaluate_structural_quality_gate(catalog)
        if not quality.passed and not allow_review:
            raise ValidationError("structural evidence bundle requires a passing quality gate")
        evaluation = evaluate_structural_fixture(catalog)
        lineage = build_structural_lineage(catalog, graph_id=f"{catalog.fixture_id}-bundle-lineage")
        receipt_by_id = {receipt.record_id: receipt for receipt in evaluation.receipts}
        entries: list[StructuralBundleEntry] = []
        ...
        quality_summary = {
            ...
            "evidence_boundary": (
                "public aggregate structural observations; consensus, ambiguity, and review state "
                "remain explicit and no individual-level claim is made"
            ),
        ...
        body = {
            ...
            "contract_manifest": default_structural_contract_registry().manifest(),
    ...
    def write(
        ...
        output: str | Path,
        *,
        output_format: StructuralBundleFormat | str | None = None,
        bundle_id: str | None = None,
        allow_review: bool = False,
    ) -> StructuralEvidenceBundle:
    ...
    def _format_for_path(
        output: Path,
        output_format: StructuralBundleFormat | str | None,
    ) -> StructuralBundleFormat:
        if output_format is not None:
            return StructuralBundleFormat(str(output_format))
        if output.suffix.casefold() == ".csv":
            return StructuralBundleFormat.CSV
        if output.suffix.casefold() in {".md", ".markdown"}:
            return StructuralBundleFormat.MARKDOWN
        return StructuralBundleFormat.JSON
...
def build_structural_evidence_bundle(
    ...
    *,
    ...
    """Build a compact structural evidence bundle."""

    return StructuralEvidenceBundleBuilder().build(
        path,
        bundle_id=bundle_id,
