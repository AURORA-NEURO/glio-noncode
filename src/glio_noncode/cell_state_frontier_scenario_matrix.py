"""Positive and control scenario assertions for Domain 08."""
...
class CellStateFrontierScenario:
    scenario_id: str
...
@dataclass(frozen=True, slots=True)
class CellStateFrontierScenarioCheck:
    scenario_id: str
    passed: bool
    observed_state: str
...
@dataclass(frozen=True, slots=True)
class CellStateFrontierScenarioReport:
    fixture_id: str
    scenarios: tuple[CellStateFrontierScenario, ...]
    checks: tuple[CellStateFrontierScenarioCheck, ...]
    content_address: str
...
def default_cell_state_frontier_scenarios() -> tuple[CellStateFrontierScenario, ...]:
    fixture = default_cell_state_frontier_fixture()
    scenarios: list[CellStateFrontierScenario] = []
    for record in fixture.records:
        ...
        scenarios.append(CellStateFrontierScenario(**body, content_address=content_hash(body)))
    return tuple(scenarios)


def evaluate_cell_state_frontier_scenarios(
    evaluation: CellStateFrontierEvaluationReport | None = None,
) -> CellStateFrontierScenarioReport:
    selected = evaluation or evaluate_cell_state_frontier_fixture()
    scenarios = default_cell_state_frontier_scenarios()
    receipt_map = {item.record_id: item for item in selected.receipts}
    checks: list[CellStateFrontierScenarioCheck] = []
    for scenario in scenarios:
        ...
        checks.append(CellStateFrontierScenarioCheck(**body, content_address=content_hash(body)))
    body = {"fixture_id": selected.fixture_id, "scenarios": scenarios, "checks": checks}
    return CellStateFrontierScenarioReport(
        selected.fixture_id,
        scenarios,
        tuple(checks),
        content_hash(body),
