...
from .downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_tr_cat_ct import (
    BOUNDARY,
    CATALOG_FIELDS,
    CATALOG_PREFIX,
    ENTRY_FIELDS,
...
def _entry_from_mapping(value: Mapping[str, Any]) -> Entry:
    if not isinstance(value, Mapping):
        raise ValidationError("fixed policy transport catalog entry must be an object")
    _strict(value, set(ENTRY_FIELDS), "fixed policy transport catalog entry")
    return Entry(*(value[field] for field in ENTRY_FIELDS))


def _catalog_from_mapping(value: Mapping[str, Any]) -> Catalog:
    if not isinstance(value, Mapping):
        raise ValidationError("fixed policy transport catalog must be an object")
    _strict(value, set(CATALOG_FIELDS), "fixed policy transport catalog")
...
def build_catalog(packages: Sequence[Any], *, entry_ids: Sequence[str] | None = None, catalog_id: str = DEFAULT_CATALOG_ID) -> Catalog:
    sources = _sequence(packages, "fixed policy transport catalog packages")
    if entry_ids is not None and (isinstance(entry_ids, (str, bytes, bytearray)) or len(entry_ids) != len(sources)):
        raise ValidationError("fixed policy transport catalog entry ID count must match package count")
    loaded: list[tuple[str, Any]] = []
    for index, source in enumerate(sources):
        package = package_model.verify_package(source)
        entry_id = _label(entry_ids[index] if entry_ids is not None else package.manifest.package_id, "fixed policy transport catalog entry ID")
    ...
    if len({entry_id for entry_id, _package in loaded}) != len(loaded) or len({package.manifest.package_id for _entry_id, package in loaded}) != len(loaded) or len({package.package_address for _entry_id, package in loaded}) != len(loaded) or len({package.manifest.content_address for _entry_id, package in loaded}) != len(loaded):
        raise ValidationError("fixed policy transport catalog IDs, package IDs, addresses, and manifests must be unique")
    ...
    if total_package_bytes > MAX_TOTAL_PACKAGE_BYTES or total_member_bytes > MAX_TOTAL_MEMBER_BYTES:
        raise ValidationError("fixed policy transport catalog byte totals exceed their bounds")
    body = {"catalog_id": _label(catalog_id, "fixed policy transport catalog ID"), "version": VERSION, "boundary": BOUNDARY, "entry_count": len(entries), "accepted_count": sum(item.policy_accepted and item.audit_accepted for item in entries), "ready_count": sum(item.policy_state == "ready" for item in entries), "blocked_count": sum(item.policy_state == "blocked" for item in entries), "total_package_bytes": total_package_bytes, "total_member_bytes": total_member_bytes, "entries": entries, "content_address": CATALOG_PREFIX + ":pending"}
...
def verify_catalog(value: Any) -> Catalog:
    ...
    if not isinstance(catalog, Catalog):
        raise ValidationError("fixed policy transport catalog verification requires a typed catalog or mapping")
    if tuple((item.entry_id, item.package_address) for item in catalog.entries) != tuple(sorted((item.entry_id, item.package_address) for item in catalog.entries)):
        raise ValidationError("fixed policy transport catalog entries are not canonically sorted")
    if _has_forbidden_key(catalog.to_dict()) or address_catalog(catalog) != catalog.content_address:
        raise ValidationError("fixed policy transport catalog address or public boundary verification failed")
    if any(address_entry(item) != item.content_address for item in catalog.entries):
        raise ValidationError("fixed policy transport catalog entry address verification failed")
    if any(item.package_byte_count > MAX_PACKAGE_BYTES for item in catalog.entries):
        raise ValidationError("fixed policy transport catalog contains an oversized package")
    return catalog
...
def load_catalog(value: str | Path | bytes | bytearray | Mapping[str, Any] | Catalog) -> Catalog:
    ...
    raw = _strict_json_loads(bytes(value).decode("utf-8") if isinstance(value, (bytes, bytearray)) else read_text(value, field="fixed policy transport catalog", max_bytes=16 * 1024 * 1024))
...
def write_catalog(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Catalog:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("fixed policy transport catalog destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("fixed policy transport catalog destination is not a file")
    _validate_parent(path.parent, "fixed policy transport catalog destination")
    atomic_write_text(path, catalog_json(catalog), field="fixed policy transport catalog destination")
    return catalog
...
def query_catalog(value: Any, *, resource: str = "summary", state: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    if resource not in {"summary", "entries", "accepted", "ready", "blocked", "lineage"} or state not in {"", "ready", "blocked"} or not isinstance(text, str) or len(text) > 4096 or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("fixed policy transport catalog query bounds are invalid")
    catalog = verify_catalog(value)
    if resource == "summary":
