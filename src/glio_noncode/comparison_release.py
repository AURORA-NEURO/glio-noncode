...
def write_comparison_release_bundle(
    ...
    for artifact in bundle.artifacts:
        ...
        atomic_write_text(
            target,
            artifact.payload,
            field="comparison release artifact path",
            encoding="utf-8",
        )
    ...
    atomic_write_text(
        ...
        field="comparison release manifest path",
        encoding="utf-8",
    )
    return root
...
def verify_comparison_release_bundle(destination: str | Path) -> ComparisonReleaseVerification:
    ...
    try:
        manifest = _strict_json_loads(
            read_text(manifest_path, field="comparison release manifest path", encoding="utf-8")
        )
    ...
    for raw_artifact in artifacts:
        ...
        if not path.is_file():
            ...
            continue
        try:
            payload = read_bytes(path, field="comparison release artifact path")
