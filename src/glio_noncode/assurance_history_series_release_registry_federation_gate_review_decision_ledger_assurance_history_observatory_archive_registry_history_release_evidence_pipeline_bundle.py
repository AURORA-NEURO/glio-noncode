...
def _write_atomic_directory(destination: Path, payload: Mapping[str, bytes], *, overwrite: bool) -> Path:
    try:
        _validate_parent(destination.parent, "release evidence bundle destination")
        if destination.is_symlink():
            raise ValidationError("release evidence bundle destination cannot be a symlink")
        ...
        raise ValidationError("release evidence bundle destination could not be prepared") from error
    temporary = Path(tempfile.mkdtemp(prefix=".gnd-release-evidence-bundle-", dir=str(destination.parent)))
    try:
        for name in FILES:
            atomic_write_bytes(temporary / name, payload[name], field=f"release evidence bundle staging artifact {name}")
        if destination.exists():
...
def _read_directory(source: str | Path) -> dict[str, bytes]:
    try:
        ...
        _validate_parent(directory.parent, "release evidence bundle input")
        ...
        raise ValidationError("release evidence bundle input directory could not be inspected") from error
    try:
        return {name: read_bytes(directory / name, field=f"release evidence bundle member {name}") for name in FILES}
    except ValidationError:
        raise
    except OSError as error:
        raise ValidationError("release evidence bundle artifact could not be read") from error
