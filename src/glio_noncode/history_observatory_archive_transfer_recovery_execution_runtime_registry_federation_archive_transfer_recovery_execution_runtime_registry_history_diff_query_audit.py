"""Independent assurance for federation runtime-registry history-diff queries."""
...
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_registry_history_diff_query as query_model
...
class RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck:
...
def address_check(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck):
        raise ValidationError("runtime registry history diff query audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit:
    """A fixed-size independently recomputed query audit."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, query_address: str, diff_address: str, diff_id: str, query_id: str, version: str, boundary: str, check_count: int, passed_count: int, failed_count: int, accepted: bool, checks: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck) else RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck.from_mapping(item) for item in _sequence(checks, "runtime registry history diff query audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit:
...
def address_audit(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit):
        raise ValidationError("runtime registry history diff query audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck(**body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffQueryAuditCheck(**(body | {"content_address": address_check(provisional)}))


def _membership(query: query_model.RecoveryExecutionRuntimeRegistryHistoryDiffQuery, diff: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff) -> bool:
...
def _semantics(row: query_model.RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow, diff: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff) -> bool:
...
def audit_query(value: query_model.RecoveryExecutionRuntimeRegistryHistoryDiffQuery, diff: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit(**audit_body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit(**(audit_body | {"content_address": address_audit(provisional)}))


def verify_audit(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit):
        raise ValidationError("runtime registry history diff query audit verification requires a typed audit")
    return RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit.from_mapping(value.to_dict())


def audit_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit:
    return RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit.from_mapping(value)


def audit_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit) -> str:
    return canonical_json(verify_audit(value).to_dict())


def audit_csv(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit) -> str:
...
def render_audit_markdown(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryAudit) -> str:
    value = verify_audit(value)
