"""Cross-view reconciliation for the Domain 05 C01–C04 evidence plane."""
...
from .regulatory_atlas_lineage import RegulatoryAtlasLineageGraph
...
from .regulatory_atlas_scenario_matrix import RegulatoryAtlasScenarioMatrix
from .serialization import content_hash, jsonable


@dataclass(frozen=True, slots=True)
class RegulatoryAtlasReconciliationCheck:
...
@dataclass(frozen=True, slots=True)
class RegulatoryAtlasReconciliationReport:
    """Reconciled public data, execution, replay, scenario, and graph views."""

    fixture_id: str
    checks: tuple[RegulatoryAtlasReconciliationCheck, ...]
    content_address: str
...
def reconcile_regulatory_atlas_views(
    fixture: RegulatoryAtlasFixture,
    data_audit: RegulatoryAtlasDataAudit,
    evaluation: RegulatoryAtlasEvaluationReport,
    replay: RegulatoryAtlasReplayReport,
    scenarios: RegulatoryAtlasScenarioMatrix,
    lineage: RegulatoryAtlasLineageGraph,
) -> RegulatoryAtlasReconciliationReport:
    """Verify that independently produced artifacts describe one boundary."""

    catalog = build_regulatory_atlas_catalog(fixture)
    checks: list[RegulatoryAtlasReconciliationCheck] = []

    def add(check_id: str, passed: bool, detail: str) -> None:
        body = {"check_id": check_id, "passed": passed, "detail": detail}
        checks.append(RegulatoryAtlasReconciliationCheck(check_id, passed, detail, _address(body)))
    ...
    add(
        ...
        "all views remain inside the declared aggregate boundary",
    )
    ...
    add("scenario-floor", len(scenarios.results) >= 12, "independent state floors remain present")
    add(
        "source-closure",
        all(
    ...
    return RegulatoryAtlasReconciliationReport(fixture.fixture_id, tuple(checks), _address(body))
