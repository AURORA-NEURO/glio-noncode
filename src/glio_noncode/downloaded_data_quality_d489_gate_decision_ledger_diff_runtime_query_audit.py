"""Independent replay audit for D489 runtime queries."""
...
from . import downloaded_data_quality_d489_gate_decision_ledger_diff_runtime_query as query_model
...
class QueryAuditCheck:
    ...
    def __init__(self, ordinal: int, check_id: str, passed: bool, actual: Any, expected: Any, detail: str, content_address: str) -> None:
        self.ordinal = _count(ordinal, "ledger diff runtime query audit ordinal", MAX_CHECKS, lower=1)
        self.check_id = _label(check_id, "ledger diff runtime query audit check ID")
        self.passed = _bool(passed, "ledger diff runtime query audit result")
        self.actual = _text(actual, "ledger diff runtime query audit actual", 32768)
        self.expected = _text(expected, "ledger diff runtime query audit expected", 32768)
        self.detail = _text(detail, "ledger diff runtime query audit detail", 4096)
        self.content_address = _address(content_address, "ledger diff runtime query audit check address", CHECK_PREFIX)
        self._validate()

    def _validate(self) -> None:
        if self.check_id not in CHECK_IDS:
            raise ValidationError("ledger diff runtime query audit identity is unsupported")
        if not self.content_address.startswith("pending:") and _address_for(self, CHECK_PREFIX) != self.content_address:
            raise ValidationError("ledger diff runtime query audit check address does not replay")
        if not _public(self.to_dict()):
            raise ValidationError("ledger diff runtime query audit check crosses the public boundary")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "QueryAuditCheck":
        value = _mapping(value, "ledger diff runtime query audit check")
        _strict(value, set(cls.FIELDS), "ledger diff runtime query audit check")
        return cls(*(value[field] for field in cls.FIELDS))


def address_check(value: QueryAuditCheck) -> str:
    if not isinstance(value, QueryAuditCheck):
        raise ValidationError("ledger diff runtime query audit check address requires a typed check")
...
class QueryAudit:
    ...
    def __init__(self, query_id: str, runtime_id: str, query_address: str, checks: Any, check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.query_id = _label(query_id, "ledger diff runtime query audit query ID")
        self.runtime_id = _label(runtime_id, "ledger diff runtime query audit runtime ID")
        self.query_address = _address(query_address, "ledger diff runtime query audit query address", query_model.QUERY_PREFIX)
        self.checks = tuple(item if isinstance(item, QueryAuditCheck) else QueryAuditCheck.from_mapping(_mapping(item, "ledger diff runtime query audit check")) for item in _sequence(checks, "ledger diff runtime query audit checks", MAX_CHECKS))
        self.check_count = _count(check_count, "ledger diff runtime query audit check count", MAX_CHECKS)
        self.passed_count = _count(passed_count, "ledger diff runtime query audit passed count", MAX_CHECKS)
        self.failed_count = _count(failed_count, "ledger diff runtime query audit failed count", MAX_CHECKS)
        self.accepted = _bool(accepted, "ledger diff runtime query audit acceptance")
        self.content_address = _address(content_address, "ledger diff runtime query audit address", AUDIT_PREFIX)
        self._validate()
    ...
    def _validate(self) -> None:
        if self.check_count != len(self.checks) or self.check_count != MAX_CHECKS or self.passed_count + self.failed_count != self.check_count or self.passed_count != sum(item.passed for item in self.checks) or self.accepted != (self.failed_count == 0):
            raise ValidationError("ledger diff runtime query audit counters do not replay")
        if tuple(item.ordinal for item in self.checks) != tuple(range(1, MAX_CHECKS + 1)) or tuple(item.check_id for item in self.checks) != CHECK_IDS:
            raise ValidationError("ledger diff runtime query audit checks are not canonical")
        if not self.content_address.startswith("pending:") and _address_for(self, AUDIT_PREFIX) != self.content_address:
            raise ValidationError("ledger diff runtime query audit address does not replay")
        if not _public(self.to_dict()):
            raise ValidationError("ledger diff runtime query audit crosses the public boundary")

    def to_dict(self) -> dict[str, Any]:
        return {"query_id": self.query_id, "runtime_id": self.runtime_id, "query_address": self.query_address, "checks": [item.to_dict() for item in self.checks], "check_count": self.check_count, "passed_count": self.passed_count, "failed_count": self.failed_count, "accepted": self.accepted, "content_address": self.content_address}

    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "QueryAudit":
        value = _mapping(value, "ledger diff runtime query audit")
        _strict(value, set(cls.FIELDS), "ledger diff runtime query audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: QueryAudit) -> str:
    if not isinstance(value, QueryAudit):
        raise ValidationError("ledger diff runtime query audit address requires a typed audit")
...
def audit_query(value: query_model.RuntimeQuery, runtime: runtime_model.LedgerDiffRuntime) -> QueryAudit:
    query_model.verify_query(value)
    runtime_model.verify_runtime(runtime)
    all_rows = query_model._rows(runtime)
    ...
    provisional = QueryAudit(value.query_id, value.runtime_id, value.content_address, checks, len(checks), sum(item.passed for item in checks), sum(not item.passed for item in checks), all(item.passed for item in checks), f"pending:{AUDIT_PREFIX}")
    return _seal(provisional, address_audit)


def verify_audit(value: QueryAudit) -> QueryAudit:
    if not isinstance(value, QueryAudit):
        raise ValidationError("ledger diff runtime query audit verification requires a typed audit")
    value._validate()
    return value
...
def render_audit_markdown(value: QueryAudit) -> str:
    ...
    lines = [f"# Ledger diff runtime query audit {value.query_id}", "", f"- Accepted: {str(value.accepted).lower()}", f"- Checks: {value.passed_count}/{value.check_count}", "", "| Check | Result | Detail |", "| --- | --- | --- |"]
    lines.extend(f"| {item.check_id} | {str(item.passed).lower()} | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "LedgerDiffRuntimeQueryAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {field: {"type": "integer" if field == "ordinal" else "boolean" if field == "passed" else "string"} for field in CHECK_FIELDS}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "LedgerDiffRuntimeQueryAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {field: {"type": "array" if field == "checks" else "integer" if field.endswith("count") else "boolean" if field == "accepted" else "string"} for field in AUDIT_FIELDS}}
