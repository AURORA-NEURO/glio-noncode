"""Source-to-receipt lineage for Domain 08 cell-state evidence."""
...
class CellStateFrontierLineageEdge:
    edge_id: str
    record_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if not self.source_ids:
            raise ValueError("cell state lineage edge requires sources")
...
@dataclass(frozen=True, slots=True)
class CellStateFrontierLineageReport:
    fixture_id: str
    fixture_address: str
    edges: tuple[CellStateFrontierLineageEdge, ...]
    source_ids: tuple[str, ...]
    content_address: str
...
def build_cell_state_frontier_lineage(
    ...
    edges: list[CellStateFrontierLineageEdge] = []
    for receipt in evaluation.receipts:
        ...
        body = {
            ...
            "transformation": "parse public aggregate cell-state rows, apply declared adapter, emit sanitized receipt",
        }
        address = content_hash(body)
        edges.append(CellStateFrontierLineageEdge("edge:" + address.split(":", 1)[1][:24], **body, content_address=address))
    ...
    return CellStateFrontierLineageReport(fixture.fixture_id, fixture.content_address, tuple(edges), body["source_ids"], content_hash(body))


def verify_cell_state_frontier_lineage(
    ...
    if tuple(item.record_id for item in lineage.edges) != tuple(item.record_id for item in evaluation.receipts):
    ...
    if any("input_text" in item.transformation for item in lineage.edges):
