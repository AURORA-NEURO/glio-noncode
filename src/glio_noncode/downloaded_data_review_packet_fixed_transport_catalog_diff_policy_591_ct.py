"""Typed contracts for module590 policy-decision transports."""
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_590_ct import BOUNDARY as POLICY_BOUNDARY, POLICY_PREFIX
...
class Member:
    ...
    def __post_init__(self) -> None:
        if self.relative_path not in PAYLOAD_NAMES or MEDIA_TYPES[self.relative_path] != self.media_type:
            raise ValidationError("module591 member path or media type is unsupported")
        _count(self.byte_count, "module591 member byte count", MAX_MEMBER_BYTES)
        _address(self.byte_address, "module591 member byte address", MEMBER_PREFIX + "-bytes")
        _address(self.content_address, "module591 member content address", MEMBER_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_member(self) != self.content_address:
            raise ValidationError("module591 member address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Member":
        if set(value) != {"relative_path", "media_type", "byte_count", "byte_address", "content_address"}:
            raise ValidationError("module591 member fields are not exact")
...
def address_member_bytes(payload: bytes) -> str:
    if not isinstance(payload, bytes) or len(payload) > MAX_MEMBER_BYTES:
        raise ValidationError("module591 member payload exceeds its byte bound")
...
class Manifest:
    ...
    def __post_init__(self) -> None:
        _label(self.package_id, "module591 package ID")
        if self.version != VERSION or self.boundary != BOUNDARY or self.package_format != PACKAGE_FORMAT:
            raise ValidationError("module591 manifest identity is not current")
        _address(self.diff_address, "module591 diff address")
        _address(self.policy_address, "module591 policy address")
        _address(self.audit_address, "module591 audit address")
        if self.policy_state not in {"ready", "blocked"} or not isinstance(self.policy_accepted, bool) or not isinstance(self.audit_accepted, bool):
            raise ValidationError("module591 decision flags are invalid")
        if tuple(item.relative_path for item in self.members) != PAYLOAD_NAMES:
            raise ValidationError("module591 payload order is not canonical")
        _address(self.content_address, "module591 manifest address", MANIFEST_PREFIX, allow_pending=True)
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Manifest":
        ...
        if set(value) != fields:
            raise ValidationError("module591 manifest fields are not exact")
...
class Transport:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.package_bytes, bytes) or len(self.package_bytes) > MAX_PACKAGE_BYTES:
            raise ValidationError("module591 package bytes exceed their bound")
        _address(self.package_address, "module591 package address", PACKAGE_PREFIX)
        if self.manifest.content_address.endswith(":pending"):
            raise ValidationError("module591 manifest must be addressed")
