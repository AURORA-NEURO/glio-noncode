"""Coverage, issue, and scale metrics for Domain 05 receipts."""
...
from .regulatory_atlas_public_data import RegulatoryAtlasOperation, RegulatoryAtlasRole
from .serialization import content_hash, jsonable, require_non_empty


@dataclass(frozen=True, slots=True)
class RegulatoryAtlasOperationMetric:
    """Per-operation count, state, and issue profile."""
...
@dataclass(frozen=True, slots=True)
class RegulatoryAtlasMetricsReport:
    ...
    supported_count: int
    review_count: int
    operation_metrics: tuple[RegulatoryAtlasOperationMetric, ...]
    issue_code_counts: tuple[tuple[str, int], ...]
...
def _operation_metric(
    operation: RegulatoryAtlasOperation, receipts: tuple[Any, ...]
    ...
    body = {
        ...
        "positive_count": sum(item.role is RegulatoryAtlasRole.POSITIVE for item in receipts),
        "control_count": sum(item.role is RegulatoryAtlasRole.CONTROL for item in receipts),
    ...
    return RegulatoryAtlasOperationMetric(**body, content_address=_address(body))


def build_regulatory_atlas_metrics(
    evaluation: RegulatoryAtlasEvaluationReport,
) -> RegulatoryAtlasMetricsReport:
    """Build deterministic metrics without copying executable payloads."""
    ...
    return RegulatoryAtlasMetricsReport(**body, content_address=_address(body))


def verify_regulatory_atlas_metrics(report: RegulatoryAtlasMetricsReport) -> tuple[str, ...]:
...
def render_regulatory_atlas_metrics(report: RegulatoryAtlasMetricsReport) -> dict[str, Any]:
