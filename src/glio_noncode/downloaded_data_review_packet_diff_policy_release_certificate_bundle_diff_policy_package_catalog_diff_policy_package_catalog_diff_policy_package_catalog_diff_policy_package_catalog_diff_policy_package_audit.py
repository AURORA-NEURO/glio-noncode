"""Independently audit policy-gated packet-package handoff catalog review packets."""
...
from . import (
    downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff as diff_model,
)
from . import (
    downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy as policy_model,
)
from . import (
    downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package as package_model,
)
from . import (
    downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_policy_audit as policy_audit_model,
)
...
class DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAuditCheck:
    ...
    def __init__(self, check_id: str, passed: bool, observed: Any, required: Any, detail: str, content_address: str) -> None:
        self.check_id = _label(check_id, "catalog diff policy packet audit check ID")
        if self.check_id not in CHECK_IDS:
            raise ValidationError("catalog diff policy packet audit check ID is unsupported")
        if not isinstance(passed, bool):
            raise ValidationError("catalog diff policy packet audit result must be boolean")
        self.passed = passed
        self.observed = observed
        self.required = required
        self.detail = _text(detail, "catalog diff policy packet audit detail", 2048)
        self.content_address = _address(content_address, "catalog diff policy packet audit check address", CHECK_PREFIX) if not content_address.endswith(":pending") else content_address
        if not content_address.endswith(":pending") and address_check(self) != content_address:
            raise ValidationError("catalog diff policy packet audit check address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAuditCheck:
        if not isinstance(value, Mapping) or set(value) != set(cls.FIELDS):
            raise ValidationError("catalog diff policy packet audit check fields are not exact")
...
class DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAudit:
    ...
    def __init__(self, package_id: str, package_address: str, checks: tuple[DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAuditCheck, ...], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.package_id = _label(package_id, "catalog diff policy packet audit package ID")
        self.package_address = _address(package_address, "catalog diff policy packet audit package address", package_model.PACKAGE_PREFIX)
        self.checks = tuple(checks)
        self.check_count = check_count
        self.passed_count = passed_count
        self.failed_count = failed_count
        self.accepted = accepted
        self.content_address = _address(content_address, "catalog diff policy packet audit address", AUDIT_PREFIX) if not content_address.endswith(":pending") else content_address
        if self.check_count != len(self.checks) or self.check_count != len(CHECK_IDS) or tuple(item.check_id for item in self.checks) != CHECK_IDS or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0):
            raise ValidationError("catalog diff policy packet audit aggregates do not replay")
        if not content_address.endswith(":pending") and address_audit(self) != content_address:
            raise ValidationError("catalog diff policy packet audit address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAudit:
        if not isinstance(value, Mapping) or set(value) != set(cls.FIELDS):
            raise ValidationError("catalog diff policy packet audit fields are not exact")
...
def audit_package(value: Any) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAudit:
    try:
        ...
        checks = tuple(_check(check_id, False, "unavailable", "replayable", f"catalog diff policy packet audit unavailable: {exc}") for check_id in CHECK_IDS)
...
def load_audit(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAudit:
    if isinstance(value, Mapping):
        ...
        raw = _strict_json_loads(read_text(value, field="catalog diff policy packet audit", max_bytes=16 * 1024 * 1024))
...
def write_audit(value: Any, destination: str | Path, *, allow_existing: bool = False) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketAudit:
    ...
    atomic_write_text(path, audit_json(audit), field="catalog diff policy packet audit destination")
    return audit
...
def query_audit(value: Any, *, passed: bool | None = None, text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if passed is not None and not isinstance(passed, bool):
        raise ValidationError("catalog diff policy packet audit passed filter must be boolean")
    if not isinstance(text, str) or len(text) > 4096 or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("catalog diff policy packet audit query bounds are invalid")
