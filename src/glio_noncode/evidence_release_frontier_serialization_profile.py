"""Detailed serialization profile plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseSerializationProfile:
    field_count: int
    accepted: bool
...
def build_evidence_release_serialization_profile(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseSerializationProfile:
    ...
    return EvidenceReleaseSerializationProfile(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_serialization_profile(receipt: EvidenceReleaseSerializationProfile, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "field_count") != expected:
        return False
    return all(
...
def summarize_serialization_profile(receipt: EvidenceReleaseSerializationProfile) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "field_count"),
...
__all__ = ["EvidenceReleaseSerializationProfile", "validate_evidence_release_serialization_profile", "build_evidence_release_serialization_profile", "summarize_serialization_profile"]
