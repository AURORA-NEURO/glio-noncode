"""Independent replay audit for D484 history-diff runtimes."""
...
class AuditCheck:
    ...
    def __init__(self, ordinal: int, check_id: str, passed: bool, actual: Any, expected: Any, detail: str, content_address: str) -> None:
        self.ordinal = _count(ordinal, "history diff runtime audit ordinal", MAX_CHECKS, lower=1); self.check_id = _label(check_id, "history diff runtime audit check ID"); self.passed = _bool(passed, "history diff runtime audit result"); self.actual = _text(actual, "history diff runtime audit actual", 32768); self.expected = _text(expected, "history diff runtime audit expected", 32768); self.detail = _text(detail, "history diff runtime audit detail", 4096); self.content_address = _address(content_address, "history diff runtime audit check address", CHECK_PREFIX); self._validate()
    def _validate(self) -> None:
        if self.check_id not in CHECK_IDS: raise ValidationError("history diff runtime audit check identity is unsupported")
        if not self.content_address.startswith("pending:") and _address_for(self, CHECK_PREFIX) != self.content_address: raise ValidationError("history diff runtime audit check address does not replay")
        if not _public(self.to_dict()): raise ValidationError("history diff runtime audit check crosses the public boundary")
...
def address_check(value: AuditCheck) -> str:
    if not isinstance(value, AuditCheck): raise ValidationError("history diff runtime audit check address requires a typed check")
...
class RuntimeAudit:
    ...
    def __init__(self, runtime_id: str, diff_id: str, runtime_address: str, checks: Any, check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.runtime_id = _label(runtime_id, "history diff runtime audit runtime ID"); self.diff_id = _label(diff_id, "history diff runtime audit diff ID"); self.runtime_address = _address(runtime_address, "history diff runtime audit runtime address", runtime_model.RUNTIME_PREFIX); self.checks = tuple(item if isinstance(item, AuditCheck) else AuditCheck.from_mapping(_mapping(item, "history diff runtime audit check")) for item in _sequence(checks, "history diff runtime audit checks", MAX_CHECKS)); self.check_count = _count(check_count, "history diff runtime audit check count", MAX_CHECKS); self.passed_count = _count(passed_count, "history diff runtime audit passed count", MAX_CHECKS); self.failed_count = _count(failed_count, "history diff runtime audit failed count", MAX_CHECKS); self.accepted = _bool(accepted, "history diff runtime audit acceptance"); self.content_address = _address(content_address, "history diff runtime audit address", AUDIT_PREFIX); self._validate()
    def _validate(self) -> None:
        if self.check_count != len(self.checks) or self.check_count != MAX_CHECKS or self.passed_count + self.failed_count != self.check_count or self.passed_count != sum(item.passed for item in self.checks) or self.accepted != (self.failed_count == 0): raise ValidationError("history diff runtime audit counters do not replay")
        if tuple(item.ordinal for item in self.checks) != tuple(range(1, MAX_CHECKS + 1)) or tuple(item.check_id for item in self.checks) != CHECK_IDS: raise ValidationError("history diff runtime audit checks are not canonical")
        if not self.content_address.startswith("pending:") and _address_for(self, AUDIT_PREFIX) != self.content_address: raise ValidationError("history diff runtime audit address does not replay")
        if not _public(self.to_dict()): raise ValidationError("history diff runtime audit crosses the public boundary")
...
def address_audit(value: RuntimeAudit) -> str:
    if not isinstance(value, RuntimeAudit): raise ValidationError("history diff runtime audit address requires a typed audit")
...
def audit_runtime(value: runtime_model.HistoryDiffRuntime, diff: diff_model.HistoryDiff | None = None) -> RuntimeAudit:
    runtime_model.verify_runtime(value, diff); checks = (
        _finding(1, "typed_runtime", isinstance(value, runtime_model.HistoryDiffRuntime), type(value).__name__, "HistoryDiffRuntime", "the audited value must use the typed runtime model"),
    ...
    ); provisional = RuntimeAudit(value.runtime_id, value.diff_id, value.content_address, checks, len(checks), sum(item.passed for item in checks), sum(not item.passed for item in checks), all(item.passed for item in checks), f"pending:{AUDIT_PREFIX}"); return _seal(provisional, address_audit)
def verify_audit(value: RuntimeAudit) -> RuntimeAudit:
    if not isinstance(value, RuntimeAudit): raise ValidationError("history diff runtime audit verification requires a typed audit")
...
def render_audit_markdown(value: RuntimeAudit) -> str:
    value = verify_audit(value); lines = [f"# History diff runtime audit {value.runtime_id}", "", f"- Accepted: {str(value.accepted).lower()}", f"- Checks: {value.passed_count}/{value.check_count}", "", "| Check | Result | Detail |", "| --- | --- | --- |"] ; lines.extend(f"| {item.check_id} | {str(item.passed).lower()} | {item.detail} |" for item in value.checks); return "\n".join(lines) + "\n"
def check_schema() -> dict[str, Any]: return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "HistoryDiffRuntimeAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {field: {"type": "integer" if field == "ordinal" else "boolean" if field == "passed" else "string"} for field in CHECK_FIELDS}}
def audit_schema() -> dict[str, Any]: return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "HistoryDiffRuntimeAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {field: {"type": "array" if field == "checks" else "integer" if field.endswith("count") else "boolean" if field == "accepted" else "string"} for field in AUDIT_FIELDS}}
def capabilities() -> dict[str, Any]: return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "check_ids": CHECK_IDS, "max_checks": MAX_CHECKS, "features": ("independent history diff runtime replay", "policy and lineage verification", "canonical check-address verification", "JSON CSV and Markdown projections"), "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}
