...
class ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow:
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery:
    ...
    def __init__(self, query_id: str, version: str, boundary: str, runtime_address: str, runtime_id: str, execution_id: str, resources: Sequence[str], state_filter: str, key_filter: str, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow], content_address: str) -> None:
        ...
        self.rows = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow) else ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow.from_mapping(item) for item in _sequence(rows, "runtime query rows", MAX_LIMIT))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery":
        value = _mapping(value, "recovery execution runtime query")
        _strict(value, set(cls.FIELDS), "recovery execution runtime query")
        return cls(value["query_id"], value["version"], value["boundary"], value["runtime_address"], value["runtime_id"], value["execution_id"], value["resources"], value["state_filter"], value["key_filter"], value["text_filter"], value["offset"], value["limit"], value["total_count"], value["returned_count"], value["truncated"], tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow.from_mapping(item) for item in _sequence(value["rows"], "runtime query rows", MAX_LIMIT)), value["content_address"])


def address_row(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow):
...
def address_query(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery):
        raise ValidationError("runtime query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def _row(resource: str, ordinal: int, key: str, value: Any, address: str, state: str, accepted: bool) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow:
    pending = ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow(resource, ordinal, key, value, address, state, accepted, "pending:runtime-query-row")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow(resource, ordinal, key, value, address, state, accepted, address_row(pending))


def _all_rows(value: runtime_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntime) -> tuple[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow, ...]:
    rows: list[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow] = []
...
def _matches(row: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow, *, state: str, key: str, text: str) -> bool:
...
def query_runtime(value: runtime_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntime, *, query_id: str = "runtime-registry-history-diff-archive-transfer-recovery-execution-runtime-query", resources: Sequence[str] | None = None, state: str = "", key: str = "", text: str = "", offset: int = 0, limit: int = MAX_LIMIT) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery:
    ...
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery(query_id, VERSION, BOUNDARY, value.content_address, value.runtime_id, value.execution_id, selected, state, key, text, offset, limit, len(rows), len(page_rows), offset + len(page_rows) < len(rows), page_rows, "pending:runtime-query")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery(query_id, VERSION, BOUNDARY, value.content_address, value.runtime_id, value.execution_id, selected, state, key, text, offset, limit, len(rows), len(page_rows), offset + len(page_rows) < len(rows), page_rows, address_query(provisional))


def query_from_mapping(value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery:
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery.from_mapping(value)


def query_json(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery) -> str:
    return canonical_json(query_from_mapping(value.to_dict()).to_dict())


def query_csv(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery) -> str:
...
def render_query_markdown(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery) -> str:
    value = query_from_mapping(value.to_dict())
...
__all__ = ["BOUNDARY", "MAX_LIMIT", "MAX_QUERY_ITEMS", "QUERY_FIELDS", "QUERY_PREFIX", "RESOURCES", "ROW_FIELDS", "ROW_PREFIX", "ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQuery", "ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeQueryRow", "VERSION", "address_query", "address_row", "capabilities", "query_csv", "query_from_mapping", "query_json", "query_runtime", "query_schema", "render_query_markdown", "row_schema"]
