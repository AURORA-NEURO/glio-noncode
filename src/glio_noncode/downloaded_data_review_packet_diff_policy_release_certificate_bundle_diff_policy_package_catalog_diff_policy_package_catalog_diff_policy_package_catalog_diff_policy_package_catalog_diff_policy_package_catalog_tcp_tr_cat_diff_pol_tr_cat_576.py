"""Build, verify, query, and persist source-free catalogs of fixed package catalog diff policy transports."""
...
from .downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_tcp_tr_cat_diff_pol_tr_cat_576_ct import (
    BOUNDARY,
    CATALOG_FIELDS,
    CATALOG_PREFIX,
    ENTRY_FIELDS,
...
def _entry_from_mapping(value: Mapping[str, Any]) -> Entry:
    if not isinstance(value, Mapping):
        raise ValidationError("fixed transport catalog-diff policy package catalog entry must be an object")
    _strict(value, set(ENTRY_FIELDS), "fixed transport catalog-diff policy package catalog entry")
    return Entry(*(value[field] for field in ENTRY_FIELDS))


def _catalog_from_mapping(value: Mapping[str, Any]) -> Catalog:
    if not isinstance(value, Mapping):
        raise ValidationError("fixed transport catalog-diff policy package catalog must be an object")
    _strict(value, set(CATALOG_FIELDS), "fixed transport catalog-diff policy package catalog")
...
def build_catalog(packages: Sequence[Any], *, entry_ids: Sequence[str] | None = None, catalog_id: str = DEFAULT_CATALOG_ID) -> Catalog:
    sources = _sequence(packages, "fixed transport catalog-diff policy package catalog packages")
    if entry_ids is not None and (isinstance(entry_ids, (str, bytes, bytearray)) or len(entry_ids) != len(sources)):
        raise ValidationError("fixed transport catalog-diff policy package catalog entry ID count must match package count")
    loaded: list[tuple[str, Any]] = []
    for index, source in enumerate(sources):
        package = package_model.verify_package(source)
        entry_id = _label(entry_ids[index] if entry_ids is not None else package.manifest.package_id, "fixed transport catalog-diff policy package catalog entry ID")
    ...
    if len({entry_id for entry_id, _package in loaded}) != len(loaded) or len({package.manifest.package_id for _entry_id, package in loaded}) != len(loaded) or len({package.package_address for _entry_id, package in loaded}) != len(loaded) or len({package.manifest.content_address for _entry_id, package in loaded}) != len(loaded):
        raise ValidationError("fixed transport catalog-diff policy package catalog IDs, package IDs, addresses, and manifests must be unique")
    ...
    if total_package_bytes > MAX_TOTAL_PACKAGE_BYTES or total_member_bytes > MAX_TOTAL_MEMBER_BYTES:
        raise ValidationError("fixed transport catalog-diff policy package catalog byte totals exceed their bounds")
    body = {"catalog_id": _label(catalog_id, "fixed transport catalog-diff policy package catalog ID"), "version": VERSION, "boundary": BOUNDARY, "entry_count": len(entries), "accepted_count": sum(item.policy_accepted and item.audit_accepted for item in entries), "ready_count": sum(item.policy_state == "ready" for item in entries), "blocked_count": sum(item.policy_state == "blocked" for item in entries), "total_package_bytes": total_package_bytes, "total_member_bytes": total_member_bytes, "entries": entries, "content_address": CATALOG_PREFIX + ":pending"}
...
def verify_catalog(value: Any) -> Catalog:
    ...
    if not isinstance(catalog, Catalog):
        raise ValidationError("fixed transport catalog-diff policy package catalog verification requires a typed catalog or mapping")
    if tuple((item.entry_id, item.package_address) for item in catalog.entries) != tuple(sorted((item.entry_id, item.package_address) for item in catalog.entries)):
        raise ValidationError("fixed transport catalog-diff policy package catalog entries are not canonically sorted")
    if _has_forbidden_key(catalog.to_dict()) or address_catalog(catalog) != catalog.content_address:
        raise ValidationError("fixed transport catalog-diff policy package catalog address or public boundary verification failed")
    if any(address_entry(item) != item.content_address for item in catalog.entries):
        raise ValidationError("fixed transport catalog-diff policy package catalog entry address verification failed")
    if any(item.package_byte_count > MAX_PACKAGE_BYTES for item in catalog.entries):
        raise ValidationError("fixed transport catalog-diff policy package catalog contains an oversized package")
    return catalog
...
def load_catalog(value: str | Path | bytes | bytearray | Mapping[str, Any] | Catalog) -> Catalog:
    ...
    raw = _strict_json_loads(bytes(value).decode("utf-8") if isinstance(value, (bytes, bytearray)) else read_text(value, field="fixed transport catalog-diff policy package catalog", max_bytes=16 * 1024 * 1024))
...
def write_catalog(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Catalog:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("fixed transport catalog-diff policy package catalog destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("fixed transport catalog-diff policy package catalog destination is not a file")
    _validate_parent(path.parent, "fixed transport catalog-diff policy package catalog destination")
    atomic_write_text(path, catalog_json(catalog), field="fixed transport catalog-diff policy package catalog destination")
    return catalog
...
def query_catalog(value: Any, *, resource: str = "summary", state: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    if resource not in {"summary", "entries", "accepted", "ready", "blocked", "lineage"} or state not in {"", "ready", "blocked"} or not isinstance(text, str) or len(text) > 4096 or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("fixed transport catalog-diff policy package catalog query bounds are invalid")
    catalog = verify_catalog(value)
    if resource == "summary":
...
def render_catalog_markdown(value: Any) -> str:
    catalog = verify_catalog(value)
    lines = ["# Downloaded-Data Fixed Transport Catalog-Diff Policy Package Catalog", "", f"- Catalog: `{catalog.catalog_id}`", f"- Entries: **{catalog.entry_count}**", f"- Accepted: **{catalog.accepted_count}**", f"- Ready / blocked: **{catalog.ready_count} / {catalog.blocked_count}**", f"- Package / member bytes: **{catalog.total_package_bytes} / {catalog.total_member_bytes}**", f"- Address: `{catalog.content_address}`", "", "| # | entry | package | state | accepted | package bytes |", "| ---: | --- | --- | --- | --- | ---:|"]
...
# Transport-catalog names make the module-564 boundary explicit.
...
query_transport_catalog = query_catalog
transport_catalog_csv = catalog_csv
render_transport_catalog_markdown = render_catalog_markdown
transport_catalog_schema = catalog_schema
transport_entry_schema = entry_schema


build_fixed_transport_catalog_diff_policy_package_catalog = build_catalog
verify_fixed_transport_catalog_diff_policy_package_catalog = verify_catalog
load_fixed_transport_catalog_diff_policy_package_catalog = load_catalog
fixed_transport_catalog_diff_policy_package_catalog_json = catalog_json
write_fixed_transport_catalog_diff_policy_package_catalog = write_catalog
query_fixed_transport_catalog_diff_policy_package_catalog = query_catalog
fixed_transport_catalog_diff_policy_package_catalog_csv = catalog_csv
render_fixed_transport_catalog_diff_policy_package_catalog_markdown = render_catalog_markdown
fixed_transport_catalog_diff_policy_package_catalog_schema = catalog_schema
fixed_transport_catalog_diff_policy_package_entry_schema = entry_schema


__all__ = ["BOUNDARY", "CATALOG_FIELDS", "CATALOG_PREFIX", "DEFAULT_CATALOG_ID", "ENTRY_FIELDS", "ENTRY_PREFIX", "MAX_ENTRIES", "MAX_LIMIT", "MAX_PACKAGE_BYTES", "MAX_TOTAL_MEMBER_BYTES", "MAX_TOTAL_PACKAGE_BYTES", "VERSION", "DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyPackageCatalogDiffPolicyPackageCatalog", "DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyPackageCatalogDiffPolicyPackageCatalogEntry", "address_catalog", "address_entry", "build_catalog", "build_fixed_transport_catalog_diff_policy_package_catalog", "build_transport_catalog", "capabilities", "catalog_csv", "catalog_from_mapping", "catalog_json", "catalog_schema", "entry_schema", "fixed_transport_catalog_diff_policy_package_catalog_csv", "fixed_transport_catalog_diff_policy_package_catalog_json", "fixed_transport_catalog_diff_policy_package_catalog_schema", "fixed_transport_catalog_diff_policy_package_entry_schema", "load_catalog", "load_fixed_transport_catalog_diff_policy_package_catalog", "load_transport_catalog", "query_catalog", "query_fixed_transport_catalog_diff_policy_package_catalog", "query_transport_catalog", "render_catalog_markdown", "render_fixed_transport_catalog_diff_policy_package_catalog_markdown", "render_transport_catalog_markdown", "transport_catalog_csv", "transport_catalog_json", "transport_catalog_schema", "transport_entry_schema", "verify_catalog", "verify_fixed_transport_catalog_diff_policy_package_catalog", "verify_transport_catalog", "write_catalog", "write_fixed_transport_catalog_diff_policy_package_catalog", "write_transport_catalog"]
