"""Bounded public queries over exact archive-transfer execution receipts."""
...
from . import exact_history_diff_archive_transfer_recovery_execution_ledger_runtime_registry_history_diff_archive_transfer_recovery_execution as execution_model
...
MAX_LIMIT = 256
MAX_QUERY_ITEMS = execution_model.MAX_OUTCOMES * 4 + 16
RESOURCES = ("summary", "outcomes", "addresses", "applied", "pending", "rejected", "state", "decisions", "bounds")
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow:
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery:
    """A deterministic, bounded execution query result."""

    FIELDS = QUERY_FIELDS

    def __init__(self, query_id: str, version: str, boundary: str, execution_address: str, execution_id: str, resources: Sequence[str], status_filter: str, index_filter: int, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[Any], content_address: str) -> None:
        self.query_id = _label(query_id, "execution query ID")
        self.version = _text(version, "execution query version", 2048, required=True)
        self.boundary = _text(boundary, "execution query boundary", 2048, required=True)
        ...
        self.truncated = bool(truncated) if isinstance(truncated, bool) else _count(truncated, "execution query truncation", 1) == 1
        self.rows = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow) else ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow.from_mapping(item) for item in _sequence(rows, "execution query rows", MAX_LIMIT))
    ...
    def _validate(self) -> None:
        ...
        if self.total_count < self.returned_count or self.offset + self.returned_count > self.total_count:
    ...
    def from_mapping(cls, value: Mapping[str, Any]):
        ...
        return cls(value["query_id"], value["version"], value["boundary"], value["execution_address"], value["execution_id"], value["resources"], value["status_filter"], value["index_filter"], value["text_filter"], value["offset"], value["limit"], value["total_count"], value["returned_count"], value["truncated"], tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow.from_mapping(item) for item in _sequence(value["rows"], "execution query rows", MAX_LIMIT)), value["content_address"])


def address_row(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow):
...
def address_query(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery):
        raise ValidationError("execution query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def _row(resource: str, ordinal: int, *, index: int = -1, action_address: str = "", content_address: str = "", offset: int = 0, size: int = 0, status: str = "", reason: str = "", outcome_address: str = "", state: str = "", decision: str = ""):
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow(resource, ordinal, index, action_address, content_address, offset, size, status, reason, outcome_address, state, decision, "pending:execution-query-row")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow(resource, ordinal, index, action_address, content_address, offset, size, status, reason, outcome_address, state, decision, address_row(provisional))


def _all_rows(value: Any) -> tuple[Any, ...]:
    rows = [_row("summary", 0, reason="execution-summary", state=value.state, decision=value.decision, offset=value.current_received_bytes, size=value.current_remaining_bytes)]
    ...
    for resource, status in (("addresses", ""), ("applied", "applied"), ("pending", "pending"), ("rejected", "rejected")):
        for item in value.outcomes:
            if not status or item.status == status:
                rows.append(_row(resource, len(rows), index=item.index, action_address=item.action_address, content_address=item.content_address, offset=item.offset, size=item.size, status=item.status, reason="address-evidence" if resource == "addresses" else item.reason, outcome_address=item.outcome_address))
    rows.append(_row("state", len(rows), reason="execution-state", state=value.state, decision=value.decision))
    rows.append(_row("decisions", len(rows), reason="execution-decision", state=value.state, decision=value.decision))
...
def _matches(row: Any, *, status: str, index: int, text: str) -> bool:
...
def query_execution(value: Any, *, query_id: str = QUERY_PREFIX, resources: Sequence[str] | None = None, status: str = "", index: int = -1, text: str = "", offset: int = 0, limit: int = MAX_LIMIT):
    if not isinstance(value, execution_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution):
        raise ValidationError("execution query requires a typed execution")
    requested = None if resources is None else tuple(resources)
    selected = tuple(resource for resource in RESOURCES if requested is None or resource in requested)
    if not selected:
    ...
    text = _text(text, "execution query text", 512)
    ...
    for ordinal, item in enumerate(page):
        page_rows.append(_row(item.resource, ordinal, index=item.index, action_address=item.action_address, content_address=item.content_address, offset=item.offset, size=item.size, status=item.status, reason=item.reason, outcome_address=item.outcome_address, state=item.state, decision=item.decision))
    result = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery(query_id, VERSION, BOUNDARY, value.content_address, value.execution_id, selected, status, index, text, offset, limit, len(rows), len(page_rows), offset + len(page_rows) < len(rows), page_rows, "pending:execution-query")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery(query_id, VERSION, BOUNDARY, value.content_address, value.execution_id, selected, status, index, text, offset, limit, len(rows), len(page_rows), offset + len(page_rows) < len(rows), page_rows, address_query(result))


def query_from_mapping(value: Mapping[str, Any]):
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery.from_mapping(value)
...
def render_query_markdown(value: Any) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# Exact execution-ledger archive transfer recovery execution query", "", f"- Execution: `{value.execution_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.returned_count}/{value.total_count}`", f"- Address: `{value.content_address}`", "", "| resource | ordinal | index | status | state | decision | reason | row address |", "| --- | ---: | ---: | --- | --- | --- | --- | --- |"]
...
def query_schema() -> dict[str, Any]:
    properties = {"query_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "execution_address": {"type": "string", "pattern": "^" + execution_model.EXECUTION_PREFIX + ":"}, "execution_id": {"type": "string"}, "resources": {"type": "array", "items": {"type": "string", "enum": list(RESOURCES)}}, "status_filter": {"type": "string", "enum": ["", *execution_model.STATUSES]}, "index_filter": {"type": "integer", "minimum": -1}, "text_filter": {"type": "string"}, "offset": {"type": "integer", "minimum": 0}, "limit": {"type": "integer", "minimum": 1, "maximum": MAX_LIMIT}, "total_count": {"type": "integer", "minimum": 0}, "returned_count": {"type": "integer", "minimum": 0}, "truncated": {"type": "boolean"}, "rows": {"type": "array", "items": row_schema()}, "content_address": {"type": "string", "pattern": "^" + QUERY_PREFIX + ":"}}
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Exact execution-ledger archive transfer recovery execution query", "type": "object", "additionalProperties": False, "required": list(QUERY_FIELDS), "properties": properties}


def capabilities() -> dict[str, Any]:
    return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "query_prefix": QUERY_PREFIX, "row_prefix": ROW_PREFIX, "resources": list(RESOURCES), "max_limit": MAX_LIMIT, "features": ["summary outcome address and status resources", "index status and text filters", "deterministic pagination", "row and query addresses", "canonical JSON CSV and Markdown projections"], "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}


__all__ = ["BOUNDARY", "MAX_LIMIT", "MAX_QUERY_ITEMS", "QUERY_FIELDS", "QUERY_PREFIX", "RESOURCES", "ROW_FIELDS", "ROW_PREFIX", "ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQuery", "ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionQueryRow", "VERSION", "address_query", "address_row", "capabilities", "query_csv", "query_execution", "query_from_mapping", "query_json", "query_schema", "render_query_markdown", "row_schema"]
