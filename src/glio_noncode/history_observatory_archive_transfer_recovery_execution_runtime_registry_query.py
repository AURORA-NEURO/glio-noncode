"""Bounded, value-free inspection queries for runtime registries."""
...
DEFAULT_QUERY_ID = "comparison-query-snapshot-registry-history-observatory-archive-transfer-recovery-execution-runtime-registry-query"
...
class RecoveryExecutionRuntimeRegistryQueryRow:
    """One deterministic row in a registry inspection page."""
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistryQueryRow":
        value = _mapping(value, "runtime registry query row")
        _strict(value, set(cls.FIELDS), "runtime registry query row")
        return cls(*(value[field] for field in cls.FIELDS))


def address_row(value: RecoveryExecutionRuntimeRegistryQueryRow) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryQueryRow):
...
def _row(resource: str, key: str, value: Any, address: str, state: str, accepted: bool, ordinal: int) -> RecoveryExecutionRuntimeRegistryQueryRow:
    ...
    provisional = RecoveryExecutionRuntimeRegistryQueryRow(**body)
    return RecoveryExecutionRuntimeRegistryQueryRow(**(body | {"row_address": address_row(provisional)}))


def _all_rows(value: registry_model.RecoveryExecutionRuntimeRegistry) -> tuple[RecoveryExecutionRuntimeRegistryQueryRow, ...]:
    value = registry_model.verify_registry(value)
    rows: list[RecoveryExecutionRuntimeRegistryQueryRow] = []
    ...
    readiness = (("accepted", value.accepted_count), ("ready", value.ready_count), ("blocked", value.blocked_count))
    for key, count in readiness:
    ...
    addresses = (("registry", value.content_address), ("manifest", value.manifest.content_address), ("entries", value.manifest.artifact_addresses[0]), ("summary", value.summary.content_address))
    for key, address in addresses:
        rows.append(_row("addresses", key, {"address": address}, address, value.state, value.accepted, len(rows) + 1))
    bounds = (("entry_count", value.entry_count), ("max_entries", registry_model.MAX_ENTRIES), ("file_count", len(registry_model.FILES)), ("files", list(registry_model.FILES)))
...
class RecoveryExecutionRuntimeRegistryQuery:
    """A bounded query page with replayable filters and row addresses."""

    FIELDS = QUERY_FIELDS

    def __init__(self, query_id: str, registry_id: str, registry_address: str, resources: Sequence[str], state_filter: str, key_filter: str, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[RecoveryExecutionRuntimeRegistryQueryRow | Mapping[str, Any]], content_address: str) -> None:
        ...
        if len(self.resources) != len(set(self.resources)) or any(item not in RESOURCES for item in self.resources):
            raise ValidationError("runtime registry query resources are unsupported or duplicated")
        if self.resources != tuple(item for item in RESOURCES if item in self.resources):
            raise ValidationError("runtime registry query resources must retain canonical order")
        ...
        self.rows = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryQueryRow) else RecoveryExecutionRuntimeRegistryQueryRow.from_mapping(item) for item in _sequence(rows, "runtime registry query rows", MAX_LIMIT))
    ...
    def _validate(self) -> None:
        if self.total_count < self.returned_count or self.returned_count != len(self.rows) or self.truncated != (self.offset + self.returned_count < self.offset + self.total_count):
            raise ValidationError("runtime registry query counts do not replay")
        if tuple(item.ordinal for item in self.rows) != tuple(range(1, len(self.rows) + 1)):
            raise ValidationError("runtime registry query row ordinals do not replay")
        if not _public(self.to_dict()):
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistryQuery":
        value = _mapping(value, "runtime registry query")
        _strict(value, set(cls.FIELDS), "runtime registry query")
        return cls(*(value[field] for field in cls.FIELDS))


def address_query(value: RecoveryExecutionRuntimeRegistryQuery) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryQuery):
        raise ValidationError("runtime registry query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def query_registry(value: registry_model.RecoveryExecutionRuntimeRegistry, *, resources: Sequence[str] | None = None, state: str = "", key: str = "", text: str = "", offset: int = 0, limit: int = MAX_LIMIT, query_id: str = DEFAULT_QUERY_ID) -> RecoveryExecutionRuntimeRegistryQuery:
    value = registry_model.verify_registry(value)
    selected_resources = tuple(resources if resources is not None else RESOURCES)
    selected_resources = tuple(selected_resources)
    if len(selected_resources) != len(set(selected_resources)) or any(item not in RESOURCES for item in selected_resources):
        raise ValidationError("runtime registry query resources are unsupported or duplicated")
    if selected_resources != tuple(item for item in RESOURCES if item in selected_resources):
        raise ValidationError("runtime registry query resources must retain canonical order")
    if state not in ("",) + STATES:
    ...
    provisional = RecoveryExecutionRuntimeRegistryQuery(**body)
    return RecoveryExecutionRuntimeRegistryQuery(**(body | {"content_address": address_query(provisional)}))


def query_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryQuery:
    return RecoveryExecutionRuntimeRegistryQuery.from_mapping(value)


def query_json(value: RecoveryExecutionRuntimeRegistryQuery) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryQuery.from_mapping(value.to_dict()).to_dict())


def query_csv(value: RecoveryExecutionRuntimeRegistryQuery) -> str:
    value = RecoveryExecutionRuntimeRegistryQuery.from_mapping(value.to_dict())
    output = io.StringIO()
    writer = csv.writer(output, lineterminator="\n")
    writer.writerow(("ordinal", "resource", "key", "value", "address", "state", "accepted", "row_address"))
...
def render_query_markdown(value: RecoveryExecutionRuntimeRegistryQuery) -> str:
    value = RecoveryExecutionRuntimeRegistryQuery.from_mapping(value.to_dict())
    lines = ["# Recovery Execution Runtime Registry Query", "", f"- Query: `{value.query_id}`", f"- Registry: `{value.registry_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.returned_count}/{value.total_count}`", f"- Address: `{value.content_address}`", "", "| # | resource | key | state | accepted | address |", "| ---: | --- | --- | --- | --- | --- |"]
    lines.extend(f"| {row.ordinal} | `{row.resource}` | `{row.key}` | `{row.state}` | `{row.accepted}` | `{row.address}` |" for row in value.rows)
    return "\n".join(lines) + "\n"


def row_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryQueryRow", "type": "object", "additionalProperties": False, "required": list(ROW_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1}, "resource": {"enum": list(RESOURCES)}, "key": {"type": "string"}, "value": {}, "address": {"type": "string"}, "state": {"enum": list(STATES)}, "accepted": {"type": "boolean"}, "row_address": {"type": "string"}}}


def query_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryQuery", "type": "object", "additionalProperties": False, "required": list(QUERY_FIELDS), "properties": {"query_id": {"type": "string"}, "registry_id": {"type": "string"}, "registry_address": {"type": "string"}, "resources": {"type": "array", "items": {"enum": list(RESOURCES)}}, "state_filter": {"enum": ["", *STATES]}, "key_filter": {"type": "string"}, "text_filter": {"type": "string"}, "offset": {"type": "integer", "minimum": 0}, "limit": {"type": "integer", "minimum": 1}, "total_count": {"type": "integer", "minimum": 0}, "returned_count": {"type": "integer", "minimum": 0}, "truncated": {"type": "boolean"}, "rows": {"type": "array", "items": row_schema()}, "content_address": {"type": "string"}}}


def capabilities() -> dict[str, Any]:
    return {"version": VERSION, "boundary": BOUNDARY, "query_prefix": QUERY_PREFIX, "row_prefix": ROW_PREFIX, "resources": list(RESOURCES), "states": list(STATES), "max_limit": MAX_LIMIT, "operations": ["query_registry", "query_from_mapping", "query_json", "query_csv", "render_query_markdown"], "filters": ["resource", "state", "key", "text", "offset", "limit"], "privacy": {"values": False, "source_paths": False, "payload_bytes": False, "agent_metadata": False, "language_metadata": False}}


__all__ = ["VERSION", "BOUNDARY", "QUERY_PREFIX", "ROW_PREFIX", "DEFAULT_QUERY_ID", "RESOURCES", "STATES", "MAX_LIMIT", "ROW_FIELDS", "QUERY_FIELDS", "RecoveryExecutionRuntimeRegistryQueryRow", "RecoveryExecutionRuntimeRegistryQuery", "address_row", "address_query", "query_registry", "query_from_mapping", "query_json", "query_csv", "render_query_markdown", "row_schema", "query_schema", "capabilities"]
