"""Detailed queue policy plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseQueuePolicy:
    queue_count: int
    accepted: bool
...
def build_evidence_release_queue_policy(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseQueuePolicy:
    ...
    return EvidenceReleaseQueuePolicy(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_queue_policy(receipt: EvidenceReleaseQueuePolicy, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "queue_count") != expected:
        return False
    return all(
...
def summarize_queue_policy(receipt: EvidenceReleaseQueuePolicy) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "queue_count"),
...
__all__ = ["EvidenceReleaseQueuePolicy", "validate_evidence_release_queue_policy", "build_evidence_release_queue_policy", "summarize_queue_policy"]
