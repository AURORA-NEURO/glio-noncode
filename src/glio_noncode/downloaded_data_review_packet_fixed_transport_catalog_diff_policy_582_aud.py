"""Independently audit module-582 policy decisions."""
...
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_581 as diff_model
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_582 as policy_model
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_582_ct as contract_model
...
class AuditCheck:
    ...
    def __post_init__(self) -> None:
        if self.check_id not in CHECK_IDS or not isinstance(self.passed, bool) or not isinstance(self.detail, str) or not self.detail:
            raise ValidationError("module582 audit check is invalid")
        _address(self.content_address, "module582 audit check address", CHECK_PREFIX)
        if not self.content_address.endswith(":pending") and address_check(self) != self.content_address:
            raise ValidationError("module582 audit check address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "AuditCheck":
        if set(value) != set(CHECK_FIELDS):
            raise ValidationError("module582 audit check fields are not exact")
...
class Audit:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.policy_id, str) or not self.policy_id or self.check_count != len(self.checks) or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0) or tuple(item.check_id for item in self.checks) != CHECK_IDS:
            raise ValidationError("module582 audit aggregates do not replay")
        _address(self.policy_address, "module582 audit policy address", policy_model.POLICY_PREFIX)
        _address(self.diff_address, "module582 audit diff address", diff_model.DIFF_PREFIX)
        _address(self.content_address, "module582 audit address", AUDIT_PREFIX)
        if not self.content_address.endswith(":pending") and address_audit(self) != self.content_address:
            raise ValidationError("module582 audit address does not replay")
...
def audit_policy(value: Any, *, diff: Any = None) -> Audit:
    ...
    recompute_ok = recomputed is None or recomputed.content_address == policy.content_address
    checks = (
        ...
        _check("audit-budgets", all(name in actual for name in ("policy-added-budget", "policy-removed-budget", "policy-changed-budget", "policy-total-change-budget")), tuple(actual.get(name).passed for name in ("policy-added-budget", "policy-removed-budget", "policy-changed-budget", "policy-total-change-budget") if name in actual), "four budget controls", "budget controls are retained"),
        ...
        _check("audit-direction-transition", all(name in actual for name in ("policy-direction-allowlist", "policy-transition-allowlist")), tuple(actual.get(name).passed for name in ("policy-direction-allowlist", "policy-transition-allowlist") if name in actual), "recorded", "direction and transition controls are retained"),
        _check("audit-posture-allowlist", all(name in actual for name in ("policy-left-posture-allowlist", "policy-right-posture-allowlist")), tuple(actual.get(name).passed for name in ("policy-left-posture-allowlist", "policy-right-posture-allowlist") if name in actual), "recorded", "posture controls are retained"),
        _check("audit-ready-control", "policy-ready-requirement" in actual, actual.get("policy-ready-requirement").passed if "policy-ready-requirement" in actual else "missing", "recorded", "readiness control is retained"),
        _check("audit-change-control", "policy-change-requirement" in actual, actual.get("policy-change-requirement").passed if "policy-change-requirement" in actual else "missing", "recorded", "change control is retained"),
        _check("audit-required-changes", "policy-required-changes" in actual, actual.get("policy-required-changes").passed if "policy-required-changes" in actual else "missing", "recorded", "required-change control is retained"),
        _check("audit-unchanged-control", "policy-unchanged-control" in actual, actual.get("policy-unchanged-control").passed if "policy-unchanged-control" in actual else "missing", "recorded", "unchanged control is retained"),
        _check("audit-public-boundary", recompute_ok and not _has_forbidden_key(policy.to_dict()), (recompute_ok, _has_forbidden_key(policy.to_dict())), (True, False), "policy recomputation and public boundary are valid"),
    )
...
def _audit_from_mapping(value: dict[str, Any]) -> Audit:
    if set(value) != set(AUDIT_FIELDS):
        raise ValidationError("module582 audit fields are not exact")
    return Audit(value["policy_id"], value["policy_address"], value["diff_address"], value["check_count"], value["passed_count"], value["failed_count"], value["accepted"], tuple(AuditCheck.from_mapping(item) for item in value["checks"]), value["content_address"])
...
def verify_audit(value: Any) -> Audit:
    ...
    if not isinstance(audit, Audit) or address_audit(audit) != audit.content_address or _has_forbidden_key(audit.to_dict()):
        raise ValidationError("module582 audit address or public boundary does not replay")
...
def load_audit(value: bytes | bytearray | str | Path | dict[str, Any]) -> Audit:
    if isinstance(value, dict):
        ...
        raw = _strict_json_loads(read_text(value, field="module582 audit", max_bytes=32 * 1024 * 1024))
...
def write_audit(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Audit:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module582 audit destination already exists")
    _validate_parent(path.parent, "module582 audit destination")
    atomic_write_text(path, audit_json(audit), field="module582 audit destination")
    return audit
...
def query_audit(value: Any, *, passed: bool | None = None, check_id: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if passed is not None and not isinstance(passed, bool) or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("module582 audit query filters are invalid")
...
def render_audit_markdown(value: Any) -> str:
    audit = verify_audit(value)
    lines = ["# Module582 Catalog Diff Policy Audit", "", f"- Policy: `{audit.policy_address}`", f"- Result: **{'accepted' if audit.accepted else 'blocked'}**", f"- Checks: **{audit.passed_count}/{audit.check_count}**", "", "| check | passed | detail |", "| --- | --- | --- |"]
    lines.extend(f"| `{item.check_id}` | {str(item.passed).lower()} | {item.detail} |" for item in audit.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module582 audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS)}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module582 audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS)}
