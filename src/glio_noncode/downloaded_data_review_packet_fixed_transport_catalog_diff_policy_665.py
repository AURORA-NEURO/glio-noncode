"""Build, verify, query, and persist source-free diffs of module664 catalogs."""
...
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_664 as catalog_model
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_665_ct as contract_model
...
def build_diff(left: Any, right: Any, *, diff_id: str = DEFAULT_DIFF_ID) -> contract_model.Diff:
    ...
    if left_catalog.catalog_id != right_catalog.catalog_id:
        raise ValidationError("module665 diff requires matching catalog IDs")
    ...
    if len(entry_ids) > MAX_ITEMS:
        raise ValidationError("module665 diff item count exceeds its bound")
...
def _item_from_mapping(value: Mapping[str, Any]) -> contract_model.DiffItem:
    if not isinstance(value, Mapping) or set(value) != set(ITEM_FIELDS):
        raise ValidationError("module665 diff item fields are not exact")
...
def _diff_from_mapping(value: Mapping[str, Any]) -> contract_model.Diff:
    if not isinstance(value, Mapping) or set(value) != set(DIFF_FIELDS):
        raise ValidationError("module665 diff fields are not exact")
...
def verify_diff(value: Any) -> contract_model.Diff:
    ...
    if not isinstance(diff, contract_model.Diff) or contract_model.address_diff(diff) != diff.content_address or _has_forbidden_key(diff.to_dict()):
        raise ValidationError("module665 diff address or public boundary does not replay")
    return _diff_from_mapping(diff.to_dict())


def load_diff(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> contract_model.Diff:
    raw = value if isinstance(value, Mapping) else _strict_json_loads(bytes(value).decode("utf-8")) if isinstance(value, (bytes, bytearray)) else _strict_json_loads(read_text(value, field="module665 diff", max_bytes=32 * 1024 * 1024))
...
def write_diff(value: Any, destination: str | Path, *, allow_existing: bool = False) -> contract_model.Diff:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module665 diff destination already exists")
    _validate_parent(path.parent, "module665 diff destination")
    atomic_write_text(path, diff_json(diff), field="module665 diff destination")
    return diff


def query_diff(value: Any, *, change: str = "", direction: str = "", state_transition: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    diff = verify_diff(value)
    change, direction, state_transition, text = (_text(change, "module665 change", 64, required=False), _text(direction, "module665 direction", 64, required=False), _text(state_transition, "module665 transition", 128, required=False), _text(text, "module665 text", required=False))
    if (change and change not in CHANGES) or (direction and direction not in DIRECTIONS) or (state_transition and state_transition not in STATE_TRANSITIONS) or not isinstance(offset, int) or offset < 0 or not isinstance(limit, int) or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("module665 query filters are invalid")
...
def render_diff_markdown(value: Any) -> str:
    diff = verify_diff(value)
    lines = ["# Module665 Fixed Transport Catalog Diff", "", f"- Catalog ID: `{diff.catalog_id}`", f"- Direction: **{diff.direction}**", f"- State transition: **{diff.state_transition}**", f"- Added / removed / changed / unchanged: **{diff.added_count} / {diff.removed_count} / {diff.changed_count} / {diff.unchanged_count}**", f"- Diff address: `{diff.content_address}`", "", "| # | entry | change | fields |", "| ---: | --- | --- | --- |"]
