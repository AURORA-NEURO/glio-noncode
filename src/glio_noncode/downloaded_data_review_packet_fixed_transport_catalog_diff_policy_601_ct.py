"""Typed contracts for longitudinal comparisons of module600 catalogs."""
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_600_ct import CATALOG_PREFIX, STATES as POSTURES
...
class DiffItem:
    ...
    def __post_init__(self) -> None:
        _count(self.ordinal, "module601 item ordinal", MAX_ITEMS)
        _label(self.entry_id, "module601 item entry ID")
        if self.change not in CHANGES or tuple(field for field in SNAPSHOT_FIELDS if field in self.changed_fields) != self.changed_fields or any(field not in SNAPSHOT_FIELDS for field in self.changed_fields):
            raise ValidationError("module601 item change fields are not canonical")
        if not isinstance(self.left_snapshot, dict) or not isinstance(self.right_snapshot, dict) or set(self.left_snapshot) - set(SNAPSHOT_FIELDS) or set(self.right_snapshot) - set(SNAPSHOT_FIELDS):
            raise ValidationError("module601 item snapshots contain unsupported fields")
        if self.change == "added" and self.left_snapshot or self.change == "removed" and self.right_snapshot or self.change == "unchanged" and self.changed_fields:
            raise ValidationError("module601 item change state does not match snapshots")
        _address(self.left_address, "module601 left item address", required=False)
        _address(self.right_address, "module601 right item address", required=False)
        _address(self.content_address, "module601 item address", ITEM_PREFIX)
        if not self.content_address.endswith(":pending") and address_item(self) != self.content_address:
            raise ValidationError("module601 item address does not replay")
...
def address_item(value: DiffItem) -> str:
    if not isinstance(value, DiffItem):
        raise ValidationError("module601 item addressing requires its typed contract")
...
class Diff:
    ...
    def __post_init__(self) -> None:
        _label(self.diff_id, "module601 diff ID")
        if self.version != VERSION or self.boundary != BOUNDARY:
            raise ValidationError("module601 diff identity is not current")
        _label(self.catalog_id, "module601 diff catalog ID")
        _address(self.left_catalog_address, "module601 left catalog address", CATALOG_PREFIX)
        _address(self.right_catalog_address, "module601 right catalog address", CATALOG_PREFIX)
        if self.left_posture not in POSTURES or self.right_posture not in POSTURES or self.state_transition not in STATE_TRANSITIONS or self.direction not in DIRECTIONS:
            raise ValidationError("module601 posture, transition, or direction is unsupported")
        for field in ("added_count", "removed_count", "changed_count", "unchanged_count"):
            _count(getattr(self, field), f"module601 {field}", MAX_ITEMS)
        if len(self.items) > MAX_ITEMS or sum((self.added_count, self.removed_count, self.changed_count, self.unchanged_count)) != len(self.items):
            raise ValidationError("module601 item counts do not replay")
        if tuple(item.ordinal for item in self.items) != tuple(range(1, len(self.items) + 1)) or tuple(item.entry_id for item in self.items) != tuple(sorted(item.entry_id for item in self.items)) or len({item.entry_id for item in self.items}) != len(self.items):
            raise ValidationError("module601 item order or identity is not canonical")
        _address(self.content_address, "module601 diff address", DIFF_PREFIX)
        if not self.content_address.endswith(":pending") and address_diff(self) != self.content_address:
            raise ValidationError("module601 diff address does not replay")
...
def address_diff(value: Diff) -> str:
    if not isinstance(value, Diff):
        raise ValidationError("module601 diff addressing requires its typed contract")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=DIFF_PREFIX)


def item_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module601 catalog diff item", "type": "object", "additionalProperties": False, "required": list(ITEM_FIELDS), "properties": {"ordinal": {"type": "integer"}, "entry_id": {"type": "string"}, "change": {"enum": list(CHANGES)}, "changed_fields": {"type": "array"}, "left_address": {"type": "string"}, "right_address": {"type": "string"}, "content_address": {"type": "string"}}}


def diff_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module601 transport catalog diff", "type": "object", "additionalProperties": False, "required": list(DIFF_FIELDS), "properties": {"version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "catalog_id": {"type": "string"}, "items": {"type": "array", "maxItems": MAX_ITEMS}, "content_address": {"type": "string"}}}
