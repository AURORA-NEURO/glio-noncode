"""Bounded evidence queries for history-diff archive-transfer recovery-execution runtime-registry history diffs."""
...
class HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow:
...
def address_row(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow):
...
def _row(resource: str, key: str, value: Any, address: str, change: str, left_address: str, right_address: str, ordinal: int) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow:
    ...
    provisional = HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(**body)
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(**(body | {"row_address": address_row(provisional)}))


def _all_rows(value: diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff) -> tuple[HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow, ...]:
    value = diff_model.verify_diff(value)
    rows: list[HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow] = []
...
class HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    ...
    def __init__(self, query_id: str, diff_id: str, diff_address: str, resources: Sequence[str], change_filter: str, key_filter: str, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.rows = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow) else HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow.from_mapping(item) for item in _sequence(rows, "runtime registry history diff query rows", MAX_LIMIT))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery:
        value = _mapping(value, "runtime registry history diff query")
        _strict(value, set(cls.FIELDS), "runtime registry history diff query")
        return cls(*(value[field] for field in cls.FIELDS))


def address_query(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery):
        raise ValidationError("runtime registry history diff query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def query_history_diff(value: diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff, *, resources: Sequence[str] | None = None, change: str = "", key: str = "", text: str = "", offset: int = 0, limit: int = MAX_LIMIT, query_id: str = DEFAULT_QUERY_ID) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    value = diff_model.verify_diff(value)
    ...
    projected = tuple(HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(item.ordinal, item.resource, item.key, item.value, item.address, item.change, item.left_address, item.right_address, item.row_address) for item in page)
    projected = tuple(HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(index, item.resource, item.key, item.value, item.address, item.change, item.left_address, item.right_address, ROW_PREFIX + ":pending") for index, item in enumerate(projected, 1))
    final_rows: list[HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow] = []
    for item in projected:
        body = item.to_dict()
        final_rows.append(HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(**(body | {"row_address": address_row(item)})))
    ...
    provisional = HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery(**body)
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery(**(body | {"content_address": address_query(provisional)}))


def verify_query(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery):
        raise ValidationError("runtime registry history diff query verification requires a typed query")
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery.from_mapping(value.to_dict())


def query_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery.from_mapping(value)


def query_json(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
    return canonical_json(verify_query(value).to_dict())


def query_csv(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
