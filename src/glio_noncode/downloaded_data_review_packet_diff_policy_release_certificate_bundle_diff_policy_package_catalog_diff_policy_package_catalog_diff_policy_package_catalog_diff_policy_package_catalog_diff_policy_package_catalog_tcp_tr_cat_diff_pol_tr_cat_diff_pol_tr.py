...
from . import (
    downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_tcp_tr_cat_diff_pol_tr_cat_diff_pol_tr_ct as contract_model,
)
from . import (
...
from .downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_tcp_tr_cat_diff_pol_tr_cat_diff_pol_tr_ct import (
    BOUNDARY,
    FILE_NAMES,
    MANIFEST_PREFIX,
    MAX_PACKAGE_BYTES,
...
def _zip_member(name: str) -> zipfile.ZipInfo:
    if name not in FILE_NAMES:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport member is outside the fixed file set")
...
def _parts(raw: bytes) -> tuple[tuple[zipfile.ZipInfo, bytes], ...]:
    try:
        ...
        raise ValidationError(f"cannot read fixed transport catalog diff policy package catalog diff policy transport ZIP: {exc}") from exc
...
def _read_package(value: bytes | bytearray | str | Path | DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket) -> bytes:
    if isinstance(value, DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket):
        ...
        raw = read_bytes(value, field="fixed transport catalog diff policy package catalog diff policy transport", max_bytes=MAX_PACKAGE_BYTES)
    if len(raw) > MAX_PACKAGE_BYTES:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport exceeds its byte bound")
    return raw
...
def build_package(diff: Any, policy: Any, audit: Any | None = None, *, package_id: str = PACKAGE_PREFIX) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket:
    ...
    if policy_value.diff_address != diff_value.content_address:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport policy does not refer to the supplied diff")
    ...
    if audit_value.policy_address != policy_value.content_address or not audit_value.accepted:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport requires an accepted independent policy audit")
...
def _manifest_and_bodies(raw: bytes) -> tuple[DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketManifest, dict[str, bytes]]:
    ...
    if names != FILE_NAMES or any(not _safe_member(name) for name in names):
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport ZIP members are not the exact safe file set")
    if any(info.date_time != _ZIP_EPOCH or info.compress_type != zipfile.ZIP_STORED or info.external_attr != _FIXED_ATTRIBUTES or info.extra or info.comment for info, _payload in parts):
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport ZIP metadata is not deterministic")
    ...
    if not isinstance(parsed, Mapping) or bodies[FILE_NAMES[0]] != (canonical_json(parsed) + "\n").encode(_UTF8):
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport manifest is not canonical")
    ...
    if address_manifest(manifest) != manifest.content_address:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport manifest address does not replay")
    for descriptor in members:
        ...
        if descriptor.byte_count != len(payload) or descriptor.byte_address != address_member_bytes(payload) or descriptor.content_address != address_member(descriptor):
            raise ValidationError(f"fixed transport catalog diff policy package catalog diff policy transport member does not replay: {descriptor.relative_path}")
    ...
    if raw != canonical_raw:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport ZIP bytes are not deterministic")
...
def verify_package(value: bytes | bytearray | str | Path | DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket:
    ...
    for path in ("catalog-diff.json", "policy.json", "policy-audit.json"):
        if bodies[path] != (canonical_json(_strict_json_loads(bodies[path].decode(_UTF8))) + "\n").encode(_UTF8):
            raise ValidationError(f"fixed transport catalog diff policy package catalog diff policy transport payload is not canonical: {path}")
    if manifest.diff_address != diff.content_address or manifest.policy_address != policy.content_address or manifest.audit_address != audit.content_address or policy.diff_address != diff.content_address or audit.policy_address != policy.content_address:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport lineage does not replay")
    if manifest.policy_state != policy.state or manifest.policy_accepted != policy.accepted or not manifest.audit_accepted or not audit.accepted:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport decision evidence does not replay")
    if _has_forbidden_key({"manifest": manifest.to_dict(), "diff": diff.to_dict(), "policy": policy.to_dict(), "audit": audit.to_dict()}):
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport crosses the public boundary")
    if bodies["review.md"].decode(_UTF8) != _review(diff, policy, audit):
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport Markdown does not replay")
    package = DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket(manifest=manifest, package_bytes=raw, package_address=hash_bytes(raw, prefix=PACKAGE_PREFIX))
    if isinstance(value, DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket) and value.to_dict() != package.to_dict():
        raise ValidationError("typed fixed transport catalog diff policy package catalog diff policy transport does not replay its bytes")
    return package
...
def write_package(value: Any, destination: str | Path, *, allow_existing: bool = False) -> DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport destination is not a file")
    _validate_parent(path.parent, "fixed transport catalog diff policy package catalog diff policy transport destination")
    atomic_write_bytes(path, package.package_bytes, field="fixed transport catalog diff policy package catalog diff policy transport destination")
    return package
...
def query_package(value: Any, *, resource: str = "summary", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    if resource not in {"summary", "members", "manifest", "diff", "policy", "audit", "review"} or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("fixed transport catalog diff policy package catalog diff policy transport query is invalid")
    package = verify_package(value)
...
build_fixed_transport_catalog_diff_policy_transport = build_package
load_fixed_transport_catalog_diff_policy_transport = load_package
fixed_transport_catalog_diff_policy_transport_json = package_json
write_fixed_transport_catalog_diff_policy_transport = write_package
query_fixed_transport_catalog_diff_policy_transport = query_package
fixed_transport_catalog_diff_policy_transport_csv = package_csv
render_fixed_transport_catalog_diff_policy_transport_markdown = render_package_markdown
verify_fixed_transport_catalog_diff_policy_transport = verify_package
fixed_transport_catalog_diff_policy_transport_schema = package_schema
fixed_transport_catalog_diff_policy_transport_manifest_schema = manifest_schema

__all__ = ["build_fixed_transport_catalog_diff_policy_transport", "build_package", "build_transport", "capabilities", "fixed_transport_catalog_diff_policy_transport_csv", "fixed_transport_catalog_diff_policy_transport_json", "fixed_transport_catalog_diff_policy_transport_manifest_schema", "fixed_transport_catalog_diff_policy_transport_schema", "load_fixed_transport_catalog_diff_policy_transport", "load_package", "load_transport", "manifest_schema", "package_csv", "package_json", "package_schema", "query_fixed_transport_catalog_diff_policy_transport", "query_package", "query_transport", "render_fixed_transport_catalog_diff_policy_transport_markdown", "render_package_markdown", "render_transport_markdown", "transport_capabilities", "transport_csv", "transport_json", "transport_manifest_schema", "transport_schema", "verify_fixed_transport_catalog_diff_policy_transport", "verify_package", "verify_transport", "write_fixed_transport_catalog_diff_policy_transport", "write_package", "write_transport"]
