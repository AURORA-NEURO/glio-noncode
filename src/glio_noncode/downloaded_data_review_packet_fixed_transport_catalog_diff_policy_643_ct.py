"""Typed contracts for module643 policy-decision transports."""
# ruff: noqa: E501
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_642_ct import BOUNDARY as POLICY_BOUNDARY, POLICY_PREFIX
...
class Member:
    ...
    def __post_init__(self) -> None:
        if self.relative_path not in PAYLOAD_NAMES or MEDIA_TYPES[self.relative_path] != self.media_type:
            raise ValidationError("module643 member path or media type is unsupported")
        _count(self.byte_count, "module643 member byte count", MAX_MEMBER_BYTES)
        _address(self.byte_address, "module643 member byte address", MEMBER_PREFIX + "-bytes")
        _address(self.content_address, "module643 member content address", MEMBER_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_member(self) != self.content_address:
            raise ValidationError("module643 member address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Member":
        if set(value) != {"relative_path", "media_type", "byte_count", "byte_address", "content_address"}:
            raise ValidationError("module643 member fields are not exact")
...
def address_member_bytes(payload: bytes) -> str:
    if not isinstance(payload, bytes) or len(payload) > MAX_MEMBER_BYTES:
        raise ValidationError("module643 member payload exceeds its byte bound")
...
class Manifest:
    ...
    def __post_init__(self) -> None:
        _label(self.package_id, "module643 package ID")
        if self.version != VERSION or self.boundary != BOUNDARY or self.package_format != PACKAGE_FORMAT:
            raise ValidationError("module643 manifest identity is not current")
        _address(self.diff_address, "module643 diff address")
        _address(self.policy_address, "module643 policy address")
        _address(self.audit_address, "module643 audit address")
        if self.policy_state not in {"ready", "blocked"} or not isinstance(self.policy_accepted, bool) or not isinstance(self.audit_accepted, bool):
            raise ValidationError("module643 decision flags are invalid")
        if tuple(item.relative_path for item in self.members) != PAYLOAD_NAMES:
            raise ValidationError("module643 payload order is not canonical")
        _address(self.content_address, "module643 manifest address", MANIFEST_PREFIX, allow_pending=True)
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Manifest":
        ...
        if set(value) != fields:
            raise ValidationError("module643 manifest fields are not exact")
...
class Transport:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.package_bytes, bytes) or len(self.package_bytes) > MAX_PACKAGE_BYTES:
            raise ValidationError("module643 package bytes exceed their bound")
        _address(self.package_address, "module643 package address", PACKAGE_PREFIX)
        if self.manifest.content_address.endswith(":pending"):
            raise ValidationError("module643 manifest must be addressed")
...
def package_schema() -> dict[str, Any]:
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module643 fixed transport", "type": "object", "additionalProperties": False, "required": list(fields), "properties": {"package_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "package_format": {"const": PACKAGE_FORMAT}, "package_byte_count": {"type": "integer"}, "manifest": {"type": "object"}, "content_address": {"type": "string"}}}


def manifest_schema() -> dict[str, Any]:
    fields = ("package_id", "version", "boundary", "diff_address", "policy_address", "audit_address", "policy_state", "policy_accepted", "audit_accepted", "members", "package_format", "content_address")
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module643 manifest", "type": "object", "additionalProperties": False, "required": list(fields), "properties": {"version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "members": {"type": "array", "minItems": len(PAYLOAD_NAMES), "maxItems": len(PAYLOAD_NAMES)}, "package_format": {"const": PACKAGE_FORMAT}}}
