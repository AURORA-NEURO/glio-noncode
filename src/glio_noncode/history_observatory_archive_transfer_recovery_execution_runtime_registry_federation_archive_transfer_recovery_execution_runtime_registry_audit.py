"""Independent audit receipts for federation recovery runtime registries."""
...
def _check(check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> RecoveryExecutionRuntimeRegistryAuditCheck:
    ...
    return RecoveryExecutionRuntimeRegistryAuditCheck(**(body | {"content_address": address_check(provisional)}))
...
def audit_registry(value: registry_model.RecoveryExecutionRuntimeRegistry) -> RecoveryExecutionRuntimeRegistryAudit:
    ...
    checks = (
        ...
        _check("runtime-linkage", all(item.runtime_version == runtime_model.VERSION and item.runtime_address.startswith(runtime_model.RUNTIME_PREFIX + ":") for item in entries), "entries retain federation runtime linkage", tuple(item.runtime_address for item in entries)),
...
def render_audit_markdown(value: RecoveryExecutionRuntimeRegistryAudit) -> str:
    value = verify_audit(value)
    lines = ["# Federation archive recovery execution runtime registry audit", "", f"- Registry: `{value.registry_id}`", f"- Passed: `{value.passed_count}/{value.check_count}`", f"- Accepted: `{value.accepted}`", f"- Address: `{value.content_address}`", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | `{item.check_id}` | `{item.passed}` | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "FederationRecoveryExecutionRuntimeRegistryAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}}, "content_address": {"type": "string"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "FederationRecoveryExecutionRuntimeRegistryAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"registry_address": {"type": "string"}, "registry_id": {"type": "string"}, "version": {"type": "string", "const": VERSION}, "boundary": {"type": "string", "const": BOUNDARY}, "check_count": {"type": "integer", "const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0}, "failed_count": {"type": "integer", "minimum": 0}, "accepted": {"type": "boolean"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "content_address": {"type": "string"}}}
