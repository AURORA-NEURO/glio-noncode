...
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer as transfer_model
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer_query as query_model
...
class HistoryDiffArchiveTransferQueryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferQueryAuditCheck:
...
class HistoryDiffArchiveTransferQueryAudit:
    """The fixed-size independent audit of a transfer query."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, transfer_address: str, query_address: str, checks: Sequence[HistoryDiffArchiveTransferQueryAuditCheck], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, HistoryDiffArchiveTransferQueryAuditCheck) else HistoryDiffArchiveTransferQueryAuditCheck.from_mapping(item) for item in _sequence(checks, "query audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferQueryAudit:
        ...
        return cls(value["transfer_address"], value["query_address"], tuple(HistoryDiffArchiveTransferQueryAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "query audit checks", MAX_CHECKS)), value["check_count"], value["passed_count"], value["failed_count"], value["accepted"], value["content_address"])


def address_check(value: HistoryDiffArchiveTransferQueryAuditCheck) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferQueryAuditCheck):
        raise ValidationError("query audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: HistoryDiffArchiveTransferQueryAudit) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferQueryAudit):
        raise ValidationError("query audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> HistoryDiffArchiveTransferQueryAuditCheck:
    provisional = HistoryDiffArchiveTransferQueryAuditCheck(ordinal, check_id, bool(passed), detail, evidence, "pending:query-audit-check")
    return HistoryDiffArchiveTransferQueryAuditCheck(ordinal, check_id, bool(passed), detail, evidence, address_check(provisional))


def audit_query(query: query_model.HistoryDiffArchiveTransferQuery, transfer: transfer_model.HistoryDiffArchiveTransfer) -> HistoryDiffArchiveTransferQueryAudit:
    if not isinstance(query, query_model.HistoryDiffArchiveTransferQuery) or not isinstance(transfer, transfer_model.HistoryDiffArchiveTransfer):
    ...
    provisional = HistoryDiffArchiveTransferQueryAudit(**body, content_address="pending:query-audit")
    return HistoryDiffArchiveTransferQueryAudit(**body, content_address=address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferQueryAudit:
    return HistoryDiffArchiveTransferQueryAudit.from_mapping(value)


def verify_audit(value: HistoryDiffArchiveTransferQueryAudit) -> HistoryDiffArchiveTransferQueryAudit:
    if not isinstance(value, HistoryDiffArchiveTransferQueryAudit):
...
def audit_json(value: HistoryDiffArchiveTransferQueryAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: HistoryDiffArchiveTransferQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    stream = io.StringIO(newline="")
    writer = csv.DictWriter(stream, fieldnames=HistoryDiffArchiveTransferQueryAuditCheck.FIELDS, lineterminator="\n")
    writer.writeheader()
...
def render_audit_markdown(value: HistoryDiffArchiveTransferQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Federation History-Diff Archive Transfer Query Audit", "", f"- Transfer: `{value.transfer_address}`", f"- Query: `{value.query_address}`", f"- Checks: `{value.passed_count}/{value.check_count}`", f"- Accepted: `{str(value.accepted).lower()}`", f"- Address: `{value.content_address}`", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | `{item.check_id}` | `{str(item.passed).lower()}` | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation history-diff archive transfer query audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"type": "string", "enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 16}, "content_address": {"type": "string", "pattern": "^" + CHECK_PREFIX + ":"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation history-diff archive transfer query audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"transfer_address": {"type": "string", "pattern": "^" + transfer_model.TRANSFER_PREFIX + ":"}, "query_address": {"type": "string", "pattern": "^" + query_model.QUERY_PREFIX + ":"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "check_count": {"type": "integer", "const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "content_address": {"type": "string", "pattern": "^" + AUDIT_PREFIX + ":"}}}
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "HistoryDiffArchiveTransferQueryAudit", "HistoryDiffArchiveTransferQueryAuditCheck", "VERSION", "address_audit", "address_check", "audit_csv", "audit_from_mapping", "audit_json", "audit_query", "audit_schema", "capabilities", "check_schema", "render_audit_markdown", "verify_audit"]
