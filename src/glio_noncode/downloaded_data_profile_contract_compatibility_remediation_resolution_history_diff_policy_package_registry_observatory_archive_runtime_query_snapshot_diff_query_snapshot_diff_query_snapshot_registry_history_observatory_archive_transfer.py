"""Resumable byte-range transport for history-observatory archives.

The transfer contract is deliberately downstream of the deterministic ZIP
archive. It records the verified archive address, a contiguous chunk plan, and
one content address per chunk. Chunk bytes are retained only in the private
in-memory transport object or in an explicitly persisted transfer directory;
the public manifest never exposes payload bytes or source paths.
"""
...
CHUNK_SUFFIX = ".bin"
DEFAULT_TRANSFER_ID = "comparison-query-snapshot-registry-history-observatory-archive-transfer"
...
class ArchiveTransferChunk:
    """A contiguous, addressed byte range in a transfer."""
...
class ArchiveTransfer:
    ...
    def __init__(self, transfer_id: str, version: str, boundary: str, archive_address: str, archive_size: int, chunk_size: int, chunk_count: int, chunks: Sequence[ArchiveTransferChunk], content_address: str, payload: Mapping[int, bytes] | None = None) -> None:
        self.transfer_id = _text(transfer_id, "transfer ID")
        self.version = _text(version, "transfer version", 2048)
        self.boundary = _text(boundary, "transfer boundary", 1024)
    ...
    def _validate(self) -> None:
        ...
        for expected_index, chunk in enumerate(self.chunks):
            if not isinstance(chunk, ArchiveTransferChunk) or chunk.index != expected_index or chunk.offset != expected_offset:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"transfer_id": self.transfer_id, "version": self.version, "boundary": self.boundary, "archive_address": self.archive_address, "archive_size": self.archive_size, "chunk_size": self.chunk_size, "chunk_count": self.chunk_count, "chunks": [chunk.to_dict() for chunk in self.chunks], "content_address": self.content_address}
...
class TransferAssemblyProgress:
    """An addressed snapshot of received and missing chunk ranges."""
...
def _build_from_parts(raw: bytes, archive_value: Any, *, transfer_id: str | None, chunk_size: int) -> ArchiveTransfer:
    ...
    if not isinstance(archive_value, archive_model.DownloadedDataProfileContractCompatibilityRemediationResolutionHistoryDiffPolicyPackageRegistryObservatoryArchiveRuntimeQuerySnapshotDiffQuerySnapshotDiffQuerySnapshotRegistryHistoryObservatoryArchive):
        raise ValidationError("transfer archive anchor is not typed")
...
def build_transfer(value: Any, *, transfer_id: str | None = None, chunk_size: int = DEFAULT_CHUNK_SIZE) -> ArchiveTransfer:
...
def _manifest(value: ArchiveTransfer) -> dict[str, Any]:
    body = {"version": VERSION, "boundary": BOUNDARY, "transfer_id": value.transfer_id, "archive_address": value.archive_address, "archive_size": value.archive_size, "chunk_size": value.chunk_size, "chunk_count": value.chunk_count, "chunks": [chunk.to_dict() for chunk in value.chunks], "transfer_address": value.content_address}
...
def _csv_text(value: ArchiveTransfer) -> str:
...
def transfer_csv(value: ArchiveTransfer) -> str:
    return _csv_text(verify_transfer(value))


def render_transfer_markdown(value: ArchiveTransfer) -> str:
    value = verify_transfer(value)
    lines = ["# Comparison-query history observatory archive transfer", "", f"- Transfer: `{value.transfer_id}`", f"- Archive address: `{value.archive_address}`", f"- Bytes: `{value.archive_size}`", f"- Chunks: `{value.chunk_count}` at `{value.chunk_size}` bytes", f"- Address: `{value.content_address}`", "", "| # | offset | bytes | chunk address |", "| ---: | ---: | ---: | --- |"]
...
def chunk_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Comparison-query history observatory archive transfer chunk", "type": "object", "additionalProperties": False, "required": list(ArchiveTransferChunk.FIELDS), "properties": {"index": {"type": "integer", "minimum": 0, "maximum": MAX_CHUNKS - 1}, "offset": {"type": "integer", "minimum": 0, "maximum": MAX_TRANSFER_BYTES}, "size": {"type": "integer", "minimum": 1, "maximum": MAX_CHUNK_SIZE}, "content_address": {"type": "string", "pattern": "^" + CHUNK_PREFIX + ":"}}}


def transfer_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Comparison-query history observatory archive transfer", "type": "object", "additionalProperties": False, "required": list(ArchiveTransfer.FIELDS), "properties": {"transfer_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "archive_address": {"type": "string", "pattern": "^" + archive_model.ARCHIVE_PREFIX + ":"}, "archive_size": {"type": "integer", "minimum": 1, "maximum": MAX_TRANSFER_BYTES}, "chunk_size": {"type": "integer", "minimum": MIN_CHUNK_SIZE, "maximum": MAX_CHUNK_SIZE}, "chunk_count": {"type": "integer", "minimum": 1, "maximum": MAX_CHUNKS}, "chunks": {"type": "array", "items": chunk_schema(), "minItems": 1, "maxItems": MAX_CHUNKS}, "content_address": {"type": "string", "pattern": "^" + TRANSFER_PREFIX + ":"}}}
