"""Deterministic baseline/candidate diff for federation runtime-registry histories."""
...
def _quality(value: history_model.RecoveryExecutionRuntimeRegistryHistory) -> tuple[int, int, int, int, int, int]:
...
def _direction(left: history_model.RecoveryExecutionRuntimeRegistryHistory, right: history_model.RecoveryExecutionRuntimeRegistryHistory, item_change: bool) -> str:
...
class RecoveryExecutionRuntimeRegistryHistoryDiffItem:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffItem:
...
def address_item(value: RecoveryExecutionRuntimeRegistryHistoryDiffItem) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffItem):
...
class RecoveryExecutionRuntimeRegistryHistoryDiffItems:
    """Ordered and addressed diff-item projection."""

    FIELDS = ITEMS_FIELDS

    def __init__(self, items: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffItem | Mapping[str, Any]], content_address: str) -> None:
        self.items = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffItem) else RecoveryExecutionRuntimeRegistryHistoryDiffItem.from_mapping(item) for item in _sequence(items, "runtime registry history diff items", MAX_ITEMS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffItems:
...
def address_items(value: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffItem]) -> str:
    typed = tuple(value)
    if any(not isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffItem) for item in typed):
...
class RecoveryExecutionRuntimeRegistryHistoryDiffManifest:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffManifest:
...
def address_manifest(value: RecoveryExecutionRuntimeRegistryHistoryDiffManifest) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffManifest):
...
class RecoveryExecutionRuntimeRegistryHistoryDiffSummary:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffSummary:
...
def address_summary(value: RecoveryExecutionRuntimeRegistryHistoryDiffSummary) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffSummary):
...
class RecoveryExecutionRuntimeRegistryHistoryDiff:
    """Fully addressed baseline/candidate history comparison."""

    FIELDS = DIFF_FIELDS

    def __init__(self, diff_id: str, version: str, boundary: str, left_history_id: str, right_history_id: str, left_history_address: str, right_history_address: str, item_count: int, added_count: int, removed_count: int, changed_count: int, unchanged_count: int, direction: str, accepted: bool, manifest: RecoveryExecutionRuntimeRegistryHistoryDiffManifest | Mapping[str, Any], summary: RecoveryExecutionRuntimeRegistryHistoryDiffSummary | Mapping[str, Any], items: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffItem | Mapping[str, Any]], content_address: str) -> None:
        self.diff_id = _label(diff_id, "runtime registry history diff ID")
        self.version = _text(version, "runtime registry history diff version", 1024)
        self.boundary = _text(boundary, "runtime registry history diff boundary", 1024)
        self.left_history_id = _label(left_history_id, "runtime registry history diff left history ID", required=False)
        ...
        self.direction = direction
        self.accepted = _bool(accepted, "runtime registry history diff acceptance")
        self.manifest = manifest if isinstance(manifest, RecoveryExecutionRuntimeRegistryHistoryDiffManifest) else RecoveryExecutionRuntimeRegistryHistoryDiffManifest.from_mapping(manifest)
        self.summary = summary if isinstance(summary, RecoveryExecutionRuntimeRegistryHistoryDiffSummary) else RecoveryExecutionRuntimeRegistryHistoryDiffSummary.from_mapping(summary)
        self.items = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffItem) else RecoveryExecutionRuntimeRegistryHistoryDiffItem.from_mapping(item) for item in _sequence(items, "runtime registry history diff items", MAX_ITEMS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiff:
        value = _mapping(value, "runtime registry history diff")
        _strict(value, set(cls.FIELDS), "runtime registry history diff")
        return cls(*(value[field] for field in cls.FIELDS))


def address_diff(value: RecoveryExecutionRuntimeRegistryHistoryDiff) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiff):
...
def _item(ordinal: int, left: history_model.RecoveryExecutionRuntimeRegistryHistoryEntry | None, right: history_model.RecoveryExecutionRuntimeRegistryHistoryEntry | None) -> RecoveryExecutionRuntimeRegistryHistoryDiffItem:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffItem(**body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffItem(**(body | {"content_address": address_item(provisional)}))


def build_diff(left: history_model.RecoveryExecutionRuntimeRegistryHistory, right: history_model.RecoveryExecutionRuntimeRegistryHistory, *, diff_id: str = DEFAULT_DIFF_ID) -> RecoveryExecutionRuntimeRegistryHistoryDiff:
    ...
    summary_provisional = RecoveryExecutionRuntimeRegistryHistoryDiffSummary(**summary_body)
    summary = RecoveryExecutionRuntimeRegistryHistoryDiffSummary(**(summary_body | {"content_address": address_summary(summary_provisional)}))
    ...
    manifest_provisional = RecoveryExecutionRuntimeRegistryHistoryDiffManifest(**manifest_body)
    manifest = RecoveryExecutionRuntimeRegistryHistoryDiffManifest(**(manifest_body | {"content_address": address_manifest(manifest_provisional)}))
    body = {
        "diff_id": diff_id,
        "version": VERSION,
        "boundary": BOUNDARY,
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiff(**body)
    return RecoveryExecutionRuntimeRegistryHistoryDiff(**(body | {"content_address": address_diff(provisional)}))


def verify_diff(value: RecoveryExecutionRuntimeRegistryHistoryDiff) -> RecoveryExecutionRuntimeRegistryHistoryDiff:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiff):
        raise ValidationError("runtime registry history diff verification requires a typed diff")
    return RecoveryExecutionRuntimeRegistryHistoryDiff.from_mapping(value.to_dict())


def diff_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiff:
    return RecoveryExecutionRuntimeRegistryHistoryDiff.from_mapping(value)


def diff_json(value: RecoveryExecutionRuntimeRegistryHistoryDiff) -> str:
    return canonical_json(verify_diff(value).to_dict())


def diff_csv(value: RecoveryExecutionRuntimeRegistryHistoryDiff) -> str:
...
def render_diff_markdown(value: RecoveryExecutionRuntimeRegistryHistoryDiff) -> str:
    value = verify_diff(value)
    lines = [
...
def items_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffItems) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryHistoryDiffItems.from_mapping(value.to_dict()).to_dict())


def summary_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffSummary) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryHistoryDiffSummary.from_mapping(value.to_dict()).to_dict())


def manifest_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffManifest) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryHistoryDiffManifest.from_mapping(value.to_dict()).to_dict())


def _write(path: Path, value: Mapping[str, Any]) -> None:
    atomic_write_text(path, canonical_json(value), field="runtime registry history diff artifact")


def persist_diff(value: RecoveryExecutionRuntimeRegistryHistoryDiff, destination: str | Path, *, overwrite: bool = False) -> Path:
    value = verify_diff(value)
    destination = Path(destination)
    if destination.is_symlink():
        raise ValidationError("runtime registry history diff destination must not be a symlink")
    ...
    try:
        items = RecoveryExecutionRuntimeRegistryHistoryDiffItems(value.items, address_items(value.items))
...
def load_diff(destination: str | Path) -> RecoveryExecutionRuntimeRegistryHistoryDiff:
    destination = Path(destination)
    ...
    manifest = RecoveryExecutionRuntimeRegistryHistoryDiffManifest.from_mapping(documents["manifest.json"])
    items = RecoveryExecutionRuntimeRegistryHistoryDiffItems.from_mapping(documents["items.json"])
    summary = RecoveryExecutionRuntimeRegistryHistoryDiffSummary.from_mapping(documents["summary.json"])
...
def run_diff(left: history_model.RecoveryExecutionRuntimeRegistryHistory, right: history_model.RecoveryExecutionRuntimeRegistryHistory, *, diff_id: str = DEFAULT_DIFF_ID, destination: str | Path | None = None, overwrite: bool = False) -> RecoveryExecutionRuntimeRegistryHistoryDiff:
