"""Compact release bundles for accepted Domain 05 C05–C08 receipts."""
...
class MolecularAtlasBundleFormat(StrEnum):
...
class MolecularAtlasBundleEntry:
    """Sanitized state or histone receipt summary."""
...
@dataclass(frozen=True, slots=True)
class MolecularAtlasBundle:
    """Bounded evidence release view."""

    fixture_id: str
    fixture_version: str
    context_key: str
    output_format: MolecularAtlasBundleFormat
    accepted_only: bool
    entries: tuple[MolecularAtlasBundleEntry, ...]
    content_address: str
...
class MolecularAtlasBundleBuilder:
    ...
    def build(
        ...
        *,
        ...
        output_format: MolecularAtlasBundleFormat = MolecularAtlasBundleFormat.JSON,
        accepted_only: bool = False,
    ) -> MolecularAtlasBundle:
        entries: list[MolecularAtlasBundleEntry] = []
        for receipt in evaluation.receipts:
            if accepted_only and receipt.role is not MolecularAtlasRole.POSITIVE:
                continue
            body = {
            ...
            entries.append(MolecularAtlasBundleEntry(**body, content_address=content_hash(body)))
        body = {
        ...
        return MolecularAtlasBundle(**body, content_address=content_hash(body))

    def render(self, bundle: MolecularAtlasBundle) -> str:
        """Render a verified-safe bundle without raw input text."""

        if bundle.output_format is MolecularAtlasBundleFormat.JSON:
            return json.dumps(bundle.to_dict(), indent=2, sort_keys=True) + "\n"
        rows = [entry.to_dict() for entry in bundle.entries]
        if bundle.output_format is MolecularAtlasBundleFormat.CSV:
            output = io.StringIO()
            fields = [
                "record_id",
                "operation",
        ...
        if bundle.output_format is MolecularAtlasBundleFormat.MARKDOWN:
            lines = [
                f"# Molecular atlas bundle: {bundle.fixture_id}",
                "",
            ...
            lines.extend(
                f"| {entry.record_id} | {entry.operation} | {entry.role} | {entry.state} | {entry.primary_count}/{entry.secondary_count} | {', '.join(entry.issue_codes) or 'none'} | {entry.accepted} |"
                for entry in bundle.entries
            )
            return "\n".join(lines) + "\n"
        raise ValidationError(f"unsupported molecular atlas bundle format: {bundle.output_format}")

    def verify(self, bundle: MolecularAtlasBundle) -> tuple[str, ...]:
    ...
    def write(self, bundle: MolecularAtlasBundle, path: str | Path) -> None:
        failures = self.verify(bundle)
        if failures:
            raise ValidationError(
                f"cannot write invalid molecular atlas bundle: {', '.join(failures)}"
            )
        output = Path(path)
