"""Operation, delegate, state, and control contract matrix for D13."""
...
def planning_architecture_contract_matrix(
    ...
    for operation in selected.operations:
        ...
        rows.append(body | {"content_address": addressed(body, "planning-contract-row")})
    return tuple(rows)


def planning_architecture_contract_matrix_summary(
    fixture: PlanningArchitectureFixture | None = None,
) -> dict[str, object]:
    rows = planning_architecture_contract_matrix(fixture)
    return {
        "row_count": len(rows),
...
__all__ = [
    "planning_architecture_contract_matrix",
    "planning_architecture_contract_matrix_summary",
]
