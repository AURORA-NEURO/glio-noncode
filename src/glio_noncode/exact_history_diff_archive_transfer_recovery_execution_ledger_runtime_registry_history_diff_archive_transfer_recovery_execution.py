"""Deterministic execution receipts over exact archive-transfer recovery plans."""
...
DEFAULT_EXECUTION_ID = EXECUTION_PREFIX
...
EXECUTION_FIELDS = (
    "execution_id", "version", "boundary", "recovery_id", "recovery_address",
    "transfer_address", "archive_address", "archive_size", "chunk_count",
    "base_received_indices", "planned_indices", "applied_indices", "pending_indices",
    "rejected_indices", "current_received_indices", "current_missing_indices",
    "planned_bytes", "applied_bytes", "pending_bytes", "rejected_bytes",
    "current_received_bytes", "current_remaining_bytes", "outcomes", "action_count",
    "applied_count", "pending_count", "rejected_count", "state", "decision",
    "safe_to_continue", "safe_to_assemble", "checkpointed", "next_index", "content_address",
)
...
def _indices(value: Any, field: str, maximum: int) -> tuple[int, ...]:
    values = tuple(_index(item, field, maximum) for item in _sequence(value, field, MAX_OUTCOMES))
    if values != tuple(sorted(values)) or len(values) != len(set(values)):
        raise ValidationError(f"{field} must be sorted and unique")
    return values


class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome:
    """One deterministic status receipt for one planned recovery action."""
    ...
    def __init__(self, index: int, action_address: str, content_address: str, offset: int, size: int, status: str, reason: str, outcome_address: str) -> None:
        self.index = _index(index, "execution outcome index", transfer_model.MAX_CHUNKS - 1)
        ...
        if not self.outcome_address.startswith("pending:") and not self.outcome_address.endswith(":pending") and address_outcome(self) != self.outcome_address:
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution:
    ...
    def __init__(self, execution_id: str, version: str, boundary: str, recovery_id: str, recovery_address: str, transfer_address: str, archive_address: str, archive_size: int, chunk_count: int, base_received_indices: Sequence[int], planned_indices: Sequence[int], applied_indices: Sequence[int], pending_indices: Sequence[int], rejected_indices: Sequence[int], current_received_indices: Sequence[int], current_missing_indices: Sequence[int], planned_bytes: int, applied_bytes: int, pending_bytes: int, rejected_bytes: int, current_received_bytes: int, current_remaining_bytes: int, outcomes: Sequence[Any], action_count: int, applied_count: int, pending_count: int, rejected_count: int, state: str, decision: str, safe_to_continue: bool, safe_to_assemble: bool, checkpointed: bool, next_index: int, content_address: str) -> None:
        ...
        self.outcomes = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome) else ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome.from_mapping(item) for item in _sequence(outcomes, "execution outcomes", MAX_OUTCOMES))
        ...
        self.next_index = _index(next_index, "execution next index", self.chunk_count - 1, allow_minus_one=True)
    ...
    def _validate(self) -> None:
        base, planned = set(self.base_received_indices), set(self.planned_indices)
        applied, pending, rejected = set(self.applied_indices), set(self.pending_indices), set(self.rejected_indices)
        current_received, current_missing = set(self.current_received_indices), set(self.current_missing_indices)
        ...
        if tuple(item.index for item in self.outcomes) != self.planned_indices:
            raise ValidationError("execution outcomes are not in plan order")
        status_sets = {status: {item.index for item in self.outcomes if item.status == status} for status in STATUSES}
        if status_sets["applied"] != applied or status_sets["pending"] != pending or status_sets["rejected"] != rejected:
            raise ValidationError("execution outcomes do not replay status sets")
        if self.action_count != len(self.planned_indices) or self.action_count != len(self.outcomes) or (self.applied_count, self.pending_count, self.rejected_count) != (len(applied), len(pending), len(rejected)):
        ...
        expected_state = "blocked" if rejected else "complete" if not pending else "in_progress" if applied else "planned"
        expected_decision = "block" if rejected else "assemble" if not pending else "resume"
        if self.state != expected_state or self.decision != expected_decision:
            raise ValidationError("execution state or decision does not follow outcomes")
        ...
        if not self.content_address.startswith("pending:") and not self.content_address.endswith(":pending") and address_execution(self) != self.content_address:
...
def address_outcome(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome):
...
def address_execution(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution):
...
def _outcomes(recovery: recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery, applied: set[int], rejected: set[int]) -> tuple[ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome, ...]:
    values = []
    for action in recovery.actions:
        status = "applied" if action.index in applied else "rejected" if action.index in rejected else "pending"
        provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome(action.index, action.action_address, action.content_address, action.offset, action.size, status, REASONS[status], "pending:execution-outcome")
        values.append(ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome(provisional.index, provisional.action_address, provisional.content_address, provisional.offset, provisional.size, provisional.status, provisional.reason, address_outcome(provisional)))
    return tuple(values)


def _build(recovery: recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery, *, applied_indices: Sequence[int], rejected_indices: Sequence[int], execution_id: str, checkpointed: bool) -> ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution:
    ...
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution(**body, content_address="pending:execution")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution(**body, content_address=address_execution(provisional))


def build_execution(recovery: recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery, *, applied_indices: Sequence[int] = (), rejected_indices: Sequence[int] = (), execution_id: str = DEFAULT_EXECUTION_ID, checkpointed: bool = False) -> ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution:
    if not isinstance(recovery, recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery):
...
def build_execution_from_assembler(recovery: recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery, assembler: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferAssembler, *, execution_id: str = DEFAULT_EXECUTION_ID, checkpointed: bool = True) -> ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution:
    if not isinstance(recovery, recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery) or not isinstance(assembler, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferAssembler):
...
def build_execution_from_directory(recovery: recovery_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecovery, source: str | Path, *, execution_id: str = DEFAULT_EXECUTION_ID) -> ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution:
...
def execution_from_mapping(value: Mapping[str, Any]):
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution.from_mapping(value)


def verify_execution(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution):
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution):
...
def execution_json(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution) -> str:
    return canonical_json(execution_from_mapping(value.to_dict()).to_dict())


def execution_csv(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution) -> str:
...
def outcome_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Exact execution-ledger archive transfer recovery execution outcome", "type": "object", "additionalProperties": False, "required": list(OUTCOME_FIELDS), "properties": {"index": {"type": "integer", "minimum": 0}, "action_address": {"type": "string", "pattern": "^" + recovery_model.ACTION_PREFIX + ":"}, "content_address": {"type": "string", "pattern": "^" + transfer_model.CHUNK_PREFIX + ":"}, "offset": {"type": "integer", "minimum": 0}, "size": {"type": "integer", "minimum": 1}, "status": {"type": "string", "enum": list(STATUSES)}, "reason": {"type": "string", "enum": list(REASONS.values())}, "outcome_address": {"type": "string", "pattern": "^" + OUTCOME_PREFIX + ":"}}}


def execution_schema() -> dict[str, Any]:
    properties = {field: {"type": "array", "items": {"type": "integer", "minimum": 0}} for field in ("base_received_indices", "planned_indices", "applied_indices", "pending_indices", "rejected_indices", "current_received_indices", "current_missing_indices")}
    properties.update({"execution_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "recovery_id": {"type": "string"}, "recovery_address": {"type": "string", "pattern": "^" + recovery_model.RECOVERY_PREFIX + ":"}, "transfer_address": {"type": "string", "pattern": "^" + transfer_model.TRANSFER_PREFIX + ":"}, "archive_address": {"type": "string", "pattern": "^" + transfer_model.archive_model.ARCHIVE_PREFIX + ":"}, "archive_size": {"type": "integer", "minimum": 1}, "chunk_count": {"type": "integer", "minimum": 1}, "planned_bytes": {"type": "integer", "minimum": 0}, "applied_bytes": {"type": "integer", "minimum": 0}, "pending_bytes": {"type": "integer", "minimum": 0}, "rejected_bytes": {"type": "integer", "minimum": 0}, "current_received_bytes": {"type": "integer", "minimum": 0}, "current_remaining_bytes": {"type": "integer", "minimum": 0}, "outcomes": {"type": "array", "items": outcome_schema()}, "action_count": {"type": "integer", "minimum": 0}, "applied_count": {"type": "integer", "minimum": 0}, "pending_count": {"type": "integer", "minimum": 0}, "rejected_count": {"type": "integer", "minimum": 0}, "state": {"type": "string", "enum": list(STATES)}, "decision": {"type": "string", "enum": list(DECISIONS)}, "safe_to_continue": {"type": "boolean"}, "safe_to_assemble": {"type": "boolean"}, "checkpointed": {"type": "boolean"}, "next_index": {"type": "integer", "minimum": -1}, "content_address": {"type": "string", "pattern": "^" + EXECUTION_PREFIX + ":"}})
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Exact execution-ledger archive transfer recovery execution", "type": "object", "additionalProperties": False, "required": list(EXECUTION_FIELDS), "properties": properties}


def capabilities() -> dict[str, Any]:
    return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "execution_prefix": EXECUTION_PREFIX, "outcome_prefix": OUTCOME_PREFIX, "statuses": list(STATUSES), "states": list(STATES), "decisions": list(DECISIONS), "max_outcomes": MAX_OUTCOMES, "features": ["applied pending and rejected outcomes", "assembler-backed verified progress", "byte and index conservation", "resume assemble or block decisions", "checkpoint and next-index projections", "canonical JSON CSV and Markdown projections"], "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}


__all__ = ["BOUNDARY", "DECISIONS", "DEFAULT_EXECUTION_ID", "EXECUTION_FIELDS", "EXECUTION_PREFIX", "MAX_OUTCOMES", "OUTCOME_FIELDS", "OUTCOME_PREFIX", "REASONS", "STATUSES", "STATES", "ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecution", "ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome", "VERSION", "address_execution", "address_outcome", "build_execution", "build_execution_from_assembler", "build_execution_from_directory", "capabilities", "execution_csv", "execution_from_mapping", "execution_json", "execution_schema", "outcome_schema", "render_execution_markdown", "verify_execution"]
