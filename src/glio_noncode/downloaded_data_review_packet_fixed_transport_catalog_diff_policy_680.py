"""Build, verify, query, and persist source-free module679 transport catalogs."""
# ruff: noqa: E501
...
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_679_aud as transport_audit_model
...
def _entry(package: Any, entry_id: str) -> contract_model.Entry:
    ...
    if not receipt.accepted:
        raise ValidationError("module680 catalog requires an accepted module679 transport audit")
...
def build_catalog(packages: Sequence[Any], *, entry_ids: Sequence[str] | None = None, catalog_id: str = DEFAULT_CATALOG_ID) -> contract_model.Catalog:
    if isinstance(packages, (str, bytes, bytearray)) or not isinstance(packages, Sequence) or len(packages) > MAX_ENTRIES:
        raise ValidationError("module680 packages must be a bounded sequence")
    if entry_ids is not None and (isinstance(entry_ids, (str, bytes, bytearray)) or not isinstance(entry_ids, Sequence) or len(entry_ids) != len(packages)):
        raise ValidationError("module680 entry IDs must align with packages")
    ...
    if len({item.package_address for item in built_entries}) != len(built_entries):
        raise ValidationError("module680 catalog contains duplicate package addresses")
...
def _catalog_from_mapping(value: Mapping[str, Any]) -> contract_model.Catalog:
    if not isinstance(value, Mapping):
        raise ValidationError("module680 catalog must be an object")
...
def load_catalog(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> contract_model.Catalog:
    raw = value if isinstance(value, Mapping) else _strict_json_loads(bytes(value).decode("utf-8")) if isinstance(value, (bytes, bytearray)) else _strict_json_loads(read_text(value, field="module680 catalog", max_bytes=32 * 1024 * 1024))
...
def verify_catalog(value: Any) -> contract_model.Catalog:
    ...
    if not isinstance(catalog, contract_model.Catalog) or contract_model.address_catalog(catalog) != catalog.content_address or _has_forbidden_key(catalog.to_dict()):
        raise ValidationError("module680 catalog address or public boundary does not replay")
...
def write_catalog(value: Any, destination: str | Path, *, allow_existing: bool = False) -> contract_model.Catalog:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module680 catalog destination already exists")
    _validate_parent(path.parent, "module680 catalog destination")
    atomic_write_text(path, catalog_json(catalog), field="module680 catalog destination")
    return catalog
...
def query_catalog(value: Any, *, resource: str = "summary", state: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if resource not in {"summary", "entries", "lineage", "state"} or state not in {"", "ready", "blocked"} or not isinstance(text, str) or len(text) > 4096 or not isinstance(offset, int) or offset < 0 or not isinstance(limit, int) or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("module680 catalog query filters are invalid")
...
def render_catalog_markdown(value: Any) -> str:
    catalog = verify_catalog(value)
    lines = ["# Module680 Fixed Policy Decision Transport Catalog", "", f"- Catalog: `{catalog.content_address}`", f"- State: **{catalog.state}**", f"- Entries: **{catalog.entry_count}**", f"- Audited: **{catalog.accepted_count}/{catalog.entry_count}**", "", "| entry | policy state | policy accepted | bytes | package |", "| --- | --- | --- | ---: | --- |"]
