"""Detailed reclassification policy plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseReclassificationPolicy:
    policy_count: int
    accepted: bool
...
def build_evidence_release_reclassification_policy(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseReclassificationPolicy:
    ...
    return EvidenceReleaseReclassificationPolicy(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_reclassification_policy(receipt: EvidenceReleaseReclassificationPolicy, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "policy_count") != expected:
        return False
    return all(
...
def summarize_reclassification_policy(receipt: EvidenceReleaseReclassificationPolicy) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "policy_count"),
...
__all__ = ["EvidenceReleaseReclassificationPolicy", "validate_evidence_release_reclassification_policy", "build_evidence_release_reclassification_policy", "summarize_reclassification_policy"]
