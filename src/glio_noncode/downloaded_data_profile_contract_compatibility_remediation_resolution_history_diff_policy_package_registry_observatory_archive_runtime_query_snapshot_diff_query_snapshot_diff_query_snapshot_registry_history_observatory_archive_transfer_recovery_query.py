"""Bounded public queries over history-observatory archive recovery plans."""
...
class RecoveryQueryRow:
    ...
    def _validate(self) -> None:
        if self.chunk_index >= 0 and self.chunk_offset + self.chunk_size > recovery_model.transfer_model.MAX_TRANSFER_BYTES:
            raise ValidationError("recovery query row range exceeds the transfer bound")
...
def render_query_markdown(value: RecoveryQuery) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# History observatory archive transfer recovery query", "", f"- Recovery: `{value.recovery_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.row_count}`", f"- Address: `{value.content_address}`", "", "| resource | ordinal | chunk | received | missing | state | decision |", "| --- | ---: | ---: | --- | --- | --- | --- |"]
