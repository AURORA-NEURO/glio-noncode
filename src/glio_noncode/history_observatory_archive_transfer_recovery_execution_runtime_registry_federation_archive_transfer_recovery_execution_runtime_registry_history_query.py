...
class RecoveryExecutionRuntimeRegistryHistoryQueryRow:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryQueryRow:
...
def address_row(value: RecoveryExecutionRuntimeRegistryHistoryQueryRow) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryQueryRow):
...
def _row(resource: str, key: str, value: Any, address: str, state: str, accepted: bool, transition: str, ordinal: int) -> RecoveryExecutionRuntimeRegistryHistoryQueryRow:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryQueryRow(**body)
    return RecoveryExecutionRuntimeRegistryHistoryQueryRow(**(body | {"row_address": address_row(provisional)}))


def _all_rows(value: history_model.RecoveryExecutionRuntimeRegistryHistory) -> tuple[RecoveryExecutionRuntimeRegistryHistoryQueryRow, ...]:
    value = history_model.verify_history(value)
    rows: list[RecoveryExecutionRuntimeRegistryHistoryQueryRow] = []
...
class RecoveryExecutionRuntimeRegistryHistoryQuery:
    ...
    def __init__(self, query_id: str, history_id: str, history_address: str, resources: Sequence[str], state_filter: str, key_filter: str, transition_filter: str, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[RecoveryExecutionRuntimeRegistryHistoryQueryRow | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.rows = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryQueryRow) else RecoveryExecutionRuntimeRegistryHistoryQueryRow.from_mapping(item) for item in _sequence(rows, "runtime registry history query rows", MAX_LIMIT))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryQuery:
        value = _mapping(value, "runtime registry history query")
        _strict(value, set(cls.FIELDS), "runtime registry history query")
        return cls(*(value[field] for field in cls.FIELDS))


def address_query(value: RecoveryExecutionRuntimeRegistryHistoryQuery) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryQuery):
        raise ValidationError("runtime registry history query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def query_history(value: history_model.RecoveryExecutionRuntimeRegistryHistory, *, resources: Sequence[str] | None = None, state: str = "", key: str = "", transition: str = "", text: str = "", offset: int = 0, limit: int = MAX_LIMIT, query_id: str = DEFAULT_QUERY_ID) -> RecoveryExecutionRuntimeRegistryHistoryQuery:
    value = history_model.verify_history(value)
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryQuery(**body)
    return RecoveryExecutionRuntimeRegistryHistoryQuery(**(body | {"content_address": address_query(provisional)}))


def query_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryQuery:
    return RecoveryExecutionRuntimeRegistryHistoryQuery.from_mapping(value)


def query_json(value: RecoveryExecutionRuntimeRegistryHistoryQuery) -> str:
    return canonical_json(query_from_mapping(value.to_dict()).to_dict())


def query_csv(value: RecoveryExecutionRuntimeRegistryHistoryQuery) -> str:
...
def render_query_markdown(value: RecoveryExecutionRuntimeRegistryHistoryQuery) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# Federation archive recovery execution runtime registry history query", "", f"History: {value.history_id}", f"Resources: {', '.join(value.resources)}", f"Rows: {value.returned_count}/{value.total_count}", f"Address: {value.content_address}", "", "| # | resource | key | state | accepted | transition |", "| ---: | --- | --- | --- | --- | --- |"]
    lines.extend(f"| {row.ordinal} | {row.resource} | {row.key} | {row.state} | {row.accepted} | {row.transition} |" for row in value.rows)
    return "\n".join(lines) + "\n"


def row_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryHistoryQueryRow", "type": "object", "additionalProperties": False, "required": list(ROW_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1}, "resource": {"enum": list(RESOURCES)}, "key": {"type": "string"}, "value": {}, "address": {"type": "string"}, "state": {"enum": list(STATES)}, "accepted": {"type": "boolean"}, "transition": {"enum": ["", *TRANSITIONS]}, "row_address": {"type": "string"}}}


def query_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryHistoryQuery", "type": "object", "additionalProperties": False, "required": list(QUERY_FIELDS), "properties": {"query_id": {"type": "string"}, "history_id": {"type": "string"}, "history_address": {"type": "string"}, "resources": {"type": "array", "items": {"enum": list(RESOURCES)}}, "state_filter": {"enum": ["", *STATES]}, "key_filter": {"type": "string"}, "transition_filter": {"enum": ["", *TRANSITIONS]}, "text_filter": {"type": "string"}, "offset": {"type": "integer", "minimum": 0}, "limit": {"type": "integer", "minimum": 1, "maximum": MAX_LIMIT}, "total_count": {"type": "integer", "minimum": 0}, "returned_count": {"type": "integer", "minimum": 0}, "truncated": {"type": "boolean"}, "rows": {"type": "array", "items": row_schema()}, "content_address": {"type": "string"}}}
