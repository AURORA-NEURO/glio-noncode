"""Independent replay audit for runtime query projections."""
...
from . import downloaded_data_quality_runtime_history_release_evidence_history_runtime_query as query_model
...
AUDIT_PREFIX = runtime_model.RUNTIME_PREFIX + "-query-audit"
CHECK_PREFIX = AUDIT_PREFIX + "-check"
CHECK_IDS = ("query_identity", "runtime_link", "history_link", "resource_set", "filter_replay", "counts", "pagination", "row_order", "row_addresses", "projection_replay", "query_address", "public_boundary")
CHECK_FIELDS = ("ordinal", "check_id", "passed", "actual", "expected", "detail", "content_address")
AUDIT_FIELDS = ("query_id", "runtime_id", "history_id", "query_address", "checks", "check_count", "passed_count", "failed_count", "accepted", "content_address")
...
class AuditCheck:
    ...
    def __init__(self, ordinal: int, check_id: str, passed: bool, actual: str, expected: str, detail: str, content_address: str) -> None:
        self.ordinal = _count(ordinal, "runtime query audit ordinal", MAX_CHECKS, lower=1)
        self.check_id = _label(check_id, "runtime query audit check ID")
        self.passed = _bool(passed, "runtime query audit result")
        self.actual = _text(actual, "runtime query audit actual", 32768)
        self.expected = _text(expected, "runtime query audit expected", 32768)
        self.detail = _text(detail, "runtime query audit detail", 4096)
        self.content_address = _address(content_address, "runtime query audit check address", CHECK_PREFIX)
        self._validate()

    def _validate(self) -> None:
        if self.check_id not in CHECK_IDS:
            raise ValidationError("runtime query audit check identity is unsupported")
...
class QueryAudit:
    ...
    def __init__(self, query_id: str, runtime_id: str, history_id: str, query_address: str, checks: Sequence[AuditCheck | Mapping[str, Any]], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.query_id = _label(query_id, "runtime query audit query ID")
        self.runtime_id = _label(runtime_id, "runtime query audit runtime ID")
        self.history_id = _label(history_id, "runtime query audit history ID")
        self.query_address = _address(query_address, "runtime query audit query address", query_model.QUERY_PREFIX)
        self.checks = tuple(item if isinstance(item, AuditCheck) else AuditCheck.from_mapping(_mapping(item, "runtime query audit check")) for item in _sequence(checks, "runtime query audit checks", MAX_CHECKS))
    ...
    def _validate(self) -> None:
        if self.check_count != len(self.checks) or self.check_count != MAX_CHECKS or tuple(item.check_id for item in self.checks) != CHECK_IDS:
            raise ValidationError("runtime query audit checks are incomplete or out of order")
        ...
        if self.accepted != (self.failed_count == 0) or any(item.content_address != address_check(item) for item in self.checks):
            raise ValidationError("runtime query audit acceptance or addresses do not replay")
...
def audit_query(value: query_model.RuntimeQuery, runtime: runtime_model.HistoryRuntime) -> QueryAudit:
    query_model.verify_query(value)
    runtime_model.verify_runtime(runtime)
    replay = query_model.query_runtime(runtime, query_id=value.query_id, resources=value.resources, state_filter=value.state_filter, passed_filter=value.passed_filter, severity_filter=value.severity_filter, text_filter=value.text_filter, offset=value.offset, limit=value.limit)
    findings = (
        _finding(1, "query_identity", value.query_id == replay.query_id, value.query_id, replay.query_id, "query identity must replay"),
        _finding(2, "runtime_link", value.runtime_id == runtime.runtime_id, value.runtime_id, runtime.runtime_id, "query must target the runtime"),
        _finding(3, "history_link", value.history_id == runtime.history_id and value.history_address == runtime.history_address, (value.history_id, value.history_address), (runtime.history_id, runtime.history_address), "query history link must replay"),
        _finding(4, "resource_set", value.resources == replay.resources, value.resources, replay.resources, "query resources must replay"),
        _finding(5, "filter_replay", (value.state_filter, value.passed_filter, value.severity_filter, value.text_filter) == (replay.state_filter, replay.passed_filter, replay.severity_filter, replay.text_filter), (value.state_filter, value.passed_filter, value.severity_filter, value.text_filter), (replay.state_filter, replay.passed_filter, replay.severity_filter, replay.text_filter), "query filters must replay"),
        ...
        _finding(10, "projection_replay", value.to_dict() == replay.to_dict(), value.content_address, replay.content_address, "query projection must replay from runtime"),
    ...
    return _seal(QueryAudit(value.query_id, value.runtime_id, value.history_id, value.content_address, sealed, len(sealed), passed, len(sealed) - passed, passed == len(sealed), f"pending:{AUDIT_PREFIX}"), address_audit)


def verify_audit(value: QueryAudit) -> QueryAudit:
    if not isinstance(value, QueryAudit):
        raise ValidationError("runtime query audit verification requires a typed audit")
    value._validate()
    return value
...
def render_audit_markdown(value: QueryAudit) -> str:
    ...
    lines = [f"# Runtime query audit {value.query_id}", "", f"- Accepted: {str(value.accepted).lower()}", f"- Checks: {value.passed_count}/{value.check_count}", "", "| Check | Result | Detail |", "| --- | --- | --- |"]
    lines.extend(f"| {item.check_id} | {'pass' if item.passed else 'fail'} | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RuntimeQueryAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {field: {"type": "integer" if field == "ordinal" else "boolean" if field == "passed" else "string"} for field in CHECK_FIELDS}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RuntimeQueryAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {field: {"type": "array" if field == "checks" else "integer" if field.endswith("count") else "boolean" if field == "accepted" else "string"} for field in AUDIT_FIELDS}}
