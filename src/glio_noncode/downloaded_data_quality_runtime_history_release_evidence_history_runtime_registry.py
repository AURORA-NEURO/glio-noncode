"""Deterministic admission registry for release-evidence history runtimes."""
...
ENTRY_FIELDS = ("ordinal", "runtime_id", "runtime_address", "history_id", "history_address", "state", "release_ready", "check_count", "passed_count", "entry_count", "content_address")
...
class RuntimeRegistryEntry:
    ...
    def __init__(self, ordinal: int, runtime_id: str, runtime_address: str, history_id: str, history_address: str, state: str, release_ready: bool, check_count: int, passed_count: int, entry_count: int, content_address: str) -> None:
        ...
        self.history_address = _address(history_address, "runtime registry history address", history_model.HISTORY_PREFIX)
        if state not in STATES[1:]:
            raise ValidationError("runtime registry entry state is unsupported")
        self.state = state
        self.release_ready = _bool(release_ready, "runtime registry entry readiness")
        if self.release_ready != (self.state == "ready"):
            raise ValidationError("runtime registry entry state and readiness disagree")
        self.check_count = _count(check_count, "runtime registry entry check count", runtime_model.MAX_CHECKS)
        self.passed_count = _count(passed_count, "runtime registry entry passed count", runtime_model.MAX_CHECKS)
        self.entry_count = _count(entry_count, "runtime registry entry history count", history_model.MAX_ENTRIES)
        if self.passed_count > self.check_count:
            raise ValidationError("runtime registry entry passed count exceeds check count")
        self.content_address = _address(content_address, "runtime registry entry address", ENTRY_PREFIX)
        self._validate()
...
def entry_from_runtime(value: runtime_model.HistoryRuntime, ordinal: int) -> RuntimeRegistryEntry:
    runtime_model.verify_runtime(value)
    body = {"ordinal": ordinal, "runtime_id": value.runtime_id, "runtime_address": value.content_address, "history_id": value.history_id, "history_address": value.history_address, "state": value.state, "release_ready": value.release_ready, "check_count": value.check_count, "passed_count": value.passed_count, "entry_count": value.entry_count, "content_address": f"pending:{ENTRY_PREFIX}"}
    provisional = RuntimeRegistryEntry(**body)
    return RuntimeRegistryEntry(**(body | {"content_address": address_entry(provisional)}))
...
class RuntimeRegistryEntries:
    ...
    def __init__(self, entries: Sequence[RuntimeRegistryEntry | Mapping[str, Any]], content_address: str) -> None:
        self.entries = tuple(item if isinstance(item, RuntimeRegistryEntry) else RuntimeRegistryEntry.from_mapping(_mapping(item, "runtime registry entry")) for item in _sequence(entries, "runtime registry entries", MAX_ENTRIES))
        self.content_address = _address(content_address, "runtime registry entries address", ENTRIES_PREFIX)
        self._validate()

    def _validate(self) -> None:
        if tuple(item.ordinal for item in self.entries) != tuple(range(1, len(self.entries) + 1)):
            raise ValidationError("runtime registry entry ordinals are not contiguous")
...
class RuntimeRegistryManifest:
    ...
    def __init__(self, registry_id: str, version: str, boundary: str, files: Sequence[str], artifact_addresses: Sequence[str], content_address: str) -> None:
        ...
        self.boundary = _text(boundary, "runtime registry manifest boundary", 2048)
        self.files = tuple(_text(item, "runtime registry manifest file", 128) for item in _sequence(files, "runtime registry manifest files", len(FILES)))
        self.artifact_addresses = tuple(_address(item, "runtime registry manifest artifact address") for item in _sequence(artifact_addresses, "runtime registry manifest artifact addresses", len(ARTIFACT_FILES)))
        self.content_address = _address(content_address, "runtime registry manifest address", MANIFEST_PREFIX)
        self._validate()

    def _validate(self) -> None:
        if self.files != FILES or len(self.artifact_addresses) != len(ARTIFACT_FILES) or self.version != VERSION or self.boundary != BOUNDARY:
            raise ValidationError("runtime registry manifest is not canonical")
        ...
        if not _public(self.to_dict()):
            raise ValidationError("runtime registry manifest crosses the public boundary")
...
class RuntimeRegistrySummary:
    ...
    def __init__(self, registry_id: str, entry_count: int, ready_count: int, blocked_count: int, accepted: bool, release_ready: bool, state: str, content_address: str) -> None:
        ...
        self.release_ready = _bool(release_ready, "runtime registry summary readiness")
        if state not in STATES:
            raise ValidationError("runtime registry summary state is unsupported")
        self.state = state
        self.content_address = _address(content_address, "runtime registry summary address", SUMMARY_PREFIX)
        self._validate()
    ...
    def _validate(self) -> None:
        ...
        if not _public(self.to_dict()):
            raise ValidationError("runtime registry summary crosses the public boundary")
...
class RuntimeRegistry:
    ...
    def __init__(self, registry_id: str, version: str, boundary: str, entry_count: int, ready_count: int, blocked_count: int, accepted: bool, release_ready: bool, state: str, manifest: Mapping[str, Any] | RuntimeRegistryManifest, summary: Mapping[str, Any] | RuntimeRegistrySummary, entries: Sequence[RuntimeRegistryEntry | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.boundary = _text(boundary, "runtime registry boundary", 2048)
        ...
        self.release_ready = _bool(release_ready, "runtime registry readiness")
        if state not in STATES:
            raise ValidationError("runtime registry state is unsupported")
        self.state = state
        self.manifest = manifest if isinstance(manifest, RuntimeRegistryManifest) else RuntimeRegistryManifest.from_mapping(_mapping(manifest, "runtime registry manifest"))
        self.summary = summary if isinstance(summary, RuntimeRegistrySummary) else RuntimeRegistrySummary.from_mapping(_mapping(summary, "runtime registry summary"))
        self.entries = tuple(item if isinstance(item, RuntimeRegistryEntry) else RuntimeRegistryEntry.from_mapping(_mapping(item, "runtime registry entry")) for item in _sequence(entries, "runtime registry entries", MAX_ENTRIES))
        self.content_address = _address(content_address, "runtime registry address", REGISTRY_PREFIX)
        self._validate()
    ...
    def _validate(self) -> None:
        if self.version != VERSION or self.boundary != BOUNDARY or self.entry_count != len(self.entries) or tuple(item.ordinal for item in self.entries) != tuple(range(1, self.entry_count + 1)):
            raise ValidationError("runtime registry identity or entry count does not replay")
        if len({item.runtime_id for item in self.entries}) != self.entry_count or len({item.runtime_address for item in self.entries}) != self.entry_count:
            raise ValidationError("runtime registry contains duplicate runtime identity or address")
        ready = sum(item.state == "ready" for item in self.entries)
        blocked = sum(item.state == "blocked" for item in self.entries)
        if (self.ready_count, self.blocked_count) != (ready, blocked):
            raise ValidationError("runtime registry state counters do not replay")
        ...
        if tuple(getattr(self.summary, field) for field in SUMMARY_FIELDS[:-1]) != expected or self.summary.content_address != address_summary(self.summary):
            raise ValidationError("runtime registry summary does not replay")
        expected_manifest = (self.registry_id, VERSION, BOUNDARY, FILES, (address_entries(self.entries), self.summary.content_address))
...
def build_registry(runtimes: Sequence[runtime_model.HistoryRuntime], *, registry_id: str = DEFAULT_REGISTRY_ID) -> RuntimeRegistry:
    typed = tuple(runtimes)
    if len(typed) > MAX_ENTRIES:
        raise ValidationError("runtime registry exceeds its entry bound")
    if any(not isinstance(item, runtime_model.HistoryRuntime) for item in typed):
        raise ValidationError("runtime registry requires typed history runtimes")
    for item in typed:
        runtime_model.verify_runtime(item)
    entries = tuple(entry_from_runtime(item, index) for index, item in enumerate(typed, 1))
    entry_count = len(entries)
    ready_count = sum(item.state == "ready" for item in entries)
    blocked_count = sum(item.state == "blocked" for item in entries)
    accepted = entry_count > 0
    release_ready = accepted and blocked_count == 0
    state = "empty" if entry_count == 0 else "ready" if release_ready else "blocked"
    summary = _seal(RuntimeRegistrySummary(registry_id, entry_count, ready_count, blocked_count, accepted, release_ready, state, f"pending:{SUMMARY_PREFIX}"), address_summary)
    manifest = _seal(RuntimeRegistryManifest(registry_id, VERSION, BOUNDARY, FILES, (address_entries(entries), summary.content_address), f"pending:{MANIFEST_PREFIX}"), address_manifest)
...
def render_registry_markdown(value: RuntimeRegistry) -> str:
    value = verify_registry(value)
    lines = [f"# Runtime registry {value.registry_id}", "", f"- State: {value.state}", f"- Release ready: {str(value.release_ready).lower()}", f"- Entries: {value.entry_count}", f"- Ready: {value.ready_count}", f"- Blocked: {value.blocked_count}", "", "| Runtime | State | Ready | Checks | History entries |", "| --- | --- | --- | ---: | ---: |"]
    lines.extend(f"| {item.runtime_id} | {item.state} | {str(item.release_ready).lower()} | {item.passed_count}/{item.check_count} | {item.entry_count} |" for item in value.entries)
    return "\n".join(lines) + "\n"
...
def persist_registry(value: RuntimeRegistry, destination: str | Path, *, overwrite: bool = False) -> Path:
    ...
    temporary = Path(tempfile.mkdtemp(prefix=".downloaded-data-quality-runtime-registry-", dir=str(destination.parent)))
    documents = {"manifest.json": value.manifest.to_dict(), "registry.json": value.to_dict(), "entries.json": {"entries": [item.to_dict() for item in value.entries], "content_address": address_entries(value.entries)}, "summary.json": value.summary.to_dict()}
    try:
        for name in FILES:
...
def load_registry(destination: str | Path) -> RuntimeRegistry:
    destination = Path(destination)
    _validate_parent(destination.parent, "runtime registry input")
    ...
    if tuple(sorted(item.name for item in children)) != tuple(sorted(FILES)) or any(item.is_symlink() or not item.is_file() for item in children):
        raise ValidationError("runtime registry directory must contain the exact regular file set")
    documents = {name: _read_json(destination / name) for name in FILES}
    for name, document in documents.items():
        serialized = canonical_json(document)
        if read_text(destination / name, field="runtime registry artifact") != serialized or len(serialized.encode("utf-8")) > MAX_REGISTRY_BYTES:
            raise ValidationError("runtime registry artifact is non-canonical or exceeds its size bound")
...
def run_registry(runtimes: Sequence[runtime_model.HistoryRuntime], *, registry_id: str = DEFAULT_REGISTRY_ID, destination: str | Path | None = None, overwrite: bool = False) -> RuntimeRegistry:
...
def entry_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RuntimeRegistryEntry", "type": "object", "additionalProperties": False, "required": list(ENTRY_FIELDS), "properties": {field: {"type": "integer" if field in ("ordinal", "check_count", "passed_count", "entry_count") else "boolean" if field == "release_ready" else "string"} for field in ENTRY_FIELDS}}


def manifest_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RuntimeRegistryManifest", "type": "object", "additionalProperties": False, "required": list(MANIFEST_FIELDS), "properties": {field: {"type": "array" if field in ("files", "artifact_addresses") else "string"} for field in MANIFEST_FIELDS}}


def summary_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RuntimeRegistrySummary", "type": "object", "additionalProperties": False, "required": list(SUMMARY_FIELDS), "properties": {field: {"type": "integer" if field.endswith("count") else "boolean" if field in ("accepted", "release_ready") else "string"} for field in SUMMARY_FIELDS}}


def registry_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RuntimeRegistry", "type": "object", "additionalProperties": False, "required": list(REGISTRY_FIELDS), "properties": {field: {"type": "array" if field == "entries" else "object" if field in ("manifest", "summary") else "integer" if field.endswith("count") else "boolean" if field in ("accepted", "release_ready") else "string"} for field in REGISTRY_FIELDS}, "$defs": {"entry": entry_schema()}}


def capabilities() -> dict[str, Any]:
    return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "files": FILES, "states": STATES, "max_entries": MAX_ENTRIES, "features": ("multi-runtime admission", "duplicate runtime identity and address rejection", "ready and blocked aggregate folding", "exact four-file persistence", "canonical reload and tamper rejection", "JSON CSV and Markdown projections"), "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}


__all__ = ["VERSION", "BOUNDARY", "REGISTRY_PREFIX", "ENTRY_PREFIX", "ENTRIES_PREFIX", "MANIFEST_PREFIX", "SUMMARY_PREFIX", "DEFAULT_REGISTRY_ID", "FILES", "ARTIFACT_FILES", "STATES", "MAX_ENTRIES", "MAX_REGISTRY_BYTES", "ENTRY_FIELDS", "ENTRIES_FIELDS", "MANIFEST_FIELDS", "SUMMARY_FIELDS", "REGISTRY_FIELDS", "RuntimeRegistryEntry", "RuntimeRegistryEntries", "RuntimeRegistryManifest", "RuntimeRegistrySummary", "RuntimeRegistry", "address_entry", "address_entries", "address_manifest", "address_summary", "address_registry", "entry_from_runtime", "build_registry", "verify_registry", "registry_from_mapping", "registry_json", "entries_json", "manifest_json", "summary_json", "registry_csv", "render_registry_markdown", "persist_registry", "load_registry", "run_registry", "entry_schema", "manifest_schema", "summary_schema", "registry_schema", "capabilities"]
