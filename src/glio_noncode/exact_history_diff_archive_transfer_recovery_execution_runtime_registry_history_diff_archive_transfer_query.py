...
from . import exact_history_diff_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer as transfer_model
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow:
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery:
    ...
    def __init__(self, transfer_address: str, transfer_id: str, resources: Sequence[str], index_filter: int | None, offset_filter: int | None, size_filter: int | None, chunk_address_filter: str, received_filter: bool | None, text_filter: str, received_indices: Sequence[int], received_bytes: int, offset: int, limit: int, total_count: int, matched_count: int, returned_count: int, truncated: bool, rows: Sequence[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow], content_address: str) -> None:
        ...
        self.rows = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow) else ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow.from_mapping(item) for item in _sequence(rows, "query rows", MAX_LIMIT))
...
def address_row(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow):
...
def address_query(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery):
        raise ValidationError("query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def _row(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer, resource: str, ordinal: int, *, index: int | None = None, received: bool | None = None, missing: bool | None = None, received_bytes: int = 0, complete: bool | None = None) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow:
    chunk = value.chunks[index] if index is not None else None
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow(resource, ordinal, value.transfer_id, value.content_address, value.archive_address, value.archive_size, value.chunk_size, value.chunk_count, None if chunk is None else chunk.index, None if chunk is None else chunk.offset, None if chunk is None else chunk.size, "" if chunk is None else chunk.content_address, received, missing, received_bytes, complete, "pending:row")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow(provisional.resource, provisional.ordinal, provisional.transfer_id, provisional.transfer_address, provisional.archive_address, provisional.archive_size, provisional.chunk_size, provisional.chunk_count, provisional.chunk_index, provisional.chunk_offset, provisional.chunk_size_value, provisional.chunk_address, provisional.received, provisional.missing, provisional.received_bytes, provisional.complete, address_row(provisional))


def _rows(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer, resources: Sequence[str], received_indices: tuple[int, ...], received_bytes: int) -> tuple[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow, ...]:
    received = set(received_indices)
    complete = len(received) == value.chunk_count
    rows: list[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow] = []
...
def _matches(row: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow, *, index: int | None, offset: int | None, size: int | None, chunk_address: str, received: bool | None, text: str) -> bool:
...
def query_transfer(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer, *, resources: Sequence[str] | None = None, received_indices: Sequence[int] | None = None, index: int | None = None, chunk_offset: int | None = None, size: int | None = None, chunk_address: str = "", received: bool | None = None, text: str = "", offset: int = 0, limit: int = MAX_LIMIT) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery:
    if not isinstance(value, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer):
    ...
    normalized = tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow(row.resource, ordinal + 1, row.transfer_id, row.transfer_address, row.archive_address, row.archive_size, row.chunk_size, row.chunk_count, row.chunk_index, row.chunk_offset, row.chunk_size_value, row.chunk_address, row.received, row.missing, row.received_bytes, row.complete, "pending:row") for ordinal, row in enumerate(page))
    rows = tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow(row.resource, row.ordinal, row.transfer_id, row.transfer_address, row.archive_address, row.archive_size, row.chunk_size, row.chunk_count, row.chunk_index, row.chunk_offset, row.chunk_size_value, row.chunk_address, row.received, row.missing, row.received_bytes, row.complete, address_row(row)) for row in normalized)
    total_count = len(all_rows)
    matched_count = len(filtered)
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery(value.content_address, value.transfer_id, resources, index, chunk_offset, size, chunk_address, received, text, indices, received_bytes, offset, limit, total_count, matched_count, len(rows), offset + len(rows) < matched_count, rows, "pending:query")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery(provisional.transfer_address, provisional.transfer_id, provisional.resources, provisional.index_filter, provisional.offset_filter, provisional.size_filter, provisional.chunk_address_filter, provisional.received_filter, provisional.text_filter, provisional.received_indices, provisional.received_bytes, provisional.offset, provisional.limit, provisional.total_count, provisional.matched_count, provisional.returned_count, provisional.truncated, provisional.rows, address_query(provisional))


def query_assembler(assembler: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferAssembler, **kwargs: Any) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery:
    if not isinstance(assembler, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferAssembler):
...
def query_from_mapping(value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery:
    ...
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery(value["transfer_address"], value["transfer_id"], value["resources"], value["index_filter"], value["offset_filter"], value["size_filter"], value["chunk_address_filter"], value["received_filter"], value["text_filter"], value["received_indices"], value["received_bytes"], value["offset"], value["limit"], value["total_count"], value["matched_count"], value["returned_count"], value["truncated"], tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow.from_mapping(row) for row in _sequence(value["rows"], "query rows", MAX_LIMIT)), value["content_address"])


def query_json(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery) -> str:
    return canonical_json(query_from_mapping(value.to_dict()).to_dict())


def query_csv(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery) -> str:
...
def render_query_markdown(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# Exact History-Diff Archive Transfer Query", "", f"- Transfer: `{value.transfer_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.returned_count}/{value.matched_count}`", f"- Address: `{value.content_address}`", "", "| # | resource | chunk | received | missing | address |", "| ---: | --- | ---: | --- | --- | --- |"]
...
__all__ = ["BOUNDARY", "MAX_LIMIT", "MAX_QUERY_ITEMS", "QUERY_FIELDS", "QUERY_PREFIX", "RESOURCES", "ROW_FIELDS", "ROW_PREFIX", "ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQuery", "ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferQueryRow", "VERSION", "address_query", "address_row", "capabilities", "query_assembler", "query_csv", "query_from_mapping", "query_json", "query_schema", "query_transfer", "render_query_markdown", "row_schema"]
