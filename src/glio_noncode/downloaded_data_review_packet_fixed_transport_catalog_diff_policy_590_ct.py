"""Typed contracts for strict and release gates over module589 catalog diffs."""
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_589_ct import CHANGES, DIFF_PREFIX, DIRECTIONS, POSTURES, STATE_TRANSITIONS, VERSION as DIFF_VERSION
...
class PolicyCheck:
    ...
    def __post_init__(self) -> None:
        _label(self.check_id, "module590 check ID")
        if self.check_id not in POLICY_CHECK_IDS:
            raise ValidationError("module590 check ID is unsupported")
        _bool(self.passed, "module590 check result")
        _text(self.detail, "module590 check detail", 2048)
        _address(self.content_address, "module590 check address", CHECK_PREFIX)
        if not self.content_address.endswith(":pending") and address_check(self) != self.content_address:
            raise ValidationError("module590 check address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "PolicyCheck":
        if not isinstance(value, Mapping):
            raise ValidationError("module590 check must be an object")
        _strict(value, set(CHECK_FIELDS), "module590 check")
...
class Policy:
    ...
    def __post_init__(self) -> None:
        _label(self.policy_id, "module590 policy ID")
        if self.version != VERSION or self.boundary != BOUNDARY:
            raise ValidationError("module590 policy identity is not current")
        _address(self.diff_address, "module590 diff address", DIFF_PREFIX)
        for field in ("maximum_added", "maximum_removed", "maximum_changed", "maximum_total_changes"):
            _count(getattr(self, field), f"module590 {field}", MAX_BUDGET)
        _ordered_labels(self.allowed_changes, "module590 allowed changes", CHANGES)
        _ordered_labels(self.allowed_directions, "module590 allowed directions", DIRECTIONS)
        _ordered_labels(self.allowed_transitions, "module590 allowed transitions", STATE_TRANSITIONS)
        _ordered_labels(self.allowed_left_postures, "module590 allowed left postures", POSTURES)
        _ordered_labels(self.allowed_right_postures, "module590 allowed right postures", POSTURES)
        _ordered_labels(self.required_changes, "module590 required changes", CHANGES, allow_empty=True)
        for field in ("require_ready", "require_change", "allow_unchanged", "accepted"):
            _bool(getattr(self, field), f"module590 {field}")
        if self.state not in STATES or not isinstance(self.checks, tuple) or tuple(item.check_id for item in self.checks) != POLICY_CHECK_IDS:
            raise ValidationError("module590 policy state or checks are invalid")
        _count(self.check_count, "module590 check count", len(POLICY_CHECK_IDS))
        _count(self.passed_count, "module590 passed count", len(POLICY_CHECK_IDS))
        _count(self.failed_count, "module590 failed count", len(POLICY_CHECK_IDS))
        if self.check_count != len(self.checks) or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0) or self.state != ("ready" if self.accepted else "blocked"):
            raise ValidationError("module590 policy aggregates do not replay")
        if _has_forbidden_key(self.to_dict()):
            raise ValidationError("module590 policy crosses the public boundary")
        _address(self.content_address, "module590 policy address", POLICY_PREFIX)
        if not self.content_address.endswith(":pending") and address_policy(self) != self.content_address:
            raise ValidationError("module590 policy address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "Policy":
        if not isinstance(value, Mapping):
            raise ValidationError("module590 policy must be an object")
        _strict(value, set(POLICY_FIELDS), "module590 policy")
...
def policy_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module590 policy", "type": "object", "additionalProperties": False, "required": list(POLICY_FIELDS), "properties": {field: {"type": "array" if field in {"allowed_changes", "allowed_directions", "allowed_transitions", "allowed_left_postures", "allowed_right_postures", "required_changes", "checks"} else "boolean" if field in {"require_ready", "require_change", "allow_unchanged", "accepted"} else "integer" if field in {"maximum_added", "maximum_removed", "maximum_changed", "maximum_total_changes", "check_count", "passed_count", "failed_count"} else "string"} for field in POLICY_FIELDS}}


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module590 policy check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"check_id": {"enum": list(POLICY_CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "content_address": {"type": "string"}}}
