"""Compare source-free fixed package-catalog policy transport catalogs."""
...
def build_diff(left: Any, right: Any, *, diff_id: str = DEFAULT_DIFF_ID) -> Diff:
    ...
    if left_catalog.catalog_id != right_catalog.catalog_id:
        raise ValidationError("fixed package-catalog policy transport catalog diff requires matching catalog IDs")
    ...
    if len(entry_ids) > MAX_ITEMS:
        raise ValidationError("fixed package-catalog policy transport catalog diff item count exceeds its bound")
...
def _item_from_mapping(value: Mapping[str, Any]) -> DiffItem:
    if not isinstance(value, Mapping) or set(value) != set(ITEM_FIELDS):
        raise ValidationError("fixed package-catalog policy transport catalog diff item fields are not exact")
...
def _diff_from_mapping(value: Mapping[str, Any]) -> Diff:
    if not isinstance(value, Mapping) or set(value) != set(DIFF_FIELDS):
        raise ValidationError("fixed package-catalog policy transport catalog diff fields are not exact")
...
def verify_diff(value: Any) -> Diff:
    ...
    if not isinstance(diff, Diff):
        raise ValidationError("fixed package-catalog policy transport catalog diff verification requires a typed diff, mapping, or JSON source")
    if address_diff(diff) != diff.content_address or _has_forbidden_key(diff.to_dict()):
        raise ValidationError("fixed package-catalog policy transport catalog diff address or public boundary does not replay")
...
def load_diff(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> Diff:
    if isinstance(value, Mapping):
        ...
        raw = _strict_json_loads(read_text(value, field="fixed package-catalog policy transport catalog diff", max_bytes=32 * 1024 * 1024))
...
def write_diff(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Diff:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("fixed package-catalog policy transport catalog diff destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("fixed package-catalog policy transport catalog diff destination is not a file")
    _validate_parent(path.parent, "fixed package-catalog policy transport catalog diff destination")
    atomic_write_text(path, diff_json(diff), field="fixed package-catalog policy transport catalog diff destination")
    return diff


def query_diff(value: Any, *, change: str = "", direction: str = "", state_transition: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    diff = verify_diff(value)
    change = _text(change, "fixed package-catalog policy transport catalog diff query change", 64, required=False)
    direction = _text(direction, "fixed package-catalog policy transport catalog diff query direction", 64, required=False)
    state_transition = _text(state_transition, "fixed package-catalog policy transport catalog diff query state transition", 128, required=False)
    text = _text(text, "fixed package-catalog policy transport catalog diff query text", required=False)
    if (change and change not in CHANGES) or (direction and direction not in DIRECTIONS) or (state_transition and state_transition not in STATE_TRANSITIONS) or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("fixed package-catalog policy transport catalog diff query filters are invalid")
