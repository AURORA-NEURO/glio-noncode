"""Sanitized JSON, CSV, and Markdown exports for Domain 08."""
...
from .cell_state_frontier_release import CellStateFrontierReleaseManifest
from .cell_state_frontier_views import CellStateFrontierView, cell_state_frontier_review_summary
...
def export_cell_state_frontier_receipts_csv(evaluation: CellStateFrontierEvaluationReport) -> str:
...
def export_cell_state_frontier_review_csv(view: CellStateFrontierView) -> str:
...
def export_cell_state_frontier_metrics_csv(metrics: CellStateFrontierMetrics) -> str:
    fields = ("operation", "record_count", "positive_count", "control_count", "supported_count", "review_count", "issue_count", "content_address")
    rows = [{"operation": item.operation.value, "record_count": item.record_count, "positive_count": item.positive_count, "control_count": item.control_count, "supported_count": item.supported_count, "review_count": item.review_count, "issue_count": item.issue_count, "content_address": item.content_address} for item in metrics.operation_metrics]
    return _csv_text(rows, fields)


def render_cell_state_frontier_review_markdown(view: CellStateFrontierView) -> str:
    summary = cell_state_frontier_review_summary(view)
    lines = ["# Cell-state frontier review", "", f"- Fixture: `{view.fixture_id}`", f"- Context: `{view.context_key}`", f"- Boundary: `{view.evidence_boundary}`", f"- Review rows: **{view.review_count}**", "", "| Record | Operation | State | Priority | Issue codes | Action |", "| --- | --- | --- | ---: | --- | --- |"]
    ...
    lines.extend(("", "## Summary", "", f"State counts: `{summary['state_counts']}`.", f"Operation counts: `{summary['operation_counts']}`.", "", "Review rows retain uncertainty and scope controls; they are not success records."))
    return "\n".join(lines) + "\n"


def render_cell_state_frontier_release_markdown(release: CellStateFrontierReleaseManifest) -> str:
    return "\n".join(("# Cell-state frontier release", "", f"- Release: `{release.release_id}`", f"- Fixture: `{release.fixture_id}`", f"- Version: `{release.fixture_version}`", f"- Run: `{release.run_id}`", f"- Context: `{release.context_key}`", f"- Boundary: `{release.evidence_boundary}`", f"- State: **{release.release_state}**", f"- Sources: `{', '.join(release.source_ids)}`", f"- Bundle address: `{release.bundle_address}`", f"- Records address: `{release.record_address}`", "", "The manifest records descriptive cell-state evidence and its review boundary.")) + "\n"


def export_cell_state_frontier_json(value: Any) -> str:
    return json.dumps(jsonable(value), sort_keys=True, indent=2) + "\n"


def cell_state_frontier_export_receipt(export_name: str, payload: str) -> dict[str, Any]:
