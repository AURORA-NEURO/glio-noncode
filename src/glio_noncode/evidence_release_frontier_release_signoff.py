"""Detailed release signoff plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseReleaseSignoff:
    signoff_count: int
    accepted: bool
...
def build_evidence_release_release_signoff(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseReleaseSignoff:
    ...
    return EvidenceReleaseReleaseSignoff(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_release_signoff(receipt: EvidenceReleaseReleaseSignoff, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "signoff_count") != expected:
        return False
    return all(
...
def summarize_release_signoff(receipt: EvidenceReleaseReleaseSignoff) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "signoff_count"),
...
__all__ = ["EvidenceReleaseReleaseSignoff", "validate_evidence_release_release_signoff", "build_evidence_release_release_signoff", "summarize_release_signoff"]
