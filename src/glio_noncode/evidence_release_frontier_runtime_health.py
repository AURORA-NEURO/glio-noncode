"""Detailed runtime health plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseRuntimeHealth:
    stage_count: int
    accepted: bool
...
def build_evidence_release_runtime_health(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseRuntimeHealth:
    ...
    return EvidenceReleaseRuntimeHealth(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_runtime_health(receipt: EvidenceReleaseRuntimeHealth, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "stage_count") != expected:
        return False
    return all(
...
def summarize_runtime_health(receipt: EvidenceReleaseRuntimeHealth) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "stage_count"),
...
__all__ = ["EvidenceReleaseRuntimeHealth", "validate_evidence_release_runtime_health", "build_evidence_release_runtime_health", "summarize_runtime_health"]
