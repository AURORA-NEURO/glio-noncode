...
def default_intake_architecture_schema() -> IntakeArchitectureSchemaManifest:
    ...
    for field_id, type_name, required, privacy_scope, description in _FIELDS:
        body = {"field_id": field_id, "type_name": type_name, "required": required, "privacy_scope": privacy_scope, "description": description}
        fields.append(IntakeArchitectureSchemaField(**body, content_address=addressed(body, "intake-schema-field")))
    body = {"schema_id": "intake-architecture-d01", "version": INTAKE_ARCHITECTURE_VERSION, "fields": tuple(fields), "accepted": all(field.privacy_scope == "public_aggregate" for field in fields)}
    return IntakeArchitectureSchemaManifest(**body, content_address=addressed(body, "intake-schema"))


def validate_intake_architecture_schema(schema: IntakeArchitectureSchemaManifest | None = None) -> tuple[str, ...]:
