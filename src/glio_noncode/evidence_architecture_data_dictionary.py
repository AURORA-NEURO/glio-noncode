"""Field dictionary for D14 public aggregate records."""
...
def evidence_architecture_data_dictionary(
    ...
    fields = (
        ...
        ("operation_id", "string", "D14 capability operation join key", "operation"),
        ("case_id", "string", "scenario-specific execution join key", "case"),
        ("family", "enum", "delegate family boundary", "case"),
        ("plane", "enum", "lifecycle or release processing plane", "case"),
...
def evidence_architecture_data_dictionary_summary(
    fixture: EvidenceArchitectureFixture | None = None,
) -> dict[str, object]:
    rows = evidence_architecture_data_dictionary(fixture)
    return {"field_count": len(rows), "fields": [item["field"] for item in rows]}


__all__ = ["evidence_architecture_data_dictionary", "evidence_architecture_data_dictionary_summary"]
