"""Independently audit longitudinal packet-package handoff catalog diffs."""
...
from . import downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff as diff_model
from .downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_contracts import BOUNDARY, DIFF_PREFIX, ITEM_PREFIX, VERSION
...
class DiffAuditCheck:
    ...
    def __init__(self, check_id: str, passed: bool, observed: Any, required: Any, detail: str, content_address: str) -> None:
        self.check_id = _label(check_id, "packet-package handoff catalog diff audit check ID")
        if self.check_id not in CHECK_IDS or not isinstance(passed, bool):
            raise ValidationError("packet-package handoff catalog diff audit check is invalid")
        self.passed = passed
        self.observed = observed
        self.required = required
        self.detail = _text(detail, "packet-package handoff catalog diff audit detail", 2048)
        self.content_address = content_address
        if not content_address.endswith(":pending"):
            _address(content_address, "packet-package handoff catalog diff audit check address", CHECK_PREFIX)
            if address_check(self) != content_address:
                raise ValidationError("packet-package handoff catalog diff audit check address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "DiffAuditCheck":
        if not isinstance(value, Mapping) or set(value) != set(cls.FIELDS):
            raise ValidationError("packet-package handoff catalog diff audit check fields are not exact")
...
class DiffAudit:
    ...
    def __init__(self, diff_id: str, diff_address: str, checks: tuple[DiffAuditCheck, ...], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.diff_id = _label(diff_id, "packet-package handoff catalog diff audit diff ID")
        self.diff_address = _address(diff_address, "packet-package handoff catalog diff audit diff address", DIFF_PREFIX)
        ...
        if self.check_count != len(self.checks) or self.check_count != len(CHECK_IDS) or tuple(item.check_id for item in self.checks) != CHECK_IDS or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0):
            raise ValidationError("packet-package handoff catalog diff audit aggregates do not replay")
        if not content_address.endswith(":pending"):
            _address(content_address, "packet-package handoff catalog diff audit address", AUDIT_PREFIX)
            if address_audit(self) != content_address:
                raise ValidationError("packet-package handoff catalog diff audit address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "DiffAudit":
        if not isinstance(value, Mapping) or set(value) != set(cls.FIELDS):
            raise ValidationError("packet-package handoff catalog diff audit fields are not exact")
...
def audit_diff(value: Any, *, left: Any | None = None, right: Any | None = None) -> DiffAudit:
    try:
        ...
        checks = (
            ...
            _check("audit-left-availability", left_value is not None, "available" if left_value is not None else "missing", "available", "left packet-package handoff catalog is supplied"),
            _check("audit-right-availability", right_value is not None, "available" if right_value is not None else "missing", "available", "right packet-package handoff catalog is supplied"),
            ...
            _check("audit-recomputation", recomputed_matches, "recomputed" if recomputed_matches else "mismatch", "recomputed diff", "diff is deterministic from supplied packet-package handoff catalogs"),
        ...
        checks = tuple(_check(check_id, False, "unavailable", "replayable", f"packet-package handoff catalog diff audit unavailable: {exc}") for check_id in CHECK_IDS)
...
def load_audit(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> DiffAudit:
    if isinstance(value, Mapping):
        ...
        raw = _strict_json_loads(read_text(value, field="packet-package handoff catalog diff audit", max_bytes=16 * 1024 * 1024))
...
def write_audit(value: Any, destination: str | Path, *, allow_existing: bool = False) -> DiffAudit:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("packet-package handoff catalog diff audit destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("packet-package handoff catalog diff audit destination is not a file")
    _validate_parent(path.parent, "packet-package handoff catalog diff audit destination")
    atomic_write_text(path, audit_json(audit), field="packet-package handoff catalog diff audit destination")
    return audit
...
def query_audit(value: Any, *, passed: bool | None = None, text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if passed is not None and not isinstance(passed, bool):
        raise ValidationError("packet-package handoff catalog diff audit passed filter must be boolean")
    if not isinstance(text, str) or len(text) > 4096 or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("packet-package handoff catalog diff audit query bounds are invalid")
...
def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded-data packet-package handoff catalog diff audit check", "type": "object", "additionalProperties": False, "required": list(DiffAuditCheck.FIELDS), "properties": {"check_id": {"enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "observed": {}, "required": {}, "detail": {"type": "string"}, "content_address": {"type": "string"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded-data packet-package handoff catalog diff audit", "type": "object", "additionalProperties": False, "required": list(DiffAudit.FIELDS), "properties": {"diff_id": {"type": "string"}, "diff_address": {"type": "string"}, "checks": {"type": "array", "minItems": len(CHECK_IDS), "maxItems": len(CHECK_IDS)}, "check_count": {"const": len(CHECK_IDS)}, "passed_count": {"type": "integer"}, "failed_count": {"type": "integer"}, "accepted": {"type": "boolean"}, "content_address": {"type": "string"}}}
