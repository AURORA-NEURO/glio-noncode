"""Detailed safety projection plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseSafetyProjection:
    field_count: int
    accepted: bool
...
def build_evidence_release_safety_projection(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseSafetyProjection:
    ...
    return EvidenceReleaseSafetyProjection(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_safety_projection(receipt: EvidenceReleaseSafetyProjection, *, expected: int | None = None) -> bool:
...
def summarize_safety_projection(receipt: EvidenceReleaseSafetyProjection) -> Mapping[str, Any]:
...
__all__ = ["EvidenceReleaseSafetyProjection", "validate_evidence_release_safety_projection", "build_evidence_release_safety_projection", "summarize_safety_projection"]
