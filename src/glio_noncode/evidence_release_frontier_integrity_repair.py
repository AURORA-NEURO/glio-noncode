"""Detailed integrity repair plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseIntegrityRepair:
    repair_count: int
    accepted: bool
...
def build_evidence_release_integrity_repair(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseIntegrityRepair:
    ...
    return EvidenceReleaseIntegrityRepair(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_integrity_repair(receipt: EvidenceReleaseIntegrityRepair, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "repair_count") != expected:
        return False
    return all(
...
def summarize_integrity_repair(receipt: EvidenceReleaseIntegrityRepair) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "repair_count"),
...
__all__ = ["EvidenceReleaseIntegrityRepair", "validate_evidence_release_integrity_repair", "build_evidence_release_integrity_repair", "summarize_integrity_repair"]
