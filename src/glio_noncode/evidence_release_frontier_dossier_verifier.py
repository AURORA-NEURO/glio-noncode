"""Detailed dossier verifier plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseDossierVerifier:
...
def build_evidence_release_dossier_verifier(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseDossierVerifier:
    ...
    return EvidenceReleaseDossierVerifier(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_dossier_verifier(receipt: EvidenceReleaseDossierVerifier, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "verification_count") != expected:
        return False
    return all(
...
def summarize_dossier_verifier(receipt: EvidenceReleaseDossierVerifier) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "verification_count"),
...
__all__ = ["EvidenceReleaseDossierVerifier", "validate_evidence_release_dossier_verifier", "build_evidence_release_dossier_verifier", "summarize_dossier_verifier"]
