...
from . import (
    downloaded_data_quality_diff_gate_remediation_resolution_history_diff_audit as audit_model,
)
from . import (
    downloaded_data_quality_diff_gate_remediation_resolution_history_diff_query as query_model,
)
from . import (
    downloaded_data_quality_diff_gate_remediation_resolution_history_diff_query_audit as query_audit_model,
)
from ._safe_persistence import atomic_write_text, read_text
from .errors import ValidationError
from .serialization import _strict_json_loads, canonical_json, content_hash

VERSION = "downloaded-data-quality-diff-gate-remediation-resolution-history-diff-runtime-v1"
BOUNDARY = "public_downloaded_data_quality_diff_gate_remediation_resolution_history_diff_runtime"
RUNTIME_PREFIX = "glio-noncode-download-quality-diff-gate-remediation-resolution-history-diff-runtime"
...
class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest:
        value = _mapping(value, "history diff runtime manifest")
        _strict(value, set(cls.FIELDS), "history diff runtime manifest")
        return cls(*(value[field] for field in cls.FIELDS))


def address_manifest(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest):
...
class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime:
    FIELDS = RUNTIME_FIELDS

    def __init__(self, runtime_id: str, version: str, boundary: str, diff_id: str, diff_address: str, audit_address: str, query_address: str, query_audit_address: str, added_count: int, removed_count: int, changed_count: int, unchanged_count: int, direction: str, state: str, accepted: bool, release_ready: bool, manifest: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest | Mapping[str, Any], diff: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff | Mapping[str, Any], audit: audit_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit | Mapping[str, Any], query: query_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffQuery | Mapping[str, Any], query_audit: Mapping[str, Any], content_address: str) -> None:
        ...
        self.manifest = manifest if isinstance(manifest, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest) else DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest.from_mapping(manifest)
        ...
        self.audit = audit if isinstance(audit, audit_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit) else audit_model.audit_from_mapping(audit)
        self.query = query if isinstance(query, query_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffQuery) else query_model.query_from_mapping(query)
        self.query_audit = query_audit if isinstance(query_audit, query_audit_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffQueryAudit) else query_audit_model.audit_from_mapping(query_audit)
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime:
        value = _mapping(value, "history diff runtime")
        _strict(value, set(cls.FIELDS), "history diff runtime")
        return cls(*(value[field] for field in cls.FIELDS))


def address_runtime(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime):
...
def build_runtime(diff: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff, *, runtime_id: str = DEFAULT_RUNTIME_ID, resources: Sequence[str] = query_model.RESOURCES, resource: str = "", change: str = "", direction: str = "", identity: str = "", text: str = "", offset: int = 0, limit: int = query_model.MAX_LIMIT) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime:
    ...
    manifest_provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest(**manifest_body, content_address=MANIFEST_PREFIX + ":pending")
    manifest = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest(**manifest_body, content_address=address_manifest(manifest_provisional))
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime(**body, content_address=RUNTIME_PREFIX + ":pending")
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime(**body, content_address=address_runtime(provisional))


def runtime_from_mapping(value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime:
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime.from_mapping(value)


def runtime_json(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime) -> str:
    return canonical_json(runtime_from_mapping(value.to_dict()).to_dict())


def runtime_csv(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime) -> str:
...
def render_runtime_markdown(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime) -> str:
    value = runtime_from_mapping(value.to_dict())
...
def persist_runtime(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime, destination: str | Path, *, overwrite: bool = False) -> Path:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime):
...
def load_runtime(destination: str | Path) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime:
    destination = Path(destination)
    ...
    runtime = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime.from_mapping(_read_json(destination / "runtime.json"))
    manifest = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffManifest.from_mapping(_read_json(destination / "manifest.json"))
...
def run_runtime(diff: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff, *, runtime_id: str = DEFAULT_RUNTIME_ID, resources: Sequence[str] = query_model.RESOURCES, resource: str = "", change: str = "", direction: str = "", identity: str = "", text: str = "", offset: int = 0, limit: int = query_model.MAX_LIMIT, destination: str | Path | None = None, overwrite: bool = False) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime:
