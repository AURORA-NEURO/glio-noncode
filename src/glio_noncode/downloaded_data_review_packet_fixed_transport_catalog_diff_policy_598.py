"""Apply deterministic release and strict gates to module597 catalog diffs."""
...
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_597 as diff_model
from . import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_598_ct as contract_model
...
def _settings(maximum_added: int, maximum_removed: int, maximum_changed: int, maximum_total_changes: int, allowed_changes: Sequence[str], allowed_directions: Sequence[str], allowed_transitions: Sequence[str], allowed_left_postures: Sequence[str], allowed_right_postures: Sequence[str], required_changes: Sequence[str], require_ready: bool, require_change: bool, allow_unchanged: bool) -> dict[str, Any]:
    return {"maximum_added": _count(maximum_added, "module598 maximum added", diff_model.MAX_ITEMS), "maximum_removed": _count(maximum_removed, "module598 maximum removed", diff_model.MAX_ITEMS), "maximum_changed": _count(maximum_changed, "module598 maximum changed", diff_model.MAX_ITEMS), "maximum_total_changes": _count(maximum_total_changes, "module598 maximum total changes", diff_model.MAX_ITEMS), "allowed_changes": _ordered_labels(allowed_changes, "module598 allowed changes", diff_model.CHANGES), "allowed_directions": _ordered_labels(allowed_directions, "module598 allowed directions", diff_model.DIRECTIONS), "allowed_transitions": _ordered_labels(allowed_transitions, "module598 allowed transitions", diff_model.STATE_TRANSITIONS), "allowed_left_postures": _ordered_labels(allowed_left_postures, "module598 allowed left postures", diff_model.POSTURES), "allowed_right_postures": _ordered_labels(allowed_right_postures, "module598 allowed right postures", diff_model.POSTURES), "required_changes": _ordered_labels(required_changes, "module598 required changes", diff_model.CHANGES, allow_empty=True), "require_ready": contract_model._bool(require_ready, "module598 require ready"), "require_change": contract_model._bool(require_change, "module598 require change"), "allow_unchanged": contract_model._bool(allow_unchanged, "module598 allow unchanged")}
...
def _checks(diff: Any, settings: Mapping[str, Any]) -> tuple[contract_model.PolicyCheck, ...]:
    ...
    return (
        _check("policy-diff-address", diff_model.address_diff(diff) == diff.content_address, diff.content_address, "replayable", "module597 diff address is valid"),
        _check("policy-diff-replay", diff_model.verify_diff(diff).content_address == diff.content_address, "replayed", "replayed", "module597 diff reload is valid"),
...
def verify_policy(value: Any) -> contract_model.Policy:
    ...
    if not isinstance(policy, contract_model.Policy) or contract_model.address_policy(policy) != policy.content_address or _has_forbidden_key(policy.to_dict()):
        raise ValidationError("module598 policy address or public boundary does not replay")
...
def load_policy(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> contract_model.Policy:
    raw = value if isinstance(value, Mapping) else _strict_json_loads(bytes(value).decode("utf-8")) if isinstance(value, (bytes, bytearray)) else _strict_json_loads(read_text(value, field="module598 policy", max_bytes=16 * 1024 * 1024))
...
def write_policy(value: Any, destination: str | Path, *, allow_existing: bool = False) -> contract_model.Policy:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("module598 policy destination already exists")
    _validate_parent(path.parent, "module598 policy destination")
    atomic_write_text(path, policy_json(policy), field="module598 policy destination")
    return policy
...
def query_policy(value: Any, *, passed: bool | None = None, check_id: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if passed is not None and not isinstance(passed, bool):
        raise ValidationError("module598 passed filter must be boolean")
    check_id = _text(check_id, "module598 check filter", 256, required=False)
    if check_id and check_id not in POLICY_CHECK_IDS:
        raise ValidationError("module598 check filter is unsupported")
    text = _text(text, "module598 text filter", required=False)
    offset = _count(offset, "module598 query offset", len(policy.checks))
    limit = _count(limit, "module598 query limit", MAX_LIMIT, positive=True)
...
def render_policy_markdown(value: Any) -> str:
    policy = verify_policy(value)
    lines = ["# Module598 Catalog Diff Policy", "", f"- Policy: `{policy.policy_id}`", f"- State: **{policy.state}**", f"- Passed / failed: **{policy.passed_count} / {policy.failed_count}**", f"- Diff: `{policy.diff_address}`", f"- Address: `{policy.content_address}`", "", "| check | passed | detail |", "| --- | --- | --- |"]
