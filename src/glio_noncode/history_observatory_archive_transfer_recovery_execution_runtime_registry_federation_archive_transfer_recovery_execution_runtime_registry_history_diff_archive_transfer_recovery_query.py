"""Bounded queries over federated history-diff recovery plans."""
...
class HistoryDiffArchiveTransferRecoveryQueryRow:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryQueryRow:
...
class HistoryDiffArchiveTransferRecoveryQuery:
    """A deterministic bounded query result over a recovery snapshot."""

    FIELDS = QUERY_FIELDS

    def __init__(self, recovery_address: str, recovery_id: str, version: str, boundary: str, resources: Sequence[str], index: int, state_filter: str, received_filter: bool | None, text_filter: str, offset: int, limit: int, rows: Sequence[HistoryDiffArchiveTransferRecoveryQueryRow], row_count: int, content_address: str) -> None:
        self.recovery_address = _address(recovery_address, "recovery query recovery address", recovery_model.RECOVERY_PREFIX)
        self.recovery_id = _label(recovery_id, "recovery query recovery ID")
        ...
        self.rows = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryQueryRow) else HistoryDiffArchiveTransferRecoveryQueryRow.from_mapping(item) for item in _sequence(rows, "recovery query rows", MAX_QUERY_ITEMS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryQuery:
        ...
        return cls(value["recovery_address"], value["recovery_id"], value["version"], value["boundary"], value["resources"], value["index"], value["state_filter"], value["received_filter"], value["text_filter"], value["offset"], value["limit"], tuple(HistoryDiffArchiveTransferRecoveryQueryRow.from_mapping(item) for item in _sequence(value["rows"], "recovery query rows", MAX_QUERY_ITEMS)), value["row_count"], value["content_address"])


def address_row(value: HistoryDiffArchiveTransferRecoveryQueryRow) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryQueryRow):
...
def address_query(value: HistoryDiffArchiveTransferRecoveryQuery) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryQuery):
        raise ValidationError("recovery query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def _row(value: recovery_model.HistoryDiffArchiveTransferRecovery, resource: str, ordinal: int, *, chunk_index: int = -1, chunk_offset: int = 0, chunk_size: int = 0, chunk_address: str = "", action_address: str = "", received: bool = True, missing: bool = False) -> HistoryDiffArchiveTransferRecoveryQueryRow:
    pending = HistoryDiffArchiveTransferRecoveryQueryRow(resource, ordinal, value.recovery_id, value.transfer_id, value.content_address, value.archive_address, value.archive_size, value.chunk_count, chunk_index, chunk_offset, chunk_size, chunk_address, action_address, received, missing, value.state, value.decision, value.safe_to_resume, value.checkpointed, value.next_index, value.remaining_bytes, "pending:recovery-query-row")
    return HistoryDiffArchiveTransferRecoveryQueryRow(resource, ordinal, value.recovery_id, value.transfer_id, value.content_address, value.archive_address, value.archive_size, value.chunk_count, chunk_index, chunk_offset, chunk_size, chunk_address, action_address, received, missing, value.state, value.decision, value.safe_to_resume, value.checkpointed, value.next_index, value.remaining_bytes, address_row(pending))


def _matches(row: HistoryDiffArchiveTransferRecoveryQueryRow, *, index: int, state: str, received: bool | None, text: str) -> bool:
...
def query_recovery(value: recovery_model.HistoryDiffArchiveTransferRecovery, *, resources: Sequence[str] | None = None, index: int | None = None, state: str = "", received: bool | None = None, text: str = "", offset: int = 0, limit: int = MAX_LIMIT) -> HistoryDiffArchiveTransferRecoveryQuery:
    ...
    rows: list[HistoryDiffArchiveTransferRecoveryQueryRow] = []
    ...
    for ordinal, row in enumerate(page, 1):
        ...
        normalized.append(HistoryDiffArchiveTransferRecoveryQueryRow(pending.resource, pending.ordinal, pending.recovery_id, pending.transfer_id, pending.recovery_address, pending.archive_address, pending.archive_size, pending.chunk_count, pending.chunk_index, pending.chunk_offset, pending.chunk_size, pending.chunk_address, pending.action_address, pending.received, pending.missing, pending.state, pending.decision, pending.safe_to_resume, pending.checkpointed, pending.next_index, pending.remaining_bytes, address_row(pending)))
    provisional = HistoryDiffArchiveTransferRecoveryQuery(value.content_address, value.recovery_id, VERSION, BOUNDARY, selected, query_index, state, received, text, offset, limit, tuple(normalized), len(normalized), "pending:recovery-query")
    return HistoryDiffArchiveTransferRecoveryQuery(value.content_address, value.recovery_id, VERSION, BOUNDARY, selected, query_index, state, received, text, offset, limit, tuple(normalized), len(normalized), address_query(provisional))


def query_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryQuery:
    return HistoryDiffArchiveTransferRecoveryQuery.from_mapping(value)


def query_json(value: HistoryDiffArchiveTransferRecoveryQuery) -> str:
    return canonical_json(query_from_mapping(value.to_dict()).to_dict())


def query_csv(value: HistoryDiffArchiveTransferRecoveryQuery) -> str:
...
def render_query_markdown(value: HistoryDiffArchiveTransferRecoveryQuery) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# Federation runtime-registry history-diff archive transfer recovery query", "", f"- Recovery: `{value.recovery_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.row_count}`", f"- Address: `{value.content_address}`", "", "| # | resource | chunk | received | missing | state | decision |", "| ---: | --- | ---: | --- | --- | --- | --- |"]
...
__all__ = ["BOUNDARY", "MAX_LIMIT", "MAX_QUERY_ITEMS", "QUERY_FIELDS", "QUERY_PREFIX", "RESOURCES", "ROW_FIELDS", "ROW_PREFIX", "HistoryDiffArchiveTransferRecoveryQuery", "HistoryDiffArchiveTransferRecoveryQueryRow", "VERSION", "address_query", "address_row", "capabilities", "query_csv", "query_from_mapping", "query_json", "query_recovery", "query_schema", "render_query_markdown", "row_schema"]
