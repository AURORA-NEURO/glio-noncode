"""Typed contracts for catalogs of module583 fixed policy transports."""
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_583_ct import PAYLOAD_NAMES, PACKAGE_PREFIX as TRANSPORT_PREFIX, VERSION as TRANSPORT_VERSION
...
class Entry:
    ...
    def __post_init__(self) -> None:
        _label(self.entry_id, "module584 entry ID")
        _label(self.package_id, "module584 package ID")
        _address(self.package_address, "module584 package address", TRANSPORT_PREFIX)
        _address(self.manifest_address, "module584 manifest address")
        _address(self.diff_address, "module584 diff address")
        _address(self.policy_address, "module584 policy address")
        _address(self.audit_address, "module584 audit address")
        if self.policy_state not in {"ready", "blocked"} or not isinstance(self.policy_accepted, bool) or not isinstance(self.audit_accepted, bool):
            raise ValidationError("module584 entry decision fields are invalid")
        _count(self.package_byte_count, "module584 package byte count", MAX_PACKAGE_BYTES)
        _count(self.member_count, "module584 member count", MAX_MEMBER_COUNT)
        _address(self.content_address, "module584 entry address", ENTRY_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_entry(self) != self.content_address:
            raise ValidationError("module584 entry address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Entry":
        _strict(value, set(ENTRY_FIELDS), "module584 entry")
        return cls(*(value[field] for field in ENTRY_FIELDS))


def address_entry(value: Entry) -> str:
    if not isinstance(value, Entry):
        raise ValidationError("module584 entry addressing requires the typed contract")
...
class Catalog:
    ...
    def __post_init__(self) -> None:
        _label(self.catalog_id, "module584 catalog ID")
        if self.version != VERSION or self.boundary != BOUNDARY or self.state not in STATES:
            raise ValidationError("module584 catalog identity or state is invalid")
        if not isinstance(self.entries, tuple) or len(self.entries) > MAX_ENTRIES or tuple(item.entry_id for item in self.entries) != tuple(sorted(item.entry_id for item in self.entries)) or len({item.entry_id for item in self.entries}) != len(self.entries) or len({item.package_address for item in self.entries}) != len(self.entries):
            raise ValidationError("module584 catalog entries are not canonical")
        _count(self.entry_count, "module584 entry count", MAX_ENTRIES)
        _count(self.accepted_count, "module584 accepted count", MAX_ENTRIES)
        _count(self.ready_count, "module584 ready count", MAX_ENTRIES)
        _count(self.blocked_count, "module584 blocked count", MAX_ENTRIES)
        if self.entry_count != len(self.entries) or self.accepted_count != sum(item.audit_accepted for item in self.entries) or self.ready_count != sum(item.policy_state == "ready" for item in self.entries) or self.blocked_count != sum(item.policy_state == "blocked" for item in self.entries):
            raise ValidationError("module584 catalog aggregates do not replay")
        ...
        if self.state != expected_state or _has_forbidden_key(self.to_dict()):
            raise ValidationError("module584 catalog state or public boundary does not replay")
        _address(self.content_address, "module584 catalog address", CATALOG_PREFIX, allow_pending=True)
        if not self.content_address.endswith(":pending") and address_catalog(self) != self.content_address:
            raise ValidationError("module584 catalog address does not replay")
    ...
    def from_mapping(cls, value: dict[str, Any]) -> "Catalog":
        _strict(value, set(CATALOG_FIELDS), "module584 catalog")
...
def address_catalog(value: Catalog) -> str:
    if not isinstance(value, Catalog):
        raise ValidationError("module584 catalog addressing requires the typed contract")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CATALOG_PREFIX)


def entry_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module584 catalog entry", "type": "object", "additionalProperties": False, "required": list(ENTRY_FIELDS), "properties": {"entry_id": {"type": "string"}, "package_id": {"type": "string"}, "package_address": {"type": "string"}, "manifest_address": {"type": "string"}, "diff_address": {"type": "string"}, "policy_address": {"type": "string"}, "audit_address": {"type": "string"}, "policy_state": {"enum": ["ready", "blocked"]}, "policy_accepted": {"type": "boolean"}, "audit_accepted": {"type": "boolean"}, "package_byte_count": {"type": "integer"}, "member_count": {"type": "integer", "maximum": MAX_MEMBER_COUNT}, "content_address": {"type": "string"}}}


def catalog_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module584 transport catalog", "type": "object", "additionalProperties": False, "required": list(CATALOG_FIELDS), "properties": {"catalog_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "entry_count": {"type": "integer", "maximum": MAX_ENTRIES}, "accepted_count": {"type": "integer", "maximum": MAX_ENTRIES}, "ready_count": {"type": "integer", "maximum": MAX_ENTRIES}, "blocked_count": {"type": "integer", "maximum": MAX_ENTRIES}, "state": {"enum": list(STATES)}, "entries": {"type": "array", "maxItems": MAX_ENTRIES}, "content_address": {"type": "string"}}}
