"""Independent assurance for exact archive-transfer execution receipts."""
...
CHECK_IDS = (
    "version", "boundary", "execution-address", "recovery-linkage", "transfer-linkage",
    "plan-conservation", "current-index-conservation", "outcome-order", "outcome-addresses",
    "status-conservation", "byte-conservation", "state-replay", "decision-replay",
    "safety-replay", "checkpoint-type", "next-index", "public-boundary", "mapping-round-trip",
)
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck:
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit:
    """Canonical audit receipt for an execution snapshot."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, execution_address: str, execution_id: str, version: str, boundary: str, checks: Sequence[Any], check_count: int, passed: bool, content_address: str) -> None:
        ...
        self.boundary = _text(boundary, "execution audit boundary", 2048)
        self.checks = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck) else ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck.from_mapping(item) for item in _sequence(checks, "execution audit checks", MAX_CHECKS))
    ...
    def from_mapping(cls, value: Mapping[str, Any]):
        ...
        return cls(value["execution_address"], value["execution_id"], value["version"], value["boundary"], tuple(ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "execution audit checks", MAX_CHECKS)), value["check_count"], value["passed"], value["content_address"])


def address_check(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck):
        raise ValidationError("execution audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit):
        raise ValidationError("execution audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, observed: Any, expected: Any):
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck(check_id, observed == expected, observed, expected, "pending:execution-audit-check")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck(check_id, provisional.passed, observed, expected, address_check(provisional))


def _expected_outcome_addresses(value: Any) -> tuple[str, ...]:
    result = []
    for item in value.outcomes:
        provisional = execution_model.ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionOutcome(item.index, item.action_address, item.content_address, item.offset, item.size, item.status, item.reason, "pending:execution-outcome")
        result.append(execution_model.address_outcome(provisional))
    return tuple(result)


def audit_execution(value: Any):
    ...
    base, planned = tuple(value.base_received_indices), tuple(value.planned_indices)
    applied, pending, rejected = set(value.applied_indices), set(value.pending_indices), set(value.rejected_indices)
    ...
    checks = (
        ...
        _check("recovery-linkage", (value.recovery_address.startswith(execution_model.recovery_model.RECOVERY_PREFIX + ":"), bool(value.recovery_id)), (True, True)),
        ...
        _check("plan-conservation", (base, planned, tuple(sorted(set(base) | set(planned))), tuple(sorted(set(base) & set(planned)))), (base, planned, universe, ())),
        _check("current-index-conservation", (value.current_received_indices, value.current_missing_indices, tuple(sorted(set(value.current_received_indices) | set(value.current_missing_indices))), tuple(sorted(set(value.current_received_indices) & set(value.current_missing_indices)))), (value.current_received_indices, value.current_missing_indices, universe, ())),
        ...
        _check("checkpoint-type", type(value.checkpointed).__name__, "bool"),
        _check("next-index", value.next_index, min(pending) if pending else min(rejected) if rejected else -1),
    ...
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit(value.content_address, value.execution_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), "pending:execution-audit")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit(value.content_address, value.execution_id, VERSION, BOUNDARY, checks, len(checks), provisional.passed, address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]):
    return ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit.from_mapping(value)
...
def render_audit_markdown(value: Any) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Exact execution-ledger archive transfer recovery execution audit", "", f"- Execution: `{value.execution_id}`", f"- Result: `{'passed' if value.passed else 'failed'}`", f"- Checks: `{value.check_count}`", f"- Address: `{value.content_address}`", "", "| check | passed | observed | expected |", "| --- | --- | --- | --- |"]
...
def capabilities() -> dict[str, Any]:
    return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "audit_prefix": AUDIT_PREFIX, "check_prefix": CHECK_PREFIX, "check_ids": list(CHECK_IDS), "features": ["independent plan and outcome replay", "index and byte conservation", "state decision and safety recomputation", "checkpoint and next-index verification", "canonical JSON CSV and Markdown projections"], "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}


__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAudit", "ExactHistoryDiffArchiveTransferRecoveryExecutionLedgerRuntimeRegistryHistoryDiffArchiveTransferRecoveryExecutionAuditCheck", "VERSION", "address_audit", "address_check", "audit_csv", "audit_execution", "audit_from_mapping", "audit_json", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
