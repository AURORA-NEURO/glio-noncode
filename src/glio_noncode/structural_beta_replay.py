"""Replay and drift checks for the Domain 02 C05-C08 fixture."""
...
class StructuralBetaReplayExpectation:
    ...
    minimum_positive_records: int = STRUCTURAL_BETA_OPERATION_FLOOR
    minimum_control_records: int = STRUCTURAL_BETA_CONTROL_FLOOR


@dataclass(frozen=True, slots=True)
class StructuralBetaReplayCase:
    """One replayed beta fixture and its address comparison result."""
...
@dataclass(frozen=True, slots=True)
class StructuralBetaReplayReport:
    """Aggregate replay result with duplicate and drift detection."""

    cases: tuple[StructuralBetaReplayCase, ...]
    issue_codes: tuple[str, ...]
    content_address: str
...
def replay_structural_beta_fixtures(
    paths: tuple[str, ...] | list[str],
    *,
    expectation: StructuralBetaReplayExpectation | None = None,
    required_context_key: str | None = None,
) -> StructuralBetaReplayReport:
    """Replay beta fixtures and reject duplicate identity or context drift."""

    if not paths:
        raise ValidationError("at least one beta fixture path is required")
    cases: list[StructuralBetaReplayCase] = []
    issues: set[str] = set()
    catalogs: list[StructuralBetaFixtureCatalog] = []
    for raw_path in paths:
        catalog = StructuralBetaFixtureCatalog.from_file(raw_path)
    ...
    return StructuralBetaReplayReport(
        cases=tuple(cases),
