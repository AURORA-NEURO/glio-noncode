"""Cross-surface quality gate for Domain 02 C05-C08."""
...
class StructuralBetaQualityCheck:
...
def evaluate_structural_beta_quality_gate(
    fixture: StructuralBetaFixtureCatalog | str,
) -> StructuralBetaQualityGateReport:
    """Run and reconcile the complete C05-C08 evidence surface."""
    ...
    address_floor = all(receipt.output_address[:7] == "sha256:" for receipt in evaluation.receipts)
    checks = (
        _check("data-audit", audit.accepted, True, audit.accepted, "public aggregate source and payload boundary"),
        _check("fixture-evaluation", evaluation.passed, True, evaluation.passed, "all beta positives and controls pass"),
        ...
        _check("operation-coverage", set(catalog.operation_ids) == {item.value for item in StructuralBetaOperation}, [item.value for item in StructuralBetaOperation], list(catalog.operation_ids), "all C05-C08 operations represented"),
        _check("contract-floor", len(contracts.contracts) == 4, 4, len(contracts.contracts), "four typed beta contracts"),
    ...
    state = StructuralBetaFixtureState.ACCEPTED if all(check.passed for check in checks) else StructuralBetaFixtureState.REVIEW
    body = {
        "fixture_id": catalog.fixture_id,
        "context_key": catalog.context_key,
        "state": state,
    ...
    return StructuralBetaQualityGateReport(
        fixture_id=catalog.fixture_id,
        context_key=catalog.context_key,
        state=state,
        checks=checks,
...
def _replay_catalog(
    ...
    return StructuralBetaReplayReport((case,), tuple(sorted(issues)), content_hash(body))


def _contract_states_match(catalog: StructuralBetaFixtureCatalog, evaluation: Any, contracts: Any) -> bool:
...
def _deterministic(catalog: StructuralBetaFixtureCatalog) -> bool:
    first = evaluate_structural_beta_fixture(catalog)
    second = evaluate_structural_beta_fixture(catalog)
...
def _sanitized(evaluation: Any) -> bool:
    ...
    return "raw_record" not in payload and "patient_id" not in payload and "subject_id" not in payload
...
def _check(check_id: str, passed: bool, expected: Any, observed: Any, detail: str) -> StructuralBetaQualityCheck:
    return StructuralBetaQualityCheck(check_id, passed, expected, observed, detail)
