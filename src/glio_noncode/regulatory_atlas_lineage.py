"""Sanitized source-to-result lineage for Domain 05 C01–C04."""
...
class RegulatoryAtlasNodeKind(StrEnum):
    """Node classes retained in the public lineage graph."""
...
class RegulatoryAtlasEdgeKind(StrEnum):
...
class RegulatoryAtlasLineageNode:
    """One stable node with bounded, non-payload attributes."""

    node_id: str
    kind: RegulatoryAtlasNodeKind
    label: str
    content_address: str
...
@dataclass(frozen=True, slots=True)
class RegulatoryAtlasLineageEdge:
    """One directed edge whose endpoints must exist in the graph."""

    edge_id: str
    source_node_id: str
    target_node_id: str
    kind: RegulatoryAtlasEdgeKind
    content_address: str
...
@dataclass(frozen=True, slots=True)
class RegulatoryAtlasLineageAudit:
...
@dataclass(frozen=True, slots=True)
class RegulatoryAtlasLineageGraph:
    """Deterministic source, record, receipt, and check graph."""

    fixture_id: str
    nodes: tuple[RegulatoryAtlasLineageNode, ...]
    edges: tuple[RegulatoryAtlasLineageEdge, ...]
    content_address: str

    def audit(self, evaluation: RegulatoryAtlasEvaluationReport) -> RegulatoryAtlasLineageAudit:
        ...
        checks: list[tuple[str, bool, str]] = [
            ...
            (
                "source-coverage",
                sum(node.kind is RegulatoryAtlasNodeKind.SOURCE for node in self.nodes) == 5,
                "five public source nodes are retained",
            ),
            (
                "record-coverage",
                sum(node.kind is RegulatoryAtlasNodeKind.RECORD for node in self.nodes)
                == len(evaluation.receipts),
                "one record node exists per receipt",
            ),
            (
                "receipt-coverage",
                sum(node.kind is RegulatoryAtlasNodeKind.RECEIPT for node in self.nodes)
                == len(evaluation.receipts),
                "one receipt node exists per record",
            ),
            (
                "check-coverage",
                sum(node.kind is RegulatoryAtlasNodeKind.CHECK for node in self.nodes)
            ...
            (
                ...
                "graph and evaluation share identity",
            ),
            (
                "address",
            ...
            (
                ...
                all(
                    ...
                    for node in self.nodes
                    if node.kind is RegulatoryAtlasNodeKind.RECEIPT
        ...
        return RegulatoryAtlasLineageAudit(
...
def _node(
    node_id: str, kind: RegulatoryAtlasNodeKind, label: str, attributes: dict[str, Any]
) -> RegulatoryAtlasLineageNode:
    body = {"node_id": node_id, "kind": kind, "label": label, "attributes": attributes}
    return RegulatoryAtlasLineageNode(**body, content_address=_address(body))


def _edge(
    edge_id: str, source: str, target: str, kind: RegulatoryAtlasEdgeKind
) -> RegulatoryAtlasLineageEdge:
    body = {"edge_id": edge_id, "source_node_id": source, "target_node_id": target, "kind": kind}
    return RegulatoryAtlasLineageEdge(**body, content_address=_address(body))


def build_regulatory_atlas_lineage(
    ...
    *,
    ...
    nodes: list[RegulatoryAtlasLineageNode] = []
    edges: list[RegulatoryAtlasLineageEdge] = []
    source_map = fixture.source_map()
    for source in fixture.sources:
        nodes.append(
            _node(
                f"source:{source.source_id}",
                RegulatoryAtlasNodeKind.SOURCE,
                source.title,
                {
    ...
    for record in fixture.records:
        nodes.append(
            _node(
                ...
                RegulatoryAtlasNodeKind.RECORD,
                record.record_id,
                {
            ...
            if source_id in source_map:
                edges.append(
                    _edge(
                        ...
                        f"source:{source_id}",
                        f"record:{record.record_id}",
                        RegulatoryAtlasEdgeKind.DECLARES,
                    )
                )
    ...
    for receipt in evaluation.receipts:
        nodes.append(
            _node(
                ...
                RegulatoryAtlasNodeKind.RECEIPT,
                receipt.record_id,
                {
        ...
        edges.append(
            _edge(
                ...
                f"receipt:{receipt.record_id}",
                RegulatoryAtlasEdgeKind.EXECUTES,
            )
        )
    ...
    for check in evaluation.checks:
        ...
        edges.append(
            _edge(
                ...
                target,
                f"check:{check_id}",
                RegulatoryAtlasEdgeKind.VERIFIES,
            )
        )
    body = {"fixture_id": fixture.fixture_id, "nodes": nodes, "edges": edges}
    return RegulatoryAtlasLineageGraph(
