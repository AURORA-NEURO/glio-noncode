"""Expected-versus-observed state reconciliation for Domain 07."""
...
class ChromatinFrontierReconciliationItem:
    record_id: str
    expected_state: str
    observed_state: str
...
@dataclass(frozen=True, slots=True)
class ChromatinFrontierReconciliationReport:
    fixture_id: str
    items: tuple[ChromatinFrontierReconciliationItem, ...]
...
def reconcile_chromatin_frontier(
    ...
    items: list[ChromatinFrontierReconciliationItem] = []
    for receipt in evaluation.receipts:
        ...
        items.append(
            ChromatinFrontierReconciliationItem(**body, content_address=content_hash(body))
        )
    checks = (
    ...
    return ChromatinFrontierReconciliationReport(
        fixture.fixture_id,
        tuple(items),
        checks,
        content_hash(body),
