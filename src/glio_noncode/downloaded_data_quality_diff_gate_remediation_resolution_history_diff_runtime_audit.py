...
from . import (
    downloaded_data_quality_diff_gate_remediation_resolution_history_diff_runtime as runtime_model,
)
from .errors import ValidationError
from .serialization import canonical_json, content_hash

VERSION = "downloaded-data-quality-diff-gate-remediation-resolution-history-diff-runtime-audit-v1"
BOUNDARY = "public_downloaded_data_quality_diff_gate_remediation_resolution_history_diff_runtime_audit"
AUDIT_PREFIX = "glio-noncode-download-quality-diff-gate-remediation-resolution-history-diff-runtime-audit"
...
class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck:
    FIELDS = CHECK_FIELDS
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck:
...
def address_check(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck):
        raise ValidationError("history diff runtime audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit:
    FIELDS = AUDIT_FIELDS

    def __init__(self, runtime_address: str, checks: Sequence[DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck | Mapping[str, Any]], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.runtime_address = _address(runtime_address, "history diff runtime audit runtime address", runtime_model.RUNTIME_PREFIX)
        self.checks = tuple(item if isinstance(item, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck) else DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck.from_mapping(item) for item in _sequence(checks, "history diff runtime audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit:
        value = _mapping(value, "history diff runtime audit")
        _strict(value, set(cls.FIELDS), "history diff runtime audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit):
        raise ValidationError("history diff runtime audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck:
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck(**body)
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck(**(body | {"content_address": address_check(provisional)}))


def audit_runtime(value: runtime_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit:
    if not isinstance(value, runtime_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntime):
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit(**body)
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit(**(body | {"content_address": address_audit(provisional)}))


def audit_from_mapping(value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit:
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit.from_mapping(value)


def audit_json(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit) -> str:
...
def render_audit_markdown(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit) -> str:
    value = audit_from_mapping(value.to_dict())
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "VERSION", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAudit", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffRuntimeAuditCheck", "address_audit", "address_check", "audit_csv", "audit_from_mapping", "audit_json", "audit_runtime", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
