"""Detailed change impact plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseChangeImpact:
    impact_count: int
    accepted: bool
...
def build_evidence_release_change_impact(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseChangeImpact:
    ...
    return EvidenceReleaseChangeImpact(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_change_impact(receipt: EvidenceReleaseChangeImpact, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "impact_count") != expected:
        return False
    return all(
...
def summarize_change_impact(receipt: EvidenceReleaseChangeImpact) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "impact_count"),
...
__all__ = ["EvidenceReleaseChangeImpact", "validate_evidence_release_change_impact", "build_evidence_release_change_impact", "summarize_change_impact"]
