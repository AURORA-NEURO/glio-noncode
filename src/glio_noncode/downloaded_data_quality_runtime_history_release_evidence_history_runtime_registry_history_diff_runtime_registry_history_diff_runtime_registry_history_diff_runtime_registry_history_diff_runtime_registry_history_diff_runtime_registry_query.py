...
def render_query_markdown(value: RegistryQuery) -> str:
    ...
    lines.extend(f"| {item.ordinal} | {item.resource} | {item.key} | {item.state} | {item.release_ready if item.release_ready is not None else '—'} | {item.value.replace('|', chr(92) + '|')} |" for item in value.rows); return "\n".join(lines) + "\n"
