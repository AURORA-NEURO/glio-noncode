"""Apply strict and release gates to handoff-catalog longitudinal diffs."""
...
from . import (
    downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_contracts as contract_model,
)
...
def _settings(maximum_added: int, maximum_removed: int, maximum_changed: int, maximum_total_changes: int, allowed_changes: Sequence[str], allowed_directions: Sequence[str], allowed_transitions: Sequence[str], allowed_left_postures: Sequence[str], allowed_right_postures: Sequence[str], required_changes: Sequence[str], require_ready: bool, require_change: bool, allow_unchanged: bool) -> dict[str, Any]:
    return {
        "maximum_added": _count(maximum_added, "maximum added handoff catalog entries", diff_model.MAX_ITEMS),
        "maximum_removed": _count(maximum_removed, "maximum removed handoff catalog entries", diff_model.MAX_ITEMS),
        "maximum_changed": _count(maximum_changed, "maximum changed handoff catalog entries", diff_model.MAX_ITEMS),
        "maximum_total_changes": _count(maximum_total_changes, "maximum total handoff catalog changes", diff_model.MAX_ITEMS),
        "allowed_changes": _ordered_labels(allowed_changes, "allowed handoff catalog entry changes", diff_model.CHANGES),
        "allowed_directions": _ordered_labels(allowed_directions, "allowed handoff catalog diff directions", diff_model.DIRECTIONS),
        "allowed_transitions": _ordered_labels(allowed_transitions, "allowed handoff catalog posture transitions", diff_model.STATE_TRANSITIONS),
        "allowed_left_postures": _ordered_labels(allowed_left_postures, "allowed left handoff catalog postures", diff_model.POSTURES),
        "allowed_right_postures": _ordered_labels(allowed_right_postures, "allowed right handoff catalog postures", diff_model.POSTURES),
        "required_changes": _ordered_labels(required_changes, "required handoff catalog entry changes", diff_model.CHANGES, allow_empty=True),
...
def _checks(diff: Any, settings: Mapping[str, Any]) -> tuple[contract_model.PolicyCheck, ...]:
    ...
    return (
        _check("policy-diff-address", diff_model.address_diff(diff) == diff.content_address, diff.content_address, "replayable", "handoff catalog diff address is valid"),
        _check("policy-diff-replay", diff_model.verify_diff(diff).content_address == diff.content_address, "replayed", "replayed", "typed handoff catalog diff reload is valid"),
        _check("policy-added-budget", diff.added_count <= settings["maximum_added"], diff.added_count, settings["maximum_added"], "added handoff catalog entries remain within budget"),
        _check("policy-removed-budget", diff.removed_count <= settings["maximum_removed"], diff.removed_count, settings["maximum_removed"], "removed handoff catalog entries remain within budget"),
        _check("policy-changed-budget", diff.changed_count <= settings["maximum_changed"], diff.changed_count, settings["maximum_changed"], "changed handoff catalog entries remain within budget"),
        _check("policy-total-change-budget", active <= settings["maximum_total_changes"], active, settings["maximum_total_changes"], "active handoff catalog changes remain within the total budget"),
        _check("policy-change-allowlist", all(item.change in settings["allowed_changes"] for item in diff.items), observed_changes, settings["allowed_changes"], "all handoff catalog entry changes are allowed"),
        _check("policy-direction-allowlist", diff.direction in settings["allowed_directions"], diff.direction, settings["allowed_directions"], "handoff catalog diff direction is allowed"),
        _check("policy-transition-allowlist", diff.state_transition in settings["allowed_transitions"], diff.state_transition, settings["allowed_transitions"], "handoff catalog posture transition is allowed"),
        _check("policy-left-posture-allowlist", diff.left_posture in settings["allowed_left_postures"], diff.left_posture, settings["allowed_left_postures"], "left handoff catalog posture is allowed"),
        _check("policy-right-posture-allowlist", diff.right_posture in settings["allowed_right_postures"], diff.right_posture, settings["allowed_right_postures"], "right handoff catalog posture is allowed"),
        _check("policy-ready-requirement", not settings["require_ready"] or diff.right_posture == "ready", diff.right_posture, "ready" if settings["require_ready"] else "not required", "right handoff catalog satisfies the release-ready requirement"),
        _check("policy-change-requirement", not settings["require_change"] or active > 0, active, "greater than zero" if settings["require_change"] else "not required", "required handoff catalog change posture is satisfied"),
        _check("policy-required-changes", set(settings["required_changes"]).issubset(set(observed_changes)), observed_changes, settings["required_changes"], "required handoff catalog change kinds are present"),
        _check("policy-unchanged-control", settings["allow_unchanged"] or diff.unchanged_count == 0, diff.unchanged_count, "allowed" if settings["allow_unchanged"] else 0, "unchanged handoff catalog evidence is explicitly controlled"),
    )
...
def verify_policy(value: Any) -> contract_model.Policy:
    ...
    if contract_model.address_policy(policy) != policy.content_address or _has_forbidden_key(policy.to_dict()):
        raise ValidationError("handoff catalog diff policy address or public boundary does not replay")
...
def load_policy(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> contract_model.Policy:
    if isinstance(value, Mapping):
        ...
        raw = _strict_json_loads(read_text(value, field="handoff catalog diff policy", max_bytes=16 * 1024 * 1024))
...
def write_policy(value: Any, destination: str | Path, *, allow_existing: bool = False) -> contract_model.Policy:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("handoff catalog diff policy destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("handoff catalog diff policy destination is not a file")
    _validate_parent(path.parent, "handoff catalog diff policy destination")
    atomic_write_text(path, policy_json(policy), field="handoff catalog diff policy destination")
    return policy
...
def query_policy(value: Any, *, passed: bool | None = None, check_id: str = "", text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if passed is not None and not isinstance(passed, bool):
        raise ValidationError("handoff catalog diff policy passed filter must be boolean")
    check_id = _text(check_id, "handoff catalog diff policy check filter", 256, required=False)
    if check_id and check_id not in POLICY_CHECK_IDS:
        raise ValidationError("handoff catalog diff policy check filter is unsupported")
    text = _text(text, "handoff catalog diff policy text filter", required=False)
    offset = _count(offset, "handoff catalog diff policy query offset", len(policy.checks))
    limit = _count(limit, "handoff catalog diff policy query limit", MAX_LIMIT, positive=True)
...
def render_policy_markdown(value: Any) -> str:
    policy = verify_policy(value)
    lines = ["# Handoff Catalog Diff Policy", "", f"- Policy: `{policy.policy_id}`", f"- State: **{policy.state}**", f"- Passed / failed: **{policy.passed_count} / {policy.failed_count}**", f"- Diff: `{policy.diff_address}`", f"- Address: `{policy.content_address}`", "", "| check | passed | detail |", "| --- | --- | --- |"]
