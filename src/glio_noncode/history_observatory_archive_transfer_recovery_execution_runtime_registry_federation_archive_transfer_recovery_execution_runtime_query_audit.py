...
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_query as query_model
...
class RecoveryExecutionRuntimeQueryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeQueryAuditCheck:
        value = _mapping(value, "runtime query audit check")
        _strict(value, set(cls.FIELDS), "runtime query audit check")
        return cls(*(value[field] for field in cls.FIELDS))


class RecoveryExecutionRuntimeQueryAudit:
    """A fixed-size audit of one runtime query result."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, query_address: str, query_id: str, version: str, boundary: str, checks: Sequence[RecoveryExecutionRuntimeQueryAuditCheck], check_count: int, passed: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, RecoveryExecutionRuntimeQueryAuditCheck) else RecoveryExecutionRuntimeQueryAuditCheck.from_mapping(item) for item in _sequence(checks, "runtime query audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeQueryAudit:
        ...
        return cls(value["query_address"], value["query_id"], value["version"], value["boundary"], tuple(RecoveryExecutionRuntimeQueryAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "runtime query audit checks", MAX_CHECKS)), value["check_count"], value["passed"], value["content_address"])


def address_check(value: RecoveryExecutionRuntimeQueryAuditCheck) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeQueryAuditCheck):
        raise ValidationError("runtime query audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: RecoveryExecutionRuntimeQueryAudit) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeQueryAudit):
        raise ValidationError("runtime query audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, observed: Any, expected: Any) -> RecoveryExecutionRuntimeQueryAuditCheck:
    provisional = RecoveryExecutionRuntimeQueryAuditCheck(check_id, observed == expected, observed, expected, "pending:runtime-query-audit-check")
    return RecoveryExecutionRuntimeQueryAuditCheck(check_id, provisional.passed, observed, expected, address_check(provisional))


def audit_query(value: query_model.RecoveryExecutionRuntimeQuery, runtime: runtime_model.RecoveryExecutionRuntime) -> RecoveryExecutionRuntimeQueryAudit:
    if not isinstance(value, query_model.RecoveryExecutionRuntimeQuery) or not isinstance(runtime, runtime_model.RecoveryExecutionRuntime):
    ...
    provisional = RecoveryExecutionRuntimeQueryAudit(value.content_address, value.query_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), "pending:runtime-query-audit")
    return RecoveryExecutionRuntimeQueryAudit(value.content_address, value.query_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeQueryAudit:
    return RecoveryExecutionRuntimeQueryAudit.from_mapping(value)


def audit_json(value: RecoveryExecutionRuntimeQueryAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: RecoveryExecutionRuntimeQueryAudit) -> str:
...
def render_audit_markdown(value: RecoveryExecutionRuntimeQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "RecoveryExecutionRuntimeQueryAudit", "RecoveryExecutionRuntimeQueryAuditCheck", "VERSION", "address_audit", "address_check", "audit_csv", "audit_from_mapping", "audit_json", "audit_query", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
