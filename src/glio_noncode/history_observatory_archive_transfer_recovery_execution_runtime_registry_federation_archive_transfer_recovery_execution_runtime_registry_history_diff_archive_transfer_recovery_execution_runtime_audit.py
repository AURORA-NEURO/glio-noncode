...
class HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck":
        value = _mapping(value, "runtime audit check")
        _strict(value, set(cls.FIELDS), "runtime audit check")
        return cls(*(value[field] for field in cls.FIELDS))


class HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit:
    """A fixed-size, value-free audit of a runtime handoff."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, runtime_address: str, runtime_id: str, version: str, boundary: str, checks: Sequence[HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck], check_count: int, passed: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck) else HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck.from_mapping(item) for item in _sequence(checks, "runtime audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit":
        value = _mapping(value, "recovery execution runtime audit")
        _strict(value, set(cls.FIELDS), "recovery execution runtime audit")
        return cls(value["runtime_address"], value["runtime_id"], value["version"], value["boundary"], tuple(HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "runtime audit checks", MAX_CHECKS)), value["check_count"], value["passed"], value["content_address"])


def address_check(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck):
        raise ValidationError("runtime audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit):
        raise ValidationError("runtime audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, observed: Any, expected: Any) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck:
    provisional = HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck(check_id, observed == expected, observed, expected, "pending:runtime-audit-check")
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck(check_id, provisional.passed, observed, expected, address_check(provisional))


def audit_runtime(value: runtime_model.HistoryDiffArchiveTransferRecoveryExecutionRuntime) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit:
    ...
    provisional = HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit(value.content_address, value.runtime_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), "pending:runtime-audit")
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit(value.content_address, value.runtime_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit:
    return HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit.from_mapping(value)


def audit_json(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit) -> str:
...
def render_audit_markdown(value: HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit) -> str:
    value = audit_from_mapping(value.to_dict())
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "HistoryDiffArchiveTransferRecoveryExecutionRuntimeAudit", "HistoryDiffArchiveTransferRecoveryExecutionRuntimeAuditCheck", "VERSION", "address_audit", "address_check", "audit_csv", "audit_from_mapping", "audit_json", "audit_runtime", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
