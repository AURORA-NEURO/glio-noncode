"""Typed contracts for source-free fixed policy transport catalog comparisons."""
...
class DiffItem:
    ...
    def __post_init__(self) -> None:
        _count(self.ordinal, "fixed policy transport catalog diff item ordinal", MAX_ITEMS)
        _label(self.entry_id, "fixed policy transport catalog diff item entry ID")
        if self.change not in CHANGES:
            raise ValidationError("fixed policy transport catalog diff item change is unsupported")
        if tuple(sorted(self.changed_fields)) != self.changed_fields or any(field not in SNAPSHOT_FIELDS for field in self.changed_fields):
            raise ValidationError("fixed policy transport catalog diff item changed fields are not canonical")
        _address(self.left_address, "fixed policy transport catalog diff item left address", required=False)
        _address(self.right_address, "fixed policy transport catalog diff item right address", required=False)
        left = _mapping(self.left_snapshot, "fixed policy transport catalog diff item left snapshot")
        right = _mapping(self.right_snapshot, "fixed policy transport catalog diff item right snapshot")
        if set(left) - set(SNAPSHOT_FIELDS) or set(right) - set(SNAPSHOT_FIELDS):
            raise ValidationError("fixed policy transport catalog diff item snapshot fields are unsupported")
        _address(self.content_address, "fixed policy transport catalog diff item content address", ITEM_PREFIX)
        if not self.content_address.endswith(":pending") and address_item(self) != self.content_address:
            raise ValidationError("fixed policy transport catalog diff item address does not replay")
...
def address_item(value: DiffItem) -> str:
    if not isinstance(value, DiffItem):
        raise ValidationError("fixed policy transport catalog diff item addressing requires its typed contract")
...
class Diff:
    ...
    def __post_init__(self) -> None:
        _label(self.diff_id, "fixed policy transport catalog diff ID")
        if self.version != VERSION or self.boundary != BOUNDARY:
            raise ValidationError("fixed policy transport catalog diff identity is not current")
        _label(self.catalog_id, "fixed policy transport catalog diff catalog ID")
        _address(self.left_catalog_address, "fixed policy transport catalog diff left catalog address", CATALOG_PREFIX)
        _address(self.right_catalog_address, "fixed policy transport catalog diff right catalog address", CATALOG_PREFIX)
        if self.left_posture not in POSTURES or self.right_posture not in POSTURES or self.state_transition not in STATE_TRANSITIONS or self.direction not in DIRECTIONS:
            raise ValidationError("fixed policy transport catalog diff posture, transition, or direction is unsupported")
        for field in ("added_count", "removed_count", "changed_count", "unchanged_count"):
            _count(getattr(self, field), f"fixed policy transport catalog diff {field}", MAX_ITEMS)
        if len(self.items) > MAX_ITEMS or sum((self.added_count, self.removed_count, self.changed_count, self.unchanged_count)) != len(self.items):
            raise ValidationError("fixed policy transport catalog diff item counts do not replay")
        if tuple(item.ordinal for item in self.items) != tuple(range(1, len(self.items) + 1)) or tuple(item.entry_id for item in self.items) != tuple(sorted(item.entry_id for item in self.items)):
            raise ValidationError("fixed policy transport catalog diff item order is not canonical")
        if len({item.entry_id for item in self.items}) != len(self.items):
            raise ValidationError("fixed policy transport catalog diff item IDs are not unique")
        _address(self.content_address, "fixed policy transport catalog diff address", DIFF_PREFIX)
        if not self.content_address.endswith(":pending") and address_diff(self) != self.content_address:
            raise ValidationError("fixed policy transport catalog diff address does not replay")
...
def address_diff(value: Diff) -> str:
    if not isinstance(value, Diff):
        raise ValidationError("fixed policy transport catalog diff addressing requires its typed contract")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=DIFF_PREFIX)


def item_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded-data fixed policy transport catalog diff item", "type": "object", "additionalProperties": False, "required": list(ITEM_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1}, "entry_id": {"type": "string"}, "change": {"enum": list(CHANGES)}, "changed_fields": {"type": "array", "items": {"enum": list(SNAPSHOT_FIELDS)}}, "left_address": {"type": "string"}, "right_address": {"type": "string"}, "left_snapshot": {"type": "object"}, "right_snapshot": {"type": "object"}, "content_address": {"type": "string"}}}


def diff_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded-data fixed policy transport catalog diff", "type": "object", "additionalProperties": False, "required": list(DIFF_FIELDS), "properties": {"diff_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "catalog_id": {"type": "string"}, "left_catalog_address": {"type": "string"}, "right_catalog_address": {"type": "string"}, "left_posture": {"enum": list(POSTURES)}, "right_posture": {"enum": list(POSTURES)}, "state_transition": {"enum": list(STATE_TRANSITIONS)}, "direction": {"enum": list(DIRECTIONS)}, "added_count": {"type": "integer", "minimum": 0}, "removed_count": {"type": "integer", "minimum": 0}, "changed_count": {"type": "integer", "minimum": 0}, "unchanged_count": {"type": "integer", "minimum": 0}, "items": {"type": "array", "items": item_schema(), "maxItems": MAX_ITEMS}, "content_address": {"type": "string"}}}
