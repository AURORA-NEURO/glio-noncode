...
from . import history_observatory_archive_transfer_recovery_execution_query as query_model
...
class RecoveryExecutionQueryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionQueryAuditCheck":
...
class RecoveryExecutionQueryAudit:
    """Canonical audit receipt for a recovery-execution query."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, query_address: str, query_id: str, version: str, boundary: str, checks: Sequence[RecoveryExecutionQueryAuditCheck], check_count: int, passed: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, RecoveryExecutionQueryAuditCheck) else RecoveryExecutionQueryAuditCheck.from_mapping(item) for item in _sequence(checks, "execution query audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionQueryAudit":
        value = _mapping(value, "execution query audit")
        _strict(value, set(cls.FIELDS), "execution query audit")
        return cls(value["query_address"], value["query_id"], value["version"], value["boundary"], tuple(RecoveryExecutionQueryAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "execution query audit checks", MAX_CHECKS)), value["check_count"], value["passed"], value["content_address"])


def address_check(value: RecoveryExecutionQueryAuditCheck) -> str:
    if not isinstance(value, RecoveryExecutionQueryAuditCheck):
        raise ValidationError("execution query audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: RecoveryExecutionQueryAudit) -> str:
    if not isinstance(value, RecoveryExecutionQueryAudit):
        raise ValidationError("execution query audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, observed: Any, expected: Any) -> RecoveryExecutionQueryAuditCheck:
    pending = RecoveryExecutionQueryAuditCheck(check_id, observed == expected, observed, expected, "pending:execution-query-audit-check")
    return RecoveryExecutionQueryAuditCheck(check_id, pending.passed, observed, expected, address_check(pending))


def audit_query(value: query_model.RecoveryExecutionQuery, execution: execution_model.RecoveryExecution) -> RecoveryExecutionQueryAudit:
    if not isinstance(value, query_model.RecoveryExecutionQuery) or not isinstance(execution, execution_model.RecoveryExecution):
    ...
    provisional = RecoveryExecutionQueryAudit(value.content_address, value.query_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), "pending:execution-query-audit")
    return RecoveryExecutionQueryAudit(value.content_address, value.query_id, VERSION, BOUNDARY, checks, len(checks), provisional.passed, address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionQueryAudit:
    return RecoveryExecutionQueryAudit.from_mapping(value)


def audit_json(value: RecoveryExecutionQueryAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: RecoveryExecutionQueryAudit) -> str:
...
def render_audit_markdown(value: RecoveryExecutionQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# History observatory archive transfer recovery execution query audit", "", f"- Query: `{value.query_id}`", f"- Result: `{'passed' if value.passed else 'failed'}`", f"- Checks: `{value.check_count}`", f"- Address: `{value.content_address}`", "", "| check | passed | observed | expected |", "| --- | --- | --- | --- |"]
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "RecoveryExecutionQueryAudit", "RecoveryExecutionQueryAuditCheck", "VERSION", "address_audit", "address_check", "audit_csv", "audit_from_mapping", "audit_json", "audit_query", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
