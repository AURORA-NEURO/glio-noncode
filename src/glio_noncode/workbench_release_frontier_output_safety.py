"""Deep output safety receipt for workbench release review.
...
class WorkbenchReleaseOutputSafety:
...
def build_workbench_release_output_safety(values: Iterable[Any] = (), *, require_non_empty: bool = True) -> WorkbenchReleaseOutputSafety:
    rows = tuple(
        {
            "sequence": index,
    ...
    return WorkbenchReleaseOutputSafety(**body, content_address=content_hash(body))


def validate_workbench_release_output_safety(receipt: WorkbenchReleaseOutputSafety) -> bool:
...
__all__ = ["WorkbenchReleaseOutputSafety", "build_workbench_release_output_safety", "validate_workbench_release_output_safety"]
