"""Bounded evidence queries for federation runtime-registry history diffs."""
...
class RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow:
...
def address_row(value: RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow):
...
def _row(resource: str, key: str, value: Any, address: str, change: str, left_address: str, right_address: str, ordinal: int) -> RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(**body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(**(body | {"row_address": address_row(provisional)}))


def _all_rows(value: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff) -> tuple[RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow, ...]:
    value = diff_model.verify_diff(value)
    rows: list[RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow] = []
...
class RecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    ...
    def __init__(self, query_id: str, diff_id: str, diff_address: str, resources: Sequence[str], change_filter: str, key_filter: str, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.rows = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow) else RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow.from_mapping(item) for item in _sequence(rows, "runtime registry history diff query rows", MAX_LIMIT))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQuery:
        value = _mapping(value, "runtime registry history diff query")
        _strict(value, set(cls.FIELDS), "runtime registry history diff query")
        return cls(*(value[field] for field in cls.FIELDS))


def address_query(value: RecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffQuery):
        raise ValidationError("runtime registry history diff query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def query_history_diff(value: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff, *, resources: Sequence[str] | None = None, change: str = "", key: str = "", text: str = "", offset: int = 0, limit: int = MAX_LIMIT, query_id: str = DEFAULT_QUERY_ID) -> RecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    value = diff_model.verify_diff(value)
    ...
    projected = tuple(RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(item.ordinal, item.resource, item.key, item.value, item.address, item.change, item.left_address, item.right_address, item.row_address) for item in page)
    projected = tuple(RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(index, item.resource, item.key, item.value, item.address, item.change, item.left_address, item.right_address, ROW_PREFIX + ":pending") for index, item in enumerate(projected, 1))
    final_rows: list[RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow] = []
    for item in projected:
        body = item.to_dict()
        final_rows.append(RecoveryExecutionRuntimeRegistryHistoryDiffQueryRow(**(body | {"row_address": address_row(item)})))
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffQuery(**body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffQuery(**(body | {"content_address": address_query(provisional)}))


def verify_query(value: RecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> RecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffQuery):
        raise ValidationError("runtime registry history diff query verification requires a typed query")
    return RecoveryExecutionRuntimeRegistryHistoryDiffQuery.from_mapping(value.to_dict())


def query_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffQuery:
    return RecoveryExecutionRuntimeRegistryHistoryDiffQuery.from_mapping(value)


def query_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
    return canonical_json(verify_query(value).to_dict())


def query_csv(value: RecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
...
def render_query_markdown(value: RecoveryExecutionRuntimeRegistryHistoryDiffQuery) -> str:
    value = verify_query(value)
