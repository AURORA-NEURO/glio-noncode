"""Compact evidence bundle projections for Domain 02 C09-C12."""
...
from .structural_haplotype_lineage import build_structural_haplotype_lineage
...
class StructuralHaplotypeBundleFormat(StrEnum):
    """Supported compact C09-C12 projections."""
...
class StructuralHaplotypeBundleEntry:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.evidence_address.startswith("sha256:"):
            raise ValidationError("structural haplotype evidence address must be addressed")
...
@dataclass(frozen=True, slots=True)
class StructuralHaplotypeEvidenceBundle:
    """Quality-gated C09-C12 bundle with stable metadata."""
    ...
    entries: tuple[StructuralHaplotypeBundleEntry, ...]
    ...
    def render(self, output_format: StructuralHaplotypeBundleFormat | str) -> str:
        selected = StructuralHaplotypeBundleFormat(str(output_format))
        if selected == StructuralHaplotypeBundleFormat.JSON:
            return json.dumps(self.to_dict(), ensure_ascii=False, indent=2, sort_keys=True) + "\n"
        if selected == StructuralHaplotypeBundleFormat.CSV:
            buffer = io.StringIO()
            fields = (
                "entry_id", "entry_class", "capability_id", "operation", "state",
                "result_state", "structural_identifier", "source_id", "evidence_address", "summary",
            )
        ...
        for entry in self.entries:
            lines.append(
                "| " + " | ".join(
                    (entry.entry_id, entry.entry_class, entry.capability_id, entry.operation,
                     entry.state, entry.result_state, entry.structural_identifier, entry.evidence_address)
                ) + " |"
            )
...
class StructuralHaplotypeEvidenceBundleBuilder:
    """Build a compact bundle only after the C09-C12 quality gate passes."""

    _capability_by_operation = {
        "phased_haplotype": "GNC-D02-C09",
        "allele_aware_sv": "GNC-D02-C10",
        "pangenome_projection": "GNC-D02-C11",
        "repeat_mobile_annotation": "GNC-D02-C12",
    }
    ...
    def build(
        ...
        *,
        ...
        catalog = StructuralHaplotypeFixtureCatalog.from_file(path)
        quality = evaluate_structural_haplotype_quality_gate(catalog)
        if not quality.passed and not allow_review:
            raise ValidationError("structural haplotype bundle requires a passing quality gate")
        ...
        entries: list[StructuralHaplotypeBundleEntry] = []
        ...
        body = {
            ...
            "contract_manifest": default_structural_haplotype_contract_registry().manifest(),
            "quality_summary": quality_summary,
            "lineage_address": lineage.content_address,
            "state": quality.state,
        }
        return StructuralHaplotypeEvidenceBundle(
            bundle_id=selected_id,
            fixture_id=catalog.fixture_id,
            fixture_version=catalog.schema_version,
            context_key=catalog.context_key,
    ...
    def write(
        ...
        output: str | Path,
        *,
        output_format: StructuralHaplotypeBundleFormat | str | None = None,
        bundle_id: str | None = None,
        allow_review: bool = False,
    ) -> StructuralHaplotypeEvidenceBundle:
    ...
    @staticmethod
    def _format_for_path(output: Path, output_format: StructuralHaplotypeBundleFormat | str | None) -> StructuralHaplotypeBundleFormat:
        if output_format is not None:
            return StructuralHaplotypeBundleFormat(str(output_format))
        if output.suffix.casefold() == ".csv":
            return StructuralHaplotypeBundleFormat.CSV
        if output.suffix.casefold() in {".md", ".markdown"}:
            return StructuralHaplotypeBundleFormat.MARKDOWN
        return StructuralHaplotypeBundleFormat.JSON


def build_structural_haplotype_evidence_bundle(
    path: str | Path,
    *,
    bundle_id: str | None = None,
    allow_review: bool = False,
) -> StructuralHaplotypeEvidenceBundle:
    """Build a compact C09-C12 evidence bundle."""

    return StructuralHaplotypeEvidenceBundleBuilder().build(path, bundle_id=bundle_id, allow_review=allow_review)
...
__all__ = [
    ...
    "build_structural_haplotype_evidence_bundle",
]
