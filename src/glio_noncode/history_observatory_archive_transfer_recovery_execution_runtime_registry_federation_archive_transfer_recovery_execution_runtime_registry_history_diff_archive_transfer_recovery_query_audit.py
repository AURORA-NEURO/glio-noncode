"""Independent assurance for federated history-diff recovery queries."""
...
from . import history_observatory_archive_transfer_recovery_execution_runtime_registry_federation_archive_transfer_recovery_execution_runtime_registry_history_diff_archive_transfer_recovery_query as query_model
...
class HistoryDiffArchiveTransferRecoveryQueryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryQueryAuditCheck:
...
class HistoryDiffArchiveTransferRecoveryQueryAudit:
    """Independent audit receipt for a bounded recovery query."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, recovery_address: str, query_address: str, checks: Sequence[HistoryDiffArchiveTransferRecoveryQueryAuditCheck], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryQueryAuditCheck) else HistoryDiffArchiveTransferRecoveryQueryAuditCheck.from_mapping(item) for item in _sequence(checks, "query audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryQueryAudit:
        ...
        return cls(value["recovery_address"], value["query_address"], tuple(HistoryDiffArchiveTransferRecoveryQueryAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "query audit checks", MAX_CHECKS)), value["check_count"], value["passed_count"], value["failed_count"], value["accepted"], value["content_address"])


def address_check(value: HistoryDiffArchiveTransferRecoveryQueryAuditCheck) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryQueryAuditCheck):
        raise ValidationError("query audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: HistoryDiffArchiveTransferRecoveryQueryAudit) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryQueryAudit):
        raise ValidationError("query audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> HistoryDiffArchiveTransferRecoveryQueryAuditCheck:
    pending = HistoryDiffArchiveTransferRecoveryQueryAuditCheck(ordinal, check_id, bool(passed), detail, evidence, "pending:query-audit-check")
    return HistoryDiffArchiveTransferRecoveryQueryAuditCheck(ordinal, check_id, bool(passed), detail, evidence, address_check(pending))


def _row_key(row: query_model.HistoryDiffArchiveTransferRecoveryQueryRow) -> tuple[Any, ...]:
    return (row.resource, row.ordinal, row.chunk_index, row.content_address)


def _semantics(rows: Sequence[query_model.HistoryDiffArchiveTransferRecoveryQueryRow]) -> tuple[bool, ...]:
    values = []
    for row in rows:
...
def audit_query(query: query_model.HistoryDiffArchiveTransferRecoveryQuery, recovery: recovery_model.HistoryDiffArchiveTransferRecovery) -> HistoryDiffArchiveTransferRecoveryQueryAudit:
    """Recompute the requested page and receipt every visible query invariant."""
    if not isinstance(query, query_model.HistoryDiffArchiveTransferRecoveryQuery) or not isinstance(recovery, recovery_model.HistoryDiffArchiveTransferRecovery):
    ...
    provisional = HistoryDiffArchiveTransferRecoveryQueryAudit(**body, content_address="pending:query-audit")
    return HistoryDiffArchiveTransferRecoveryQueryAudit(**body, content_address=address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryQueryAudit:
    return HistoryDiffArchiveTransferRecoveryQueryAudit.from_mapping(value)


def verify_audit(value: HistoryDiffArchiveTransferRecoveryQueryAudit) -> HistoryDiffArchiveTransferRecoveryQueryAudit:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryQueryAudit):
...
def audit_json(value: HistoryDiffArchiveTransferRecoveryQueryAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: HistoryDiffArchiveTransferRecoveryQueryAudit) -> str:
...
def render_audit_markdown(value: HistoryDiffArchiveTransferRecoveryQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Federation runtime-registry history-diff archive transfer recovery query audit", "", f"- Recovery: `{value.recovery_address}`", f"- Query: `{value.query_address}`", f"- Checks: `{value.passed_count}/{value.check_count}`", f"- Accepted: `{str(value.accepted).lower()}`", f"- Address: `{value.content_address}`", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | `{item.check_id}` | `{str(item.passed).lower()}` | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer recovery query audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"type": "string", "enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 16}, "content_address": {"type": "string", "pattern": "^" + CHECK_PREFIX + ":"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer recovery query audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"recovery_address": {"type": "string", "pattern": "^" + recovery_model.RECOVERY_PREFIX + ":"}, "query_address": {"type": "string", "pattern": "^" + query_model.QUERY_PREFIX + ":"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "check_count": {"type": "integer", "const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "content_address": {"type": "string", "pattern": "^" + AUDIT_PREFIX + ":"}}}
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "HistoryDiffArchiveTransferRecoveryQueryAudit", "HistoryDiffArchiveTransferRecoveryQueryAuditCheck", "MAX_CHECKS", "VERSION", "address_audit", "address_check", "audit_csv", "audit_from_mapping", "audit_json", "audit_query", "audit_schema", "capabilities", "check_schema", "render_audit_markdown", "verify_audit"]
