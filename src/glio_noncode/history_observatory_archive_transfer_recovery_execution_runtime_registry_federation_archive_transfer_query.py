"""Bounded inspection queries for federation-archive transfers."""
...
class TransferQuery:
    ...
    def _validate(self) -> None:
        if self.transfer_id == "" or any(isinstance(index, bool) or not isinstance(index, int) or index < 0 or index >= transfer_model.MAX_CHUNKS for index in self.received_indices) or tuple(sorted(set(self.received_indices))) != self.received_indices:
...
def query_transfer(value: transfer_model.ArchiveTransfer, *, resources: Sequence[str] | None = None, received_indices: Sequence[int] | None = None, index: int | None = None, chunk_offset: int | None = None, size: int | None = None, chunk_address: str = "", received: bool | None = None, text: str = "", offset: int = 0, limit: int = MAX_LIMIT) -> TransferQuery:
    ...
    normalized = tuple(TransferQueryRow(row.resource, ordinal + 1, row.transfer_id, row.transfer_address, row.archive_address, row.archive_size, row.chunk_size, row.chunk_count, row.chunk_index, row.chunk_offset, row.chunk_size_value, row.chunk_address, row.received, row.missing, row.received_bytes, row.complete, "pending:row") for ordinal, row in enumerate(page))
    rows = tuple(TransferQueryRow(row.resource, row.ordinal, row.transfer_id, row.transfer_address, row.archive_address, row.archive_size, row.chunk_size, row.chunk_count, row.chunk_index, row.chunk_offset, row.chunk_size_value, row.chunk_address, row.received, row.missing, row.received_bytes, row.complete, address_row(row)) for row in normalized)
    total_count = len(all_rows)
    matched_count = len(filtered)
    provisional = TransferQuery(value.content_address, value.transfer_id, resources, index, chunk_offset, size, chunk_address, received, text, indices, received_bytes, offset, limit, total_count, matched_count, len(rows), offset + len(rows) < matched_count, rows, "pending:query")
    return TransferQuery(provisional.transfer_address, provisional.transfer_id, provisional.resources, provisional.index_filter, provisional.offset_filter, provisional.size_filter, provisional.chunk_address_filter, provisional.received_filter, provisional.text_filter, provisional.received_indices, provisional.received_bytes, provisional.offset, provisional.limit, provisional.total_count, provisional.matched_count, provisional.returned_count, provisional.truncated, provisional.rows, address_query(provisional))


def query_assembler(assembler: transfer_model.TransferAssembler, **kwargs: Any) -> TransferQuery:
    if not isinstance(assembler, transfer_model.TransferAssembler):
        raise ValidationError("assembler query requires a typed transfer assembler")
...
def query_from_mapping(value: Mapping[str, Any]) -> TransferQuery:
    ...
    _strict(value, set(QUERY_FIELDS), "transfer query")
    return TransferQuery(value["transfer_address"], value["transfer_id"], value["resources"], value["index_filter"], value["offset_filter"], value["size_filter"], value["chunk_address_filter"], value["received_filter"], value["text_filter"], value["received_indices"], value["received_bytes"], value["offset"], value["limit"], value["total_count"], value["matched_count"], value["returned_count"], value["truncated"], tuple(TransferQueryRow.from_mapping(row) for row in _sequence(value["rows"], "query rows", MAX_LIMIT)), value["content_address"])
...
def render_query_markdown(value: TransferQuery) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# Runtime Registry Federation Archive Transfer Query", "", f"- Transfer: `{value.transfer_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.returned_count}/{value.matched_count}`", f"- Address: `{value.content_address}`", "", "| # | resource | chunk | received | missing | address |", "| ---: | --- | ---: | --- | --- | --- |"]
...
__all__ = ["BOUNDARY", "MAX_LIMIT", "MAX_QUERY_ITEMS", "QUERY_FIELDS", "QUERY_PREFIX", "RESOURCES", "ROW_FIELDS", "ROW_PREFIX", "TransferQuery", "TransferQueryRow", "VERSION", "address_query", "address_row", "capabilities", "query_assembler", "query_csv", "query_from_mapping", "query_json", "query_schema", "query_transfer", "render_query_markdown", "row_schema"]
