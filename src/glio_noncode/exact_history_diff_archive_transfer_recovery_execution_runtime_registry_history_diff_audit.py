"""Independent assurance receipt for exact runtime-registry history diffs."""
...
class ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck:
...
def address_check(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck):
        raise ValidationError("runtime registry history diff audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    """A fixed-size independently recomputed history-diff audit."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, diff_address: str, diff_id: str, version: str, boundary: str, check_count: int, passed_count: int, failed_count: int, accepted: bool, checks: Sequence[ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck) else ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck.from_mapping(item) for item in _sequence(checks, "runtime registry history diff audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit:
        value = _mapping(value, "runtime registry history diff audit")
        _strict(value, set(cls.FIELDS), "runtime registry history diff audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit):
...
def _expected_direction(value: diff_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff) -> str:
    left = value.to_dict()
    right = value.to_dict()
...
def _check(check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck:
    ...
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck(**body)
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck(**(body | {"content_address": address_check(provisional)}))


def audit_diff(value: diff_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    ...
    provisional = ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit(**audit_body)
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit(**(audit_body | {"content_address": address_audit(provisional)}))


def verify_audit(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit):
        raise ValidationError("runtime registry history diff audit verification requires a typed audit")
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit.from_mapping(value.to_dict())


def audit_from_mapping(value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    return ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit.from_mapping(value)


def audit_json(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
    return canonical_json(verify_audit(value).to_dict())


def audit_csv(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
...
def render_audit_markdown(value: ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
    value = verify_audit(value)
    lines = ["# Federation runtime-registry history diff audit", "", f"- Diff: {value.diff_id}", f"- Checks: {value.passed_count}/{value.check_count}", f"- Accepted: {value.accepted}", f"- Address: {value.content_address}", "", "| # | check | passed | detail |", "| ---: | --- | :---: | --- |"]
    lines.extend(f"| {check.ordinal} | {check.check_id} | {check.passed} | {check.detail} |" for check in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "FederationRuntimeRegistryHistoryDiffAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1}, "check_id": {"enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}}, "content_address": {"type": "string"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "FederationRuntimeRegistryHistoryDiffAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"diff_address": {"type": "string"}, "diff_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "check_count": {"type": "integer", "minimum": 0}, "passed_count": {"type": "integer", "minimum": 0}, "failed_count": {"type": "integer", "minimum": 0}, "accepted": {"type": "boolean"}, "checks": {"type": "array", "items": check_schema(), "maxItems": MAX_CHECKS}, "content_address": {"type": "string"}}}
