"""Public-boundary compliance checks for D15 payload projections."""
...
class WorkbenchArchitectureComplianceReport:
    fixture_id: str
...
def assess_workbench_architecture_compliance(
    fixture: WorkbenchArchitectureFixture | None = None,
) -> WorkbenchArchitectureComplianceReport:
    selected = fixture or default_workbench_architecture_fixture()
    hits = [
        hit for case in selected.cases for hit in _walk_keys(case.payload, f"cases.{case.case_id}")
    ]
    checks = (
        {
    ...
    return WorkbenchArchitectureComplianceReport(
        selected.fixture_id,
        checks,
        all(item["passed"] for item in checks),
        forbidden_keys,
        addressed(body, "workbench-architecture-compliance"),
    )


def workbench_architecture_compliance_summary(
    report: WorkbenchArchitectureComplianceReport,
) -> dict[str, object]:
    return {
...
__all__ = [
    "WorkbenchArchitectureComplianceReport",
    "assess_workbench_architecture_compliance",
    "workbench_architecture_compliance_summary",
]
