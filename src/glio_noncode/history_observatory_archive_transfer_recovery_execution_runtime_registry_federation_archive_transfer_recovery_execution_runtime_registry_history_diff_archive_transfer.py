"""Resumable addressed transport for federation runtime-registry history-diff archives."""
...
CHUNK_SUFFIX = ".bin"
DEFAULT_TRANSFER_ID = "runtime-registry-history-diff-archive-transfer"
...
class HistoryDiffArchiveTransferChunk:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferChunk:
...
class HistoryDiffArchiveTransfer:
    ...
    def __init__(self, transfer_id: str, version: str, boundary: str, archive_address: str, archive_size: int, chunk_size: int, chunk_count: int, chunks: Sequence[HistoryDiffArchiveTransferChunk], content_address: str, payload: Mapping[int, bytes] | None = None) -> None:
        ...
        self.chunks = tuple(item if isinstance(item, HistoryDiffArchiveTransferChunk) else HistoryDiffArchiveTransferChunk.from_mapping(item) for item in _sequence(chunks, "transfer chunks", MAX_CHUNKS))
...
class HistoryDiffArchiveTransferProgress:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferProgress:
...
def address_transfer(value: HistoryDiffArchiveTransfer) -> str:
    if not isinstance(value, HistoryDiffArchiveTransfer):
...
def address_progress(value: HistoryDiffArchiveTransferProgress) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferProgress):
...
def _progress_from_parts(value: HistoryDiffArchiveTransfer, parts: Mapping[int, bytes]) -> HistoryDiffArchiveTransferProgress:
    ...
    provisional = HistoryDiffArchiveTransferProgress(value.content_address, value.archive_address, value.archive_size, value.chunk_count, received, missing, received_bytes, not missing, "pending:progress")
    return HistoryDiffArchiveTransferProgress(value.content_address, value.archive_address, value.archive_size, value.chunk_count, received, missing, received_bytes, not missing, address_progress(provisional))


class HistoryDiffArchiveTransferAssembler:
    """Idempotent receiver accepting verified chunks in any arrival order."""

    def __init__(self, value: HistoryDiffArchiveTransfer) -> None:
        if not isinstance(value, HistoryDiffArchiveTransfer):
    ...
    def add_chunk(self, index: int, raw: bytes) -> HistoryDiffArchiveTransferProgress:
    ...
    def add_chunks(self, parts: Mapping[int, bytes]) -> HistoryDiffArchiveTransferProgress:
    ...
    def progress(self) -> HistoryDiffArchiveTransferProgress:
...
def _build_from_parts(raw: bytes, archive_value: archive_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchive, *, transfer_id: str | None, chunk_size: int) -> HistoryDiffArchiveTransfer:
    ...
    chunks = tuple(HistoryDiffArchiveTransferChunk(index, index * chunk_size, len(part), address_chunk(part)) for index, part in parts.items())
    ...
    provisional = HistoryDiffArchiveTransfer(**body, content_address="pending:transfer", payload=parts)
    return HistoryDiffArchiveTransfer(**body, content_address=address_transfer(provisional), payload=parts)


def build_transfer(value: archive_model.RecoveryExecutionRuntimeRegistryHistoryDiffArchive, *, transfer_id: str | None = None, chunk_size: int = DEFAULT_CHUNK_SIZE) -> HistoryDiffArchiveTransfer:
...
def build_transfer_from_bytes(raw: bytes, *, transfer_id: str | None = None, chunk_size: int = DEFAULT_CHUNK_SIZE) -> HistoryDiffArchiveTransfer:
...
def transfer_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransfer:
    value = _mapping(value, "history diff archive transfer")
    _strict(value, set(HistoryDiffArchiveTransfer.FIELDS), "history diff archive transfer")
    chunks = tuple(HistoryDiffArchiveTransferChunk.from_mapping(item) for item in _sequence(value["chunks"], "transfer chunks", MAX_CHUNKS))
    return HistoryDiffArchiveTransfer(value["transfer_id"], value["version"], value["boundary"], value["archive_address"], value["archive_size"], value["chunk_size"], value["chunk_count"], chunks, value["content_address"])


def _assemble_parts(value: HistoryDiffArchiveTransfer, parts: Mapping[int, bytes]) -> bytes:
...
def assemble_archive_bytes(value: HistoryDiffArchiveTransfer, parts: Mapping[int, bytes] | None = None) -> bytes:
    if not isinstance(value, HistoryDiffArchiveTransfer):
...
def verify_transfer(value: HistoryDiffArchiveTransfer) -> HistoryDiffArchiveTransfer:
    if not isinstance(value, HistoryDiffArchiveTransfer):
...
def _manifest(value: HistoryDiffArchiveTransfer) -> dict[str, Any]:
...
def manifest_document(value: HistoryDiffArchiveTransfer) -> dict[str, Any]:
    return _manifest(verify_transfer(value))


def manifest_json(value: HistoryDiffArchiveTransfer) -> str:
    return canonical_json(manifest_document(value))


def transfer_json(value: HistoryDiffArchiveTransfer) -> str:
    return canonical_json(transfer_from_mapping(value.to_dict()).to_dict())


def _expected_names(value: HistoryDiffArchiveTransfer, *, indices: Sequence[int] | None = None) -> set[str]:
...
def _validate_chunk_directory(directory: Path, value: HistoryDiffArchiveTransfer, *, require_complete: bool) -> tuple[int, ...]:
...
def _write_atomic_directory(destination: Path, value: HistoryDiffArchiveTransfer, *, parts: Mapping[int, bytes], overwrite: bool) -> Path:
    try:
...
def write_transfer(value: HistoryDiffArchiveTransfer, destination: str | Path, *, overwrite: bool = False) -> Path:
    verify_transfer(value)
    return _write_atomic_directory(Path(destination), value, parts=value.payload_bytes(), overwrite=overwrite)


def write_partial_transfer(assembler: HistoryDiffArchiveTransferAssembler, destination: str | Path, *, overwrite: bool = False) -> Path:
    if not isinstance(assembler, HistoryDiffArchiveTransferAssembler):
...
def _read_manifest(directory: Path) -> HistoryDiffArchiveTransfer:
    ...
    value = transfer_from_mapping({field: document[field] for field in HistoryDiffArchiveTransfer.FIELDS if field != "content_address"} | {"content_address": document["transfer_address"]})
...
def load_transfer(source: str | Path) -> HistoryDiffArchiveTransfer:
    ...
    loaded = HistoryDiffArchiveTransfer(value.transfer_id, value.version, value.boundary, value.archive_address, value.archive_size, value.chunk_size, value.chunk_count, value.chunks, value.content_address, payload=parts)
    return verify_transfer(loaded)


def load_partial_transfer(source: str | Path) -> HistoryDiffArchiveTransferAssembler:
    directory = Path(source)
    try:
        _validate_parent(directory.parent, "transfer input")
        if directory.is_symlink() or not directory.is_dir():
    ...
    assembler = HistoryDiffArchiveTransferAssembler(value)
    for index in indices:
        assembler.add_chunk(
            index,
...
def verify_transfer_directory(source: str | Path) -> HistoryDiffArchiveTransfer:
    return load_transfer(source)


def verify_partial_transfer(source: str | Path) -> HistoryDiffArchiveTransferProgress:
...
def transfer_csv(value: HistoryDiffArchiveTransfer) -> str:
    value = verify_transfer(value)
    stream = io.StringIO(newline="")
    writer = csv.DictWriter(stream, fieldnames=HistoryDiffArchiveTransferChunk.FIELDS, lineterminator="\n")
    writer.writeheader()
    for chunk in value.chunks:
...
def render_transfer_markdown(value: HistoryDiffArchiveTransfer) -> str:
    value = verify_transfer(value)
    lines = ["# Federation Runtime-Registry History-Diff Archive Transfer", "", f"- Transfer: `{value.transfer_id}`", f"- Archive address: `{value.archive_address}`", f"- Bytes: `{value.archive_size}`", f"- Chunks: `{value.chunk_count}` at `{value.chunk_size}` bytes", f"- Address: `{value.content_address}`", "", "| index | offset | bytes | chunk address |", "| ---: | ---: | ---: | --- |"]
    lines.extend(f"| {chunk.index} | {chunk.offset} | {chunk.size} | {chunk.content_address} |" for chunk in value.chunks)
    return "\n".join(lines) + "\n"


def progress_json(value: HistoryDiffArchiveTransferProgress) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferProgress):
        raise ValidationError("progress JSON requires a typed progress receipt")
    return canonical_json(value.to_dict())


def progress_csv(value: HistoryDiffArchiveTransferProgress) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferProgress):
        raise ValidationError("progress CSV requires a typed progress receipt")
    stream = io.StringIO(newline="")
    writer = csv.DictWriter(stream, fieldnames=HistoryDiffArchiveTransferProgress.FIELDS, lineterminator="\n")
    writer.writeheader()
    writer.writerow(value.to_dict())
    return stream.getvalue()


def render_progress_markdown(value: HistoryDiffArchiveTransferProgress) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferProgress):
        raise ValidationError("progress Markdown requires a typed progress receipt")
    status = "complete" if value.complete else "partial"
    return "\n".join(("# Federation Runtime-Registry History-Diff Archive Transfer Progress", "", f"- Transfer address: `{value.transfer_address}`", f"- Status: `{status}`", f"- Received: `{len(value.received_indices)}/{value.chunk_count}` chunks", f"- Received bytes: `{value.received_bytes}/{value.archive_size}`", f"- Missing indices: `{', '.join(str(index) for index in value.missing_indices) or 'none'}`", f"- Address: `{value.content_address}`", ""))


def chunk_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer chunk", "type": "object", "additionalProperties": False, "required": list(HistoryDiffArchiveTransferChunk.FIELDS), "properties": {"index": {"type": "integer", "minimum": 0, "maximum": MAX_CHUNKS - 1}, "offset": {"type": "integer", "minimum": 0, "maximum": MAX_TRANSFER_BYTES}, "size": {"type": "integer", "minimum": 1, "maximum": MAX_CHUNK_SIZE}, "content_address": {"type": "string", "pattern": "^" + CHUNK_PREFIX + ":"}}}


def transfer_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer", "type": "object", "additionalProperties": False, "required": list(HistoryDiffArchiveTransfer.FIELDS), "properties": {"transfer_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "archive_address": {"type": "string", "pattern": "^" + archive_model.ARCHIVE_PREFIX + ":"}, "archive_size": {"type": "integer", "minimum": 1, "maximum": MAX_TRANSFER_BYTES}, "chunk_size": {"type": "integer", "minimum": MIN_CHUNK_SIZE, "maximum": MAX_CHUNK_SIZE}, "chunk_count": {"type": "integer", "minimum": 1, "maximum": MAX_CHUNKS}, "chunks": {"type": "array", "items": chunk_schema(), "minItems": 1, "maxItems": MAX_CHUNKS}, "content_address": {"type": "string", "pattern": "^" + TRANSFER_PREFIX + ":"}}}
...
def manifest_schema() -> dict[str, Any]:
    ...
    required = [field for field in HistoryDiffArchiveTransfer.FIELDS if field != "content_address"] + ["transfer_address", "manifest_address"]
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer manifest", "type": "object", "additionalProperties": False, "required": required, "properties": properties}


def progress_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Federation runtime-registry history-diff archive transfer progress", "type": "object", "additionalProperties": False, "required": list(HistoryDiffArchiveTransferProgress.FIELDS), "properties": {"transfer_address": {"type": "string", "pattern": "^" + TRANSFER_PREFIX + ":"}, "archive_address": {"type": "string", "pattern": "^" + archive_model.ARCHIVE_PREFIX + ":"}, "archive_size": {"type": "integer", "minimum": 1}, "chunk_count": {"type": "integer", "minimum": 1}, "received_indices": {"type": "array", "items": {"type": "integer", "minimum": 0}}, "missing_indices": {"type": "array", "items": {"type": "integer", "minimum": 0}}, "received_bytes": {"type": "integer", "minimum": 0}, "complete": {"type": "boolean"}, "content_address": {"type": "string", "pattern": "^" + PROGRESS_PREFIX + ":"}}}
