"""Detailed evidence requirements plane for the D14 C13-C16 release boundary.
...
class EvidenceReleaseEvidenceRequirements:
...
def build_evidence_release_evidence_requirements(values: Iterable[Any] = (), *, required: int = 1) -> EvidenceReleaseEvidenceRequirements:
    ...
    return EvidenceReleaseEvidenceRequirements(
        **body,
        content_address=content_hash(body),
    )


def validate_evidence_release_evidence_requirements(receipt: EvidenceReleaseEvidenceRequirements, *, expected: int | None = None) -> bool:
    ...
    if not receipt.closed:
        return False
    if expected is not None and getattr(receipt, "requirement_count") != expected:
        return False
    return all(
...
def summarize_evidence_requirements(receipt: EvidenceReleaseEvidenceRequirements) -> Mapping[str, Any]:
    """Return a safe projection suitable for a release summary."""
    return {
        "count": getattr(receipt, "requirement_count"),
...
__all__ = ["EvidenceReleaseEvidenceRequirements", "validate_evidence_release_evidence_requirements", "build_evidence_release_evidence_requirements", "summarize_evidence_requirements"]
