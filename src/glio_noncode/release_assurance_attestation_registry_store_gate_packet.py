...
def _atomic_write(path: Path, payload: bytes) -> None:
    atomic_write_bytes(path, payload, field="gate packet artifact")
...
def write_release_assurance_attestation_registry_store_gate_packet(
    ...
    *,
    ...
    _validate_parent(root.parent, "gate packet destination")
    if root.exists() and root.is_symlink():
        raise ValidationError("gate packet destination must not be a symlink")
    if root.exists() and not root.is_dir():
        raise ValidationError("gate packet destination must be a directory")
    ...
    for artifact in packet.artifacts:
        ...
        _validate_parent(path.parent, "gate packet artifact")
...
def _read_manifest(
    ...
    try:
        value = _strict_json_loads(read_text(path, field="gate packet manifest"))
...
def verify_release_assurance_attestation_registry_store_gate_packet(
    ...
    for item in listed:
        ...
        if not target.is_file():
            ...
            continue
        try:
            payload = read_bytes(target, field=f"gate packet artifact {path}")
...
def load_release_assurance_attestation_registry_store_gate_packet(
    ...
    try:
        gate_payload = _strict_json_loads(
            read_text(path, field="gate packet gate payload")
        )
