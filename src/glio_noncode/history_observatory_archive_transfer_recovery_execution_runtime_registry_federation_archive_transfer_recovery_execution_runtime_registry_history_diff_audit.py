"""Independent assurance receipt for federation runtime-registry history diffs."""
...
class RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck:
...
def address_check(value: RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck):
        raise ValidationError("runtime registry history diff audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class RecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    """A fixed-size independently recomputed history-diff audit."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, diff_address: str, diff_id: str, version: str, boundary: str, check_count: int, passed_count: int, failed_count: int, accepted: bool, checks: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck) else RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck.from_mapping(item) for item in _sequence(checks, "runtime registry history diff audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffAudit:
        value = _mapping(value, "runtime registry history diff audit")
        _strict(value, set(cls.FIELDS), "runtime registry history diff audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: RecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffAudit):
...
def _expected_direction(value: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff) -> str:
    left = value.to_dict()
    right = value.to_dict()
...
def _check(check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck(**body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffAuditCheck(**(body | {"content_address": address_check(provisional)}))


def audit_diff(value: diff_model.RecoveryExecutionRuntimeRegistryHistoryDiff) -> RecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffAudit(**audit_body)
    return RecoveryExecutionRuntimeRegistryHistoryDiffAudit(**(audit_body | {"content_address": address_audit(provisional)}))


def verify_audit(value: RecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> RecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffAudit):
        raise ValidationError("runtime registry history diff audit verification requires a typed audit")
    return RecoveryExecutionRuntimeRegistryHistoryDiffAudit.from_mapping(value.to_dict())


def audit_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffAudit:
    return RecoveryExecutionRuntimeRegistryHistoryDiffAudit.from_mapping(value)


def audit_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
    return canonical_json(verify_audit(value).to_dict())


def audit_csv(value: RecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
...
def render_audit_markdown(value: RecoveryExecutionRuntimeRegistryHistoryDiffAudit) -> str:
    value = verify_audit(value)
