"""Operation-level metrics for Domain 07 evidence review."""
...
from .chromatin_frontier_public_data import ChromatinFrontierOperation, ChromatinFrontierRole
from .serialization import content_hash, jsonable


@dataclass(frozen=True, slots=True)
class ChromatinFrontierOperationMetric:
...
@dataclass(frozen=True, slots=True)
class ChromatinFrontierMetrics:
    fixture_id: str
    total_records: int
    ...
    operation_metrics: tuple[ChromatinFrontierOperationMetric, ...]
    content_address: str
...
def compute_chromatin_frontier_metrics(
    evaluation: ChromatinFrontierEvaluationReport,
) -> ChromatinFrontierMetrics:
    operation_metrics: list[ChromatinFrontierOperationMetric] = []
    for operation in ChromatinFrontierOperation:
        ...
        body = {
            ...
            "positive_count": sum(item.role is ChromatinFrontierRole.POSITIVE for item in receipts),
            "control_count": sum(item.role is ChromatinFrontierRole.CONTROL for item in receipts),
        ...
        operation_metrics.append(
            ChromatinFrontierOperationMetric(**body, content_address=content_hash(body))
        )
    body = {
    ...
    return ChromatinFrontierMetrics(**body, content_address=content_hash(body))
