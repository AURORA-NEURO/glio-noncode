"""Typed policy and gate contracts for policy-set packet diffs."""
...
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_VERSION = (
    "module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-v1"
)
...
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_DEFAULT_LIMIT = 50
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_MAX_LIMIT = 512
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_MAX_CHECKS = 16
...
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicy:
    """Immutable admission thresholds for a portable packet comparison."""

    policy_id: str
    maximum_changed_count: int
    allowed_directions: tuple[str, ...]
    allowed_state_transitions: tuple[str, ...]
    require_previous_accepted: bool
    require_current_accepted: bool
    allow_unchanged: bool
    content_address: str

    def __post_init__(self) -> None:
        _text(self.policy_id, "policy_id")
        _count(self.maximum_changed_count, "maximum_changed_count")
...
@dataclass(frozen=True, slots=True)
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyCheck:
    """One independently inspectable packet-diff policy result."""
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.passed, bool):
            raise ValidationError("packet-diff policy check result must be boolean")
...
@dataclass(frozen=True, slots=True)
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyGate:
    """A conserved readiness decision over one portable packet comparison."""

    diff_address: str
    policy: ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicy
    checks: tuple[ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyCheck, ...]
    accepted: bool
    content_address: str
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(
            ...
            raise ValidationError("packet-diff gate policy must be typed")
        if (
            not self.checks
            or len(self.checks)
            > MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_MAX_CHECKS
        ):
            raise ValidationError("packet-diff gate checks are missing or excessive")
        ids = tuple(item.check_id for item in self.checks)
        if ids != tuple(sorted(ids)) or len(ids) != len(set(ids)):
            raise ValidationError("packet-diff gate checks must be sorted and unique")
        if not isinstance(self.accepted, bool):
            raise ValidationError("packet-diff gate acceptance must be boolean")
        if self.accepted != all(item.passed for item in self.checks):
            raise ValidationError("packet-diff gate acceptance does not conserve checks")
        _text(self.content_address, "content_address")
...
def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy(
...
def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_check(
    value: ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyCheck,
) -> str:
    return _address(
        value,
        "module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-check",
    )


def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_gate(
    value: ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyGate,
) -> str:
    return _address(
        value,
        "module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-gate",
    )
