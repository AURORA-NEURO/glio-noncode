"""Independent audit projections for D480 history-diff runtimes."""
...
def audit_runtime(value: runtime_model.DiffRuntime, diff: diff_model.HistoryDiff | None = None) -> RuntimeAudit:
    ...
    checks = (
        _finding(1, "typed_runtime", isinstance(value, runtime_model.DiffRuntime), type(value).__name__, "DiffRuntime", "the audited value must use the typed runtime model"),
        ...
        _finding(10, "transition_link", diff is None or value.state_transition == diff.state_transition, value.state_transition, diff.state_transition if diff is not None else value.state_transition, "state transition must link to the diff"),
