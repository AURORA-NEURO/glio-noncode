...
def audit_runtime(value: runtime_model.DownloadedDataQualityDiffGateRuntime) -> DownloadedDataQualityDiffGateRuntimeAudit:
    ...
    checks = (
        ...
        _check(9, "manifest", value.manifest.runtime_id == value.runtime_id and value.manifest.files == runtime_model.FILES and value.manifest.artifact_addresses == (value.gate_address, value.audit_address, value.query_address, value.query_audit_address), "manifest retains the exact runtime file set and component addresses", (value.manifest.content_address,)),
