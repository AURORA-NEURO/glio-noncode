...
REGISTRY_VERSION = "glio-noncode-d451-history-diff-v1-runtime-v1-registry-v1"
REGISTRY_BOUNDARY = "glio-noncode-d451-history-diff_runtime_registry"
REGISTRY_PREFIX = "glio-noncode-d453-runtime-registry"
...
class RuntimeRegistryEntrySnapshot:
    """Compact value-only view of one D453 runtime admission entry."""
    ...
    def __init__(self, **values: Any) -> None:
        _strict(values, set(self.FIELDS), "D453 runtime registry entry")
        for field in self.FIELDS:
            setattr(self, field, values[field])
        if not isinstance(self.ordinal, int) or isinstance(self.ordinal, bool) or not 1 <= self.ordinal <= REGISTRY_MAX_ENTRIES:
            raise ValidationError("D453 runtime registry entry ordinal is outside its bound")
        if not isinstance(self.runtime_id, str) or not self.runtime_id.strip() or not isinstance(self.diff_id, str) or not self.diff_id.strip():
            raise ValidationError("D453 runtime registry entry identity is invalid")
        if self.state not in REGISTRY_STATES[1:] or not isinstance(self.release_ready, bool) or self.release_ready != (self.state == "ready"):
            raise ValidationError("D453 runtime registry entry disposition is invalid")
        for field in ("check_count", "passed_count", "item_count"):
            ...
            if not isinstance(value, int) or isinstance(value, bool) or value < 0 or value > REGISTRY_MAX_ENTRIES:
                raise ValidationError(f"D453 runtime registry entry {field} is outside its bound")
        if self.passed_count > self.check_count:
            raise ValidationError("D453 runtime registry entry passed count exceeds checks")
        _address(self.runtime_address, "D453 runtime registry runtime address")
        _address(self.diff_address, "D453 runtime registry diff address")
        _address(self.content_address, "D453 runtime registry entry address", REGISTRY_ENTRY_PREFIX)
        self._validate()
    ...
    def _validate(self) -> None:
        if not self.content_address.startswith("pending:") and _address_for(self, REGISTRY_ENTRY_PREFIX) != self.content_address:
            raise ValidationError("D453 runtime registry entry address does not replay")


class RuntimeRegistrySnapshot:
    """Minimal D453 registry projection used without importing old layers."""
    ...
    def __init__(self, **values: Any) -> None:
        _strict(values, set(self.FIELDS), "D453 runtime registry")
        for field in self.FIELDS:
            setattr(self, field, values[field])
        ...
        if not isinstance(self.manifest, Mapping) or not isinstance(self.summary, Mapping):
            raise ValidationError("D453 runtime registry components must be objects")
        self._validate()
    ...
    def _validate(self) -> None:
        if (self.version, self.boundary) != (REGISTRY_VERSION, REGISTRY_BOUNDARY):
            raise ValidationError("D453 runtime registry identity does not replay")
        if not isinstance(self.registry_id, str) or not self.registry_id.strip() or len(self.entries) != self.entry_count:
            raise ValidationError("D453 runtime registry identity or count does not replay")
        if tuple(item.ordinal for item in self.entries) != tuple(range(1, self.entry_count + 1)):
            raise ValidationError("D453 runtime registry entry ordinals do not replay")
        if len({item.runtime_id for item in self.entries}) != self.entry_count or len({item.runtime_address for item in self.entries}) != self.entry_count:
            raise ValidationError("D453 runtime registry contains duplicate runtime identity or address")
        ...
        if (self.ready_count, self.blocked_count, self.state) != (ready, blocked, expected_state):
            raise ValidationError("D453 runtime registry state counters do not replay")
        if self.accepted != (self.entry_count > 0) or self.release_ready != (self.state == "ready"):
            raise ValidationError("D453 runtime registry acceptance does not replay")
        ...
        if tuple(summary.get(field) for field in summary_fields) != (self.registry_id, self.entry_count, self.ready_count, self.blocked_count, self.accepted, self.release_ready, self.state):
            raise ValidationError("D453 runtime registry summary does not replay")
        if summary.get("content_address") != content_hash(dict(summary) | {"content_address": None}, prefix=REGISTRY_SUMMARY_PREFIX):
            raise ValidationError("D453 runtime registry summary address does not replay")
        ...
        if tuple(self.manifest.get(field) for field in manifest_fields[:3]) + (tuple(self.manifest.get("files", ())), tuple(self.manifest.get("artifact_addresses", ()))) != (self.registry_id, REGISTRY_VERSION, REGISTRY_BOUNDARY, REGISTRY_FILES, (content_hash({"entries": [item.to_dict() for item in self.entries]}, prefix=REGISTRY_ENTRIES_PREFIX), summary["content_address"])):
            raise ValidationError("D453 runtime registry manifest does not replay")
        if self.manifest.get("content_address") != content_hash(dict(self.manifest) | {"content_address": None}, prefix=REGISTRY_MANIFEST_PREFIX):
            raise ValidationError("D453 runtime registry manifest address does not replay")
        if not isinstance(self.content_address, str) or (not self.content_address.startswith("pending:") and not self.content_address.startswith(REGISTRY_PREFIX + ":")):
            raise ValidationError("D453 runtime registry address has the wrong namespace")
        if not self.content_address.startswith("pending:") and _address_for(self, REGISTRY_PREFIX) != self.content_address:
            raise ValidationError("D453 runtime registry address does not replay")

    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RuntimeRegistrySnapshot":
        value = _mapping(value, "D453 runtime registry")
        _strict(value, set(cls.FIELDS), "D453 runtime registry")
        return cls(**dict(value))


class _RegistryModel:
    """Compatibility facade exposing only the D453 surface D454 needs."""
    ...
    def verify_registry(value: RuntimeRegistrySnapshot) -> RuntimeRegistrySnapshot:
        if not isinstance(value, RuntimeRegistrySnapshot):
            raise ValidationError("D453 runtime registry verification requires a compact snapshot")
        value._validate()
        return value
...
def build_registry_snapshot(runtimes: Sequence[Any], *, registry_id: str = "glio-noncode-d453-runtime-registry") -> RuntimeRegistrySnapshot:
    """Build a canonical D453 registry from runtime mappings or typed values.

    This narrow constructor is intentionally local to D454. It lets the history
    layer consume persisted D453 decisions without importing the entire
    generated compatibility ancestry.
    """

    runtimes = tuple(runtimes)
    if len(runtimes) > REGISTRY_MAX_ENTRIES:
        raise ValidationError("D453 runtime registry exceeds its entry bound")
...
def load_registry_snapshot(destination: str | Path) -> RuntimeRegistrySnapshot:
    """Load and independently verify the exact four-file D453 registry."""

    destination = Path(destination)
    _validate_parent(destination.parent, "D453 registry input")
    if not destination.is_dir() or destination.is_symlink():
        raise ValidationError("D453 registry source must be a regular directory")
    ...
    if tuple(sorted(item.name for item in children)) != tuple(sorted(REGISTRY_FILES)) or any(item.is_symlink() or not item.is_file() for item in children):
        raise ValidationError("D453 registry directory must contain the exact regular file set")
    documents = {name: _read_json_document(destination / name, "D453 registry artifact") for name in REGISTRY_FILES}
    for name, document in documents.items():
        if read_text(destination / name, field="D453 registry artifact") != canonical_json(document):
            raise ValidationError("D453 registry artifact is not canonical")
    ...
    if any(canonical_json(documents[name]) != canonical_json(document) for name, document in expected.items()):
        raise ValidationError("D453 registry component documents do not replay registry.json")
