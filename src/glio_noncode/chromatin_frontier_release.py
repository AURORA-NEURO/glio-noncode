"""Release manifest for Domain 07 chromatin frontier outputs."""
...
from .chromatin_frontier_quality_gate import ChromatinFrontierQualityReport
from .chromatin_frontier_runtime import ChromatinFrontierRuntimeResult
from .serialization import content_hash, jsonable, require_non_empty


@dataclass(frozen=True, slots=True)
class ChromatinFrontierReleaseManifest:
    release_id: str
    fixture_id: str
    fixture_version: str
    run_id: str
    ...
    def __post_init__(self) -> None:
        ...
        if self.release_state not in {"ready", "blocked"}:
            raise ValueError("chromatin frontier release state must be ready or blocked")
...
def build_chromatin_frontier_release(
    ...
    *,
    ...
    release_state = "ready" if quality.accepted and runtime.accepted else "blocked"
    body = {
        ...
        "record_address": bundle.records_address,
    ...
    release_id = "chromatin-frontier-release:" + content_hash(body).split(":", 1)[1][:24]
    final_body = body | {"release_id": release_id}
    return ChromatinFrontierReleaseManifest(
        release_id,
        bundle.fixture_id,
        ...
        bundle.records_address,
        bundle.source_ids,
        operation_ids,
        content_hash(final_body),
    )


__all__ = ["ChromatinFrontierReleaseManifest", "build_chromatin_frontier_release"]
