"""Path-free recovery plans for exact history-diff archive transfers."""
...
class ExactHistoryDiffArchiveTransferRecoveryAction:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecoveryAction:
...
class ExactHistoryDiffArchiveTransferRecovery:
    ...
    def __init__(self, recovery_id: str, version: str, boundary: str, transfer_id: str, transfer_address: str, archive_address: str, archive_size: int, chunk_count: int, received_indices: Sequence[int], missing_indices: Sequence[int], received_bytes: int, remaining_bytes: int, actions: Sequence[ExactHistoryDiffArchiveTransferRecoveryAction], action_count: int, state: str, decision: str, safe_to_resume: bool, checkpointed: bool, next_index: int, content_address: str) -> None:
        ...
        self.actions = tuple(item if isinstance(item, ExactHistoryDiffArchiveTransferRecoveryAction) else ExactHistoryDiffArchiveTransferRecoveryAction.from_mapping(item) for item in _sequence(actions, "recovery actions", MAX_ACTIONS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecovery:
...
def address_recovery(value: ExactHistoryDiffArchiveTransferRecovery) -> str:
    if not isinstance(value, ExactHistoryDiffArchiveTransferRecovery):
...
def _actions(transfer: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer, missing: Sequence[int]) -> tuple[ExactHistoryDiffArchiveTransferRecoveryAction, ...]:
    actions = []
    for index in missing:
        chunk = transfer.chunks[index]
        pending = ExactHistoryDiffArchiveTransferRecoveryAction(chunk.index, chunk.offset, chunk.size, chunk.content_address, "pending:recovery-action")
        actions.append(ExactHistoryDiffArchiveTransferRecoveryAction(pending.index, pending.offset, pending.size, pending.content_address, address_action(pending)))
    return tuple(actions)


def build_recovery(value: transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer | transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferAssembler, *, recovery_id: str = DEFAULT_RECOVERY_ID, checkpointed: bool = False) -> ExactHistoryDiffArchiveTransferRecovery:
    """Derive a path-free recovery plan from a complete transfer or receiver assembler."""
    if isinstance(value, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransferAssembler):
        transfer = value.value
        parts = dict(value._parts)
    elif isinstance(value, transfer_model.ExactHistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffArchiveTransfer):
        transfer = value
    ...
    provisional = ExactHistoryDiffArchiveTransferRecovery(**body, content_address="pending:recovery")
    return ExactHistoryDiffArchiveTransferRecovery(**body, content_address=address_recovery(provisional))


def build_recovery_from_directory(source: str | Path, *, recovery_id: str = DEFAULT_RECOVERY_ID) -> ExactHistoryDiffArchiveTransferRecovery:
...
def recovery_from_mapping(value: Mapping[str, Any]) -> ExactHistoryDiffArchiveTransferRecovery:
    return ExactHistoryDiffArchiveTransferRecovery.from_mapping(value)


def recovery_json(value: ExactHistoryDiffArchiveTransferRecovery) -> str:
    return canonical_json(recovery_from_mapping(value.to_dict()).to_dict())


def recovery_csv(value: ExactHistoryDiffArchiveTransferRecovery) -> str:
