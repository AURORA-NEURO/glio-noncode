"""Independent replay audit for runtime registry history diff runtime queries."""
...
from . import downloaded_data_quality_runtime_history_release_evidence_history_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime as runtime_model
from . import downloaded_data_quality_runtime_history_release_evidence_history_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_registry_history_diff_runtime_query as query_model
...
CHECK_IDS = ("query_identity", "runtime_link", "resource_set", "filter_replay", "counts", "pagination", "row_order", "row_addresses", "projection_replay", "query_address", "resource_bounds", "public_boundary")
CHECK_FIELDS = ("ordinal", "check_id", "passed", "actual", "expected", "detail", "content_address")
AUDIT_FIELDS = ("query_id", "runtime_id", "runtime_address", "query_address", "checks", "check_count", "passed_count", "failed_count", "accepted", "content_address")
...
class QueryAuditCheck:
    ...
    def __init__(self, ordinal: int, check_id: str, passed: bool, actual: Any, expected: Any, detail: str, content_address: str) -> None:
        self.ordinal = _count(ordinal, "history diff runtime query audit check ordinal", MAX_CHECKS, lower=1)
        self.check_id = _label(check_id, "history diff runtime query audit check ID")
        self.passed = _bool(passed, "history diff runtime query audit check result")
        self.actual = _text(canonical_json(actual), "history diff runtime query audit actual", 16384)
        self.expected = _text(canonical_json(expected), "history diff runtime query audit expected", 16384)
        self.detail = _text(detail, "history diff runtime query audit detail", 4096)
        self.content_address = _address(content_address, "history diff runtime query audit check address", CHECK_PREFIX)
        self._validate()

    def _validate(self) -> None:
        if self.check_id not in CHECK_IDS:
            raise ValidationError("history diff runtime query audit check ID is unsupported")
        if not self.content_address.startswith("pending:") and _address_for(self, CHECK_PREFIX) != self.content_address:
            raise ValidationError("history diff runtime query audit check address does not replay")
        if not _public(self.to_dict()):
            raise ValidationError("history diff runtime query audit check crosses the public boundary")
...
def address_check(value: QueryAuditCheck) -> str:
    if not isinstance(value, QueryAuditCheck):
        raise ValidationError("history diff runtime query audit check address requires a typed check")
...
class QueryAudit:
    ...
    def __init__(self, query_id: str, runtime_id: str, runtime_address: str, query_address: str, checks: Sequence[QueryAuditCheck | Mapping[str, Any]], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.query_id = _label(query_id, "history diff runtime query audit query ID")
        self.runtime_id = _label(runtime_id, "history diff runtime query audit runtime ID")
        self.runtime_address = _address(runtime_address, "history diff runtime query audit runtime address", runtime_model.RUNTIME_PREFIX)
        self.query_address = _address(query_address, "history diff runtime query audit query address", query_model.QUERY_PREFIX)
        self.checks = tuple(item if isinstance(item, QueryAuditCheck) else QueryAuditCheck.from_mapping(_mapping(item, "history diff runtime query audit check")) for item in _sequence(checks, "history diff runtime query audit checks", MAX_CHECKS))
        self.check_count = _count(check_count, "history diff runtime query audit check count", MAX_CHECKS)
        self.passed_count = _count(passed_count, "history diff runtime query audit passed count", MAX_CHECKS)
        self.failed_count = _count(failed_count, "history diff runtime query audit failed count", MAX_CHECKS)
        self.accepted = _bool(accepted, "history diff runtime query audit acceptance")
        self.content_address = _address(content_address, "history diff runtime query audit address", AUDIT_PREFIX)
        self._validate()
    ...
    def _validate(self) -> None:
        if self.check_count != len(self.checks) or self.check_count != MAX_CHECKS or self.passed_count + self.failed_count != self.check_count or self.passed_count != sum(item.passed for item in self.checks) or self.accepted != (self.failed_count == 0):
            raise ValidationError("history diff runtime query audit counters do not replay")
        if tuple(item.ordinal for item in self.checks) != tuple(range(1, MAX_CHECKS + 1)) or tuple(item.check_id for item in self.checks) != CHECK_IDS:
            raise ValidationError("history diff runtime query audit check order does not replay")
        if any(item.content_address != address_check(item) for item in self.checks):
            raise ValidationError("history diff runtime query audit check addresses do not replay")
        if not self.content_address.startswith("pending:") and _address_for(self, AUDIT_PREFIX) != self.content_address:
            raise ValidationError("history diff runtime query audit address does not replay")
        if not _public(self.to_dict()):
            raise ValidationError("history diff runtime query audit crosses the public boundary")
...
def address_audit(value: QueryAudit) -> str:
    if not isinstance(value, QueryAudit):
        raise ValidationError("history diff runtime query audit address requires a typed audit")
...
def audit_query(value: query_model.RuntimeQuery, runtime: runtime_model.DiffRuntime) -> QueryAudit:
    ...
    expected = query_model.query_runtime(runtime, query_id=value.query_id, resources=value.resources, state_filter=value.state_filter, passed_filter=value.passed_filter, severity_filter=value.severity_filter, text_filter=value.text_filter, offset=value.offset, limit=value.limit)
    filters_ok = all((not value.state_filter or item.state == value.state_filter) and (value.passed_filter is None or item.passed == value.passed_filter) and (not value.severity_filter or item.severity == value.severity_filter) and (not value.text_filter or value.text_filter.casefold() in item.text.casefold()) for item in value.rows)
    checks = (
        _finding(1, "query_identity", value.query_id == expected.query_id and value.runtime_id == runtime.runtime_id, (value.query_id, value.runtime_id), (expected.query_id, runtime.runtime_id), "query and runtime identity must replay"),
        _finding(2, "runtime_link", value.runtime_address == runtime.content_address, value.runtime_address, runtime.content_address, "query must link to the source runtime"),
    ...
    return _seal(QueryAudit(value.query_id, value.runtime_id, value.runtime_address, value.content_address, sealed, len(sealed), passed, len(sealed) - passed, passed == len(sealed), f"pending:{AUDIT_PREFIX}"), address_audit)


def verify_audit(value: QueryAudit) -> QueryAudit:
    if not isinstance(value, QueryAudit):
        raise ValidationError("history diff runtime query audit verification requires a typed audit")
...
def render_audit_markdown(value: QueryAudit) -> str:
    value = verify_audit(value); lines = [f"# Runtime registry history diff runtime query audit {value.query_id}", "", f"- Accepted: {str(value.accepted).lower()}", f"- Checks: {value.passed_count}/{value.check_count}", "", "| Check | Result | Detail |", "| --- | --- | --- |"]
    lines.extend(f"| {item.check_id} | {'pass' if item.passed else 'fail'} | {item.detail} |" for item in value.checks); return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "HistoryDiffRuntimeQueryAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {field: {"type": "integer" if field == "ordinal" else "boolean" if field == "passed" else "string"} for field in CHECK_FIELDS}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "HistoryDiffRuntimeQueryAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {field: {"type": "array" if field == "checks" else "integer" if field.endswith("count") else "boolean" if field == "accepted" else "string"} for field in AUDIT_FIELDS}}
