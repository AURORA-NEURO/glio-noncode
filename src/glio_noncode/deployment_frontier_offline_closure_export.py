"""Exact-byte export packets for the D16 closure handoff."""
...
@dataclass(frozen=True, slots=True)
class DeploymentFrontierClosureExportManifest:
    version: str
    bundle_id: str
    artifact_count: int
...
@dataclass(frozen=True, slots=True)
class DeploymentFrontierClosureExportPacket:
    bundle_id: str
    artifacts: tuple[DeploymentFrontierClosureExportArtifact, ...]
    manifest: DeploymentFrontierClosureExportManifest
    accepted: bool
    content_address: str
...
@dataclass(frozen=True, slots=True)
class DeploymentFrontierClosureExportVerification:
    bundle_id: str
...
def _json_artifact(relative_path: str, value: Any) -> DeploymentFrontierClosureExportArtifact:
    content = (canonical_json(jsonable(_value(value))) + "\n").encode("utf-8")
    return DeploymentFrontierClosureExportArtifact(
        relative_path=relative_path,
        media_type="application/json",
        byte_count=len(content),
        content_address=hash_bytes(content, prefix="deployment-frontier-closure-export"),
        content=content,
    )


def build_deployment_frontier_closure_export(
    bundle: DeploymentFrontierOfflineBundle,
) -> DeploymentFrontierClosureExportPacket:
    boundary = {
    ...
    index_audit = audit_deployment_frontier_closure_indexes(bundle, indexes)
    ...
    certification = certify_deployment_frontier_closure(bundle)
    observability = build_deployment_frontier_closure_observability(bundle)
    graph = build_deployment_frontier_closure_graph(bundle)
    failure = build_deployment_frontier_closure_failure_report(bundle)
    runtime = run_deployment_frontier_closure_runtime(
    ...
    manifest_body = {
        "version": DEPLOYMENT_FRONTIER_CLOSURE_EXPORT_VERSION,
        "bundle_id": bundle.bundle_id,
        "artifact_count": len(artifacts),
        "artifacts": tuple(item.to_dict() for item in artifacts),
        "accepted": accepted
        and len(artifacts) == DEPLOYMENT_FRONTIER_CLOSURE_EXPORT_ARTIFACT_COUNT,
    }
    manifest = DeploymentFrontierClosureExportManifest(
        **manifest_body,
        content_address=hash_bytes(
            canonical_json(manifest_body).encode("utf-8"),
            prefix="deployment-frontier-closure-export-manifest",
        ),
    )
    body = {
        "bundle_id": bundle.bundle_id,
    ...
    return DeploymentFrontierClosureExportPacket(
        **body,
        content_address=hash_bytes(
            canonical_json(content_body).encode("utf-8"),
            prefix="deployment-frontier-closure-export-packet",
        ),
    )


def write_deployment_frontier_closure_export(
...
def verify_deployment_frontier_closure_export(
    packet: DeploymentFrontierClosureExportPacket, destination: str | Path
) -> DeploymentFrontierClosureExportVerification:
    root = Path(destination)
    ...
    return DeploymentFrontierClosureExportVerification(
        **body,
        content_address=hash_bytes(
            canonical_json(body).encode("utf-8"),
            prefix="deployment-frontier-closure-export-verification",
        ),
    )


__all__ = [
    "DEPLOYMENT_FRONTIER_CLOSURE_EXPORT_ARTIFACT_COUNT",
    "DEPLOYMENT_FRONTIER_CLOSURE_EXPORT_VERSION",
    "DeploymentFrontierClosureExportArtifact",
    "DeploymentFrontierClosureExportManifest",
    "DeploymentFrontierClosureExportPacket",
    "DeploymentFrontierClosureExportVerification",
