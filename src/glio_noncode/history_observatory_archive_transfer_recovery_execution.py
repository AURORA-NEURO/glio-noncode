...
from . import downloaded_data_profile_contract_compatibility_remediation_resolution_history_diff_policy_package_registry_observatory_archive_runtime_query_snapshot_diff_query_snapshot_diff_query_snapshot_registry_history_observatory_archive_transfer_recovery as recovery_model
...
DEFAULT_EXECUTION_ID = "comparison-query-snapshot-registry-history-observatory-archive-transfer-recovery-execution"
...
class RecoveryExecutionOutcome:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionOutcome":
...
class RecoveryExecution:
    ...
    def __init__(self, execution_id: str, version: str, boundary: str, recovery_id: str, recovery_address: str, transfer_address: str, archive_address: str, archive_size: int, chunk_count: int, base_received_indices: Sequence[int], planned_indices: Sequence[int], applied_indices: Sequence[int], pending_indices: Sequence[int], rejected_indices: Sequence[int], current_received_indices: Sequence[int], current_missing_indices: Sequence[int], planned_bytes: int, applied_bytes: int, pending_bytes: int, rejected_bytes: int, current_received_bytes: int, current_remaining_bytes: int, outcomes: Sequence[RecoveryExecutionOutcome], action_count: int, applied_count: int, pending_count: int, rejected_count: int, state: str, decision: str, safe_to_continue: bool, safe_to_assemble: bool, checkpointed: bool, next_index: int, content_address: str) -> None:
        ...
        self.outcomes = tuple(item if isinstance(item, RecoveryExecutionOutcome) else RecoveryExecutionOutcome.from_mapping(item) for item in _sequence(outcomes, "execution outcomes", MAX_OUTCOMES))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecution":
        value = _mapping(value, "recovery execution")
        _strict(value, set(cls.FIELDS), "recovery execution")
        return cls(*(value[field] for field in cls.FIELDS))


def address_outcome(value: RecoveryExecutionOutcome) -> str:
    if not isinstance(value, RecoveryExecutionOutcome):
...
def address_execution(value: RecoveryExecution) -> str:
    if not isinstance(value, RecoveryExecution):
...
def _outcomes(recovery: recovery_model.TransferRecovery, applied: set[int], rejected: set[int]) -> tuple[RecoveryExecutionOutcome, ...]:
    values = []
    for action in recovery.actions:
        status = "applied" if action.index in applied else "rejected" if action.index in rejected else "pending"
        pending = RecoveryExecutionOutcome(action.index, action.action_address, action.content_address, action.offset, action.size, status, REASONS[status], "pending:execution-outcome")
        values.append(RecoveryExecutionOutcome(pending.index, pending.action_address, pending.content_address, pending.offset, pending.size, pending.status, pending.reason, address_outcome(pending)))
    return tuple(values)


def _build(recovery: recovery_model.TransferRecovery, *, applied_indices: Sequence[int], rejected_indices: Sequence[int], execution_id: str, checkpointed: bool) -> RecoveryExecution:
    ...
    provisional = RecoveryExecution(**body, content_address="pending:execution")
    return RecoveryExecution(**body, content_address=address_execution(provisional))


def build_execution(recovery: recovery_model.TransferRecovery, *, applied_indices: Sequence[int] = (), rejected_indices: Sequence[int] = (), execution_id: str = DEFAULT_EXECUTION_ID, checkpointed: bool = False) -> RecoveryExecution:
    if not isinstance(recovery, recovery_model.TransferRecovery):
...
def build_execution_from_assembler(recovery: recovery_model.TransferRecovery, assembler: transfer_model.TransferAssembler, *, execution_id: str = DEFAULT_EXECUTION_ID, checkpointed: bool = True) -> RecoveryExecution:
    if not isinstance(recovery, recovery_model.TransferRecovery) or not isinstance(assembler, transfer_model.TransferAssembler):
...
def build_execution_from_directory(recovery: recovery_model.TransferRecovery, source: str | Path, *, execution_id: str = DEFAULT_EXECUTION_ID) -> RecoveryExecution:
...
def execution_from_mapping(value: Mapping[str, Any]) -> RecoveryExecution:
    return RecoveryExecution.from_mapping(value)


def verify_execution(value: RecoveryExecution) -> RecoveryExecution:
    if not isinstance(value, RecoveryExecution):
...
def execution_json(value: RecoveryExecution) -> str:
    return canonical_json(execution_from_mapping(value.to_dict()).to_dict())


def execution_csv(value: RecoveryExecution) -> str:
...
def execution_schema() -> dict[str, Any]:
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "History observatory archive transfer recovery execution", "type": "object", "additionalProperties": False, "required": list(EXECUTION_FIELDS), "properties": properties}
...
__all__ = ["BOUNDARY", "DECISIONS", "DEFAULT_EXECUTION_ID", "EXECUTION_FIELDS", "EXECUTION_PREFIX", "MAX_OUTCOMES", "OUTCOME_FIELDS", "OUTCOME_PREFIX", "REASONS", "STATUSES", "STATES", "RecoveryExecution", "RecoveryExecutionOutcome", "VERSION", "address_execution", "address_outcome", "build_execution", "build_execution_from_assembler", "build_execution_from_directory", "capabilities", "execution_csv", "execution_from_mapping", "execution_json", "execution_schema", "outcome_schema", "render_execution_markdown", "verify_execution"]
