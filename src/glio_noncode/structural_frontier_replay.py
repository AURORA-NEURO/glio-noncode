"""Replay integrity checks for Domain 02 C13-C16 fixtures."""
...
from .structural_frontier_public_data import StructuralFrontierFixtureCatalog


@dataclass(frozen=True, slots=True)
class StructuralFrontierReplayExpectation:
    """Minimum identity and evidence floors for a replay."""
    ...
    def __post_init__(self) -> None:
        ...
        if not self.source_ids:
            raise ValidationError("structural frontier replay source IDs must not be empty")
        if min(self.minimum_checks, self.minimum_positive_records, self.minimum_control_records) < 1:
            raise ValidationError("structural frontier replay floors must be positive")


@dataclass(frozen=True, slots=True)
class StructuralFrontierReplayCase:
...
@dataclass(frozen=True, slots=True)
class StructuralFrontierReplayReport:
    """Deterministic report across one or more replay paths."""

    fixture_id: str
    required_context_key: str
    cases: tuple[StructuralFrontierReplayCase, ...]
    passed: bool
    content_address: str
...
def replay_structural_frontier_fixtures(
    ...
    *,
    ...
    if not paths:
        raise ValidationError("structural frontier replay requires at least one path")
    required_context = required_context_key or expectation.context_key
    cases: list[StructuralFrontierReplayCase] = []
    for path in paths:
        ...
        if catalog.context_key != required_context or catalog.context_key != expectation.context_key:
    ...
    return StructuralFrontierReplayReport(
        fixture_id=expectation.fixture_id,
        required_context_key=required_context,
        cases=tuple(cases),
        passed=passed,
