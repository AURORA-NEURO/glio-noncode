"""Typed contracts for strict and release gates over packet-package catalog diffs."""
...
class PolicyCheck:
    """One deterministic packet-package catalog diff policy condition."""
    ...
    def __post_init__(self) -> None:
        _label(self.check_id, "packet-package catalog diff policy check ID")
        if self.check_id not in POLICY_CHECK_IDS:
            raise ValidationError("packet-package catalog diff policy check ID is unsupported")
        _bool(self.passed, "packet-package catalog diff policy check result")
        _text(self.detail, "packet-package catalog diff policy check detail", 2048)
        if not self.content_address.endswith(":pending"):
            _address(self.content_address, "packet-package catalog diff policy check address", CHECK_PREFIX)
            if address_check(self) != self.content_address:
                raise ValidationError("packet-package catalog diff policy check address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "PolicyCheck":
        if not isinstance(value, Mapping):
            raise ValidationError("packet-package catalog diff policy check must be an object")
        _strict(value, set(CHECK_FIELDS), "packet-package catalog diff policy check")
...
@dataclass(frozen=True)
class Policy:
    """A source-free ready or blocked gate over one packet-package catalog diff."""
    ...
    def __post_init__(self) -> None:
        _label(self.policy_id, "packet-package catalog diff policy ID")
        if self.version != VERSION or self.boundary != BOUNDARY:
            raise ValidationError("packet-package catalog diff policy version or boundary is not current")
        _address(self.diff_address, "packet-package catalog diff policy diff address", DIFF_PREFIX)
        for field in ("maximum_added", "maximum_removed", "maximum_changed", "maximum_total_changes"):
            _count(getattr(self, field), f"packet-package catalog diff policy {field}", 512)
        _ordered_labels(self.allowed_changes, "packet-package catalog diff policy changes", CHANGES)
        _ordered_labels(self.allowed_directions, "packet-package catalog diff policy directions", DIRECTIONS)
        _ordered_labels(self.allowed_transitions, "packet-package catalog diff policy transitions", STATE_TRANSITIONS)
        _ordered_labels(self.allowed_left_postures, "packet-package catalog diff policy left postures", POSTURES)
        _ordered_labels(self.allowed_right_postures, "packet-package catalog diff policy right postures", POSTURES)
        _ordered_labels(self.required_changes, "packet-package catalog diff policy required changes", CHANGES, allow_empty=True)
        for field in ("require_ready", "require_change", "allow_unchanged", "accepted"):
            _bool(getattr(self, field), f"packet-package catalog diff policy {field}")
        if self.state not in STATES:
            raise ValidationError("packet-package catalog diff policy state is unsupported")
        if not isinstance(self.checks, tuple) or len(self.checks) != len(POLICY_CHECK_IDS) or any(not isinstance(item, PolicyCheck) for item in self.checks):
            raise ValidationError("packet-package catalog diff policy checks are invalid")
        _count(self.check_count, "packet-package catalog diff policy check count", len(POLICY_CHECK_IDS))
        _count(self.passed_count, "packet-package catalog diff policy passed count", len(POLICY_CHECK_IDS))
        _count(self.failed_count, "packet-package catalog diff policy failed count", len(POLICY_CHECK_IDS))
        if self.check_count != len(self.checks) or tuple(item.check_id for item in self.checks) != POLICY_CHECK_IDS or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0) or self.state != ("ready" if self.accepted else "blocked"):
            raise ValidationError("packet-package catalog diff policy decision aggregates do not replay")
        if _has_forbidden_key(self.to_dict()):
            raise ValidationError("packet-package catalog diff policy crosses the public boundary")
        if not self.content_address.endswith(":pending"):
            _address(self.content_address, "packet-package catalog diff policy address", POLICY_PREFIX)
            if address_policy(self) != self.content_address:
                raise ValidationError("packet-package catalog diff policy address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "Policy":
        if not isinstance(value, Mapping):
            raise ValidationError("packet-package catalog diff policy must be an object")
        _strict(value, set(POLICY_FIELDS), "packet-package catalog diff policy")
...
def policy_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data packet-package catalog diff policy", "type": "object", "additionalProperties": False, "required": list(POLICY_FIELDS), "properties": {field: {"type": "array" if field in {"allowed_changes", "allowed_directions", "allowed_transitions", "allowed_left_postures", "allowed_right_postures", "required_changes", "checks"} else "boolean" if field in {"require_ready", "require_change", "allow_unchanged", "accepted"} else "integer" if field in {"maximum_added", "maximum_removed", "maximum_changed", "maximum_total_changes", "check_count", "passed_count", "failed_count"} else "string"} for field in POLICY_FIELDS}}


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data packet-package catalog diff policy check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"check_id": {"enum": list(POLICY_CHECK_IDS)}, "passed": {"type": "boolean"}, "observed": {}, "required": {}, "detail": {"type": "string"}, "content_address": {"type": "string"}}}
