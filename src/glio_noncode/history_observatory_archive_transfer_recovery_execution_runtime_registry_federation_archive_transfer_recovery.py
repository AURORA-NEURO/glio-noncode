"""Path-free recovery plans for federation archive transfers."""
...
def build_recovery(value: transfer_model.ArchiveTransfer | transfer_model.TransferAssembler, *, recovery_id: str = DEFAULT_RECOVERY_ID, checkpointed: bool = False) -> TransferRecovery:
    """Derive a recovery plan from a verified full transfer or partial assembler."""
...
def build_recovery_from_directory(source: str | Path, *, recovery_id: str = DEFAULT_RECOVERY_ID) -> TransferRecovery:
    """Load complete or partial transfer state and checkpoint a recovery plan."""
...
def action_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "History observatory federation archive transfer recovery action", "type": "object", "additionalProperties": False, "required": list(ACTION_FIELDS), "properties": {"index": {"type": "integer", "minimum": 0, "maximum": transfer_model.MAX_CHUNKS - 1}, "offset": {"type": "integer", "minimum": 0}, "size": {"type": "integer", "minimum": 1}, "content_address": {"type": "string", "pattern": "^" + transfer_model.CHUNK_PREFIX + ":"}, "action_address": {"type": "string", "pattern": "^" + ACTION_PREFIX + ":"}}}
...
def recovery_schema() -> dict[str, Any]:
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "History observatory federation archive transfer recovery", "type": "object", "additionalProperties": False, "required": list(RECOVERY_FIELDS), "properties": properties}
