"""Publication manifest and verification for Domain 04 C09–C12."""
...
from .reference_governance_quality_gate import ReferenceGovernanceQualityGateReport
from .reference_governance_replay import ReferenceGovernanceReplayReport
from .serialization import content_hash, jsonable, require_non_empty


class ReferenceGovernanceReleaseState(StrEnum):
    """Release state derived from independent verification checks."""
...
class ReferenceGovernanceReleaseCheck:
    """One release check and its short reason."""
...
@dataclass(frozen=True, slots=True)
class ReferenceGovernanceReleaseManifest:
    """Reproducible publication decision for the C09–C12 evidence plane."""

    release_id: str
    fixture_id: str
    fixture_version: str
    context_key: str
    state: ReferenceGovernanceReleaseState
    checks: tuple[ReferenceGovernanceReleaseCheck, ...]
    ...
    def publishable(self) -> bool:
        return self.state is ReferenceGovernanceReleaseState.PUBLISHED and all(
...
def _check(check_id: str, passed: bool, detail: str) -> ReferenceGovernanceReleaseCheck:
    body = {"check_id": check_id, "passed": passed, "detail": detail}
    return ReferenceGovernanceReleaseCheck(check_id, passed, detail, _address(body))


def build_reference_governance_release_manifest(
    evaluation: ReferenceGovernanceEvaluationReport,
    quality: ReferenceGovernanceQualityGateReport,
    bundle: ReferenceGovernanceBundle,
    replay: ReferenceGovernanceReplayReport,
    *,
    fixture: ReferenceGovernanceFixture,
    release_id: str = "reference-governance-c09-c12",
) -> ReferenceGovernanceReleaseManifest:
    """Build a release decision stricter than a local adapter pass."""

    checks: list[ReferenceGovernanceReleaseCheck] = []
    ...
    add(
        "bundle",
        not ReferenceGovernanceBundleBuilder().verify(bundle),
        "bundle verification is empty",
    )
    add(
        "positive-count",
    ...
    add(
        "control-exclusion",
        all(entry.role.value == "positive" for entry in bundle.entries),
    ...
    add(
        "no-input-copy",
        all(
            "records" not in entry.to_dict() and "restrictions" not in entry.to_dict()
            for entry in bundle.entries
        ),
        "release bundle contains no input collections",
    )
    if all(check.passed for check in checks):
        state = ReferenceGovernanceReleaseState.PUBLISHED
    elif evaluation.accepted and quality.accepted:
        state = ReferenceGovernanceReleaseState.REVIEW
    else:
        state = ReferenceGovernanceReleaseState.BLOCKED
    body = {
    ...
    return ReferenceGovernanceReleaseManifest(**body, content_address=_address(body))


def verify_reference_governance_release_manifest(
    manifest: ReferenceGovernanceReleaseManifest,
) -> tuple[str, ...]:
    """Verify addresses, state, and every release check."""

    failures: list[str] = []
    if manifest.content_address != _address(
        {
            key: value
    ...
    if manifest.state is ReferenceGovernanceReleaseState.PUBLISHED and any(
...
def write_reference_governance_release_manifest(
    manifest: ReferenceGovernanceReleaseManifest, path: str | Path
) -> None:
    """Write a verified JSON release manifest."""

    failures = verify_reference_governance_release_manifest(manifest)
    if failures:
