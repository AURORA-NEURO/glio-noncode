"""Build, verify, query, and persist portable packet-catalog diff policy packages."""
...
from .downloaded_data_review_packet_diff_policy_release_certificate_bundle_diff_policy_package_catalog_diff_policy_package_catalog_diff_policy_package_contracts import (
    BOUNDARY, FILE_NAMES, MANIFEST_PREFIX, MAX_PACKAGE_BYTES, MEDIA_TYPES, MEMBER_PREFIX, PACKAGE_FORMAT, PACKAGE_PREFIX, PAYLOAD_NAMES, VERSION as CONTRACT_VERSION,
    DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyPackageCatalogDiffPolicyPackage as Package,
    DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyPackageCatalogDiffPolicyPackageManifest as Manifest,
    DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyPackageCatalogDiffPolicyPackageMember as Member,
...
def _zip_member(name: str) -> zipfile.ZipInfo:
    if name not in FILE_NAMES:
        raise ValidationError("packet-catalog diff policy package member is outside the fixed file set")
...
def _parts(raw: bytes) -> tuple[tuple[zipfile.ZipInfo, bytes], ...]:
    try:
        ...
        raise ValidationError(f"cannot read packet-catalog diff policy package ZIP: {exc}") from exc
...
def _read_package(value: bytes | bytearray | str | Path | Package) -> bytes:
    if isinstance(value, Package):
        ...
        raw = read_bytes(value, field="packet-catalog diff policy package", max_bytes=MAX_PACKAGE_BYTES)
    if len(raw) > MAX_PACKAGE_BYTES:
        raise ValidationError("packet-catalog diff policy package exceeds its byte bound")
    return raw


def _review(diff: Any, policy: Any, audit: Any) -> str:
    return "\n".join((
        "# Downloaded Data Packet-Catalog Diff Policy Package", "",
        ...
        "This package carries source-free packet-catalog comparison evidence, its applied gate, and an independent audit. It does not carry source archive bytes or record values.", "",
    ))
...
def _member(path: str, payload: bytes) -> Member:
    ...
    provisional = Member(**body)
    return Member(**(body | {"content_address": address_member(provisional)}))
...
def build_package(diff: Any, policy: Any, audit: Any | None = None, *, package_id: str = PACKAGE_PREFIX) -> Package:
    ...
    if policy_value.diff_address != diff_value.content_address:
        raise ValidationError("packet-catalog diff policy package policy does not refer to the supplied diff")
    ...
    if audit_value.policy_address != policy_value.content_address or not audit_value.accepted:
        raise ValidationError("packet-catalog diff policy package requires an accepted independent policy audit")
    ...
    provisional = Manifest(**(body | {"content_address": MANIFEST_PREFIX + ":pending"}))
    manifest = Manifest(**(body | {"content_address": address_manifest(provisional)}))
...
def _manifest_and_bodies(raw: bytes) -> tuple[Manifest, dict[str, bytes]]:
    parts = _parts(raw)
    names = tuple(info.filename for info, _payload in parts)
    if names != FILE_NAMES or any(not _safe_member(name) for name in names):
        raise ValidationError("packet-catalog diff policy package ZIP members are not the exact safe file set")
    if any(info.date_time != _ZIP_EPOCH or info.compress_type != zipfile.ZIP_STORED or info.external_attr != _FIXED_ATTRIBUTES or info.extra or info.comment for info, _payload in parts):
        raise ValidationError("packet-catalog diff policy package ZIP metadata is not deterministic")
    ...
    if not isinstance(parsed, Mapping) or bodies[FILE_NAMES[0]] != (canonical_json(parsed) + "\n").encode(_UTF8):
        raise ValidationError("packet-catalog diff policy package manifest is not canonical")
    members = tuple(Member(**item) for item in parsed.get("members", ()))
    manifest = Manifest(package_id=parsed.get("package_id"), version=parsed.get("version"), boundary=parsed.get("boundary"), diff_address=parsed.get("diff_address"), policy_address=parsed.get("policy_address"), audit_address=parsed.get("audit_address"), policy_state=parsed.get("policy_state"), policy_accepted=parsed.get("policy_accepted"), audit_accepted=parsed.get("audit_accepted"), members=members, package_format=parsed.get("package_format"), content_address=parsed.get("content_address"))
    if address_manifest(manifest) != manifest.content_address:
        raise ValidationError("packet-catalog diff policy package manifest address does not replay")
    for descriptor in members:
        ...
        if descriptor.byte_count != len(payload) or descriptor.byte_address != address_member_bytes(payload) or descriptor.content_address != address_member(descriptor):
            raise ValidationError(f"packet-catalog diff policy package member does not replay: {descriptor.relative_path}")
    ...
    if raw != canonical_raw:
        raise ValidationError("packet-catalog diff policy package ZIP bytes are not deterministic")
...
def verify_package(value: bytes | bytearray | str | Path | Package) -> Package:
    ...
    for path in ("catalog-diff.json", "policy.json", "policy-audit.json"):
        ...
        if bodies[path] != (canonical_json(parsed) + "\n").encode(_UTF8):
            raise ValidationError(f"packet-catalog diff policy package payload is not canonical: {path}")
    if manifest.diff_address != diff.content_address or manifest.policy_address != policy.content_address or manifest.audit_address != audit.content_address or policy.diff_address != diff.content_address or audit.policy_address != policy.content_address:
        raise ValidationError("packet-catalog diff policy package lineage does not replay")
    if manifest.policy_state != policy.state or manifest.policy_accepted != policy.accepted or not manifest.audit_accepted or not audit.accepted:
        raise ValidationError("packet-catalog diff policy package decision evidence does not replay")
    if _has_forbidden_key({"manifest": manifest.to_dict(), "diff": diff.to_dict(), "policy": policy.to_dict(), "audit": audit.to_dict()}):
        raise ValidationError("packet-catalog diff policy package crosses the public boundary")
    if bodies["review.md"].decode(_UTF8) != _review(diff, policy, audit):
        raise ValidationError("packet-catalog diff policy package Markdown does not replay")
    ...
    if isinstance(value, Package) and value.to_dict() != package.to_dict():
        raise ValidationError("typed packet-catalog diff policy package does not replay its bytes")
    return package
...
def write_package(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Package:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("packet-catalog diff policy package destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("packet-catalog diff policy package destination is not a file")
    _validate_parent(path.parent, "packet-catalog diff policy package destination")
    atomic_write_bytes(path, package.package_bytes, field="packet-catalog diff policy package destination")
    return package
...
def query_package(value: Any, *, resource: str = "summary", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    if resource not in {"summary", "members", "manifest", "diff", "policy", "audit", "review"} or offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("packet-catalog diff policy package query is invalid")
    package = verify_package(value)
