"""Independently audit handoff-catalog longitudinal diff policy decisions."""
...
class AuditCheck:
    ...
    def __init__(self, check_id: str, passed: bool, observed: Any, required: Any, detail: str, content_address: str) -> None:
        self.check_id = _label(check_id, "handoff catalog diff policy audit check ID")
        if self.check_id not in CHECK_IDS:
            raise ValidationError("handoff catalog diff policy audit check ID is unsupported")
        if not isinstance(passed, bool):
            raise ValidationError("handoff catalog diff policy audit check result must be boolean")
        self.passed = passed
        self.observed = observed
        self.required = required
        self.detail = _text(detail, "handoff catalog diff policy audit detail", 2048)
        self.content_address = content_address if content_address.endswith(":pending") else _address(content_address, "handoff catalog diff policy audit check address", CHECK_PREFIX)
        if not content_address.endswith(":pending") and address_check(self) != content_address:
            raise ValidationError("handoff catalog diff policy audit check address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> AuditCheck:
        if not isinstance(value, Mapping) or set(value) != set(cls.FIELDS):
            raise ValidationError("handoff catalog diff policy audit check fields are not exact")
...
class Audit:
    ...
    def __init__(self, policy_id: str, policy_address: str, checks: tuple[AuditCheck, ...], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.policy_id = _label(policy_id, "handoff catalog diff policy audit policy ID")
        self.policy_address = _address(policy_address, "handoff catalog diff policy audit policy address", policy_model.POLICY_PREFIX)
        self.checks = tuple(checks)
        self.check_count = check_count
        self.passed_count = passed_count
        self.failed_count = failed_count
        self.accepted = accepted
        self.content_address = content_address if content_address.endswith(":pending") else _address(content_address, "handoff catalog diff policy audit address", AUDIT_PREFIX)
        if self.check_count != len(self.checks) or self.check_count != len(CHECK_IDS) or tuple(item.check_id for item in self.checks) != CHECK_IDS or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0):
            raise ValidationError("handoff catalog diff policy audit aggregates do not replay")
        if not content_address.endswith(":pending") and address_audit(self) != content_address:
            raise ValidationError("handoff catalog diff policy audit address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> Audit:
        if not isinstance(value, Mapping) or set(value) != set(cls.FIELDS):
            raise ValidationError("handoff catalog diff policy audit fields are not exact")
...
def audit_policy(value: Any, *, diff: Any | None = None) -> Audit:
    try:
        ...
        checks = (
            ...
            _check("audit-diff-lineage", policy.diff_address.startswith(diff_model.DIFF_PREFIX + ":"), policy.diff_address, "diff namespace", "policy retains a handoff catalog diff lineage address"),
        ...
        checks = tuple(_check(check_id, False, "unavailable", "replayable", f"handoff catalog diff policy audit unavailable: {exc}") for check_id in CHECK_IDS)
...
def load_audit(value: bytes | bytearray | str | Path | Mapping[str, Any]) -> Audit:
    if isinstance(value, Mapping):
        ...
        raw = _strict_json_loads(read_text(value, field="handoff catalog diff policy audit", max_bytes=16 * 1024 * 1024))
...
def write_audit(value: Any, destination: str | Path, *, allow_existing: bool = False) -> Audit:
    ...
    if path.exists() and not allow_existing:
        raise ValidationError("handoff catalog diff policy audit destination already exists")
    if path.exists() and not path.is_file():
        raise ValidationError("handoff catalog diff policy audit destination is not a file")
    _validate_parent(path.parent, "handoff catalog diff policy audit destination")
    atomic_write_text(path, audit_json(audit), field="handoff catalog diff policy audit destination")
    return audit
...
def query_audit(value: Any, *, passed: bool | None = None, text: str = "", offset: int = 0, limit: int = 50) -> dict[str, Any]:
    ...
    if passed is not None and not isinstance(passed, bool):
        raise ValidationError("handoff catalog diff policy audit passed filter must be boolean")
    text = _text(text, "handoff catalog diff policy audit text filter") if text else ""
    if offset < 0 or limit < 1 or limit > MAX_LIMIT:
        raise ValidationError("handoff catalog diff policy audit query bounds are invalid")
...
def render_audit_markdown(value: Any) -> str:
    audit = verify_audit(value)
    lines = ["# Handoff Catalog Diff Policy Audit", "", f"- Policy: `{audit.policy_id}`", f"- Passed / failed: **{audit.passed_count} / {audit.failed_count}**", f"- Accepted: **{str(audit.accepted).lower()}**", f"- Address: `{audit.content_address}`", "", "| check | passed | detail |", "| --- | --- | --- |"]
    lines.extend(f"| `{item.check_id}` | {str(item.passed).lower()} | {item.detail} |" for item in audit.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data handoff catalog diff policy audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"check_id": {"enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "observed": {}, "required": {}, "detail": {"type": "string"}, "content_address": {"type": "string"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data handoff catalog diff policy audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"policy_id": {"type": "string"}, "policy_address": {"type": "string"}, "checks": {"type": "array", "minItems": len(CHECK_IDS), "maxItems": len(CHECK_IDS)}, "check_count": {"const": len(CHECK_IDS)}, "passed_count": {"type": "integer"}, "failed_count": {"type": "integer"}, "accepted": {"type": "boolean"}, "content_address": {"type": "string"}}}
