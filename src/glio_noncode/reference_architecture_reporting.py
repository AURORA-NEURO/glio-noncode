"""Deterministic reports for the D04 reference runtime."""
...
from .reference_architecture_depth import reference_architecture_depth_percent
from .reference_architecture_metrics import ReferenceArchitectureMetrics
from .serialization import jsonable


@dataclass(frozen=True, slots=True)
class ReferenceArchitectureReport:
    fixture_id: str
    title: str
...
def build_reference_architecture_report(
    fixture: ReferenceArchitectureFixture,
    runtime: ReferenceArchitectureRuntime,
    ...
    summary = {
        ...
        "depth_percent": reference_architecture_depth_percent(
    ...
    body = {
        ...
        "title": "D04 Reference Context and Release Architecture",
        "state": runtime.state,
        "summary": summary,
        "sections": sections,
    }
    return ReferenceArchitectureReport(
        fixture_id=fixture.fixture_id,
        title="D04 Reference Context and Release Architecture",
        state=runtime.state,
        summary=summary,
        sections=sections,
        content_address=addressed(body, "reference-report"),
    )


def render_reference_architecture_markdown(report: ReferenceArchitectureReport) -> str:
    lines = [
        f"# {report.title}",
        "",
...
def reference_architecture_receipts_csv(runtime: ReferenceArchitectureRuntime) -> str:
    rows = (
        (
            item.case_id,
            item.operation_id,
...
def reference_architecture_review_csv(queue: ReferenceArchitectureReviewQueue) -> str:
    rows = (
        (
            item.review_id,
            item.case_id,
