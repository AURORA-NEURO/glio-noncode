"""Independent audit for history-observatory archive transfer queries."""
...
from . import downloaded_data_profile_contract_compatibility_remediation_resolution_history_diff_policy_package_registry_observatory_archive_runtime_query_snapshot_diff_query_snapshot_diff_query_snapshot_registry_history_observatory_archive_transfer_query as query_model
...
class TransferQueryAuditCheck:
    ...
    def __init__(self, ordinal: int, check_id: str, passed: bool, detail: str, evidence_addresses: Sequence[str], content_address: str) -> None:
        ...
        if self.ordinal == 0 or check_id not in CHECK_IDS:
            raise ValidationError("query audit check identity is invalid")
...
class TransferQueryAudit:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"transfer_address": self.transfer_address, "query_address": self.query_address, "checks": [item.to_dict() for item in self.checks], "check_count": self.check_count, "passed_count": self.passed_count, "failed_count": self.failed_count, "accepted": self.accepted, "content_address": self.content_address}
...
def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> TransferQueryAuditCheck:
    provisional = TransferQueryAuditCheck(ordinal, check_id, passed, detail, evidence, "pending:query-audit-check")
    return TransferQueryAuditCheck(ordinal, check_id, passed, detail, evidence, address_check(provisional))
...
def audit_query(query: query_model.TransferQuery, transfer: transfer_model.ArchiveTransfer) -> TransferQueryAudit:
    ...
    expected = query_model.query_transfer(transfer, resources=query.resources, index=query.index_filter, chunk_offset=query.offset_filter, size=query.size_filter, chunk_address=query.chunk_address_filter, received=query.received_filter, text=query.text_filter, offset=query.offset, limit=query.limit)
    ...
    checks = (
        ...
        ("filter-replay", expected.total_count == query.total_count and expected.matched_count == query.matched_count, "query filters replay against the transfer"),
        ...
        ("resource-semantics", all(item.resource in query.resources and (item.received or item.missing or item.resource in {"summary", "archive", "progress"}) for item in query.rows), "query rows retain resource semantics"),
...
def render_audit_markdown(value: TransferQueryAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Comparison-query history observatory archive transfer query audit", "", f"- Transfer: `{value.transfer_address}`", f"- Query: `{value.query_address}`", f"- Checks: `{value.passed_count}/{value.check_count}`", f"- Accepted: `{str(value.accepted).lower()}`", f"- Address: `{value.content_address}`", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | {item.check_id} | {str(item.passed).lower()} | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "History observatory archive transfer query audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"type": "string", "enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 16}, "content_address": {"type": "string", "pattern": "^" + CHECK_PREFIX + ":"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "History observatory archive transfer query audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"transfer_address": {"type": "string", "pattern": "^" + transfer_model.TRANSFER_PREFIX + ":"}, "query_address": {"type": "string", "pattern": "^" + query_model.QUERY_PREFIX + ":"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "check_count": {"type": "integer", "const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "content_address": {"type": "string", "pattern": "^" + AUDIT_PREFIX + ":"}}}


def capabilities() -> dict[str, Any]:
    return {"public": True, "value_free": True, "version": VERSION, "boundary": BOUNDARY, "audit_prefix": AUDIT_PREFIX, "check_prefix": CHECK_PREFIX, "check_count": MAX_CHECKS, "checks": list(CHECK_IDS), "features": ["independent filter replay", "row address replay", "transfer linkage", "canonical JSON CSV and Markdown projections"], "public_boundary": {"source_paths": False, "source_records": False, "payload_bytes": False, "private_metadata": False}}
