"""Typed contracts for policy gates over module-581 catalog diffs."""
...
from .downloaded_data_review_packet_fixed_transport_catalog_diff_policy_581_ct import CHANGES, DIFF_PREFIX, DIRECTIONS, POSTURES, STATE_TRANSITIONS, VERSION as DIFF_VERSION
...
class PolicyCheck:
    ...
    def __post_init__(self) -> None:
        _label(self.check_id, "module582 check ID")
        if self.check_id not in POLICY_CHECK_IDS:
            raise ValidationError("module582 check ID is unsupported")
        _bool(self.passed, "module582 check result")
        _text(self.detail, "module582 check detail", 2048)
        _address(self.content_address, "module582 check address", CHECK_PREFIX)
        if not self.content_address.endswith(":pending") and address_check(self) != self.content_address:
            raise ValidationError("module582 check address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "PolicyCheck":
        if not isinstance(value, Mapping):
            raise ValidationError("module582 check must be an object")
        _strict(value, set(CHECK_FIELDS), "module582 check")
...
class Policy:
    ...
    def __post_init__(self) -> None:
        _label(self.policy_id, "module582 policy ID")
        if self.version != VERSION or self.boundary != BOUNDARY:
            raise ValidationError("module582 policy identity is not current")
        _address(self.diff_address, "module582 diff address", DIFF_PREFIX)
        for field in ("maximum_added", "maximum_removed", "maximum_changed", "maximum_total_changes"):
            _count(getattr(self, field), f"module582 {field}", MAX_BUDGET)
        _ordered_labels(self.allowed_changes, "module582 allowed changes", CHANGES)
        _ordered_labels(self.allowed_directions, "module582 allowed directions", DIRECTIONS)
        _ordered_labels(self.allowed_transitions, "module582 allowed transitions", STATE_TRANSITIONS)
        _ordered_labels(self.allowed_left_postures, "module582 allowed left postures", POSTURES)
        _ordered_labels(self.allowed_right_postures, "module582 allowed right postures", POSTURES)
        _ordered_labels(self.required_changes, "module582 required changes", CHANGES, allow_empty=True)
        for field in ("require_ready", "require_change", "allow_unchanged", "accepted"):
            _bool(getattr(self, field), f"module582 {field}")
        if self.state not in STATES or not isinstance(self.checks, tuple) or tuple(item.check_id for item in self.checks) != POLICY_CHECK_IDS:
            raise ValidationError("module582 policy state or checks are invalid")
        _count(self.check_count, "module582 check count", len(POLICY_CHECK_IDS))
        _count(self.passed_count, "module582 passed count", len(POLICY_CHECK_IDS))
        _count(self.failed_count, "module582 failed count", len(POLICY_CHECK_IDS))
        if self.check_count != len(self.checks) or self.passed_count != sum(item.passed for item in self.checks) or self.failed_count != sum(not item.passed for item in self.checks) or self.accepted != (self.failed_count == 0) or self.state != ("ready" if self.accepted else "blocked"):
            raise ValidationError("module582 policy aggregates do not replay")
        if _has_forbidden_key(self.to_dict()):
            raise ValidationError("module582 policy crosses the public boundary")
        _address(self.content_address, "module582 policy address", POLICY_PREFIX)
        if not self.content_address.endswith(":pending") and address_policy(self) != self.content_address:
            raise ValidationError("module582 policy address does not replay")
    ...
    def from_mapping(cls, value: Mapping[str, Any]) -> "Policy":
        if not isinstance(value, Mapping):
            raise ValidationError("module582 policy must be an object")
        _strict(value, set(POLICY_FIELDS), "module582 policy")
...
def policy_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module582 policy", "type": "object", "additionalProperties": False, "required": list(POLICY_FIELDS), "properties": {field: {"type": "array" if field in {"allowed_changes", "allowed_directions", "allowed_transitions", "allowed_left_postures", "allowed_right_postures", "required_changes", "checks"} else "boolean" if field in {"require_ready", "require_change", "allow_unchanged", "accepted"} else "integer" if field in {"maximum_added", "maximum_removed", "maximum_changed", "maximum_total_changes", "check_count", "passed_count", "failed_count"} else "string"} for field in POLICY_FIELDS}}


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Module582 policy check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"check_id": {"enum": list(POLICY_CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "content_address": {"type": "string"}}}


def capabilities() -> dict[str, Any]:
    return {"version": VERSION, "boundary": BOUNDARY, "public": True, "source_free": True, "content_addressed": True, "states": STATES, "check_ids": POLICY_CHECK_IDS, "diff_version": DIFF_VERSION, "controls": ("budgets", "allowlists", "postures", "required_changes", "readiness", "unchanged"), "operations": ("build_policy", "policy_json", "policy_csv", "render_policy_markdown", "query_policy", "verify_policy")}
