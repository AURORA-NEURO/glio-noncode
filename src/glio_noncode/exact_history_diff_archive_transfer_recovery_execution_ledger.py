...
def _write(path: Path, raw: bytes) -> None:
    atomic_write_bytes(path, raw, field=f"ledger member {path.name}")
...
def persist_ledger(value: ExactHistoryDiffArchiveTransferRecoveryExecutionLedger, destination: str | Path, *, overwrite: bool = False) -> Path:
    ...
    _validate_parent(target.parent, "ledger destination")
    if target.exists():
        if not overwrite:
...
def _read_json(path: Path) -> tuple[Mapping[str, Any], bytes]:
    try:
        raw = read_bytes(path, field=f"ledger member {path.name}")
...
def load_ledger(destination: str | Path) -> ExactHistoryDiffArchiveTransferRecoveryExecutionLedger:
    ...
    for filename in FILES:
        if read_bytes(root / filename, field=f"ledger member {filename}") != expected_members[filename]:
