"""Deep context guard receipt for workbench release review.
...
class WorkbenchReleaseContextGuard:
...
def build_workbench_release_context_guard(values: Iterable[Any] = (), *, require_non_empty: bool = True) -> WorkbenchReleaseContextGuard:
    rows = tuple(
        {
            "sequence": index,
    ...
    return WorkbenchReleaseContextGuard(**body, content_address=content_hash(body))


def validate_workbench_release_context_guard(receipt: WorkbenchReleaseContextGuard) -> bool:
...
__all__ = ["WorkbenchReleaseContextGuard", "build_workbench_release_context_guard", "validate_workbench_release_context_guard"]
