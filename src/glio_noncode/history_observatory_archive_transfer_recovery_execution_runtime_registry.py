"""Deterministic registries for independently verified recovery runtimes."""
...
DEFAULT_REGISTRY_ID = "comparison-query-snapshot-registry-history-observatory-archive-transfer-recovery-execution-runtime-registry"
...
class RecoveryExecutionRuntimeRegistryEntry:
    """One value-free admission row for a recovery execution runtime."""
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistryEntry":
        value = _mapping(value, "runtime registry entry")
        _strict(value, set(cls.FIELDS), "runtime registry entry")
        return cls(*(value[field] for field in cls.FIELDS))


def address_entry(value: RecoveryExecutionRuntimeRegistryEntry) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryEntry):
...
def entry_from_runtime(value: runtime_model.RecoveryExecutionRuntime, ordinal: int) -> RecoveryExecutionRuntimeRegistryEntry:
    ...
    provisional = RecoveryExecutionRuntimeRegistryEntry(**body)
    return RecoveryExecutionRuntimeRegistryEntry(**(body | {"content_address": address_entry(provisional)}))


class RecoveryExecutionRuntimeRegistryEntries:
    """Addressed ordered entry projection persisted beside a registry."""

    FIELDS = ENTRIES_FIELDS

    def __init__(self, entries: Sequence[RecoveryExecutionRuntimeRegistryEntry | Mapping[str, Any]], content_address: str) -> None:
        self.entries = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryEntry) else RecoveryExecutionRuntimeRegistryEntry.from_mapping(item) for item in _sequence(entries, "runtime registry entries", MAX_ENTRIES))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistryEntries":
...
def address_entries(value: Sequence[RecoveryExecutionRuntimeRegistryEntry]) -> str:
    typed = tuple(value)
    if any(not isinstance(item, RecoveryExecutionRuntimeRegistryEntry) for item in typed):
...
class RecoveryExecutionRuntimeRegistryManifest:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistryManifest":
        value = _mapping(value, "runtime registry manifest")
        _strict(value, set(cls.FIELDS), "runtime registry manifest")
        return cls(*(value[field] for field in cls.FIELDS))


def address_manifest(value: RecoveryExecutionRuntimeRegistryManifest) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryManifest):
...
class RecoveryExecutionRuntimeRegistrySummary:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistrySummary":
        value = _mapping(value, "runtime registry summary")
        _strict(value, set(cls.FIELDS), "runtime registry summary")
        return cls(*(value[field] for field in cls.FIELDS))


def address_summary(value: RecoveryExecutionRuntimeRegistrySummary) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistrySummary):
...
def _state(entries: Sequence[RecoveryExecutionRuntimeRegistryEntry]) -> str:
    if not entries:
        return "empty"
    return "ready" if all(item.accepted for item in entries) else "blocked"


class RecoveryExecutionRuntimeRegistry:
    """A deterministic multi-runtime admission registry."""

    FIELDS = REGISTRY_FIELDS

    def __init__(self, registry_id: str, version: str, boundary: str, entry_count: int, accepted_count: int, ready_count: int, blocked_count: int, state: str, accepted: bool, manifest: RecoveryExecutionRuntimeRegistryManifest | Mapping[str, Any], summary: RecoveryExecutionRuntimeRegistrySummary | Mapping[str, Any], entries: Sequence[RecoveryExecutionRuntimeRegistryEntry | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.manifest = manifest if isinstance(manifest, RecoveryExecutionRuntimeRegistryManifest) else RecoveryExecutionRuntimeRegistryManifest.from_mapping(manifest)
        self.summary = summary if isinstance(summary, RecoveryExecutionRuntimeRegistrySummary) else RecoveryExecutionRuntimeRegistrySummary.from_mapping(summary)
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> "RecoveryExecutionRuntimeRegistry":
        value = _mapping(value, "runtime registry")
        _strict(value, set(cls.FIELDS), "runtime registry")
        return cls(*(value[field] for field in cls.FIELDS))


def address_registry(value: RecoveryExecutionRuntimeRegistry) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistry):
...
def build_registry(runtimes: Sequence[runtime_model.RecoveryExecutionRuntime], *, registry_id: str = DEFAULT_REGISTRY_ID) -> RecoveryExecutionRuntimeRegistry:
    runtimes = tuple(_sequence(runtimes, "runtime registry runtimes", MAX_ENTRIES))
    if any(not isinstance(item, runtime_model.RecoveryExecutionRuntime) for item in runtimes):
    ...
    summary_provisional = RecoveryExecutionRuntimeRegistrySummary(**summary_body)
    summary = RecoveryExecutionRuntimeRegistrySummary(**(summary_body | {"content_address": address_summary(summary_provisional)}))
    ...
    manifest_provisional = RecoveryExecutionRuntimeRegistryManifest(**manifest_body)
    manifest = RecoveryExecutionRuntimeRegistryManifest(**(manifest_body | {"content_address": address_manifest(manifest_provisional)}))
    ...
    provisional = RecoveryExecutionRuntimeRegistry(**body, content_address=REGISTRY_PREFIX + ":pending")
    return RecoveryExecutionRuntimeRegistry(**body, content_address=address_registry(provisional))


def verify_registry(value: RecoveryExecutionRuntimeRegistry) -> RecoveryExecutionRuntimeRegistry:
    if not isinstance(value, RecoveryExecutionRuntimeRegistry):
        raise ValidationError("runtime registry verification requires a typed registry")
    return RecoveryExecutionRuntimeRegistry.from_mapping(value.to_dict())


def registry_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistry:
    return RecoveryExecutionRuntimeRegistry.from_mapping(value)


def registry_json(value: RecoveryExecutionRuntimeRegistry) -> str:
    return canonical_json(verify_registry(value).to_dict())


def registry_csv(value: RecoveryExecutionRuntimeRegistry) -> str:
...
def render_registry_markdown(value: RecoveryExecutionRuntimeRegistry) -> str:
    value = verify_registry(value)
    lines = ["# Recovery Execution Runtime Registry", "", f"- Registry: `{value.registry_id}`", f"- State: `{value.state}`", f"- Entries: `{value.entry_count}`", f"- Accepted: `{value.accepted_count}/{value.entry_count}`", f"- Address: `{value.content_address}`", "", "| ordinal | runtime | state | accepted | execution | address |", "| ---: | --- | --- | --- | --- | --- |"]
...
def entries_json(value: RecoveryExecutionRuntimeRegistryEntries) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryEntries.from_mapping(value.to_dict()).to_dict())


def summary_json(value: RecoveryExecutionRuntimeRegistrySummary) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistrySummary.from_mapping(value.to_dict()).to_dict())


def manifest_json(value: RecoveryExecutionRuntimeRegistryManifest) -> str:
    return canonical_json(RecoveryExecutionRuntimeRegistryManifest.from_mapping(value.to_dict()).to_dict())
...
def persist_registry(value: RecoveryExecutionRuntimeRegistry, destination: str | Path, *, overwrite: bool = False) -> Path:
    ...
    _validate_parent(parent, "runtime registry destination")
    parent.mkdir(parents=True, exist_ok=True)
    if destination.is_symlink():
        raise ValidationError("runtime registry destination must not be a symlink")
    temporary = Path(tempfile.mkdtemp(prefix=".recovery-execution-runtime-registry-", dir=str(parent)))
    try:
        entries = RecoveryExecutionRuntimeRegistryEntries(value.entries, address_entries(value.entries))
...
def load_registry(destination: str | Path) -> RecoveryExecutionRuntimeRegistry:
    destination = Path(destination)
    ...
    try:
        ...
        raise ValidationError("runtime registry directory could not be inspected") from error
    ...
    manifest = RecoveryExecutionRuntimeRegistryManifest.from_mapping(documents["manifest.json"])
    entries = RecoveryExecutionRuntimeRegistryEntries.from_mapping(documents["entries.json"])
    summary = RecoveryExecutionRuntimeRegistrySummary.from_mapping(documents["summary.json"])
...
def entry_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryEntry", "type": "object", "additionalProperties": False, "required": list(ENTRY_FIELDS), "properties": {field: {"type": ["integer", "string", "boolean"]} for field in ENTRY_FIELDS}}


def entries_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryEntries", "type": "object", "additionalProperties": False, "required": list(ENTRIES_FIELDS), "properties": {"entries": {"type": "array", "items": entry_schema()}, "content_address": {"type": "string"}}}


def manifest_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryManifest", "type": "object", "additionalProperties": False, "required": list(MANIFEST_FIELDS), "properties": {"registry_id": {"type": "string"}, "version": {"type": "string"}, "boundary": {"type": "string"}, "files": {"type": "array", "items": {"type": "string"}}, "artifact_addresses": {"type": "array", "items": {"type": "string"}}, "content_address": {"type": "string"}}}


def summary_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistrySummary", "type": "object", "additionalProperties": False, "required": list(SUMMARY_FIELDS), "properties": {"registry_id": {"type": "string"}, "entry_count": {"type": "integer", "minimum": 0}, "accepted_count": {"type": "integer", "minimum": 0}, "ready_count": {"type": "integer", "minimum": 0}, "blocked_count": {"type": "integer", "minimum": 0}, "state": {"enum": list(STATES)}, "accepted": {"type": "boolean"}, "content_address": {"type": "string"}}}
...
def registry_schema() -> dict[str, Any]:
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistry", "type": "object", "additionalProperties": False, "required": list(REGISTRY_FIELDS), "properties": properties}


def capabilities() -> dict[str, Any]:
    return {"version": VERSION, "boundary": BOUNDARY, "registry_prefix": REGISTRY_PREFIX, "entry_prefix": ENTRY_PREFIX, "files": list(FILES), "states": list(STATES), "max_entries": MAX_ENTRIES, "operations": ["build_registry", "verify_registry", "registry_from_mapping", "persist_registry", "load_registry", "registry_json", "registry_csv", "render_registry_markdown"], "privacy": {"values": False, "source_paths": False, "payload_bytes": False, "agent_metadata": False, "language_metadata": False}}


__all__ = ["VERSION", "BOUNDARY", "REGISTRY_PREFIX", "ENTRY_PREFIX", "ENTRIES_PREFIX", "MANIFEST_PREFIX", "SUMMARY_PREFIX", "DEFAULT_REGISTRY_ID", "FILES", "ARTIFACT_FILES", "MAX_ENTRIES", "STATES", "ENTRY_FIELDS", "ENTRIES_FIELDS", "MANIFEST_FIELDS", "SUMMARY_FIELDS", "REGISTRY_FIELDS", "RecoveryExecutionRuntimeRegistryEntry", "RecoveryExecutionRuntimeRegistryEntries", "RecoveryExecutionRuntimeRegistryManifest", "RecoveryExecutionRuntimeRegistrySummary", "RecoveryExecutionRuntimeRegistry", "address_entry", "address_entries", "address_manifest", "address_summary", "address_registry", "entry_from_runtime", "build_registry", "verify_registry", "registry_from_mapping", "registry_json", "registry_csv", "render_registry_markdown", "entries_json", "summary_json", "manifest_json", "persist_registry", "load_registry", "entry_schema", "entries_schema", "manifest_schema", "summary_schema", "registry_schema", "capabilities"]
