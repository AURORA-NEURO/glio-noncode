...
class RecoveryExecutionRuntimeRegistryHistoryAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryAuditCheck:
...
def address_check(value: RecoveryExecutionRuntimeRegistryHistoryAuditCheck) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryAuditCheck):
        raise ValidationError("runtime registry history audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class RecoveryExecutionRuntimeRegistryHistoryAudit:
    """A fixed-size independently recomputed history audit."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, history_address: str, history_id: str, version: str, boundary: str, check_count: int, passed_count: int, failed_count: int, accepted: bool, checks: Sequence[RecoveryExecutionRuntimeRegistryHistoryAuditCheck | Mapping[str, Any]], content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryAuditCheck) else RecoveryExecutionRuntimeRegistryHistoryAuditCheck.from_mapping(item) for item in _sequence(checks, "runtime registry history audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryAudit:
        value = _mapping(value, "runtime registry history audit")
        _strict(value, set(cls.FIELDS), "runtime registry history audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: RecoveryExecutionRuntimeRegistryHistoryAudit) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryAudit):
        raise ValidationError("runtime registry history audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> RecoveryExecutionRuntimeRegistryHistoryAuditCheck:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryAuditCheck(**body)
    return RecoveryExecutionRuntimeRegistryHistoryAuditCheck(**(body | {"content_address": address_check(provisional)}))


def audit_history(value: history_model.RecoveryExecutionRuntimeRegistryHistory) -> RecoveryExecutionRuntimeRegistryHistoryAudit:
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryAudit(**body)
    return RecoveryExecutionRuntimeRegistryHistoryAudit(**(body | {"content_address": address_audit(provisional)}))


def verify_audit(value: RecoveryExecutionRuntimeRegistryHistoryAudit) -> RecoveryExecutionRuntimeRegistryHistoryAudit:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryAudit):
        raise ValidationError("runtime registry history audit verification requires a typed audit")
    return RecoveryExecutionRuntimeRegistryHistoryAudit.from_mapping(value.to_dict())


def audit_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryAudit:
    return RecoveryExecutionRuntimeRegistryHistoryAudit.from_mapping(value)


def audit_json(value: RecoveryExecutionRuntimeRegistryHistoryAudit) -> str:
    return canonical_json(verify_audit(value).to_dict())


def audit_csv(value: RecoveryExecutionRuntimeRegistryHistoryAudit) -> str:
...
def render_audit_markdown(value: RecoveryExecutionRuntimeRegistryHistoryAudit) -> str:
    value = verify_audit(value)
    lines = ["# Federation archive recovery execution runtime registry history audit", "", f"History: {value.history_id}", f"Passed: {value.passed_count}/{value.check_count}", f"Accepted: {value.accepted}", f"Address: {value.content_address}", "", "| # | check | passed | detail |", "| ---: | --- | --- | --- |"]
    lines.extend(f"| {item.ordinal} | {item.check_id} | {item.passed} | {item.detail} |" for item in value.checks)
    return "\n".join(lines) + "\n"


def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryHistoryAuditCheck", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1, "maximum": MAX_CHECKS}, "check_id": {"enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}}, "content_address": {"type": "string"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "RecoveryExecutionRuntimeRegistryHistoryAudit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"history_address": {"type": "string"}, "history_id": {"type": "string"}, "version": {"const": VERSION}, "boundary": {"const": BOUNDARY}, "check_count": {"const": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "content_address": {"type": "string"}}}
...
__all__ = ["VERSION", "BOUNDARY", "AUDIT_PREFIX", "CHECK_PREFIX", "CHECK_IDS", "CHECK_FIELDS", "AUDIT_FIELDS", "MAX_CHECKS", "RecoveryExecutionRuntimeRegistryHistoryAuditCheck", "RecoveryExecutionRuntimeRegistryHistoryAudit", "address_check", "address_audit", "audit_history", "verify_audit", "audit_from_mapping", "audit_json", "audit_csv", "render_audit_markdown", "check_schema", "audit_schema", "capabilities"]
