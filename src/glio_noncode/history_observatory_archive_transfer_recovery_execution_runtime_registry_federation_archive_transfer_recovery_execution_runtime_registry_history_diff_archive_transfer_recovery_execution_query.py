...
class HistoryDiffArchiveTransferRecoveryExecutionQueryRow:
...
class HistoryDiffArchiveTransferRecoveryExecutionQuery:
    ...
    def __init__(self, query_id: str, version: str, boundary: str, execution_address: str, execution_id: str, resources: Sequence[str], status_filter: str, index_filter: int, text_filter: str, offset: int, limit: int, total_count: int, returned_count: int, truncated: bool, rows: Sequence[HistoryDiffArchiveTransferRecoveryExecutionQueryRow], content_address: str) -> None:
        ...
        self.rows = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryExecutionQueryRow) else HistoryDiffArchiveTransferRecoveryExecutionQueryRow.from_mapping(item) for item in _sequence(rows, "execution query rows", MAX_LIMIT))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionQuery:
        value = _mapping(value, "execution query")
        _strict(value, set(cls.FIELDS), "execution query")
        return cls(value["query_id"], value["version"], value["boundary"], value["execution_address"], value["execution_id"], value["resources"], value["status_filter"], value["index_filter"], value["text_filter"], value["offset"], value["limit"], value["total_count"], value["returned_count"], value["truncated"], tuple(HistoryDiffArchiveTransferRecoveryExecutionQueryRow.from_mapping(item) for item in _sequence(value["rows"], "execution query rows", MAX_LIMIT)), value["content_address"])


def address_row(value: HistoryDiffArchiveTransferRecoveryExecutionQueryRow) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionQueryRow):
...
def address_query(value: HistoryDiffArchiveTransferRecoveryExecutionQuery) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionQuery):
        raise ValidationError("execution query address requires a typed query")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=QUERY_PREFIX)


def _row(resource: str, ordinal: int, *, index: int = -1, action_address: str = "", content_address: str = "", offset: int = 0, size: int = 0, status: str = "", reason: str = "", outcome_address: str = "", state: str = "", decision: str = "") -> HistoryDiffArchiveTransferRecoveryExecutionQueryRow:
    pending = HistoryDiffArchiveTransferRecoveryExecutionQueryRow(resource, ordinal, index, action_address, content_address, offset, size, status, reason, outcome_address, state, decision, "pending:execution-query-row")
    return HistoryDiffArchiveTransferRecoveryExecutionQueryRow(resource, ordinal, index, action_address, content_address, offset, size, status, reason, outcome_address, state, decision, address_row(pending))


def _all_rows(value: execution_model.HistoryDiffArchiveTransferRecoveryExecution) -> tuple[HistoryDiffArchiveTransferRecoveryExecutionQueryRow, ...]:
    rows: list[HistoryDiffArchiveTransferRecoveryExecutionQueryRow] = []
...
def _matches(row: HistoryDiffArchiveTransferRecoveryExecutionQueryRow, *, status: str, index: int, text: str) -> bool:
...
def query_execution(value: execution_model.HistoryDiffArchiveTransferRecoveryExecution, *, query_id: str = "runtime-registry-history-diff-archive-transfer-recovery-execution-query", resources: Sequence[str] | None = None, status: str = "", index: int = -1, text: str = "", offset: int = 0, limit: int = MAX_LIMIT) -> HistoryDiffArchiveTransferRecoveryExecutionQuery:
    if not isinstance(value, execution_model.HistoryDiffArchiveTransferRecoveryExecution):
    ...
    for ordinal, item in enumerate(page):
        pending = HistoryDiffArchiveTransferRecoveryExecutionQueryRow(item.resource, ordinal, item.index, item.action_address, item.content_address, item.offset, item.size, item.status, item.reason, item.outcome_address, item.state, item.decision, "pending:execution-query-row")
        page_rows.append(HistoryDiffArchiveTransferRecoveryExecutionQueryRow(item.resource, ordinal, item.index, item.action_address, item.content_address, item.offset, item.size, item.status, item.reason, item.outcome_address, item.state, item.decision, address_row(pending)))
    page = tuple(page_rows)
    result = HistoryDiffArchiveTransferRecoveryExecutionQuery(query_id, VERSION, BOUNDARY, value.content_address, value.execution_id, selected, status, index, text, offset, limit, len(rows), len(page), offset + len(page) < len(rows), page, "pending:execution-query")
    return HistoryDiffArchiveTransferRecoveryExecutionQuery(query_id, VERSION, BOUNDARY, value.content_address, value.execution_id, selected, status, index, text, offset, limit, len(rows), len(page), offset + len(page) < len(rows), page, address_query(result))


def query_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionQuery:
    return HistoryDiffArchiveTransferRecoveryExecutionQuery.from_mapping(value)


def query_json(value: HistoryDiffArchiveTransferRecoveryExecutionQuery) -> str:
    return canonical_json(query_from_mapping(value.to_dict()).to_dict())


def query_csv(value: HistoryDiffArchiveTransferRecoveryExecutionQuery) -> str:
...
def render_query_markdown(value: HistoryDiffArchiveTransferRecoveryExecutionQuery) -> str:
    value = query_from_mapping(value.to_dict())
    lines = ["# Runtime-registry history-diff archive transfer recovery execution query", "", f"- Execution: `{value.execution_id}`", f"- Resources: `{', '.join(value.resources)}`", f"- Rows: `{value.returned_count}/{value.total_count}`", f"- Address: `{value.content_address}`", "", "| resource | ordinal | index | status | state | decision | reason | row address |", "| --- | ---: | ---: | --- | --- | --- | --- | --- |"]
