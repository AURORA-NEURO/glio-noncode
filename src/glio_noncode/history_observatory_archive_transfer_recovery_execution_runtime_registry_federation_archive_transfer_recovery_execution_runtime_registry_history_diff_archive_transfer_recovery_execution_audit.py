...
class HistoryDiffArchiveTransferRecoveryExecutionAuditCheck:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionAuditCheck:
...
class HistoryDiffArchiveTransferRecoveryExecutionAudit:
    """Canonical audit receipt for a recovery execution snapshot."""

    FIELDS = AUDIT_FIELDS

    def __init__(self, execution_address: str, execution_id: str, version: str, boundary: str, checks: Sequence[HistoryDiffArchiveTransferRecoveryExecutionAuditCheck], check_count: int, passed: bool, content_address: str) -> None:
        ...
        self.checks = tuple(item if isinstance(item, HistoryDiffArchiveTransferRecoveryExecutionAuditCheck) else HistoryDiffArchiveTransferRecoveryExecutionAuditCheck.from_mapping(item) for item in _sequence(checks, "execution audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionAudit:
        value = _mapping(value, "execution audit")
        _strict(value, set(cls.FIELDS), "execution audit")
        return cls(value["execution_address"], value["execution_id"], value["version"], value["boundary"], tuple(HistoryDiffArchiveTransferRecoveryExecutionAuditCheck.from_mapping(item) for item in _sequence(value["checks"], "execution audit checks", MAX_CHECKS)), value["check_count"], value["passed"], value["content_address"])


def address_check(value: HistoryDiffArchiveTransferRecoveryExecutionAuditCheck) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionAuditCheck):
        raise ValidationError("execution audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


def address_audit(value: HistoryDiffArchiveTransferRecoveryExecutionAudit) -> str:
    if not isinstance(value, HistoryDiffArchiveTransferRecoveryExecutionAudit):
        raise ValidationError("execution audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(check_id: str, observed: Any, expected: Any) -> HistoryDiffArchiveTransferRecoveryExecutionAuditCheck:
    pending = HistoryDiffArchiveTransferRecoveryExecutionAuditCheck(check_id, observed == expected, observed, expected, "pending:execution-audit-check")
    return HistoryDiffArchiveTransferRecoveryExecutionAuditCheck(check_id, pending.passed, observed, expected, address_check(pending))


def _expected_outcome_addresses(value: execution_model.HistoryDiffArchiveTransferRecoveryExecution) -> tuple[str, ...]:
    result = []
    for item in value.outcomes:
        pending = execution_model.HistoryDiffArchiveTransferRecoveryExecutionOutcome(item.index, item.action_address, item.content_address, item.offset, item.size, item.status, item.reason, "pending:execution-outcome")
        result.append(execution_model.address_outcome(pending))
    return tuple(result)


def audit_execution(value: execution_model.HistoryDiffArchiveTransferRecoveryExecution) -> HistoryDiffArchiveTransferRecoveryExecutionAudit:
    ...
    provisional = HistoryDiffArchiveTransferRecoveryExecutionAudit(value.content_address, value.execution_id, VERSION, BOUNDARY, checks, len(checks), all(item.passed for item in checks), "pending:execution-audit")
    return HistoryDiffArchiveTransferRecoveryExecutionAudit(value.content_address, value.execution_id, VERSION, BOUNDARY, checks, len(checks), provisional.passed, address_audit(provisional))


def audit_from_mapping(value: Mapping[str, Any]) -> HistoryDiffArchiveTransferRecoveryExecutionAudit:
    return HistoryDiffArchiveTransferRecoveryExecutionAudit.from_mapping(value)


def audit_json(value: HistoryDiffArchiveTransferRecoveryExecutionAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: HistoryDiffArchiveTransferRecoveryExecutionAudit) -> str:
...
def render_audit_markdown(value: HistoryDiffArchiveTransferRecoveryExecutionAudit) -> str:
    value = audit_from_mapping(value.to_dict())
    lines = ["# Runtime-registry history-diff archive transfer recovery execution audit", "", f"- Execution: `{value.execution_id}`", f"- Result: `{'passed' if value.passed else 'failed'}`", f"- Checks: `{value.check_count}`", f"- Address: `{value.content_address}`", "", "| check | passed | observed | expected |", "| --- | --- | --- | --- |"]
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "HistoryDiffArchiveTransferRecoveryExecutionAudit", "HistoryDiffArchiveTransferRecoveryExecutionAuditCheck", "VERSION", "address_audit", "address_check", "audit_csv", "audit_execution", "audit_from_mapping", "audit_json", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
