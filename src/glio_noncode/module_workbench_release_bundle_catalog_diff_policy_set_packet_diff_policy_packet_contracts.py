"""Typed contracts for portable packet-diff policy review packets."""

# The public contract names intentionally mirror the full module boundary.
# ruff: noqa: E501
...
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_VERSION = (
    "module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-packet-v1"
)
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_BOUNDARY = "public_aggregate_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_packet"
...
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_MAX_ENTRIES = 5
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_MAX_CHECKS = 32
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_MAX_BYTES = (
    32 * 1024 * 1024
)
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_DEFAULT_LIMIT = 50
MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_MAX_LIMIT = 512


class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketCheckPlane(StrEnum):
    """The independent surface on which a packet invariant is checked."""
...
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketMember:
    """One exact member of the deterministic review ZIP."""
    ...
    def __post_init__(self) -> None:
        ...
        if not _safe_path(self.relative_path):
            raise ValidationError("packet-diff policy packet member path is unsafe")
...
def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_packet_member(
    value: ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketMember,
) -> str:
    body = value.to_dict()
    body.pop("content_address", None)
    return content_hash(
        body,
        prefix="module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-packet-member",
    )


@dataclass(frozen=True, slots=True)
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketCheck:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(
            ...
            raise ValidationError("packet-diff policy packet check plane is invalid")
        if not isinstance(self.passed, bool):
            raise ValidationError("packet-diff policy packet check result must be boolean")
...
def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_packet_check(
    value: ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketCheck,
) -> str:
    body = value.to_dict()
    body.pop("content_address", None)
    return content_hash(
        body,
        prefix="module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-packet-check",
    )


@dataclass(frozen=True, slots=True)
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacket:
    """A portable packet carrying comparison, gate, audit, and review."""
    ...
    def __post_init__(self) -> None:
        ...
        _text(self.packet_diff_address, "packet_diff_address", 512)
        _text(self.policy_gate_address, "policy_gate_address", 512)
        _text(self.policy_audit_address, "policy_audit_address", 512)
        ...
        if (
            ...
            raise ValidationError("packet-diff policy packet must contain exactly five members")
        if tuple(item.ordinal for item in self.members) != tuple(range(5)):
            raise ValidationError("packet-diff policy packet members must use canonical ordinals")
        if len({item.relative_path for item in self.members}) != len(self.members):
            raise ValidationError("packet-diff policy packet member paths must be unique")
        if (
            not isinstance(self.packet_bytes, bytes)
            or len(self.packet_bytes) != self.packet_byte_count
        ):
            raise ValidationError(
                "packet-diff policy packet bytes do not conserve packet_byte_count"
            )
        _text(self.content_address, "content_address", 512)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "packet_diff_address": self.packet_diff_address,
            "policy_gate_address": self.policy_gate_address,
            "policy_audit_address": self.policy_audit_address,
...
def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_packet(
    ...
    return content_hash(
        body,
        prefix="module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-packet",
    )


@dataclass(frozen=True, slots=True)
class ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketVerification:
    """Verification receipt that remains useful for blocked gate evidence."""
    ...
    def __post_init__(self) -> None:
        ...
        if self.present_count + self.missing_count != self.entry_count:
            raise ValidationError("packet-diff policy packet entry counts do not conserve")
        if (
            not self.checks
            or len(self.checks)
            > MODULE_WORKBENCH_RELEASE_BUNDLE_CATALOG_DIFF_POLICY_SET_PACKET_DIFF_POLICY_PACKET_MAX_CHECKS
        ):
            raise ValidationError("packet-diff policy packet checks are missing or excessive")
        ids = tuple(item.check_id for item in self.checks)
        if ids != tuple(sorted(ids)) or len(ids) != len(set(ids)):
            raise ValidationError("packet-diff policy packet checks must be sorted and unique")
        if not isinstance(self.accepted, bool):
            raise ValidationError("packet-diff policy packet acceptance must be boolean")
...
def address_module_workbench_release_bundle_catalog_diff_policy_set_packet_diff_policy_packet_verification(
    value: ModuleWorkbenchReleaseBundleCatalogDiffPolicySetPacketDiffPolicyPacketVerification,
) -> str:
    body = value.to_dict()
    body.pop("content_address", None)
    return content_hash(
        body,
        prefix="module-workbench-release-bundle-catalog-diff-policy-set-packet-diff-policy-packet-verification",
    )
