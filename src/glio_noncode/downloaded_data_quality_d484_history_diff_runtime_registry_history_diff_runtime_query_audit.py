"""Independent replay audit for D484 runtime queries."""
...
from . import downloaded_data_quality_d484_history_diff_runtime_registry_history_diff_runtime_query as query_model
...
def audit_query(value: query_model.RuntimeQuery, runtime: runtime_model.HistoryDiffRuntime) -> QueryAudit:
    query_model.verify_query(value); runtime_model.verify_runtime(runtime); all_rows = query_model._rows(runtime); expected = tuple(item for item in all_rows if item.resource in value.resources and (not value.state_filter or item.state == value.state_filter) and (value.passed_filter is None or item.passed == value.passed_filter) and (not value.severity_filter or item.severity == value.severity_filter) and (not value.text_filter or value.text_filter.casefold() in item.text.casefold())); page = expected[value.offset:value.offset + value.limit]
    checks = (_finding(1, "typed_query", isinstance(value, query_model.RuntimeQuery), type(value).__name__, "RuntimeQuery", "the audited value must use the typed query model"), _finding(2, "runtime_identity", (value.runtime_id, value.runtime_address) == (runtime.runtime_id, runtime.content_address), (value.runtime_id, value.runtime_address), (runtime.runtime_id, runtime.content_address), "query identity must point to the requested runtime"), _finding(3, "resource_selection", bool(value.resources) and len(set(value.resources)) == len(value.resources) and all(item in query_model.RESOURCES for item in value.resources), value.resources, query_model.RESOURCES, "resource selection must be bounded and known"), _finding(4, "filter_replay", all((not value.state_filter or item.state == value.state_filter) and (value.passed_filter is None or item.passed == value.passed_filter) and (not value.severity_filter or item.severity == value.severity_filter) and (not value.text_filter or value.text_filter.casefold() in item.text.casefold()) for item in value.rows), True, "all filters", "returned rows must satisfy every filter"), _finding(5, "offset_bounds", 0 <= value.offset <= query_model.MAX_ROWS, value.offset, query_model.MAX_ROWS, "offset must remain bounded"), _finding(6, "limit_bounds", 1 <= value.limit <= query_model.MAX_LIMIT, value.limit, query_model.MAX_LIMIT, "limit must remain bounded"), _finding(7, "count_replay", (value.total_count, value.returned_count, value.truncated) == (len(expected), len(page), value.offset + len(page) < len(expected)), (value.total_count, value.returned_count, value.truncated), "replayed counts", "query counters must derive from filtered runtime rows"), _finding(8, "row_order", tuple(item.ordinal for item in value.rows) == tuple(range(value.offset + 1, value.offset + value.returned_count + 1)), [item.ordinal for item in value.rows], "contiguous page ordinals", "rows must preserve source order"), _finding(9, "row_addresses", all(query_model.address_row(item) == item.content_address for item in value.rows), True, "replayed row addresses", "every returned row must be independently addressed"), _finding(10, "runtime_address", value.runtime_address == runtime.content_address, value.runtime_address, runtime.content_address, "query runtime address must match source runtime"), _finding(11, "public_boundary", _public(value.to_dict()), True, "public value", "queries must contain no source paths or private records"), _finding(12, "canonical_query", canonical_json(value.to_dict()) == canonical_json(query_model.query_from_mapping(_strict_json_loads(query_model.query_json(value))).to_dict()), True, "canonical round-trip", "query serialization must be deterministic")); provisional = QueryAudit(value.query_id, value.runtime_id, value.content_address, checks, len(checks), sum(item.passed for item in checks), sum(not item.passed for item in checks), all(item.passed for item in checks), f"pending:{AUDIT_PREFIX}"); return _seal(provisional, address_audit)
