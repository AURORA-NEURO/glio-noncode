...
def write_package(value: RegistryFederationConsensusGatePackage, directory: str | Path, *, overwrite: bool = False) -> Path:
    ...
    try:
        _validate_parent(destination.parent, "gate package destination")
        if destination.is_symlink():
            raise ValidationError("gate package destination cannot be a symlink")
        ...
        raise ValidationError("gate package destination could not be prepared") from error
    ...
    try:
        ...
        for name, raw in package_bytes(value).items():
            atomic_write_bytes(staging / name, raw, field=f"gate package staging artifact {name}")
...
def load_package(directory: str | Path) -> RegistryFederationConsensusGatePackage:
    try:
        ...
        _validate_parent(source.parent, "gate package input")
    ...
    if tuple(sorted(path.name for path in members)) != tuple(sorted(FILES)) or any(path.is_symlink() or not path.is_file() for path in members):
        raise ValidationError("gate package directory does not contain exact canonical members")
    try:
        raw = {name: read_bytes(source / name, field=f"gate package member {name}") for name in FILES}
    except ValidationError:
        raise
    except OSError as error:
