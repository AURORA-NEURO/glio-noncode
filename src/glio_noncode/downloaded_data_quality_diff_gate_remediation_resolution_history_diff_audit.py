...
VERSION = "downloaded-data-quality-diff-gate-remediation-resolution-history-diff-audit-v1"
BOUNDARY = "public_downloaded_data_quality_diff_gate_remediation_resolution_history_diff_audit"
AUDIT_PREFIX = "glio-noncode-download-quality-diff-gate-remediation-resolution-history-diff-audit"
...
class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck:
    FIELDS = CHECK_FIELDS
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck:
        value = _mapping(value, "history diff audit check")
        _strict(value, set(cls.FIELDS), "history diff audit check")
        return cls(*(value[field] for field in cls.FIELDS))


def address_check(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck):
        raise ValidationError("history diff audit check address requires a typed check")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=CHECK_PREFIX)


class DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit:
    FIELDS = AUDIT_FIELDS

    def __init__(self, diff_address: str, checks: Sequence[DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck | Mapping[str, Any]], check_count: int, passed_count: int, failed_count: int, accepted: bool, content_address: str) -> None:
        self.diff_address = _address(diff_address, "history diff audit diff address", diff_model.DIFF_PREFIX)
        self.checks = tuple(item if isinstance(item, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck) else DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck.from_mapping(item) for item in _sequence(checks, "history diff audit checks", MAX_CHECKS))
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit:
        value = _mapping(value, "history diff audit")
        _strict(value, set(cls.FIELDS), "history diff audit")
        return cls(*(value[field] for field in cls.FIELDS))


def address_audit(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit) -> str:
    if not isinstance(value, DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit):
        raise ValidationError("history diff audit address requires a typed audit")
    return content_hash(value.to_dict() | {"content_address": None}, prefix=AUDIT_PREFIX)


def _check(ordinal: int, check_id: str, passed: bool, detail: str, evidence: Sequence[str]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck:
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck(**body)
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck(**(body | {"content_address": address_check(provisional)}))


def _expected_attributes(item: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffItem) -> tuple[str, ...]:
...
def _direction(value: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff) -> str:
...
def audit_diff(value: diff_model.DownloadedDataQualityDiffGateRemediationResolutionHistoryDiff) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit:
    ...
    provisional = DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit(**body)
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit(**(body | {"content_address": address_audit(provisional)}))


def audit_from_mapping(value: Mapping[str, Any]) -> DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit:
    return DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit.from_mapping(value)


def audit_json(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit) -> str:
    return canonical_json(audit_from_mapping(value.to_dict()).to_dict())


def audit_csv(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit) -> str:
...
def render_audit_markdown(value: DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit) -> str:
    value = audit_from_mapping(value.to_dict())
...
def check_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data quality diff gate remediation resolution history diff audit check", "type": "object", "additionalProperties": False, "required": list(CHECK_FIELDS), "properties": {"ordinal": {"type": "integer", "minimum": 1}, "check_id": {"enum": list(CHECK_IDS)}, "passed": {"type": "boolean"}, "detail": {"type": "string"}, "evidence_addresses": {"type": "array", "items": {"type": "string"}, "minItems": 1, "maxItems": 8}, "content_address": {"type": "string"}}}


def audit_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "Downloaded data quality diff gate remediation resolution history diff audit", "type": "object", "additionalProperties": False, "required": list(AUDIT_FIELDS), "properties": {"diff_address": {"type": "string"}, "checks": {"type": "array", "items": check_schema(), "minItems": MAX_CHECKS, "maxItems": MAX_CHECKS}, "check_count": {"type": "integer", "minimum": MAX_CHECKS, "maximum": MAX_CHECKS}, "passed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "failed_count": {"type": "integer", "minimum": 0, "maximum": MAX_CHECKS}, "accepted": {"type": "boolean"}, "content_address": {"type": "string"}}}
...
__all__ = ["AUDIT_FIELDS", "AUDIT_PREFIX", "BOUNDARY", "CHECK_FIELDS", "CHECK_IDS", "CHECK_PREFIX", "MAX_CHECKS", "VERSION", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAudit", "DownloadedDataQualityDiffGateRemediationResolutionHistoryDiffAuditCheck", "address_audit", "address_check", "audit_csv", "audit_diff", "audit_from_mapping", "audit_json", "audit_schema", "capabilities", "check_schema", "render_audit_markdown"]
