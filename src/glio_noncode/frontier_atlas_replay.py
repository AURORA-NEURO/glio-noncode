"""Deterministic replay floors for Domain 05 C13-C16."""
...
class FrontierAtlasReplayExpectation:
    fixture_id: str
    fixture_version: str
    context_key: str
...
@dataclass(frozen=True, slots=True)
class FrontierAtlasReplayCheck:
    check_id: str
    passed: bool
    detail: str
    content_address: str
...
@dataclass(frozen=True, slots=True)
class FrontierAtlasReplayReport:
    expectation: FrontierAtlasReplayExpectation
    current_evaluation_address: str
    checks: tuple[FrontierAtlasReplayCheck, ...]
    content_address: str
...
def _check(check_id: str, passed: bool, detail: str) -> FrontierAtlasReplayCheck:
    body = {"check_id": check_id, "passed": passed, "detail": detail}
    return FrontierAtlasReplayCheck(check_id, passed, detail, content_hash(body))


def build_frontier_atlas_expectation(
    ...
    return FrontierAtlasReplayExpectation(**body, content_address=content_hash(body))


def replay_frontier_atlas_evaluation(
    evaluation: FrontierAtlasEvaluationReport, *, fixture: FrontierAtlasFixture | None = None
) -> FrontierAtlasReplayReport:
    """Re-execute and compare identity, state, issue, role, and sanitation floors."""

    selected = fixture or default_frontier_atlas_fixture()
    expectation = build_frontier_atlas_expectation(evaluation)
    current = evaluate_frontier_atlas_fixture(selected)
    checks: list[FrontierAtlasReplayCheck] = []
    ...
    add(
        "context",
        current.context_key == FRONTIER_ATLAS_CONTEXT_KEY == expectation.context_key,
        "exact frontier atlas context is replayed",
    )
    add(
        "record-order",
    ...
    add(
        ...
        all(
            ...
            for item in current.receipts
            if item.role is FrontierAtlasRole.CONTROL
        ),
    ...
    return FrontierAtlasReplayReport(
