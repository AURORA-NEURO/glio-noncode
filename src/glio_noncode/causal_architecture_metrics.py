"""D11 coverage and evidence metrics."""
...
def causal_architecture_metrics(
    ...
    return metrics | {"content_address": addressed(metrics, "causal-metrics")}


def causal_architecture_metric_invariants(metrics: dict[str, Any]) -> tuple[str, ...]:
    return tuple(
        key
        for key, expected in (
            ("source_count", 20),
...
__all__ = ["causal_architecture_metric_invariants", "causal_architecture_metrics"]
