"""Deep fixture diff receipt for workbench release review.
...
class WorkbenchReleaseFixtureDiff:
...
def build_workbench_release_fixture_diff(values: Iterable[Any] = (), *, require_non_empty: bool = True) -> WorkbenchReleaseFixtureDiff:
    rows = tuple(
        {
            "sequence": index,
    ...
    return WorkbenchReleaseFixtureDiff(**body, content_address=content_hash(body))


def validate_workbench_release_fixture_diff(receipt: WorkbenchReleaseFixtureDiff) -> bool:
...
__all__ = ["WorkbenchReleaseFixtureDiff", "build_workbench_release_fixture_diff", "validate_workbench_release_fixture_diff"]
