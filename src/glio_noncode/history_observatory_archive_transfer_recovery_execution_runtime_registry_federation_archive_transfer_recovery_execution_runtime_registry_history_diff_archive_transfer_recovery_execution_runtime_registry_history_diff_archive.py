...
DEFAULT_ARCHIVE_ID = "comparison-query-snapshot-registry-history-observatory-archive-transfer-recovery-execution-runtime-registry-federation-history-diff-archive"
...
def _embedded_diff(value: diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff) -> dict[str, bytes]:
    checked = diff_model.verify_diff(value)
    items = diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiffItems(checked.items, diff_model.address_items(checked.items))
    documents = {
...
class RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact:
    ...
    @classmethod
    def from_mapping(cls, value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact:
...
class RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    ...
    def __init__(
        ...
        artifact_count: int,
        files: Sequence[str],
        artifacts: Sequence[RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact | Mapping[str, Any]],
        archive_size: int,
        content_address: str,
        raw: Mapping[str, bytes] | None = None,
        diff: diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff | None = None,
    ) -> None:
        ...
        self.artifacts = tuple(item if isinstance(item, RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact) else RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact.from_mapping(item) for item in _sequence(artifacts, "history diff archive artifacts", len(EMBEDDED_FILES)))
    ...
    @property
    def diff(self) -> diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff | None:
        return self._diff


def address_archive(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> str:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffArchive):
...
def _artifact(index: int, name: str, raw: bytes) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact:
    return RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact(index, name, len(raw), hash_bytes(raw, prefix=ARTIFACT_PREFIX))


def build_archive(value: diff_model.HistoryDiffArchiveTransferRecoveryExecutionRuntimeRegistryHistoryDiff, *, archive_id: str = DEFAULT_ARCHIVE_ID) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    checked = diff_model.verify_diff(value)
    ...
    provisional = RecoveryExecutionRuntimeRegistryHistoryDiffArchive(**body, content_address="pending:archive", raw=raw, diff=checked)
    content_address = address_archive(provisional)
    candidate = RecoveryExecutionRuntimeRegistryHistoryDiffArchive(**body, content_address=content_address, raw=raw, diff=checked)
    archive_size = len(_zip_bytes_unchecked(candidate))
    return RecoveryExecutionRuntimeRegistryHistoryDiffArchive(**(body | {"archive_size": archive_size}), content_address=content_address, raw=raw, diff=checked)


def build_archive_from_directory(directory: str | Path, *, archive_id: str = DEFAULT_ARCHIVE_ID) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    return build_archive(diff_model.load_diff(directory), archive_id=archive_id)


def archive_from_mapping(value: Mapping[str, Any]) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    value = _mapping(value, "history diff archive")
    _strict(value, set(RecoveryExecutionRuntimeRegistryHistoryDiffArchive.FIELDS), "history diff archive")
    artifacts = tuple(RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact.from_mapping(item) for item in _sequence(value["artifacts"], "history diff archive artifacts", len(EMBEDDED_FILES)))
    result = RecoveryExecutionRuntimeRegistryHistoryDiffArchive(value["archive_id"], value["version"], value["boundary"], value["diff_id"], value["diff_address"], value["artifact_count"], value["files"], artifacts, value["archive_size"], value["content_address"])
    return verify_archive(result)


def verify_archive(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    if not isinstance(value, RecoveryExecutionRuntimeRegistryHistoryDiffArchive):
...
def _archive_manifest(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> dict[str, Any]:
    body = {
        "version": VERSION,
        "boundary": BOUNDARY,
...
def manifest_document(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> dict[str, Any]:
    return _archive_manifest(verify_archive(value))


def _zip_bytes_unchecked(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> bytes:
...
def archive_bytes(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> bytes:
...
def archive_json(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> str:
...
def load_archive(source: str | Path | bytes) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    ...
    artifacts = tuple(RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact.from_mapping(item) for item in _sequence(manifest["artifacts"], "history diff archive artifacts", len(EMBEDDED_FILES)))
    value = RecoveryExecutionRuntimeRegistryHistoryDiffArchive(manifest["archive_id"], manifest["version"], manifest["boundary"], manifest["diff_id"], manifest["diff_address"], manifest["artifact_count"], manifest["files"], artifacts, physical_size, manifest["archive_address"], raw={name: raw[name] for name in EMBEDDED_FILES}, diff=diff)
...
def load_archive_bytes(raw: bytes) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    return load_archive(raw)


def verify_archive_file(source: str | Path) -> RecoveryExecutionRuntimeRegistryHistoryDiffArchive:
    return load_archive(source)


def archive_csv(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> str:
    value = verify_archive(value)
...
def render_archive_markdown(value: RecoveryExecutionRuntimeRegistryHistoryDiffArchive) -> str:
    value = verify_archive(value)
    lines = [
...
def artifact_schema() -> dict[str, Any]:
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "FederationRuntimeRegistryHistoryDiffArchiveArtifact", "type": "object", "additionalProperties": False, "required": list(RecoveryExecutionRuntimeRegistryHistoryDiffArchiveArtifact.FIELDS), "properties": {"index": {"type": "integer", "minimum": 0, "maximum": len(EMBEDDED_FILES) - 1}, "name": {"type": "string", "enum": list(EMBEDDED_FILES)}, "size": {"type": "integer", "minimum": 1}, "hash": {"type": "string", "pattern": "^" + ARTIFACT_PREFIX + ":"}}}
...
def archive_schema() -> dict[str, Any]:
    ...
    return {"$schema": "https://json-schema.org/draft/2020-12/schema", "title": "FederationRuntimeRegistryHistoryDiffArchive", "type": "object", "additionalProperties": False, "required": list(RecoveryExecutionRuntimeRegistryHistoryDiffArchive.FIELDS), "properties": fields}
