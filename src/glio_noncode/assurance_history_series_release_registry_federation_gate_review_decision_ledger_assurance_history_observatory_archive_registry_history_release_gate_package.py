...
def _write_atomic_directory(destination: Path, payload: Mapping[str, bytes], *, overwrite: bool) -> Path:
    try:
        _validate_parent(destination.parent, "registry history release gate package destination")
        if destination.is_symlink():
            raise ValidationError("registry history release gate package destination cannot be a symlink")
        ...
        raise ValidationError("registry history release gate package destination could not be prepared") from error
    temporary = Path(tempfile.mkdtemp(prefix=".gnd-history-release-gate-", dir=str(destination.parent)))
    try:
        for name in FILES:
            atomic_write_bytes(temporary / name, payload[name], field=f"registry history release gate package staging artifact {name}")
        if destination.exists():
...
def _read_directory(source: str | Path) -> dict[str, bytes]:
    try:
        ...
        _validate_parent(directory.parent, "registry history release gate package input")
        ...
        raise ValidationError("registry history release gate package input directory could not be inspected") from error
    try:
        return {name: read_bytes(directory / name, field=f"registry history release gate package member {name}") for name in FILES}
    except ValidationError:
        raise
    except OSError as error:
        raise ValidationError("registry history release gate package artifact could not be read") from error
