"""Typed contracts for portable review-packet catalog diff policy transports."""
...
VERSION = "downloaded-data-review-packet-catalog-diff-policy-transport-v1"
BOUNDARY = "public_downloaded_data_review_packet_catalog_diff_policy_transport"
PACKAGE_PREFIX = "glio-noncode-review-catalog-diff-policy-transport"
...
class DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketMember:
    ...
    def __post_init__(self) -> None:
        if self.relative_path not in PAYLOAD_NAMES:
            raise ValidationError("catalog diff policy transport member path is unsupported")
        if MEDIA_TYPES[self.relative_path] != self.media_type:
            raise ValidationError("catalog diff policy transport member media type does not replay")
        _count(self.byte_count, "catalog diff policy transport member byte count", MAX_MEMBER_BYTES)
        _address(self.byte_address, "catalog diff policy transport member byte address", MEMBER_PREFIX + "-bytes")
        _address(self.content_address, "catalog diff policy transport member content address", MEMBER_PREFIX)
...
def address_member(value: DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketMember) -> str:
    if not isinstance(value, DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketMember):
        raise ValidationError("catalog diff policy transport member addressing requires its typed contract")
...
def address_member_bytes(payload: bytes) -> str:
    if not isinstance(payload, bytes) or len(payload) > MAX_MEMBER_BYTES:
        raise ValidationError("catalog diff policy transport member payload exceeds its byte bound")
...
class DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketManifest:
    ...
    def __post_init__(self) -> None:
        _label(self.package_id, "catalog diff policy transport ID")
        if self.version != VERSION or self.boundary != BOUNDARY or self.package_format != PACKAGE_FORMAT:
            raise ValidationError("catalog diff policy transport identity is not current")
        _address(self.diff_address, "catalog diff policy transport diff address")
        _address(self.policy_address, "catalog diff policy transport policy address")
        _address(self.audit_address, "catalog diff policy transport audit address")
        if self.policy_state not in {"ready", "blocked"} or not isinstance(self.policy_accepted, bool) or not isinstance(self.audit_accepted, bool):
            raise ValidationError("catalog diff policy transport decision flags are invalid")
        if tuple(item.relative_path for item in self.members) != PAYLOAD_NAMES:
            raise ValidationError("catalog diff policy transport payload order is not canonical")
        _address(self.content_address, "catalog diff policy transport manifest address", MANIFEST_PREFIX)
...
def address_manifest(value: DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketManifest) -> str:
    if not isinstance(value, DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacketManifest):
        raise ValidationError("catalog diff policy transport manifest addressing requires its typed contract")
...
class DownloadedDataReviewPacketDiffPolicyReleaseCertificateBundleDiffPolicyPackageCatalogDiffPolicyReviewPacket:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.package_bytes, bytes) or len(self.package_bytes) > MAX_PACKAGE_BYTES:
            raise ValidationError("catalog diff policy transport bytes exceed their bound")
        _address(self.package_address, "catalog diff policy transport address", PACKAGE_PREFIX)
        if self.manifest.content_address.endswith(":pending"):
            raise ValidationError("catalog diff policy transport manifest must be addressed")
