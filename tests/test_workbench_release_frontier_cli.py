...
class WorkbenchReleaseFrontierCliTests(unittest.TestCase):
    ...
    def test_data_audit(self) -> None:
        result = self.run_command("workbench-release-frontier-data-audit")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertTrue(json.loads(result.stdout)["accepted"])

    def test_evaluation(self) -> None:
        result = self.run_command("workbench-release-frontier-evaluate")
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_pipeline(self) -> None:
        result = self.run_command("workbench-release-frontier-pipeline")
        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertTrue(payload["accepted"])
        self.assertEqual(len(payload["stages"]), 49)

    def test_review_csv(self) -> None:
        result = self.run_command("workbench-release-frontier-review-csv")
        self.assertEqual(result.returncode, 0, result.stderr)
