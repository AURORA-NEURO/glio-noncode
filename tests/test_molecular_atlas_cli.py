...
class MolecularAtlasCliTests(unittest.TestCase):
    def setUp(self) -> None:
        self.example = Path("examples/molecular-atlas-public-aggregate.json")
        self.pipeline_example = Path("examples/molecular-atlas-pipeline-accepted.json")
    ...
    def test_contracts_and_metrics_commands(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            directory = Path(directory_name)
            contracts = self._run(["molecular-atlas-contracts"], directory)
            self.assertEqual(len(contracts["contracts"]), 4)
            metrics = self._run(["molecular-atlas-metrics", str(self.example)], directory)
        ...
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            for output_format, suffix in (("json", ".json"), ("csv", ".csv"), ("markdown", ".md")):
                ...
                if output_format == "markdown":
                    self.assertIn("# Molecular atlas bundle", text)

    def test_lineage_and_reconciliation_commands(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            directory = Path(directory_name)
            lineage = self._run(["molecular-atlas-lineage", str(self.example)], directory)
            self.assertTrue(lineage["accepted"])
            self.assertEqual(lineage["audit"]["node_count"], 157)
            self.assertEqual(lineage["audit"]["edge_count"], 158)
            reconciliation = self._run(
                ["molecular-atlas-reconciliation", str(self.example)], directory
            )
