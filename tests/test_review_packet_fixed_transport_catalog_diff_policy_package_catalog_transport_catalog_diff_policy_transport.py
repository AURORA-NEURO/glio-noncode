"""Deep contracts for longitudinal package-catalog policy handoff transports."""
...
class FixedTransportCatalogDiffPolicyTransportTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.diff_model = diff573
        self.policy_model = policy574
        self.policy_audit_model = policy_audit574
        self.transport = _local_module("*_tcp_tr_cat_diff_pol_tr_cat_diff_pol_tr.py")
        self.transport_audit = _local_module("*_tcp_tr_cat_diff_pol_tr_cat_diff_pol_tr_aud.py")

        source = legacy_diff.catalog_model.build_catalog((), catalog_id="module575-transport-source")
        source_diff = legacy_diff.build_diff(source, source, diff_id="module575-transport-source-diff")
        source_policy = policy566.release_policy(source_diff, policy_id="module575-transport-source-policy", require_ready=False)
        source_packages = {
            name: transport567.build_package(source_diff, source_policy, package_id=f"module575-source-{name}")
        ...
        left_inner = catalog568.build_catalog(
            ...
            catalog_id="module575-inner-catalog",
        )
        ...
        inner_diff = diff569.build_diff(left_inner, right_inner, diff_id="module575-inner-diff")
        inner_policy = policy570.release_fixed_transport_catalog_diff_policy_package_catalog_policy(inner_diff, policy_id="module575-inner-policy")
        ...
        inner_transports = {
            name: transport571.build_package(inner_diff, inner_policy, inner_audit, package_id=f"module575-inner-{name}")
            for name in ("unchanged", "changed-left", "changed-right", "removed", "added")
        }
        left_outer = catalog572.build_catalog(
            ...
            entry_ids=("entry-unchanged", "entry-changed", "entry-removed"),
            catalog_id="module575-outer-catalog",
        )
        ...
        self.diff = diff573.build_diff(left_outer, right_outer, diff_id="module575-outer-diff")
        self.policy = policy574.release_fixed_transport_catalog_diff_policy_package_catalog_policy(self.diff, policy_id="module575-transport-policy")
        self.policy_audit = policy_audit574.audit_fixed_transport_catalog_diff_policy_package_catalog_policy(self.policy, diff=self.diff)
        self.package = self.transport.build_package(self.diff, self.policy, self.policy_audit, package_id="module575-transport-package")

    def test_deterministic_round_trip_and_independent_audit(self) -> None:
        first = self.transport.build_package(self.diff, self.policy, self.policy_audit, package_id="module575-transport-package")
        second = self.transport.build_package(self.diff, self.policy, self.policy_audit, package_id="module575-transport-package")
    ...
    def test_cli_http_schema_and_capabilities(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_575_COMMAND
            ...
            thread.start()
            try:
                base = "/v1/downloaded-data/review-packet/diff/policy/release-"
                ...
                built = get(base, [("input", diff_path), ("profile", "release"), ("package_id", "module575-http"), ("destination", package_path), ("overwrite", "true")])
