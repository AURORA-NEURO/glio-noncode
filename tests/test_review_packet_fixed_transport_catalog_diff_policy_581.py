"""Deep contracts for module-581 longitudinal catalog comparisons."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_581_aud as audit_model
...
def _entry(entry_id: str, *, state: str = "ready", package_byte_count: int = 100) -> Entry:
    ...
    body = {
        "entry_id": entry_id,
        "package_id": "package-" + entry_id,
        "package_address": PACKAGE_PREFIX + ":" + f"{seed:064x}",
        "manifest_address": "manifest:" + f"{seed + 10:064x}",
        "diff_address": "diff:" + f"{seed + 20:064x}",
        "policy_address": "policy:" + f"{seed + 30:064x}",
        "audit_address": "audit:" + f"{seed + 40:064x}",
        "policy_state": state,
        "policy_accepted": state == "ready",
        "audit_accepted": True,
...
def _catalog(entries: tuple[Entry, ...], catalog_id: str = "module581-catalog") -> Catalog:
    ...
    body = {
        ...
        "boundary": catalog_model.BOUNDARY,
        "entry_count": len(entries),
        "accepted_count": sum(item.audit_accepted for item in entries),
        "ready_count": ready_count,
        "blocked_count": blocked_count,
        "state": "empty" if not entries else "ready" if ready_count == len(entries) else "blocked" if blocked_count == len(entries) else "mixed",
        "entries": entries,
        "content_address": CATALOG_PREFIX + ":pending",
    }
    provisional = Catalog(**body)
    return Catalog(**(body | {"content_address": address_catalog(provisional)}))


class FixedTransportCatalogDiffPolicyTransportCatalogDiff581Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        self.value = diff_model.build_diff(self.left, self.right, diff_id="module581-diff")

    def test_deterministic_classification_transition_and_direction(self) -> None:
        repeated = diff_model.build_diff(self.left, self.right, diff_id="module581-diff")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_581_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(left_path), str(right_path), "--diff-id", "cli-module581", "--destination", str(diff_path), "--format", "summary"]), 0)
            ...
            thread.start()
            try:
                base = "/v1/downloaded-data/review-packet/diff/policy/release-"
                ...
                built = get(base, [("left", left_path), ("right", right_path), ("diff_id", "http-module581"), ("destination", diff_path), ("overwrite", "true")])
                ...
                self.assertEqual((audited["check_count"], audited["failed_count"], audited["accepted"]), (14, 0, True))
