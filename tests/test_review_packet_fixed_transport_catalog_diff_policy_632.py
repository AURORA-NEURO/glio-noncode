"""Deep contracts for module632 policy-decision transport catalogs."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_628 as catalog_seed_model
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_631 as transport_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_632 as catalog_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_632_aud as audit_model
...
def _catalog(entries: tuple[Entry, ...], catalog_id: str = "module632-seed") -> Catalog:
...
class FixedTransportCatalogDiffPolicyCatalog632Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        diff = diff_model.build_diff(left, right, diff_id="module632-diff")
        release = policy_model.release_policy(diff, policy_id="module632-release")
        strict = policy_model.strict_policy(diff, policy_id="module632-strict")
        ...
        self.release_package = transport_model.build_package(diff, release, release_audit, package_id="module632-release")
        self.strict_package = transport_model.build_package(diff, strict, strict_audit, package_id="module632-strict")
        self.value = catalog_model.build_catalog((self.release_package, self.strict_package), entry_ids=("release", "strict"), catalog_id="module632-catalog")

    def test_mixed_fold_lineage_and_independent_audit(self) -> None:
        repeated = catalog_model.build_catalog((self.release_package, self.strict_package), entry_ids=("release", "strict"), catalog_id="module632-catalog")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_632_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(release_path), str(strict_path), "--entry-id", "release", "--entry-id", "strict", "--catalog-id", "cli-module632", "--destination", str(catalog_path)]), 0)
            ...
            try:
                ...
                base = policy_base + "" + "/diff" + "/policy" + "/transport" + "/catalog"
                ...
                built = get(base, [("package", release_path), ("package", strict_path), ("entry_id", "release"), ("entry_id", "strict"), ("catalog_id", "http-module632"), ("destination", catalog_path), ("overwrite", "true")])
