"""Deep contracts for policy gates over fixed transport package-catalog diffs."""
...
class FixedTransportPackageCatalogPolicyTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.audit_model = _local_module("*_tcp_tr_cat_diff_pol_aud.py")
        source = legacy_diff.catalog_model.build_catalog((), catalog_id="module570-policy-source")
        source_diff = legacy_diff.build_diff(source, source, diff_id="module570-policy-source-diff")
        release = policy566.release_policy(source_diff, policy_id="module570-policy-source-policy", require_ready=False)
        packages = {
            name: transport567.build_package(source_diff, release, package_id=f"module570-policy-{name}")
            for name in ("unchanged", "changed-left", "changed-right", "removed", "added")
        }
        self.left = catalog568.build_catalog(
            ...
            catalog_id="module570-policy-catalog",
        )
        self.right = catalog568.build_catalog(
        ...
        self.diff = diff569.build_diff(self.left, self.right, diff_id="module570-policy-diff")

    def test_strict_release_and_independent_audit(self) -> None:
        strict = self.policy_model.strict_fixed_transport_catalog_diff_policy_package_catalog_policy(self.diff, policy_id="module570-strict")
        release = self.policy_model.release_fixed_transport_catalog_diff_policy_package_catalog_policy(self.diff, policy_id="module570-release")
    ...
    def test_persistence_tamper_and_public_boundary(self) -> None:
        policy = self.policy_model.release_fixed_transport_catalog_diff_policy_package_catalog_policy(self.diff, policy_id="module570-persistence")
    ...
    def test_cli_http_schema_and_capabilities(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            diff_path, policy_path, audit_path = root / "diff.json", root / "policy.json", root / "audit.json"
            self.diff_model.write_diff(self.diff, diff_path)
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_570_COMMAND
            ...
            thread.start()
            try:
                base = "/v1/downloaded-data/review-packet/diff/policy/release-"
