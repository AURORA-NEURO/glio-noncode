"""Focused regression coverage for the D266 registry history layer."""
...
class D266RegistryHistoryTests(unittest.TestCase):
    def test_history_replays_append_query_and_head_guards(self) -> None:
        ...
        source_registry = source_registry_model.build_registry((), registry_id="d266-source-registry")
        left = source_history_model.build_history(source_registry, history_id="d266-source-history", snapshot_id="baseline")
        right = source_history_model.build_history(source_registry, history_id="d266-source-history", snapshot_id="candidate")
        diff = runtime_model.diff_model.build_diff(left, right, diff_id="d266-diff")
        ready_runtime = runtime_model.build_runtime(diff, runtime_id="d266-ready-runtime", policy=runtime_model.build_policy("d266-ready-policy", diff.diff_id, maximum_changed=1))
        blocked_runtime = runtime_model.build_runtime(diff, runtime_id="d266-blocked-runtime", policy=runtime_model.build_policy("d266-blocked-policy", diff.diff_id, maximum_changed=0))
        blocked_registry = registry_model.build_registry((blocked_runtime,), registry_id="d266-registry")
        ready_registry = registry_model.build_registry((ready_runtime,), registry_id="d266-registry")
        history = history_model.build_history(blocked_registry, history_id="d266-history", snapshot_id="blocked")
