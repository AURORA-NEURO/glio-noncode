"""Deep contracts for module597 longitudinal module596 catalog diffs."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_597_aud as audit_model
...
def _catalog(entries: tuple[Entry, ...], catalog_id: str = "module597-catalog") -> Catalog:
...
class FixedTransportCatalogDiffPolicyTransportCatalogDiff597Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        self.value = diff_model.build_diff(self.left, self.right, diff_id="module597-diff")

    def test_deterministic_classification_transition_and_direction(self) -> None:
        repeated = diff_model.build_diff(self.left, self.right, diff_id="module597-diff")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_597_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(left_path), str(right_path), "--diff-id", "cli-module597", "--destination", str(diff_path), "--format", "summary"]), 0)
            ...
            try:
                ...
                base = policy_base + "/transport/catalog/diff/policy/transport/catalog/diff/policy/transport/catalog/diff/policy/transport/catalog/diff"
                ...
                built = get(base, [("left", left_path), ("right", right_path), ("diff_id", "http-module597"), ("destination", diff_path), ("overwrite", "true")])
