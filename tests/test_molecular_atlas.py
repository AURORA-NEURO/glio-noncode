...
class MolecularAtlasQualityRuntimeReleaseTests(unittest.TestCase):
    ...
    def test_runtime_request_rejects_coerced_flags_and_symlinked_file(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            invalid.write_text(
                json.dumps({"fixture": "default_molecular_atlas_fixture", "accepted_only": "false"}),
                encoding="utf-8",
            )
            with self.assertRaises(ValidationError):
                MolecularAtlasPipelineRequest.from_file(invalid)
            target = root / "target.json"
            target.write_text(
                json.dumps({"fixture": "default_molecular_atlas_fixture"}), encoding="utf-8"
            )
            ...
            with self.assertRaises(ValidationError):
                MolecularAtlasPipelineRequest.from_file(link)
