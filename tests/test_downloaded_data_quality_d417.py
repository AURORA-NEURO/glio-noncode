"""Focused regression coverage for the D417 runtime registry layer."""
...
from glio_noncode import downloaded_data_quality_d417_runtime_registry_audit as audit_model
from glio_noncode import downloaded_data_quality_d417_runtime_registry_query as query_model
from glio_noncode import downloaded_data_quality_d417_runtime_registry_query_audit as query_audit_model
from glio_noncode.errors import ValidationError
from glio_noncode.serialization import canonical_json


class D417RuntimeRegistryTests(unittest.TestCase):
    def test_registry_replays_aggregate_audit_query_and_duplicate_guard(self) -> None:
        ...
        source_registry = source_registry_model.build_registry((), registry_id="d417-source-registry")
        left = source_history_model.build_history(source_registry, history_id="d417-source-history", snapshot_id="baseline")
        right = source_history_model.build_history(source_registry, history_id="d417-source-history", snapshot_id="candidate")
        diff = runtime_model.diff_model.build_diff(left, right, diff_id="d417-diff")
        ready = runtime_model.build_runtime(diff, runtime_id="d417-ready-runtime", policy=runtime_model.build_policy("d417-ready-policy", diff.diff_id, maximum_changed=1))
        blocked = runtime_model.build_runtime(diff, runtime_id="d417-blocked-runtime", policy=runtime_model.build_policy("d417-blocked-policy", diff.diff_id, maximum_changed=0))
        registry = registry_model.build_registry((blocked, ready), registry_id="d417-registry")
        ...
        with self.assertRaises(ValidationError):
            registry_model.build_registry((ready, ready), registry_id="d417-duplicate")
