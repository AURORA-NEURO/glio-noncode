"""Deep contracts for module630 strict and release gates over module629 diffs."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_630_aud as audit_model
...
def _catalog(entries: tuple[Entry, ...], catalog_id: str = "module630-catalog") -> Catalog:
...
class FixedTransportCatalogDiffPolicy630Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        self.diff = diff_model.build_diff(self.left, self.right, diff_id="module630-diff")

    def test_release_accepts_strict_retains_failures_and_audit_is_independent(self) -> None:
        release = policy_model.release_policy(self.diff, policy_id="release-module630")
        strict = policy_model.strict_policy(self.diff, policy_id="strict-module630")
    ...
    def test_persistence_tamper_and_custom_budget(self) -> None:
        value = policy_model.build_policy(self.diff, policy_id="custom-module630", maximum_added=1, maximum_removed=1, maximum_changed=1, maximum_total_changes=3, require_change=True)
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            diff_command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_629_COMMAND
            policy_command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_630_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([diff_command, str(left_path), str(right_path), "--diff-id", "cli-module630", "--destination", str(diff_path)]), 0)
            ...
            try:
                ...
                base = policy_base + "" + "/diff" + "/policy"
