"""Deep contracts for module667 policy-decision transports."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_667_aud as audit_model
...
def _catalog(entries: tuple[Entry, ...]) -> Catalog:
    ...
    body = {"catalog_id": "module667-catalog", "version": catalog_model.VERSION, "boundary": catalog_model.BOUNDARY, "entry_count": len(entries), "accepted_count": sum(item.audit_accepted for item in entries), "ready_count": ready_count, "blocked_count": blocked_count, "state": "empty" if not entries else "ready" if ready_count == len(entries) else "blocked" if blocked_count == len(entries) else "mixed", "entries": entries, "content_address": CATALOG_PREFIX + ":pending"}
    provisional = Catalog(**body)
    return Catalog(**(body | {"content_address": address_catalog(provisional)}))


class FixedTransportCatalogDiffPolicyTransport667Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        self.diff = diff_model.build_diff(left, right, diff_id="module667-diff")
        self.release = policy_model.release_policy(self.diff, policy_id="module667-release")
        self.strict = policy_model.strict_policy(self.diff, policy_id="module667-strict")
    ...
    def test_deterministic_nested_lineage_and_blocked_evidence(self) -> None:
        release = package_model.build_package(self.diff, self.release, self.release_audit, package_id="module667-release")
        repeated = package_model.build_package(self.diff, self.release, self.release_audit, package_id="module667-release")
        blocked = package_model.build_package(self.diff, self.strict, self.strict_audit, package_id="module667-strict")
    ...
    def test_persistence_tamper_and_public_boundary(self) -> None:
        package = package_model.build_package(self.diff, self.release, self.release_audit, package_id="module667-persist")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_667_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(diff_path), str(policy_path), "--audit", str(audit_path), "--package-id", "cli-module667", "--destination", str(package_path)]), 0)
            ...
            try:
                ...
                base = '/v1/downloaded-data/review-packet/diff/policy/release-'
                ...
                built = get(base, [("diff", diff_path), ("policy", policy_path), ("audit", audit_path), ("package_id", "http-module667"), ("destination", package_path), ("overwrite", "true")])
                self.assertEqual((built["policy_state"], built["audit_accepted"]), ("ready", True))
                loaded = get(base + "/load", [("input", package_path)])
                self.assertEqual(loaded["policy"]["policy_id"], "module667-release")
