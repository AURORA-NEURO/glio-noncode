...
from glio_noncode.module_fabric_catalog import default_module_fabric_catalog, validate_module_fabric_catalog
...
from glio_noncode.module_fabric_exports import export_module_fabric_review_csv, module_fabric_json, module_fabric_summary, render_module_fabric_review_markdown
from glio_noncode.module_fabric_failures import run_module_fabric_failure_injections
from glio_noncode.module_fabric_fixture_eval import evaluate_module_fabric_fixture, execute_module_fabric_record
from glio_noncode.module_fabric_governance import build_module_fabric_review_queue, default_module_fabric_claim_boundary
from glio_noncode.module_fabric_lineage import build_module_fabric_lineage, verify_module_fabric_lineage
from glio_noncode.module_fabric_metrics import measure_module_fabric, module_fabric_reference_rate
from glio_noncode.module_fabric_observability import build_module_fabric_trace, verify_module_fabric_trace
from glio_noncode.module_fabric_operations import evaluate_module_fabric_record
from glio_noncode.module_fabric_public_data import audit_module_fabric_data, default_module_fabric_fixture, load_module_fabric_fixture, module_fabric_fixture_json
from glio_noncode.module_fabric_quality_gate import run_module_fabric_quality_gate
from glio_noncode.module_fabric_release import build_module_fabric_release, verify_module_fabric_release
from glio_noncode.module_fabric_replay import replay_module_fabric
from glio_noncode.module_fabric_reports import module_fabric_report, render_module_fabric_runtime_markdown
...
from glio_noncode.module_fabric_schema import default_module_fabric_schema, validate_module_fabric_schema
from glio_noncode.module_fabric_source_registry import build_module_fabric_source_registry, source_registry_for
from glio_noncode.module_fabric_support import all_resolved, contains_private_key, parse_reference, reference_set_receipts, resolve_reference
...
class ModuleFabricTests(unittest.TestCase):
    ...
    def test_evaluation_is_accepted_with_eight_checks_per_record(self) -> None:
        ...
        self.assertEqual(len(self.evaluation.checks), 256)
    ...
    def test_runtime_is_ordered_and_accepted(self) -> None:
        ...
        self.assertEqual(tuple(item.ordinal for item in report.stages), tuple(range(1, 21)))
        trace = build_module_fabric_trace(report)
        self.assertTrue(trace.accepted)
        self.assertEqual(verify_module_fabric_trace(trace), ())
        self.assertEqual(module_fabric_report(report)["stage_count"], 20)
