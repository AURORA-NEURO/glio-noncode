...
class EditingDesignFrontierCliTests(unittest.TestCase):
    ...
    def test_data_audit(self) -> None:
        result = self.run_command("editing-design-frontier-data-audit")
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertTrue(json.loads(result.stdout)["accepted"])

    def test_evaluation(self) -> None:
        result = self.run_command("editing-design-frontier-evaluate")
        self.assertEqual(result.returncode, 0, result.stderr)
    ...
    def test_pipeline(self) -> None:
        result = self.run_command("editing-design-frontier-pipeline")
        self.assertEqual(result.returncode, 0, result.stderr)
        payload = json.loads(result.stdout)
        self.assertTrue(payload["accepted"])
    ...
    def test_review_csv(self) -> None:
        result = self.run_command("editing-design-frontier-review-csv")
        self.assertEqual(result.returncode, 0, result.stderr)
