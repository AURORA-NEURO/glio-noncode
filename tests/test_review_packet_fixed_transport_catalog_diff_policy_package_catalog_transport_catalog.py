"""Deep contracts for catalogs of fixed package-catalog policy transports."""
...
class FixedTransportPackageCatalogPolicyTransportCatalogTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.catalog_model = _local_module("*_tcp_tr_cat_diff_pol_tr_cat.py")
        self.audit_model = _local_module("*_tcp_tr_cat_diff_pol_tr_cat_aud.py")

        source = legacy_diff.catalog_model.build_catalog((), catalog_id="module572-catalog-source")
        source_diff = legacy_diff.build_diff(source, source, diff_id="module572-catalog-source-diff")
        source_policy = policy566.release_policy(source_diff, policy_id="module572-catalog-source-policy", require_ready=False)
        source_packages = {
            name: transport567.build_package(source_diff, source_policy, package_id=f"module572-source-{name}")
        ...
        left = catalog568.build_catalog(
            ...
            catalog_id="module572-package-catalog",
        )
        ...
        diff = diff569.build_diff(left, right, diff_id="module572-catalog-diff")
        policy = policy570.release_fixed_transport_catalog_diff_policy_package_catalog_policy(diff, policy_id="module572-catalog-policy")
        policy_audit = policy_audit570.audit_fixed_transport_catalog_diff_policy_package_catalog_policy(policy, diff=diff)
        self.transport_model = transport571
        self.package_a = transport571.build_package(diff, policy, policy_audit, package_id="module572-transport-a")
        self.package_b = transport571.build_package(diff, policy, policy_audit, package_id="module572-transport-b")
        self.catalog = self.catalog_model.build_catalog(
            ...
            catalog_id="module572-transport-catalog",
        )
    ...
    def test_persistence_tamper_duplicate_and_public_boundary_rejection(self) -> None:
        ...
        with self.assertRaises(ValidationError):
            self.catalog_model.build_catalog((self.package_a.package_bytes, self.package_a.package_bytes), catalog_id="module572-duplicate")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_572_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(package_b_path), str(package_a_path), "--entry-id", "entry-b", "--entry-id", "entry-a", "--catalog-id", "cli-module572", "--destination", str(catalog_path), "--format", "summary"]), 0)
            ...
            thread.start()
            try:
                base = "/v1/downloaded-data/review-packet/diff/policy/release-"
                ...
                built = get(base, [("package", package_b_path), ("package", package_a_path), ("entry_id", "entry-b"), ("entry_id", "entry-a"), ("catalog_id", "http-module572")])
