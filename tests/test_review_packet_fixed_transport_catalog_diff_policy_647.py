"""Deep contracts for module647 policy-decision transports."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_644 as catalog_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_645 as diff_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_646 as policy_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_646_aud as policy_audit_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_647 as package_model
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_647_aud as audit_model
from glio_noncode.downloaded_data_review_packet_fixed_transport_catalog_diff_policy_644_ct import TRANSPORT_PREFIX as PACKAGE_PREFIX
from glio_noncode.downloaded_data_review_packet_fixed_transport_catalog_diff_policy_644_ct import CATALOG_PREFIX, ENTRY_PREFIX, Catalog, Entry, address_catalog, address_entry
...
class FixedTransportCatalogDiffPolicyTransport647Test(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.diff = diff_model.build_diff(left, right, diff_id="module647-diff")
        self.release = policy_model.release_policy(self.diff, policy_id="module647-release")
        self.strict = policy_model.strict_policy(self.diff, policy_id="module647-strict")
    ...
    def test_deterministic_nested_lineage_and_blocked_evidence(self) -> None:
        release = package_model.build_package(self.diff, self.release, self.release_audit, package_id="module647-release")
        repeated = package_model.build_package(self.diff, self.release, self.release_audit, package_id="module647-release")
        blocked = package_model.build_package(self.diff, self.strict, self.strict_audit, package_id="module647-strict")
    ...
    def test_persistence_tamper_and_public_boundary(self) -> None:
        package = package_model.build_package(self.diff, self.release, self.release_audit, package_id="module647-persist")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_647_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(diff_path), str(policy_path), "--audit", str(audit_path), "--package-id", "cli-module647", "--destination", str(package_path)]), 0)
            ...
            try:
                ...
                base = policy_base + "" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport"
                ...
                built = get(base, [("diff", diff_path), ("policy", policy_path), ("audit", audit_path), ("package_id", "http-module647"), ("destination", package_path), ("overwrite", "true")])
                self.assertEqual((built["policy_state"], built["audit_accepted"]), ("ready", True))
                loaded = get(base + "/load", [("input", package_path)])
                self.assertEqual(loaded["policy"]["policy_id"], "module647-release")
