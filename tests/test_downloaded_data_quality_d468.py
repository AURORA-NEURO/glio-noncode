"""Focused regression coverage for the D468 history diff runtime layer."""
...
class D468DiffRuntimeTests(unittest.TestCase):
    def test_runtime_replays_strict_release_audit_and_query(self) -> None:
        ...
        def runtime(runtime_id: str, state: str, release_ready: bool, address: str) -> dict[str, object]:
            return {
                ...
                "diff_id": "d468-diff",
                "diff_address": "glio-noncode-d468-diff:fixture",
                "state": state,
        ...
        blocked_registry = history_model.build_registry_snapshot(
            (runtime("d468-blocked-runtime", "blocked", False, "glio-noncode-d468-runtime:blocked"),),
            registry_id="d468-registry",
        )
        ready_registry = history_model.build_registry_snapshot(
            (runtime("d468-ready-runtime", "ready", True, "glio-noncode-d468-runtime:ready"),),
            registry_id="d468-registry",
        )
        baseline = history_model.build_history(blocked_registry, history_id="d468-history", snapshot_id="blocked")
        ...
        diff = diff_model.build_diff(baseline, candidate, diff_id="d468-diff")
        strict = runtime_model.run_runtime(diff, runtime_id="d468-strict-runtime", policy=runtime_model.build_policy("d468-strict-policy", diff.diff_id, maximum_added=0, maximum_removed=0, maximum_changed=0, allowed_directions=("improved", "changed", "unchanged"), require_accepted=True, require_state_change=False, allow_unchanged=True))
        release = runtime_model.run_runtime(diff, runtime_id="d468-release-runtime", policy=runtime_model.build_policy("d468-release-policy", diff.diff_id, maximum_added=1, maximum_removed=0, maximum_changed=0, allowed_directions=("improved",), require_accepted=True, require_state_change=True, allow_unchanged=True))
