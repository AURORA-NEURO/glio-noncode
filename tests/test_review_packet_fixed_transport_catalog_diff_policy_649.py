"""Deep contracts for module649 longitudinal module648 catalog diffs."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_649_aud as audit_model
...
def _catalog(entries: tuple[Entry, ...], catalog_id: str = "module649-catalog") -> Catalog:
...
class FixedTransportCatalogDiffPolicyTransportCatalogDiff649Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        self.value = diff_model.build_diff(self.left, self.right, diff_id="module649-diff")

    def test_deterministic_classification_transition_and_direction(self) -> None:
        repeated = diff_model.build_diff(self.left, self.right, diff_id="module649-diff")
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_649_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(left_path), str(right_path), "--diff-id", "cli-module649", "--destination", str(diff_path), "--format", "summary"]), 0)
            ...
            try:
                ...
                base = policy_base + "" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff" + "/policy" + "/transport" + "/catalog" + "/diff"
                ...
                built = get(base, [("left", left_path), ("right", right_path), ("diff_id", "http-module649"), ("destination", diff_path), ("overwrite", "true")])
