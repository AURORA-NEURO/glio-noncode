...
from glio_noncode.cell_state_frontier_replay import (
...
from glio_noncode.cell_state_frontier_scenario_matrix import (
...
class CellStateFrontierDepthTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.runtime = run_cell_state_frontier_pipeline(CellStateFrontierRuntimeOptions(run_id="d08-depth"), fixture=self.fixture)
    ...
    def test_fixture_constants_match_declared_shape(self) -> None:
        self.assertEqual(self.fixture.fixture_version, CELL_STATE_FRONTIER_FIXTURE_VERSION)
        ...
        self.assertEqual(len(self.fixture.sources), CELL_STATE_FRONTIER_SOURCE_COUNT)
        self.assertEqual((len(self.fixture.positive_records), len(self.fixture.control_records)), (4, 12))
    ...
    def test_record_receipts_are_unique_and_reference_known_sources(self) -> None:
        ...
        self.assertTrue(all(set(item.source_ids) <= known_sources for item in self.fixture.records))
    ...
    def test_each_operation_has_one_positive_and_three_controls(self) -> None:
        for operation in CellStateFrontierOperation:
            ...
            self.assertEqual(sum(item.role is CellStateFrontierRole.POSITIVE for item in records), 1)
            self.assertEqual(sum(item.role is CellStateFrontierRole.CONTROL for item in records), 3)
    ...
    def test_catalog_rejects_duplicate_ids(self) -> None:
        ...
        with self.assertRaises(ValidationError):
            CellStateFrontierCatalog(catalog.fixture, (catalog.source_ids[0], *catalog.source_ids), catalog.record_ids, catalog.operations, catalog.content_address)
        with self.assertRaises(ValidationError):
            CellStateFrontierCatalog(catalog.fixture, catalog.source_ids, (catalog.record_ids[0], *catalog.record_ids), catalog.operations, catalog.content_address)
    ...
    def test_fixture_json_round_trip_retains_address(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            loaded = load_cell_state_frontier_fixture(path)
        self.assertEqual(loaded.to_dict(), self.fixture.to_dict())
        self.assertEqual(loaded.content_address, self.fixture.content_address)
        self.assertEqual(loaded.record_map()["C13-POS-001"].operation, CellStateFrontierOperation.ABUNDANCE_INTERVAL)
    ...
    def test_data_audit_contains_all_boundary_checks(self) -> None:
        ...
        self.assertGreaterEqual(len(audit.checks), 9)
    ...
    def test_data_audit_detects_boundary_drift(self) -> None:
        with self.assertRaises(ValidationError):
    ...
    def test_evaluation_receipt_addresses_are_unique(self) -> None:
        ...
        self.assertEqual(len(addresses), len(set(addresses)))
        self.assertTrue(all(address.startswith("sha256:") for address in addresses))
        self.assertEqual(tuple(item.record_id for item in self.evaluation.receipts), tuple(item.record_id for item in self.fixture.records))

    def test_evaluation_is_deterministic_across_repeated_runs(self) -> None:
        repeated = evaluate_cell_state_frontier_fixture(self.fixture)
        ...
        self.assertEqual(build_cell_state_frontier_expectation(repeated).to_dict(), build_cell_state_frontier_expectation(self.evaluation).to_dict())
    ...
    def test_positive_records_are_supported_and_controls_are_not(self) -> None:
        positives = tuple(item for item in self.evaluation.receipts if item.role is CellStateFrontierRole.POSITIVE)
        controls = tuple(item for item in self.evaluation.receipts if item.role is CellStateFrontierRole.CONTROL)
        self.assertEqual((len(positives), len(controls)), (4, 12))
    ...
    def test_expected_issue_floors_are_observed(self) -> None:
        ...
        for receipt in self.evaluation.receipts:
            self.assertTrue(set(records[receipt.record_id].expected_issue_codes) <= set(receipt.observed_issue_codes), receipt.record_id)

    def test_state_matrix_is_explicit_for_each_operation(self) -> None:
        states = {operation: tuple(item.adapter_state for item in self.evaluation.receipts if item.operation is operation) for operation in CellStateFrontierOperation}
        self.assertEqual(states[CellStateFrontierOperation.ABUNDANCE_INTERVAL], ("supported", "partial", "out_of_domain", "partial"))
        self.assertEqual(states[CellStateFrontierOperation.REFERENCE_MAPPING], ("supported", "partial", "out_of_domain", "partial"))
        self.assertEqual(states[CellStateFrontierOperation.OOD_DETECTION], ("supported", "partial", "out_of_domain", "partial"))
        self.assertEqual(states[CellStateFrontierOperation.CONTEXT_PUBLICATION], ("supported", "partial", "out_of_domain", "partial"))
    ...
    def test_contract_registry_resolves_every_operation(self) -> None:
        ...
        for operation in CellStateFrontierOperation:
            ...
            self.assertTrue(contract.contract_id.startswith("GNC-D08-"))
            self.assertTrue(contract.required_payload_fields)
            self.assertIn("supported", contract.positive_states)

    def test_contract_vocabularies_are_disjoint_and_bounded(self) -> None:
        registry = default_cell_state_frontier_contracts()
    ...
    def test_replay_report_has_eight_passed_checks(self) -> None:
        replay = replay_cell_state_frontier_evaluation(self.evaluation, fixture=self.fixture)
    ...
    def test_replay_detects_changed_fixture_identity(self) -> None:
        changed = replace(self.fixture, fixture_id="cell-state-frontier-changed")
        evaluation = evaluate_cell_state_frontier_fixture(changed)
    ...
    def test_scenario_matrix_matches_receipt_order(self) -> None:
        scenarios = default_cell_state_frontier_scenarios()
        report = evaluate_cell_state_frontier_scenarios(self.evaluation)
        self.assertTrue(report.accepted)
        self.assertEqual(len(scenarios), 16)
        self.assertEqual(tuple(item.record_id for item in scenarios), tuple(item.record_id for item in self.evaluation.receipts))
    ...
    def test_scenario_matrix_detects_state_drift(self) -> None:
        ...
        report = evaluate_cell_state_frontier_scenarios(evaluation)
        self.assertFalse(report.accepted)
        self.assertIn("scenario:C13-POS-001", {item.scenario_id for item in report.checks if not item.passed})
    ...
    def test_policy_declares_bounded_dispositions(self) -> None:
        ...
        self.assertTrue(all(item.disposition_on_failure in {CellStateFrontierPolicyDisposition.DENY, CellStateFrontierPolicyDisposition.REVIEW} for item in rules))
        self.assertEqual({operation for item in rules for operation in item.applies_to}, set(CellStateFrontierOperation))

    def test_policy_report_contains_one_check_per_rule(self) -> None:
        policy = evaluate_cell_state_frontier_policy(self.fixture, self.evaluation)
        ...
        self.assertTrue(all(item.disposition is CellStateFrontierPolicyDisposition.PASS for item in policy.checks))

    def test_policy_detects_participant_key_in_payload(self) -> None:
        ...
        policy = evaluate_cell_state_frontier_policy(fixture)
    ...
    def test_schema_manifest_has_four_operation_schemas(self) -> None:
        ...
        manifest = cell_state_frontier_schema_manifest(schemas)
    ...
    def test_schema_report_detects_prohibited_claim_in_summary(self) -> None:
        ...
        self.assertFalse(report.accepted)
        self.assertIn("cell_state_abundance_interval:claims", report.failed_check_ids)
    ...
    def test_view_has_one_operation_view_per_operation(self) -> None:
        ...
        self.assertEqual(tuple(item.operation for item in self.view.operation_views), tuple(CellStateFrontierOperation))
    ...
    def test_view_filters_compose_states_operations_and_priority(self) -> None:
        out_of_domain = filter_cell_state_frontier_review_queue(self.view, states=("out_of_domain",))
        self.assertEqual(len(out_of_domain), 4)
        abundance = filter_cell_state_frontier_review_queue(self.view, operations=(CellStateFrontierOperation.ABUNDANCE_INTERVAL,))
        self.assertEqual(len(abundance), 3)
        bounded = filter_cell_state_frontier_review_queue(self.view, maximum_priority=2)
        self.assertEqual(len(bounded), 8)
        combined = filter_cell_state_frontier_review_queue(self.view, states=("partial",), operations=(CellStateFrontierOperation.REFERENCE_MAPPING,), maximum_priority=2)
        self.assertEqual(len(combined), 2)

    def test_view_summary_counts_are_conservative(self) -> None:
        summary = cell_state_frontier_review_summary(self.view)
        ...
        self.assertEqual(summary["source_count"], CELL_STATE_FRONTIER_SOURCE_COUNT)
    ...
    def test_source_matrix_closes_over_all_fixture_records(self) -> None:
        ...
        for row in self.view.source_matrix:
            ...
            self.assertEqual(set(row.positive_record_ids) | set(row.control_record_ids), set(row.record_ids))
        self.assertEqual(matrix_ids, {item.record_id for item in self.fixture.records})
    ...
    def test_lineage_verification_detects_address_and_order_drift(self) -> None:
        ...
        self.assertIn("fixture-identity", verify_cell_state_frontier_lineage(changed, self.fixture, self.evaluation))
        reordered = replace(lineage, edges=tuple(reversed(lineage.edges)))
        self.assertIn("record-order", verify_cell_state_frontier_lineage(reordered, self.fixture, self.evaluation))
    ...
    def test_quality_gate_has_named_checks_and_accepted_bundle(self) -> None:
        ...
        self.assertEqual(set(self.quality.bundle.record_ids), {item.record_id for item in self.evaluation.receipts})
    ...
    def test_bundle_address_changes_when_evidence_changes(self) -> None:
        ...
        fixture = replace(self.fixture, records=(record, *self.fixture.records[1:]))
        changed = run_cell_state_frontier_quality_gate(fixture)
    ...
    def test_runtime_acceptance_and_context_boundary(self) -> None:
        ...
        self.assertIsNone(self.runtime.requested_context_key)
        self.assertTrue(self.runtime.content_address.startswith("sha256:"))
        context = run_cell_state_frontier_pipeline(CellStateFrontierRuntimeOptions(run_id="d08-wrong-context", requested_context_key="GRCh38|glioma|adult|differentiated|tumor|unknown"), fixture=self.fixture)
    ...
    def test_strict_runtime_rejects_review_rows(self) -> None:
        strict = run_cell_state_frontier_pipeline(CellStateFrontierRuntimeOptions(run_id="d08-strict", fail_on_review=True), fixture=self.fixture)
    ...
    def test_runtime_options_reject_unsupported_source_mode(self) -> None:
        with self.assertRaises(ValueError):
            CellStateFrontierRuntimeOptions(run_id="d08-network", source_mode="remote")
        with self.assertRaises(ValidationError):
            CellStateFrontierRuntimeOptions(run_id="")

    def test_release_manifest_is_ready_and_closes_addresses(self) -> None:
        release = build_cell_state_frontier_release(self.quality, self.runtime)
    ...
    def test_release_blocks_when_strict_runtime_rejects_review(self) -> None:
        strict = run_cell_state_frontier_pipeline(CellStateFrontierRuntimeOptions(run_id="d08-release-block", fail_on_review=True), fixture=self.fixture)
        release = build_cell_state_frontier_release(self.quality, strict)
    ...
    def test_trace_has_monotonic_nine_stage_sequence(self) -> None:
        trace = build_cell_state_frontier_trace(self.runtime)
    ...
    def test_trace_stage_names_cover_runtime_components(self) -> None:
        trace = build_cell_state_frontier_trace(self.runtime)
        self.assertEqual({item.stage.value for item in trace.stage_receipts}, {"data_audit", "evaluation", "replay", "scenarios", "policy", "schema", "lineage", "reconciliation", "bundle"})
    ...
    def test_run_comparison_is_equivalent_for_same_fixture(self) -> None:
        left = run_cell_state_frontier_pipeline(CellStateFrontierRuntimeOptions(run_id="d08-left"), fixture=self.fixture)
        right = run_cell_state_frontier_pipeline(CellStateFrontierRuntimeOptions(run_id="d08-right"), fixture=self.fixture)
    ...
    def test_review_budget_is_content_addressed(self) -> None:
        ...
        priority_two = cell_state_frontier_review_budget(self.view, maximum_priority=2)
        self.assertEqual(all_rows["eligible_review_count"], 12)
        self.assertEqual(priority_two["eligible_review_count"], 8)
    ...
    def test_json_export_is_sorted_and_sanitized(self) -> None:
        payload = export_cell_state_frontier_json(self.quality.bundle)
    ...
    def test_export_receipt_reports_payload_size_and_address(self) -> None:
        payload = export_cell_state_frontier_json(self.evaluation)
        receipt = cell_state_frontier_export_receipt("evaluation-json", payload)
        ...
        self.assertEqual(receipt["content_address"], cell_state_frontier_export_receipt("evaluation-json", payload)["content_address"])

    def test_jsonable_round_trip_contains_enum_values(self) -> None:
        value = jsonable(self.evaluation.receipts[0])
        self.assertEqual(value["operation"], CellStateFrontierOperation.ABUNDANCE_INTERVAL.value)
        self.assertEqual(value["role"], CellStateFrontierRole.POSITIVE.value)
    ...
    def test_content_addresses_are_sensitive_to_semantic_fields(self) -> None:
        receipt = self.evaluation.receipts[0]
        self.assertNotEqual(content_hash({"record_id": receipt.record_id, "state": receipt.adapter_state}), content_hash({"record_id": receipt.record_id, "state": "partial"}))

    def test_public_records_do_not_include_subject_level_keys(self) -> None:
        prohibited = {"patient", "subject", "donor", "participant"}
    ...
    def test_source_receipt_constructor_enforces_required_fields(self) -> None:
        with self.assertRaises(ValidationError):
            CellStateFrontierSourceReceipt(source_id="", title="source", uri="https://example.org", source_kind="public", release="2026", scope="aggregate", content_address="sha256:source")
        with self.assertRaises(ValidationError):
            CellStateFrontierSourceReceipt(source_id="source", title="source", uri="http://example.org", source_kind="public", release="2026", scope="aggregate", content_address="sha256:source")

    def test_record_constructor_enforces_typed_operation_and_role(self) -> None:
        record = self.fixture.records[0]
        with self.assertRaises(ValidationError):
            CellStateFrontierRecord(record.record_id, "cell_state_abundance_interval", record.role, record.context_key, record.source_ids, record.payload, record.expected_state, record.expected_issue_codes, record.description, record.content_address)
        with self.assertRaises(ValidationError):
            CellStateFrontierRecord(record.record_id, record.operation, "positive", record.context_key, record.source_ids, record.payload, record.expected_state, record.expected_issue_codes, record.description, record.content_address)

    def test_quality_bundle_contains_all_component_addresses(self) -> None:
        bundle = self.quality.bundle
        components = (bundle.data_audit, bundle.evaluation, bundle.replay, bundle.scenarios, bundle.policy, bundle.lineage, bundle.reconciliation, bundle.metrics)
        ...
        self.assertEqual((len(bundle.record_ids), len(bundle.source_ids)), (16, 5))
    ...
    def test_view_retains_context_on_every_review_entry(self) -> None:
        ...
        self.assertTrue(all(item.context_key == self.fixture.context_key for item in self.view.review_queue))
    ...
    def test_runtime_quality_reuses_same_bundle_identity(self) -> None:
        self.assertEqual(self.runtime.quality.bundle.bundle_address, self.quality.bundle.bundle_address)
    ...
    def test_fixture_to_dict_is_json_serializable(self) -> None:
        decoded = json.loads(json.dumps(self.fixture.to_dict(), sort_keys=True))
        self.assertEqual(decoded["fixture_id"], self.fixture.fixture_id)
        self.assertEqual((len(decoded["records"]), len(decoded["sources"])), (16, 5))

    def test_operation_values_are_stable_public_identifiers(self) -> None:
        self.assertEqual(tuple(item.value for item in CellStateFrontierOperation), ("cell_state_abundance_interval", "single_cell_reference_mapping", "cell_state_ood_detection", "cell_state_context_publication"))
