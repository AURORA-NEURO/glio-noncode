"""Focused regression coverage for the D465 runtime registry layer."""
...
class D465RuntimeRegistryTests(unittest.TestCase):
    def test_registry_replays_aggregate_audit_query_and_duplicate_guard(self) -> None:
        ...
        from glio_noncode import downloaded_data_quality_d465_runtime_registry as registry_model
        from glio_noncode import downloaded_data_quality_d465_runtime_registry_audit as audit_model
        from glio_noncode import downloaded_data_quality_d465_runtime_registry_query as query_model
        from glio_noncode import downloaded_data_quality_d465_runtime_registry_query_audit as query_audit_model
        ...
        def runtime(runtime_id: str, state: str, release_ready: bool, address: str) -> dict[str, object]:
            return {
                ...
                "diff_id": "d465-diff",
                "diff_address": "glio-noncode-d465-diff:fixture",
                "state": state,
        ...
        blocked_source = history_model.build_registry_snapshot(
            (runtime("d465-source-blocked", "blocked", False, "glio-noncode-d465-source-runtime:blocked"),),
            registry_id="d465-registry",
        )
        ready_source = history_model.build_registry_snapshot(
            (runtime("d465-source-ready", "ready", True, "glio-noncode-d465-source-runtime:ready"),),
            registry_id="d465-registry",
        )
        left = history_model.build_history(blocked_source, history_id="d465-source-history", snapshot_id="baseline")
        right = history_model.append_history(left, ready_source, snapshot_id="ready", expected_head=left.entries[-1].content_address)
        diff = diff_model.build_diff(left, right, diff_id="d465-diff")
        ready = runtime_model.run_runtime(diff, runtime_id="d465-ready-runtime", policy=runtime_model.build_policy("d465-ready-policy", diff.diff_id, maximum_added=1, maximum_removed=0, maximum_changed=0, allowed_directions=("improved",), require_accepted=True, require_state_change=True, allow_unchanged=True))
        blocked = runtime_model.run_runtime(diff, runtime_id="d465-blocked-runtime", policy=runtime_model.build_policy("d465-blocked-policy", diff.diff_id, maximum_added=0, maximum_removed=0, maximum_changed=0, allowed_directions=("improved", "changed", "unchanged"), require_accepted=True, require_state_change=False, allow_unchanged=True))
        registry = registry_model.build_registry((blocked, ready), registry_id="d465-registry")
        ...
        with self.assertRaises(ValidationError):
            registry_model.build_registry((ready, ready), registry_id="d465-duplicate")
