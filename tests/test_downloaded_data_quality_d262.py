"""Focused regression coverage for the D262 registry history layer."""
...
from glio_noncode import downloaded_data_quality_d262_runtime_registry_history_audit as audit_model
from glio_noncode import downloaded_data_quality_d262_runtime_registry_history_query as query_model
from glio_noncode import downloaded_data_quality_d262_runtime_registry_history_query_audit as query_audit_model
from glio_noncode.errors import ValidationError
from glio_noncode.serialization import canonical_json


class D262RegistryHistoryTests(unittest.TestCase):
    def test_history_replays_append_query_and_head_guards(self) -> None:
        ...
        source_registry = source_registry_model.build_registry((), registry_id="d262-source-registry")
        left = source_history_model.build_history(source_registry, history_id="d262-source-history", snapshot_id="baseline")
        right = source_history_model.build_history(source_registry, history_id="d262-source-history", snapshot_id="candidate")
        diff = runtime_model.diff_model.build_diff(left, right, diff_id="d262-diff")
        ready_runtime = runtime_model.build_runtime(diff, runtime_id="d262-ready-runtime", policy=runtime_model.build_policy("d262-ready-policy", diff.diff_id, maximum_changed=1))
        blocked_runtime = runtime_model.build_runtime(diff, runtime_id="d262-blocked-runtime", policy=runtime_model.build_policy("d262-blocked-policy", diff.diff_id, maximum_changed=0))
        blocked_registry = registry_model.build_registry((blocked_runtime,), registry_id="d262-registry")
        ready_registry = registry_model.build_registry((ready_runtime,), registry_id="d262-registry")
        history = history_model.build_history(blocked_registry, history_id="d262-history", snapshot_id="blocked")
