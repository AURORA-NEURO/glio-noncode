...
class CausalArchitectureCliTests(unittest.TestCase):
    def test_fixture_evaluation_runtime_and_report_commands(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(main(["causal-architecture-fixture", "--output", str(fixture)]), 0)
            self.assertEqual(
                main(
                    [
                        "evaluate-causal-architecture",
                        "--input",
                        str(fixture),
                        "--output",
                        str(evaluation),
    ...
    def test_bundle_validation_and_query_commands(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(
                main(
                    [
                        "causal-architecture-validation",
                        "--input",
                        str(fixture),
                        "--output",
                        str(validation),
