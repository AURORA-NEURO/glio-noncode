"""Deep contracts for module681 longitudinal module680 catalog diffs."""
...
from glio_noncode import downloaded_data_review_packet_fixed_transport_catalog_diff_policy_681_aud as audit_model
...
def _catalog(entries: tuple[Entry, ...], catalog_id: str = "module681-catalog") -> Catalog:
...
class FixedTransportCatalogDiffPolicyTransportCatalogDiff681Test(unittest.TestCase):
    def setUp(self) -> None:
        alpha = _entry("alpha")
        ...
        self.value = diff_model.build_diff(self.left, self.right, diff_id="module681-diff")

    def test_deterministic_classification_transition_and_direction(self) -> None:
        repeated = diff_model.build_diff(self.left, self.right, diff_id="module681-diff")
    ...
    def test_posture_improvement_outweighs_catalog_size_reduction(self) -> None:
        left = _catalog((_entry("release"), _entry("strict", state="blocked")))
        right = _catalog((_entry("release"),))
        value = diff_model.build_diff(left, right, diff_id="posture-priority")
        self.assertEqual((value.state_transition, value.direction), ("mixed-to-ready", "improved"))
    ...
    def test_cli_http_and_schema_surfaces(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = _legacy_cli._REVIEW_PACKET_TRANSPORT_CATALOG_POLICY_TRANSPORT_CATALOG_DIFF_POLICY_TRANSPORT_CATALOG_DIFF_681_COMMAND

            def invoke(arguments: list[str]) -> int:
                with contextlib.redirect_stdout(io.StringIO()):
                    return main(arguments)

            self.assertEqual(invoke([command, str(left_path), str(right_path), "--diff-id", "cli-module681", "--destination", str(diff_path), "--format", "summary"]), 0)
            ...
            try:
                ...
                base = '/v1/downloaded-data/review-packet/diff/policy/release-'
                ...
                built = get(base, [("left", left_path), ("right", right_path), ("diff_id", "http-module681"), ("destination", diff_path), ("overwrite", "true")])
